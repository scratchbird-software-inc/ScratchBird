...
# AEIC_PRODUCTION_AGENT_CONFIGURE_GATE_MATRIX: top-level configure proof for
# non-bypassable production agent build separation. These cases configure in
# throwaway build directories under the active build tree.
...
foreach(_sb_forwarded_var
  ...
  if(DEFINED ${_sb_forwarded_var} AND NOT "${${_sb_forwarded_var}}" STREQUAL "")
    list(APPEND _sb_agent_configure_common_args
...
if(DEFINED SB_CONFIGURE_GATE_GENERATOR AND NOT "${SB_CONFIGURE_GATE_GENERATOR}" STREQUAL "")
  set(_sb_agent_configure_generator "${SB_CONFIGURE_GATE_GENERATOR}")
elseif(DEFINED CMAKE_GENERATOR AND NOT "${CMAKE_GENERATOR}" STREQUAL "")
  set(_sb_agent_configure_generator "${CMAKE_GENERATOR}")
else()
  set(_sb_agent_configure_generator "")
endif()
if(NOT "${_sb_agent_configure_generator}" STREQUAL "")
  string(REGEX REPLACE "^['\"]|['\"]$" "" _sb_agent_configure_generator
         "${_sb_agent_configure_generator}")
  list(APPEND _sb_agent_configure_generator_args -G "${_sb_agent_configure_generator}")
endif()

function(_sb_agent_configure_case case_name expected_success)
  set(_sb_case_dir "${SB_CONFIGURE_GATE_BINARY_ROOT}/${case_name}")
  file(REMOVE_RECURSE "${_sb_case_dir}")
  execute_process(
    COMMAND "${CMAKE_COMMAND}"
            ${_sb_agent_configure_generator_args}
            -S "${SB_PROJECT_SOURCE_DIR}"
            -B "${_sb_case_dir}"
            -DSB_BUILD_TESTS=OFF
            ${_sb_agent_configure_common_args}
            ${ARGN}
...
_sb_agent_configure_case(enforcement_off FALSE
  -DSB_NONCLUSTER_ENGINE_PROFILE=release-complete
  -DCMAKE_BUILD_TYPE=Release
  -DSB_AGENT_ENFORCE_PRODUCTION_BUILD_GATE=OFF)
_sb_agent_configure_case(fixture_auth FALSE
  -DSB_NONCLUSTER_ENGINE_PROFILE=release-complete
  -DCMAKE_BUILD_TYPE=Release
  -DSB_AGENT_ALLOW_FIXTURE_AUTH_IN_PRODUCTION=ON)
_sb_agent_configure_case(simulated_actuators FALSE
  -DSB_NONCLUSTER_ENGINE_PROFILE=release-complete
  -DCMAKE_BUILD_TYPE=Release
  -DSB_AGENT_ALLOW_SIMULATED_ACTUATORS_IN_PRODUCTION=ON)
_sb_agent_configure_case(debug_logs FALSE
  -DSB_NONCLUSTER_ENGINE_PROFILE=release-complete
  -DCMAKE_BUILD_TYPE=Release
  -DSCRATCHBIRD_ENABLE_DEBUG_LOGS=ON)
_sb_agent_configure_case(cluster_stub_link TRUE
  -DSB_NONCLUSTER_ENGINE_PROFILE=release-complete
  -DCMAKE_BUILD_TYPE=Release
  -DSB_ENABLE_CLUSTER_PROVIDER=ON
  -DSB_CLUSTER_PROVIDER_STUB=ON)
_sb_agent_configure_case(empty_build_type_fixture_auth FALSE
  -DSB_NONCLUSTER_ENGINE_PROFILE=release-complete
  -DSB_AGENT_ALLOW_FIXTURE_AUTH_IN_PRODUCTION=ON)
_sb_agent_configure_case(bootstrap_fixture_success TRUE
  -DSB_NONCLUSTER_ENGINE_PROFILE=bootstrap
  -DCMAKE_BUILD_TYPE=Release
  -DSB_AGENT_ALLOW_FIXTURE_AUTH_IN_PRODUCTION=ON
  -DSB_AGENT_ALLOW_SIMULATED_ACTUATORS_IN_PRODUCTION=ON)

message(STATUS "agent_production_build_configure_gate=passed")
