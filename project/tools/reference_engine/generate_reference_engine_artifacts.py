...
"""Generate profile-derived reference reference artifacts.

The reference compatibility gate is intentionally data-driven: the canonical reference
...
PROFILE_MANIFEST = REPO / "project/src/parsers/compatibility/CompatibilityProfileManifest.csv"
...
Status: profile-derived private reference compatibility reference.

Reference family: `{row['family_id']}`.
Release profile: `{row['release_profile']}`.
Profile class: `{row['profile_class']}`.

## Authority Boundary

This artifact records ScratchBird compatibility profile evidence only. Reference
syntax, reference wire bytes, reference catalogs, reference logs, and reference source packets
...
`reference_seed_manifest_conformance` and the canonical reference compatibility profile
manifest.
"""
...
def write_conformance_manifest(row: dict[str, str]) -> bool:
    ...
    if path.exists():
        return False
    data = {
        "manifest_id": f"reference-{row['family_id']}-go-readiness-exact-extraction-closure-conformance",
        ...
        "gates": [
            {
                "gate": "reference_seed_manifest_conformance",
                "expected": "pass",
                "evidence": [
                    row["release_evidence_manifest"],
                    row["seed_manifest_path"],
                    "docs/reference/reference_catalog_seeds/actual_per_family_seed_manifest_index.yaml",
                ],
            }
        ],
    }
...
def write_release_evidence(row: dict[str, str]) -> bool:
    ...
    (regression_root / "README.md").write_text(
        ...
        "Status: profile-derived regression import root for structural reference compatibility proof.\n",
        encoding="utf-8",
    )
...
def seed_manifest(row: dict[str, str]) -> dict[str, Any]:
    ...
    base = {
        ...
        "authority": "reference_catalog_seed_manifest",
        "catalog_object_set": [
            {
                "object_id": f"{family}.compatibility_root",
                "object_kind": "reference_catalog_projection_root",
                "display_name": row["display_name"],
                "visibility": "reference_profile_visible",
            }
        ],
        "catalog_column_set": [
            {
        ...
        "visibility_rule_set": [
            {
                ...
                "rule": "visible_to_admitted_reference_profile_sessions_only",
            }
        ],
        "redaction_rule_set": [
            {
...
def write_seed_index(entries: list[dict[str, Any]]) -> None:
    path = repo_path("docs/reference/reference_catalog_seeds/actual_per_family_seed_manifest_index.yaml")
    dump_yaml(path, {"manifests": entries})


def main() -> int:
    rows = read_profiles()
    true_rows = [row for row in rows if row["profile_class"] == "reference_emulation"]
    ...
    print(
        "generated reference reference artifacts: "
