...
struct Args {
  std::filesystem::path matrix_path =
      "docs" "/execution-plans/datatypes-and-domains-final-coverage-closure/DONOR_DATATYPE_COVERAGE_MATRIX.csv";
};
...
std::vector<DonorRow> LoadRows(const std::filesystem::path& path, std::vector<std::string>* errors) {
  std::ifstream in(path);
  std::vector<DonorRow> rows;
  if (!in) {
  ...
  if (header.size() != 8 || header[0] != "donor_engine" || header[1] != "donor_type_or_family" ||
...
bool IsUnsupportedRow(const DonorRow& row) {
...
EngineDescriptor DescriptorForRow(const DonorRow& row) {
  ...
  if (row.sb_support_path == "domain_over_native") {
    descriptor.encoded_descriptor = "domain_uuid=generated-by-conformance;name=" + row.donor_type_or_family +
...
bool GeneratePositiveCases(const DonorRow& row) {
  const auto descriptor = DescriptorForRow(row);
  const auto metadata = RenderWireDriverMetadata(descriptor, row.donor_engine, row.donor_type_or_family);
  EngineTypedValue value;
  value.descriptor = descriptor;
  value.encoded_value = "sample:" + row.donor_engine + ":" + row.donor_type_or_family;
  EngineDatatypeTransportRecord record;
  record.transport_scope = "replication";
  record.descriptor = descriptor;
  record.value = value;
  record.donor_dialect = row.donor_engine;
  record.donor_label = row.donor_type_or_family;
  ...
  return !metadata.driver_display_type.empty() && metadata.donor_label_alias_only && encoded.ok && decoded.ok;
}
...
int main(int argc, char** argv) {
  ...
  if (!ParseArgs(argc, argv, &args)) {
    std::cerr << "usage: sb_donor_datatype_coverage_probe [--matrix PATH]\n";
    return 2;
  }
  ...
  for (const auto& row : rows) {
    if (row.donor_engine.empty() || row.donor_type_or_family.empty() || row.normalized_family.empty() ||
        row.sb_support_path.empty() || row.sb_descriptor_or_domain.empty() || row.wire_behavior.empty() ||
        row.status.empty() || row.notes.empty()) {
      required_fields_present = false;
    }
    const std::string key = row.donor_engine + "|" + row.donor_type_or_family;
