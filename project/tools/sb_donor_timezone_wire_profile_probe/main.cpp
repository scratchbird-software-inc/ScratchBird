...
DonorTemporalWireProfileRequest Request(const TimezoneSeedAuthority& seed,
                                        const char* donor,
                                        const char* donor_type,
                                        const char* profile,
                                        const char* value) {
  DonorTemporalWireProfileRequest request;
  request.donor_engine = donor;
  request.donor_type_or_family = donor_type;
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (key == "--seed-pack-root" && i + 1 < argc) {
      ...
      std::cerr << "usage: sb_donor_timezone_wire_profile_probe [--seed-pack-root PATH]\n";
      return 2;
    }
  }
