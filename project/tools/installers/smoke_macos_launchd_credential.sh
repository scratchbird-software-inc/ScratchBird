...
for index in "${!resolved_group_ids[@]}"; do
  ...
  # supplementary process credentials. A group-readable canary for either
  # would grant access by definition. The raw kernel-group check still proves
  # launchd did not copy their numeric GIDs into the service credential.
  case "$name" in
