...
struct Args {
  std::filesystem::path matrix_path =
      "docs" "/execution-plans/datatypes-and-domains-final-coverage-closure/DONOR_EXACT_DATATYPE_REGISTRY_MATRIX.csv";
};

struct ExactRow {
  std::string donor_engine;
  std::string source_row;
  std::string exact_donor_type;
...
std::vector<ExactRow> LoadRows(const std::filesystem::path& path, std::vector<std::string>* errors) {
  ...
  const std::vector<std::string> expected = {
      "donor_engine", "source_row", "exact_donor_type", "exact_aliases", "normalized_family",
...
int main(int argc, char** argv) {
  ...
  if (!ParseArgs(argc, argv, &args)) {
    std::cerr << "usage: sb_donor_exact_registry_probe [--matrix PATH]\n";
    return 2;
  }
  ...
  for (const auto& row : rows) {
    if (row.donor_engine.empty() || row.source_row.empty() || row.exact_donor_type.empty() ||
    ...
    const std::string key = row.donor_engine + "|" + row.exact_donor_type;
    if (!keys.insert(key).second) { no_duplicate_exact_rows = false; }
    source_rows.insert(row.donor_engine + "|" + row.source_row);
    ...
    const bool has_parse_case = !row.exact_donor_type.empty();
