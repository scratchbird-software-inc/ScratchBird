...
fn::FunctionCallResult CallAlias(const fn::FunctionRegistry& registry,
  ...
  Expect(row->alias_source == fn::FunctionAliasSource::reference,
         std::string(alias_name) + " is not classified as reference alias", errors);
...
int main() {
  ...
  projection_request.parser_profile = "reference-alias-conformance";
  ...
  std::set<std::string> reference_packages;
  for (const auto& row : rows) {
    if (row.alias_source == fn::FunctionAliasSource::reference) reference_packages.insert(row.source_package);
  }
  ...
  for (const auto& package_name : {"postgresql", "mysql", "firebird", "sqlite", "redis", "neo4j", "opensearch"}) {
    Expect(reference_packages.count(package_name) == 1, std::string("reference package missing from projection: ") + package_name, &errors);
  }
  const auto authority = fn::ValidateFunctionParserProjectionAuthority(projection_request);
  Expect(authority.allowed, "metadata-only reference projection authority was refused", &errors);
  ...
  std::cout << "  \"probe\": \"sb_sblr_reference_alias_probe\",\n";
  std::cout << "  \"projection_rows\": " << rows.size() << ",\n";
  std::cout << "  \"reference_packages\": " << reference_packages.size() << ",\n";
  std::cout << "  \"errors\": [";
