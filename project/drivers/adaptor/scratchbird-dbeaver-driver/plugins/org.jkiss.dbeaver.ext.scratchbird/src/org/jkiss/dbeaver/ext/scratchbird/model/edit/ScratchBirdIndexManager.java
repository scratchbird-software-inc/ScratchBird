...
public class ScratchBirdIndexManager extends GenericIndexManager {
    ...
    public boolean canCreateObject(@NotNull Object container) {
        ...
        return ScratchBirdManagerSupport.canCreateIndex(container);
    }
    ...
    protected GenericTableIndex createDatabaseObject(
        ...
        GenericTableIndex index = tableBase.getDataSource().getMetaModel().createIndexImpl(
            tableBase,
            true,
            null,
            0,
            getNewChildName(monitor, tableBase, "new_index"),
            DBSIndexType.OTHER,
            false);
        try {
            GenericTableColumn firstColumn = ScratchBirdManagerSupport.firstTableColumn(monitor, tableBase);
            index.addColumn(new GenericTableIndexColumn(index, firstColumn, 1, true));
        } catch (Exception ignored) {
            // Keep the draft index creatable even when the table has no discoverable columns yet.
        }
        return index;
    }
}
