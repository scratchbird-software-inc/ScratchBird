...
"""Validate P4 documentation/sample and reference-driver route evidence."""
...
REFERENCE_EVIDENCE_NAME = "DSR_045_REFERENCE_DRIVER_COMPATIBILITY_ROUTE_EVIDENCE.json"
...
REFERENCE_ROUTE_TOKENS = ("sbwp", "listener", "parser", "server", "engine")
...
def validate_reference_route(ctx: Context, route: dict[str, Any]) -> list[str]:
    ...
    for token in REFERENCE_ROUTE_TOKENS:
        if token not in command_lower:
            errors.append(f"{prefix} ScratchBird route command missing route token {token}")
    reference_tool = str(route.get("reference_tool", "")).strip().lower()
    profile = str(route.get("compatibility_profile", "")).strip().lower()
    if reference_tool and reference_tool not in command_lower:
        errors.append(f"{prefix} ScratchBird route command must cite reference tool {reference_tool}")
    ...
    if "reference_driver_compatibility_route" not in ctest_label:
        errors.append(f"{prefix} ctest_label must include reference_driver_compatibility_route")
...
def validate_reference_evidence(ctx: Context) -> int:
    path = ctx.artifacts_root / REFERENCE_EVIDENCE_NAME
    try:
        data = load_json(path)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        return fail(str(exc))
    ...
    if data.get("gate_id") != "DSR-045":
        errors.append(f"{REFERENCE_EVIDENCE_NAME} gate_id must be DSR-045")
    if is_empty(data.get("route_requirement")):
        errors.append(f"{REFERENCE_EVIDENCE_NAME} missing route_requirement")
    errors.extend(validate_shortcut_rejection(data, "DSR-045"))

    routes = data.get("routes")
    if not isinstance(routes, list) or not routes:
        errors.append(f"{REFERENCE_EVIDENCE_NAME} routes must be a non-empty list")
        routes = []
    ...
    for route in routes:
        if not isinstance(route, dict):
            errors.append(f"{REFERENCE_EVIDENCE_NAME} routes contains a non-object item")
            continue
        route_id = str(route.get("route_id", "")).strip()
        ...
        family = str(route.get("reference_family", "")).strip()
        if family:
            families.add(family)
        errors.extend(validate_reference_route(ctx, route))

    if duplicate_ids:
        errors.append(f"{REFERENCE_EVIDENCE_NAME} duplicate routes: {', '.join(sorted(duplicate_ids))}")

    required_families = data.get("required_reference_families")
    if not isinstance(required_families, list) or not required_families:
        errors.append(f"{REFERENCE_EVIDENCE_NAME} required_reference_families must be a non-empty list")
    else:
        missing_families = sorted(set(str(item) for item in required_families) - families)
        if missing_families:
            errors.append(f"{REFERENCE_EVIDENCE_NAME} missing required reference families: {', '.join(missing_families)}")

    return print_result(errors, f"DSR-045 reference driver route evidence ok: {len(route_ids)} routes validated")


def print_result(errors: list[str], ok_message: str) -> int:
    if errors:
        print("failed: P4 docs/reference gate errors:", file=sys.stderr)
...
def main() -> int:
    ...
    if args.mode == "reference-driver-route-evidence":
        return validate_reference_evidence(ctx)
    doc_result = validate_documentation_evidence(ctx)
    reference_result = validate_reference_evidence(ctx)
    return 1 if doc_result != 0 or reference_result != 0 else 0
