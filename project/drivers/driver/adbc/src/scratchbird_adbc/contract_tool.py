"""Host-route runner for the ScratchBird ADBC native tool."""
...
def transport_impl(route: str, sslmode: str) -> str:
    ...
    if route == "ipc_local":
        return "adbc_driver_manager_unix_domain_socket"
    return "adbc_driver_manager_tcp_tls" if sslmode != "disable" else "adbc_driver_manager_tcp_plain"
...
class AdbcRunner:
    ...
    def connect(self) -> None:
        driver = os.environ.get("SCRATCHBIRD_ADBC_DRIVER", "").strip()
        if not driver:
            raise ContractError(
                f"{DIAG_PREFIX}_RUNTIME_DEPENDENCY_MISSING",
                "SCRATCHBIRD_ADBC_DRIVER must name the ScratchBird ADBC driver library",
                dependency="SCRATCHBIRD_ADBC_DRIVER",
            )
        try:
            ...
            raise ContractError(
                f"{DIAG_PREFIX}_RUNTIME_DEPENDENCY_MISSING",
                "Python package adbc_driver_manager is required for sb_isql_adbc",
                dependency="adbc_driver_manager",
            ) from exc
        uri = self._uri()
        kwargs: dict[str, Any] = {
            "driver": driver,
        ...
        entrypoint = os.environ.get("SCRATCHBIRD_ADBC_ENTRYPOINT", "").strip()
        if entrypoint:
            kwargs["entrypoint"] = entrypoint
        self.conn = dbapi.connect(**kwargs)
        self.api_hits["AdbcDatabase"] += 1
        self.api_hits["AdbcConnection"] += 1

    def _uri(self) -> str:
        if self.args.route == "ipc_local":
            if not self.args.ipc_path:
                raise ContractError(f"{DIAG_PREFIX}_IPC_PATH_REQUIRED", "ipc_local route requires --ipc-path")
            return "scratchbird+unix://" + quote(self.args.ipc_path, safe="")
        if self.args.route == "embedded":
            raise ContractError(
                f"{DIAG_PREFIX}_EMBEDDED_UNSUPPORTED",
                "ADBC contract lane has no ScratchBird C++ embedded library boundary",
            )
        scheme = "scratchbird"
        return f"{scheme}://{self.args.host}:{self.args.port}/{quote(self.args.database)}?sslmode={quote(self.args.sslmode)}"

    def execute(self, statement: str) -> tuple[list[Any], int]:
        if self.conn is None:
            raise RuntimeError("ADBC connection is not open")
        cursor = self.conn.cursor()
        self.api_hits["AdbcStatement"] += 1
        cursor.execute(statement)
        rows = cursor.fetchall() if getattr(cursor, "description", None) else []
        self.api_hits["ArrowArrayStream"] += 1
        return rows, len(rows)

    def commit(self) -> None:
        if self.conn is not None:
            self.conn.commit()
            self.api_hits["AdbcConnection.commit"] += 1

    def rollback(self) -> None:
        if self.conn is not None:
            self.conn.rollback()
            self.api_hits["AdbcConnection.rollback"] += 1
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.create_database:
        raise ContractError(
            ...
            "ADBC create-database mode requires a live ScratchBird ADBC extension surface",
        )
...
def run_script(args: argparse.Namespace) -> int:
    ...
    api_hits = {
        ...
        "AdbcStatement": 0,
        "ArrowArrayStream": 0,
        "AdbcConnection.commit": 0,
        "AdbcConnection.rollback": 0,
    }
    started = now_ns()
    route_env = route_environment(args, None, "fail", "not_probed")
    runner = AdbcRunner(args, api_hits)
    statements: list[str] = []
    try:
        ...
        try:
            ...
            if rows:
                ...
                if isinstance(first, dict):
                    actual = int(first.get("page_size_bytes")) if first.get("page_size_bytes") is not None else None
    ...
    write_text(artifacts["code-example-review.json"], json.dumps({"driver": DRIVER, "public_api_only": True, "shells_out_to_other_driver": False, "source_is_canonical_example": True, "host_api": "Apache Arrow ADBC"}, indent=2, sort_keys=True) + "\n")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="ScratchBird ADBC host-route native tool")
