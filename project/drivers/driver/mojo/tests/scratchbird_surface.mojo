...
def _assert_config_session_pooling_manager_extensions() raises:
    ...
    var cfg_prepare_threshold_alias = scratchbird.ScratchBirdConfig(
    ...
    var cfg_rewrite_batched_alias = scratchbird.ScratchBirdConfig(
    ...
    var cfg_logger_aliases = scratchbird.ScratchBirdConfig(
        "scratchbird://user:pass@localhost:3092/testdb?sslmode=require&logLevel=debug&log_file=/tmp/sb_mojo_surface.log"
    )
    _require(cfg_logger_aliases.logger_level == "DEBUG", "logLevel alias normalization mismatch")
    _require(cfg_logger_aliases.logger_file == "/tmp/sb_mojo_surface.log", "log_file alias mismatch")
    ...
    var cfg_pooling_overrides = scratchbird.ScratchBirdConfig(
    ...
    var cfg_manager_overrides = scratchbird.ScratchBirdConfig(
    ...
    var cfg_mcp_database_alias = scratchbird.ScratchBirdConfig(
    ...
    var cfg_ssl_alias = scratchbird.ScratchBirdConfig(
...
def _assert_config_parsing_extensions() raises:
    ...
    var cfg_protocol_scratchbird = scratchbird.ScratchBirdConfig(
    ...
    var cfg_protocol_scratchbird_native = scratchbird.ScratchBirdConfig(
    ...
    var cfg_manager_dash = scratchbird.ScratchBirdConfig(
        "scratchbird://user:pass@localhost:3092/testdb?sslmode=require&front_door_mode=manager-proxy&manager_auth_token=mode_token"
    )
...
def main() raises:
    ...
    try:
        ...
        _require(
            scratchbird.extract_sqlstate(String(e)) == "07001",
            "parameter mismatch should report 07001",
        )
    ...
    try:
        ...
        _require(
            scratchbird.extract_sqlstate(String(e)) == "22023",
    ...
    try:
        ...
        _require(
            scratchbird.extract_sqlstate(String(e)) == "07001",
            "prepared mismatch should report 07001",
        )
    try:
