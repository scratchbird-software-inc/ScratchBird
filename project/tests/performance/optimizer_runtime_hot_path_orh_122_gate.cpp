...
parser::CacheKey ParserCacheKey() {
  ...
  key.connection_uuid = scratchbird::tests::FixtureUuid(1453, 11);
  ...
  key.policy_profile = scratchbird::tests::FixtureUuid(1453, 12);
...
void ProveParserFrontDoorConcurrentChurnInvalidation() {
  ...
  ProveParserChurnCause(
      "policy profile",
      [](parser::CacheKey& key) { key.policy_profile = scratchbird::tests::FixtureUuid(1453, 13); },
      [](parser::SblrTemplateCache& cache, const parser::CacheKey& key) {
...
opt::OptimizerPlanCacheKeyInput OptimizerInput() {
  ...
  input.object_uuids = {scratchbird::tests::FixtureUuid(1587, 1)};
  input.function_uuids = {scratchbird::tests::FixtureUuid(1587, 2)};
  input.index_uuids = {scratchbird::tests::FixtureUuid(1587, 3)};
  input.filespace_uuids = {scratchbird::tests::FixtureUuid(1587, 4)};
  return input;
}
...
exec::SnapshotSafeCacheKey SnapshotKey() {
  ...
  key.descriptor_identity_digest = "descriptor:orh122-rowset:v1";
  key.descriptor_epoch = 127;
...
exec::SnapshotSafeCachePayload CachePayload() {
  ...
  Require(binding.ok, "ORH-122 int64 fixture binding unavailable");
  auto value = exec::EncodeInt64Value(42);
  value.descriptor.descriptor_uuid = scratchbird::tests::FixtureUuid(1587, 501);
...
void ProveSnapshotSafeResultCacheChurnMissesAndRefusals() {
  ...
  for (bool security_identity : {false, true}) {
    lookup = SnapshotLookupRequest();
    auto& identity = security_identity ? lookup.key.security_context_uuid
                                       : lookup.key.bound_sblr_tree_uuid;
    identity.bytes.back() ^= 1;
    decision = cache.Lookup(lookup);
    Require(decision.action == exec::SnapshotSafeCacheAction::kRefuse &&
                decision.fail_closed && !decision.cache_hit &&
                decision.diagnostic_code == "EXECUTOR.SNAPSHOT_RESULT_CACHE.CATALOG_IDENTITY_REQUIRED",
            "cache key identity conflicting with selected DAG was accepted");
  }

  lookup = SnapshotLookupRequest();
  lookup.key.catalog_epoch_uuid =
      scratchbird::tests::FixtureUuidLiteral("019f0000-0000-7500-8000-000000006198");
  decision = cache.Lookup(lookup);
  Require(decision.action == exec::SnapshotSafeCacheAction::kRefuse &&
              !decision.cache_hit,
...
api::EngineRequestContext NoSqlContext(const std::filesystem::path& path) {
  ...
  context.database_uuid = scratchbird::tests::FixtureUuid(1208, 3101);
  context.current_schema_uuid = scratchbird::tests::FixtureUuid(1208, 3102);
...
void ProveNoSqlProviderGenerationChurnRefusals() {
  ...
  const auto filespace_id = scratchbird::core::uuid::MakeTypedUuid(
      scratchbird::core::platform::UuidKind::filespace, scratchbird::tests::FixtureUuid(1587, 10));
  Require(database_id.ok() && filespace_id.ok(), "ORH-122 invalid database identities");
  ...
  Require(scratchbird::storage::database::CreateDatabaseFile(create).ok(), "ORH-122 database creation failed");
  context.principal_uuid = scratchbird::tests::FixtureUuid(1587, 11);
  context.session_uuid = scratchbird::tests::FixtureUuid(1587, 12);
