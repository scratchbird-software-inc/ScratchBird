...
void CheckFirebirdMappingInventory() {
  ...
  for (const auto& mapping : fb::FirebirdLifecycleMappings()) {
    ...
    Require(!mapping.reference_engine_sql_executed, "DBLC-014 Firebird mapping permits reference SQL execution");
...
void CheckFirebirdParse(std::string_view sql,
  ...
  Require(!parsed.reference_engine_sql_executed,
          "DBLC-014 Firebird parse permits reference SQL execution");
  ...
  Require(Contains(parsed.sblr_envelope, "\"reference_engine_sql_executed\":false"),
          "DBLC-014 Firebird envelope missing reference SQL exclusion");
  Require(!Contains(parsed.sblr_envelope, std::string(sql)),
          "DBLC-014 Firebird envelope leaked reference SQL text");
}
