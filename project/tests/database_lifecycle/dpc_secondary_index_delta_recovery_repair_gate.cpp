...
Fixture CopyFixtureSidecars(const Fixture& source, std::string name, platform::u64 salt) {
  ...
  restored.owner_context = source.owner_context;
  restored.owner_context.session_uuid = api::GenerateCrudEngineUuid("object");
  ...
  restored.session = std::make_shared<scratchbird::tests::FixtureEngineSession>(
      BaseContext(restored, "dpc-restored-session"));
  return restored;
}
...
int main() {
  ...
  policy.policy_name = "dpc_secondary_index_delta_recovery_repair_gate";
  Require(scratchbird::core::memory::ConfigureDefaultMemoryManagerForFixture(
              policy, "dpc_secondary_index_delta_recovery_repair_gate").ok(), "DPC memory configuration failed");
