...
server::HostedEngineState MakeEngineState() {
  ...
  database.database_path =
      "/tmp/ipar_prepared_execution_cache_authority.sbdb";
  database.database_uuid = scratchbird::tests::FixtureUuid(1208, 2101);
...
server::ServerSessionRecord MakeSession() {
  ...
  session.database_path =
      "/tmp/ipar_prepared_execution_cache_authority.sbdb";
  session.database_uuid = scratchbird::tests::FixtureUuid(1208, 2101);
  ...
  session.transaction_uuid = scratchbird::core::platform::Uuid{Uuid(0x50)};
...
void RequireTransactionUnchanged(const server::ServerSessionRecord& before,
                                 const server::ServerSessionRecord& after) {
  Require(after.local_transaction_id == before.local_transaction_id &&
          ...
          "retired prepare input mutated session transaction authority");
  ...
  Require(before_transaction != before.transactions_by_local_id.end() &&
          ...
          "retired prepare input changed active transaction state");
}
...
int main() {
  ...
  Require(registry.prepared_by_uuid.empty(),
          "retired prepare input published prepared metadata");
  ...
  Require(registry.authority_cache_by_key.empty(),
          "retired prepare input published cache authority");
  Require(registry.object_handles_by_key.empty(),
          "retired prepare input published a session object handle");
  ...
  Require(registry.public_abi_sessions_by_session_uuid.empty(),
          "retired prepare input opened a public ABI session");
  Require(registry.statement_contexts_by_statement_uuid.empty(),
          "retired prepare input acquired a statement receipt");
  ...
  RequireTransactionUnchanged(before, registry.sessions_by_uuid.at(session_key));
  return EXIT_SUCCESS;
}
