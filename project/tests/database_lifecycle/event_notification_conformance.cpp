...
void CreateChannel(const api::EngineRequestContext& base,
  ...
  request.option_envelopes.push_back(api::BinaryViewUuidOption("channel_uuid:", channel_uuid));
...
api::EngineUuid NotifyCommitted(const api::EngineRequestContext& base,
                            const api::EngineUuid& channel_uuid,
  ...
  const api::EngineUuid event_uuid = notified.publication.event_uuid;
  CommitTx(tx_context);
  return event_uuid;
}
...
server::ParserServerEventSession EventSession(const Fixture& fixture,
                                              const api::EngineUuid& parser_channel_uuid,
  ...
  session.engine_context.database_uuid = fixture.database_uuid;
  session.engine_context.session_uuid = session_uuid;
  session.engine_context.principal_uuid = scratchbird::tests::FixtureUuidLiteral("019e13c0-0000-7000-8000-0000000000aa");
...
server::PsEventSubscribeResult Subscribe(server::ParserServerEventIpcRuntime* runtime,
  ...
  request.request_uuid = NewUuid(UuidKind::object, 1779130902000);
  request.session = session;
  request.channel_uuid = channel_uuid;
  request.rendering_profile_uuid = scratchbird::tests::FixtureUuid(1258, 1);
...
void TestParserIpcEngineAuthorizedDeliveryAckAndDisconnect() {
  ...
  const auto session =
      EventSession(fixture,
                   scratchbird::tests::FixtureUuidLiteral("019e13c0-0000-7000-8000-000000000202"),
                   scratchbird::tests::FixtureUuidLiteral("019e13c0-0000-7000-8000-000000000302"));
  const auto subscribed = Subscribe(&runtime, session, channel_uuid);
  Require(subscribed.outcome == "accepted" && !subscribed.subscription_uuid.is_nil(),
  ...
  const api::EngineUuid event_uuid = NotifyCommitted(admin, channel_uuid, "payload-1");
  server::PsEventDeliveryPumpRequest pump;
  pump.request_uuid = scratchbird::tests::FixtureUuid(1258, 2);
  ...
  ack.request_uuid = scratchbird::tests::FixtureUuid(1258, 3);
  ack.session = session;
  ...
  Require(acked.outcome == "accepted" && !acked.acknowledgement_uuid.is_nil(),
  ...
  bad_ack.request_uuid = scratchbird::tests::FixtureUuid(1258, 4);
  bad_ack.event_uuid = scratchbird::tests::FixtureUuidLiteral("019e13c0-0000-7000-8000-00000000dead");
...
void TestRollbackSavepointRedactionBackpressureAndClusterRefusal() {
  ...
  const auto session =
      EventSession(fixture,
                   scratchbird::tests::FixtureUuidLiteral("019e13c0-0000-7000-8000-000000000601"),
                   scratchbird::tests::FixtureUuidLiteral("019e13c0-0000-7000-8000-000000000701"));
  const auto subscribed = Subscribe(&runtime, session, channel_uuid);
  ...
  server::PsEventDeliveryPumpRequest pump;
  pump.request_uuid = scratchbird::tests::FixtureUuid(1258, 5);
  pump.session = session;
  pump.max_events = 16;
  ...
  savepoint_request.option_envelopes.push_back(api::BinaryViewUuidOption("channel_uuid:", channel_uuid));
  ...
  const api::EngineUuid redacted_event =
  ...
  for (const auto& [key, value] : redacted_delivery.notifications.front().notification_vector.fields) {
    if (key == "payload" && scratchbird::tests::EvidenceTextEquals(value, "<redacted>")) { redacted_payload_seen = true; }
    Require(!scratchbird::tests::EvidenceTextEquals(value, "secret-payload"), "DBLC-013Y protected event payload leaked");
  }
  ...
  pressure.request_uuid = scratchbird::tests::FixtureUuid(1258, 6);
...
void TestParserOwnedServerFramesRejected() {
  ...
  frame.request_uuid = scratchbird::tests::FixtureUuid(1258, 7);
