...
Fixture MakeFixture(std::string name, platform::u64 salt) {
  ...
  fixture.non_unique_index_uuid =
      NewIdentity(platform::UuidKind::object, salt + 11);
...
bool HasEvidence(const std::vector<api::EngineEvidenceReference>& evidence,
  ...
  for (const auto& entry : evidence) {
    if (entry.evidence_kind == kind && scratchbird::tests::EvidenceTextEquals(entry.evidence_id, value)) {
      return true;
    }
  }
  return false;
...
int main() {
  ...
  policy.policy_name = "dpc_secondary_index_garbage_cleanup_agent_gate";
  Require(scratchbird::core::memory::ConfigureDefaultMemoryManagerForFixture(
              policy, "dpc_secondary_index_garbage_cleanup_agent_gate").ok(), "DPC memory configuration failed");
