...
Fixture MakeFixture(
  ...
  RequireDiagnosticOk(scratchbird::tests::PublishMgaTableFixture(
                          context, Table(fixture, context),
                          {"character", "character", "character"},
                          {Index(fixture, context, fixture.non_unique_index_uuid,
                                 "name", false, non_unique_family),
  ...
  fixture.session = std::make_shared<scratchbird::tests::FixtureEngineSession>(
      BaseContext(fixture, "dpc022-session"));
  return fixture;
}
...
void ValidateDeferredDeleteWritesTombstoneDelta() {
  ...
  const api::EngineUuid row_uuid = SeedCommittedRow(fixture,
                                                "id-delete",
                                                "alpha",
                                                DeferredOptions());
...
int main() {
  ...
  policy.policy_name = "dpc022_statement_fixture";
  Require(scratchbird::core::memory::ConfigureDefaultMemoryManagerForFixture(
              policy, "dpc022_statement_fixture").ok(),
          "DPC-022 memory policy configuration failed");
