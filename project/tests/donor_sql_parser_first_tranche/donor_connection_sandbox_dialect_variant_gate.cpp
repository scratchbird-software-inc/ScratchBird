...
using scratchbird::parser::donor::ConnectionSandboxReportJson;
using scratchbird::parser::donor::DialectProfile;
using scratchbird::parser::donor::DialectVariantReportJson;
...
std::vector<DonorProfile> DonorProfiles() {
  return {
...
bool CheckSandboxReport(const DonorProfile& donor) {
  const auto report = ConnectionSandboxReportJson(*donor.profile);
  const std::string label(donor.donor);
  return Expect(Contains(report, "\"ok\":true"), label + " sandbox ok missing") &&
         ...
         Expect(Contains(report, "donor_connection_schema_root_v1"),
...
bool CheckVariantReport(const DonorProfile& donor) {
  const auto report = DialectVariantReportJson(*donor.profile);
  const std::string label(donor.donor);
  bool ok = Expect(Contains(report, "\"ok\":true"), label + " variant ok missing") &&
            Expect(Contains(report, "\"dialect\":\"" + label + "\""),
                   label + " variant dialect missing") &&
            Expect(Contains(report, "donor_supported_variant_surface_v1"),
            ...
            Expect(Contains(report, "\"reasonable_subset_policy\":\"declared_and_tested_per_donor_variant\""),
  ...
  } else if (donor.donor == "postgresql") {
...
bool CheckSharedWorkerReports() {
  ...
  const auto sandbox =
      scratchbird::parser::donor::HandleWorkerCommand("CONNECTION_SANDBOX_REPORT",
                                                      profile,
                                                      &close);
  const auto variants =
      scratchbird::parser::donor::HandleWorkerCommand("DIALECT_VARIANT_REPORT",
                                                      profile,
                                                      &close);
  return Expect(!close, "worker metadata reports closed the session") &&
         ...
         Expect(Contains(sandbox, "donor_connection_schema_root_v1"),
...
int main() {
  bool ok = true;
  const auto donors = DonorProfiles();
  ok = ok && Expect(donors.size() == 25, "donor profile count drift");
  for (const auto& donor : donors) {
    ok = CheckSandboxReport(donor) && ok;
    ok = CheckVariantReport(donor) && ok;
  }
