...
P4_REL = (
    "public_execution_plan"
    "PARSER_REFUSAL_REDUCTION_MATRIX.csv"
)
...
def validate_policy_fields(row: dict[str, str]) -> None:
    ...
    require(row["test_destination"] ==
            "project/tests/donor_sql_parser_first_tranche/parser_refusal_reduction_gate.py",
    ...
    require("donor_sql_not_executed" in row["donor_authority_policy"],
            f"donor execution policy missing: {row['refusal_id']}")
    ...
    if row["runtime_disposition"] == "exact_pre_provider_refusal":
        ...
        require(row["provider_policy"] == "no_provider_or_donor_execution_for_exact_refusal",
