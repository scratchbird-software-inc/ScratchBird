...
"""First-tranche donor parser CLI proof gate."""
...
def check_parser(name: str, binary: Path, positive_sql: str, refusal_sql: str, refusal_code: str) -> dict[str, object]:
    ...
    require(
        '"donor_sql_execution":false' in identity.stdout
        or '"donor_engine_sql_executed":false' in identity.stdout,
        f"{name} identity did not deny donor SQL execution",
    )
    ...
    require('"donor_engine_sql_executed":false' in positive.stdout, f"{name} positive parse allowed donor SQL execution")
...
def main() -> int:
    ...
    evidence = {
        "gate": "donor_sql_parser_first_tranche_cli_evidence",
        ...
        "authority_invariants": {
            ...
            "donor_sql_execution_forbidden": True,
            "donor_file_effects_forbidden": True,
