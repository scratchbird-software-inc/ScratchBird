# Donor Logical Remote Stream Backup and Restore Policy

Search key: `SB_TEST_DONOR_LOGICAL_REMOTE_STREAM_BACKUP_RESTORE_POLICY`

This tracked test policy records the donor-by-donor backup/restore boundary
that must survive public release packaging and full CTest runs.

A donor parser may support full or partial backup and restore only when the
...
donor-level record stream that can be translated into SBLR or parser-support
UDR calls.
...
parser decodes donor records and lowers them to ScratchBird calls. The engine
...
authorized engine reads from the single connected emulated donor database and
renders donor-compatible logical records back to the client. Full and partial
streams are allowed only where the donor normally exposes that scope.

The matrix for each donor parser is:

`project/tests/donor_sql_parser_first_tranche/logical_stream_backup_restore_policy_matrix.csv`

The implementation route is not considered complete until the relevant donor
...
Firebird must explicitly allow donor-real `gbak` logical remote service
