...
bool ExpectFunctionalEncoding(std::string_view payload,
  ...
  ok &= ExpectField(
      ...
      label);
  ok &= ExpectBool(payload, "donor_cst_materialized", true, label);
  ok &= ExpectBool(payload, "donor_ast_materialized", true, label);
  ok &= ExpectBool(payload, "donor_bound_ast_materialized", true, label);
  ...
  if (firebird_payload) {
    ...
    ok &= ExpectField(payload, "runtime_equivalence_status",
                      "pending_donor_native_psql_replay", label);
  }
...
bool ExpectDirectResult(const Result& result,
  ...
  ok &= ExpectBool(result.sblr_envelope, "donor_engine_sql_executed", false,
                   label);
...
bool ExpectUdrResult(const UdrResult& result,
  ...
  ok &= ExpectBool(result.payload, "donor_engine_sql_executed", false, label);
