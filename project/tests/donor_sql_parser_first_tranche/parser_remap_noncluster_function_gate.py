...
"""FPR-P1 non-cluster donor remap proof gate."""
...
P1_REL = (
    "public_execution_plan"
    "SBLR_OPERATION_EXPANSION_REGISTER.csv"
)
...
def validate_authority(row: dict[str, str], proof: dict[str, str]) -> None:
    ...
    require("donor_execution_storage_transaction_finality_forbidden" in combined,
...
def validate_symbols(
    ...
    for row in matrix_rows:
        ...
        for token in uppercase_tokens:
            if token.startswith("SBLR_DONOR_"):
                donor_symbol_rows += 1
                require(token in backfill_symbols, f"donor SBLR symbol missing from backfill: {token}")
            else:
                native_symbol_rows += 1
                require(token in engine_symbols, f"native SBLR symbol missing from engine matrix: {token}")
    return {
        "donor_prefixed_sblr_symbol_rows": donor_symbol_rows,
...
def main() -> int:
    ...
    for row in matrix_rows:
        ...
        require(row["donor"] == inventory["donor"], f"donor mismatch for {row['remap_id']}")
        require(row["parser_surface"] == inventory["parser_surface"],
                f"parser surface mismatch for {row['remap_id']}")
        require(row["donor_surface"] == inventory["donor_surface"],
                f"donor surface mismatch for {row['remap_id']}")
    ...
    require(backfill_symbols, "donor backfill register has no parser symbols")
    ...
    evidence = {
        ...
        "donor_backfill_symbols_read": len(backfill_symbols),
        **symbol_counts,
    }
