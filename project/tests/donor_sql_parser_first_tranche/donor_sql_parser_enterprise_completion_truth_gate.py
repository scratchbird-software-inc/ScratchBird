...
"""Enterprise completion truth gate for first-tranche donor SQL parsers.
...
PROCEDURAL_DATATYPE_EXECUTION_PLAN = (
    EXECUTION_PLAN_ROOT + "/" + "donor-parser-procedural-datatype-enterprise-closure"
)
...
DIALECTS = {
    "firebird": {
        "display": "FirebirdSQL",
        "native_tool_manifest": "project/tests/donor_regression/firebird/native_tool_harness/native_tool_harness_manifest.csv",
        "execution-plans": (
            EXECUTION_PLAN_ROOT + "/donor-parser-firebird-implementation-readiness",
            EXECUTION_PLAN_ROOT + "/full-firebirdsql-parser-udr-emulation-implementation-closure",
        ),
        "completed_execution-plans": (
            COMPLETED_EXECUTION_PLAN_ROOT + "/donor-parser-firebird-implementation-readiness",
            COMPLETED_EXECUTION_PLAN_ROOT + "/full-firebirdsql-parser-udr-emulation-implementation-closure",
        ),
        "project_tests": (
            "project/tests/donor_regression/firebird",
            "project/tests/firebird_parser_worker/fixtures/full_firebirdsql_parser_udr_emulation_closure",
        ),
    },
    "mysql": {
        "display": "MySQL",
        "native_tool_manifest": "project/tests/donor_regression/mysql/native_tool_harness/native_tool_harness_manifest.csv",
        "execution-plans": (
            EXECUTION_PLAN_ROOT + "/donor-parser-mysql-implementation-readiness",
        ),
        "completed_execution-plans": (
            COMPLETED_EXECUTION_PLAN_ROOT + "/donor-parser-mysql-implementation-readiness",
        ),
        "project_tests": (
            "project/tests/donor_regression/mysql",
        ),
    },
    "postgresql": {
        "display": "PostgreSQL",
        "native_tool_manifest": "project/tests/donor_regression/postgresql/native_tool_harness/native_tool_harness_manifest.csv",
        "execution-plans": (
            EXECUTION_PLAN_ROOT + "/donor-parser-postgresql-implementation-readiness",
        ),
        "completed_execution-plans": (
            COMPLETED_EXECUTION_PLAN_ROOT + "/donor-parser-postgresql-implementation-readiness",
        ),
        "project_tests": (
            "project/tests/donor_regression/postgresql",
        ),
    },
}
...
NON_BLOCKING_STATUSES = {
    "available",
    "verified_present",
    "source_reviewed_from_recorded_donor_release_packet",
...
CSV_ENTERPRISE_CLAIM_PHRASES = (
    ...
    "observable donor equivalence",
    "semantic defaults observable equivalence",
    "procedural datatype",
    "procedural/API encoding",
    "exact datatypes",
    "donor-native proof behavior",
)

NATIVE_REPLAY_COMPLETION_STATUSES = {
    "native_replay_passed",
    "original_donor_replay_passed",
    "original_donor_tool_replay_passed",
    "passed",
}
    ...
    if not root.is_dir():
        blockers.append(
            Blocker(
                ...
                row_id="donor-parser-procedural-datatype-enterprise-closure",
...
def collect_native_tool_replay(repo_root: pathlib.Path,
    ...
    if not path.is_file():
        return {
            ...
            "original_donor_tools_required": True,
...
def collect_first_tranche_replay_evidence(path: pathlib.Path | None) -> dict[str, object]:
    ...
    if data.get("gate") != "donor_sql_first_tranche_original_tool_replay_gate":
    ...
    if data.get("donor_tools_are_storage_authority") is not False:
        raise AssertionError("first-tranche replay evidence gives donor tools storage authority")
    if data.get("donor_tools_are_transaction_authority") is not False:
        raise AssertionError("first-tranche replay evidence gives donor tools transaction authority")
        ...
        dialect_evidence.append(
            {
                ...
                "original_donor_tool_replay": native_tool_replay,
    ...
    native_replay_incomplete = [
        {
            ...
            "manifest": item["original_donor_tool_replay"]["manifest"],
            "incomplete_replay_rows": item["original_donor_tool_replay"].get(
                "incomplete_replay_rows", []
            ),
            "missing_tool_locators": item["original_donor_tool_replay"].get(
                "missing_tool_locators", []
            ),
        }
        for item in dialect_evidence
        if not item["original_donor_tool_replay"]["replay_completed"]
    ]
    all_blockers.extend(global_blockers)
    all_blockers.extend(runtime_replay_blockers)

    return {
        "gate": "donor_sql_parser_enterprise_completion_truth_gate",
        ...
        "original_donor_tool_replay_required": True,
...
def main(argv: list[str]) -> int:
    ...
    if args.strict_release and evidence["native_replay_incomplete"]:
        raise AssertionError(
            "strict release mode requires original donor-tool replay completion: " +
            json.dumps(evidence["native_replay_incomplete"], sort_keys=True)
        )

    print(
        "donor_sql_parser_enterprise_completion_truth_gate="
...
if __name__ == "__main__":
    try:
        ...
        print(f"donor_sql_parser_enterprise_completion_truth_gate: {exc}",
              file=sys.stderr)
        raise SystemExit(1)
