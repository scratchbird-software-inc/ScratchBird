...
struct ScalarExpected {
  ...
  bool donor_conditional_function_surface{false};
};
...
bool ExpectScalarEvidence(std::string_view payload,
  ...
  ok &= ExpectField(payload, "evidence_contract",
                    "donor_scalar_expression_semantic_descriptor_evidence.v1",
                    label);
  ok &= ExpectField(payload, "descriptor_resolution", "uuid_required", label);
  ...
  ok &= ExpectBool(payload, "donor_conditional_function_surface",
                   expected.donor_conditional_function_surface, label);
