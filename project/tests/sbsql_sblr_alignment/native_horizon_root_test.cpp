...
Bytes Oracle(const p::NativeHorizonRoot& v){const auto& h=v.header;Bytes b(h.page_size_bytes,0);
  ...
  std::copy_n("SBHOR002",8,b.begin()+128);Num(b,136,2,2);Num(b,138,2,384);Num(b,140,4,512+192*v.records.size());Put(b,144,v.object_uuid);Num(b,160,8,v.epoch);Put(b,168,v.creator_transaction_uuid);Num(b,184,8,v.creator_local_transaction_id);
  ...
  for(std::size_t i=0;i<v.records.size();++i){const auto at=512+i*192;const auto& r=v.records[i];Num(b,at,2,static_cast<u16>(r.kind));Num(b,at+2,2,static_cast<u16>(r.owner_kind));Num(b,at+4,4,r.flags);Num(b,at+8,8,r.local_boundary);
    Put(b,at+16,r.owner_uuid);Put(b,at+32,r.pin_uuid);Put(b,at+48,r.checkpoint_object_uuid);Num(b,at+64,8,r.checkpoint_generation);Put(b,at+72,r.diagnostic_uuid);if(r.checkpoint)Ref(b,at+88,*r.checkpoint);Put(b,at+136,r.horizon_uuid);Put(b,at+152,r.timeline_uuid);
  }Seal(b);return b;
}
...
p::NativeHorizonRoot Example(unsigned profile=0){const auto z=Zero(profile,2);p::NativeHorizonRoot v;
  ...
  p::NativeHorizonRecord r;r.flags=1;r.local_boundary=8;r.owner_uuid=Id(90);r.pin_uuid=Id(92);r.checkpoint_object_uuid=Id(69);r.checkpoint_generation=2;r.diagnostic_uuid=Id(93);r.checkpoint=d::NativePageReference{Id(2),19,109,z.bootstrap.page_size_profile_uuid};r.horizon_uuid=Id(180);r.timeline_uuid=Id(95);v.records.push_back(r);return v;
}
...
void Images(){
  ...
  for(unsigned profile=0;profile<5;++profile){auto v=Example(profile);v.records.clear();v.total_records=0;v.minimum_blocker=0;Check(p::EncodeNativeHorizonRoot(v).ok()&&p::DecodeNativeHorizonRoot(Oracle(v)).ok(),"represent empty image without cleanup grant");
    v=Example(profile);auto r=v.records[0];r.owner_uuid=Id(91);r.horizon_uuid=Id(181);r.local_boundary=4;v.records.push_back(r);v.total_records=2;v.minimum_blocker=4;Check(p::EncodeNativeHorizonRoot(v).bytes==Oracle(v),"minimum blocker is independently computed");
    const unsigned capacity=(v.header.page_size_bytes-512)/192;v.records.clear();v.total_records=capacity;v.minimum_blocker=0;r.flags=0;r.diagnostic_uuid={};
    for(unsigned i=0;i<capacity;++i){r.owner_uuid=Id(0);r.owner_uuid.bytes[14]=i>>8;r.owner_uuid.bytes[15]=i&255;r.horizon_uuid=r.owner_uuid;r.horizon_uuid.bytes[13]=10;v.records.push_back(r);}
  ...
  for(auto offset:{128u,136u,138u,140u,216u,220u,440u,680u,704u}){auto b=good;b[offset]^=1;Seal(b);Empty(p::DecodeNativeHorizonRoot(b));}
  ...
  for(unsigned n=0;n<4;++n){auto value=Example();auto& id=n<2?value.records[0].horizon_uuid:value.records[0].timeline_uuid;if(n%2)id.bytes[6]=0x40;else id={};Empty(p::EncodeNativeHorizonRoot(value));Empty(p::DecodeNativeHorizonRoot(Oracle(value)));}
  auto primary=Example();auto duplicate=primary.records[0];duplicate.owner_uuid=Id(200);primary.records.push_back(duplicate);primary.total_records=2;Empty(p::EncodeNativeHorizonRoot(primary));Empty(p::DecodeNativeHorizonRoot(Oracle(primary)));
  auto unique=Example();duplicate=unique.records[0];duplicate.horizon_uuid=Id(181);duplicate.pin_uuid=Id(200);unique.records.push_back(duplicate);unique.total_records=2;Empty(p::EncodeNativeHorizonRoot(unique));Empty(p::DecodeNativeHorizonRoot(Oracle(unique)));
  auto legacy=Oracle(Example());std::copy_n("SBHOR001",8,legacy.begin()+128);Num(legacy,136,2,1);Seal(legacy);Empty(p::DecodeNativeHorizonRoot(legacy));
}
...
const auto persist=[&](auto a,auto b){const auto bytes=Oracle(b);if(a.next)a.next_sha256=Hash(bytes);else a.next_sha256.fill(0);const auto zero1=d::EncodeFilespacePageZero(z1),zero2=d::EncodeFilespacePageZero(z2);Check(zero1.ok()&&zero2.ok(),"encode enclosing bootstrap fixtures");put(first,0,z1.bootstrap.page_size_bytes,*zero1.bytes);put(second,0,z2.bootstrap.page_size_bytes,*zero2.bytes);put(first,24,z1.bootstrap.page_size_bytes,Oracle(a));put(second,25,z2.bootstrap.page_size_bytes,bytes);};
  ...
  auto duplicate_id=tail;duplicate_id.records[0].horizon_uuid=head.records[0].horizon_uuid;persist(head,duplicate_id);result=read(budget);Empty(result);Check(result.error==E::chain_mismatch,"primary horizon identity unique across actual leaves");
  auto duplicate_key=tail;duplicate_key.records[0].kind=head.records[0].kind;duplicate_key.records[0].pin_uuid=Id(200);persist(head,duplicate_key);result=read(budget);Empty(result);Check(result.error==E::chain_mismatch,"different pins do not create distinct catalog unique keys");
  auto other_timeline=tail;other_timeline.records[0].kind=head.records[0].kind;other_timeline.records[0].timeline_uuid=Id(96);persist(head,other_timeline);result=read(budget);Check(result.ok()&&result.pages[0].root->records[0].horizon_uuid==Id(180)&&result.pages[1].root->records[0].horizon_uuid==Id(181)&&result.pages[1].root->records[0].timeline_uuid==Id(96),"distinct lineage and stable identities preserved in actual chain");
