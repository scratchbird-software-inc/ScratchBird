...
[[noreturn]] void Fail(const char* message) {
  std::cerr << "CSC-TEST-003615: " << message << '\n';
...
struct Fixture {
  ...
  ~Fixture() {
    ...
    std::filesystem::remove(
        database_path +
            ".sb.sblr_executor_availability_registry.v1.name_resolve",
        ignored);
  }
};
...
api::EngineRequestContext Context(const Fixture& fixture,
  ...
  context.database_uuid = scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003615");
...
api::SblrExecutorAvailabilityRowIdentity Identity() {
  return {api::kSblrNameResolveExecutorId,
          api::kSblrNameResolveOpcodeCode,
          api::kSblrNameResolveOpcodeVersion,
          api::kSblrNameResolveOperandDescriptorId,
          api::kSblrNameResolveResultDescriptorId,
          api::kSblrNameResolveResultDescriptorVersion};
}
...
int main() {
  ...
  auto admission_context = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003616"));
  ...
  Require(admitted.ok && admitted.cohort != nullptr &&
          ...
          "name-resolve admission cohort was not installed");
  ...
  revoke.reason_code = "CSC-TEST-003615";
  const auto revoked =
  ...
  Require(revoked.ok && !revoked.snapshot.installed &&
          ...
          "name-resolve executor revocation was not published");
  ...
  Require(diagnostic.code == "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING" &&
              ...
              !current.installed,
          "pinned cohort hid current name-resolve revocation");

  auto next_statement = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003617"));
  ...
  Require(observed.ok && observed.cohort != nullptr &&
              ...
              observed.rows.front().snapshot.availability_state ==
                  api::SblrExecutorAvailabilityState::revoked,
          "next statement did not observe revoked name-resolve authority");
  return EXIT_SUCCESS;
}
