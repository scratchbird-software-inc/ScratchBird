...
int main(){EngineRequestContext c;c.security_context_present=true;c.statement_metadata_snapshot_engine_owned=true;c.statement_uuid=scratchbird::tests::FixtureUuid(0xc008, 1);c.trace_tags={"private_bitemporal_for_versions_between_binder","cluster_provider_admitted","cluster_route_fence_admitted"};auto q=CompileSblrBitemporalForVersionsBetweenDescriptor(c,c.statement_uuid,7);assert(q.ok);c.trace_tags.push_back("private_bitemporal_for_versions_between");assert(ConsumeSblrBitemporalForVersionsBetweenDescriptor(c,q.descriptor).ok);assert(!ConsumeSblrBitemporalForVersionsBetweenDescriptor(c,q.descriptor).ok);return 0;}
