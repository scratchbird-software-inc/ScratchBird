...
int main() {
  ...
  SblrDmlCounterAddDescriptorV1 d; d.availability = 1;
  ...
  assert(EncodeSblrDmlCounterAddDescriptorV1(d2, true) == dw); dw[416] ^= 1;
  assert(!DecodeSblrDmlCounterAddDescriptorV1(dw.data(), dw.size(), &d2, nullptr, true));
  SblrDmlCounterAddResultV1 r; r.availability = 1; r.publication_barrier[0] = 4;
  ...
  assert(EncodeSblrDmlCounterAddResultV1(r2) == rw); rw[256] ^= 1;
  assert(!DecodeSblrDmlCounterAddResultV1(rw.data(), rw.size(), &r2, nullptr));
  return 0;
}
