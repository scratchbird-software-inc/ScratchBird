...
struct Fixture {
  ...
  ~Fixture() {
    ...
    std::filesystem::remove(
        database_path +
            ".sb.sblr_executor_availability_registry.v1.parameter_bind",
        ignored);
  }
};
...
Fixture MakeFixture() {
  ...
  fixture.database_path =
      (std::filesystem::temp_directory_path() /
       ("sb_parameter_bind_availability_" +
...
api::EngineRequestContext Context(const Fixture& fixture,
  ...
  context.database_uuid = scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003595");
...
api::SblrExecutorAvailabilityRowIdentity ParameterBindIdentity() {
  return {api::kSblrParameterBindExecutorId,
          api::kSblrParameterBindOpcodeCode,
          api::kSblrParameterBindOpcodeVersion,
          api::kSblrParameterBindOperandDescriptorId,
          api::kSblrParameterBindResultDescriptorId,
          api::kSblrParameterBindResultDescriptorVersion};
}
...
int main() {
  ...
  auto admission_context = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003596"));
  const auto identity = ParameterBindIdentity();
  ...
  Require(admitted.ok && admitted.cohort != nullptr &&
          ...
          "parameter-bind admission cohort was not installed");
  ...
  revoke.expected_snapshot_uuid =
      admitted.rows.front().snapshot.snapshot_uuid;
  ...
  revoke.reason_code = "CSC-TEST-003595";
  const auto revoked =
  ...
  Require(revoked.ok && !revoked.snapshot.installed &&
          ...
          "parameter-bind executor revocation was not published");
  ...
  Require(diagnostic.code == "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING" &&
              ...
              !current.installed,
          "pinned admission cohort hid the current executor revocation");

  auto next_statement = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003597"));
