...
[[noreturn]] void Fail(const char* message) {
  std::cerr << "CSC-TEST-003635: " << message << '\n';
...
struct Fixture {
  ...
  ~Fixture() {
    ...
    std::filesystem::remove(
        database_path +
            ".sb.sblr_executor_availability_registry.v1.database_attach",
        ignored);
  }
};
...
Fixture MakeFixture() {
  ...
  fixture.database_path =
      (std::filesystem::temp_directory_path() /
       ("sb_database_attach_availability_" +
...
api::EngineRequestContext Context(const Fixture& fixture,
  ...
  context.database_uuid = scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003634");
...
api::SblrExecutorAvailabilityRowIdentity Identity() {
  return {api::kSblrDatabaseAttachExecutorId,
          api::kSblrDatabaseAttachOpcodeCode,
          api::kSblrDatabaseAttachOpcodeVersion,
          api::kSblrDatabaseAttachOperandDescriptorId,
          api::kSblrDatabaseAttachResultDescriptorId,
          api::kSblrDatabaseAttachResultDescriptorVersion};
}
...
int main() {
  ...
  auto statement = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003635"));
  ...
  Require(admitted.ok && admitted.cohort != nullptr &&
          ...
          "database-attach admission cohort was not installed");
  ...
  revoke.database_uuid = statement.database_uuid;
  ...
  revoke.reason_code = "CSC-TEST-003635";
  ...
  Require(revoked.ok && !revoked.snapshot.installed &&
          ...
          "database-attach executor revocation was not published");
  ...
  Require(diagnostic.code == "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING" &&
              ...
              !current.installed,
          "pinned cohort hid current database-attach revocation");

  auto next_statement = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003636"));
  ...
  Require(observed.ok && observed.cohort != nullptr &&
              ...
              observed.rows.front().snapshot.availability_state ==
                  api::SblrExecutorAvailabilityState::revoked,
          "next statement did not observe revoked database-attach authority");
  return EXIT_SUCCESS;
}
