...
int inventory_allocation_route=-1,inventory_allocation_shard=-1;const unsigned char* replacement_bytes=nullptr;std::size_t replacement_length=0;off_t replacement_offset=0;unsigned replacement_at=0,replacement_seen=0;}
...
struct Bundle {
 ...
 Bundle(Graph& g,unsigned inventory_count=0,unsigned placement_offset=0):graph(g){const u64 size=g.fixture.size,count=((g.after.size()+inventory_count)*size+size-385)/(size-384);
