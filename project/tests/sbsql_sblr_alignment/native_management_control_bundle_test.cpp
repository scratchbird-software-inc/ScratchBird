...
int main(){try{for(unsigned profile=0;profile<5;++profile){InventoryBundle(profile);Test(profile);}std::cout<<"PASS management control bundle checks="<<checks<<" reconstruction_input_only=true not_allocation_publication_or_SQL_E2E=true\n";return 0;}catch(const std::exception& e){allocation_budget=-1;hash_fault=0;io_counting=false;std::cerr<<"FAIL management control bundle checks="<<checks<<" "<<e.what()<<'\n';return 1;}}
