...
int main() {
  ...
  context.statement_uuid = scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000002960");
  context.trace_tags = {"private_ddl_drop_type_binder"};
  context.query_cancellation_requested = [&]() {
    ++cancellation_probes;
    return true;
  };

  const auto refused = api::CompileSblrDdlDropTypeDescriptor(
  ...
  const auto refused_again = api::CompileSblrDdlDropTypeDescriptor(
      context, context.statement_uuid, 1, 1, 1);
  ...
  const auto missing_occurrence = api::CompileSblrDdlDropTypeDescriptor(
  ...
  const auto missing_domain_occurrence = api::CompileSblrDdlDropTypeDescriptor(
  ...
  const auto missing_availability = api::CompileSblrDdlDropTypeDescriptor(
  ...
  const auto unbound = api::CompileSblrDdlDropTypeDescriptor(
  ...
  const auto snapshot_invalid = api::CompileSblrDdlDropTypeDescriptor(
  ...
  const auto receipt_invalid = api::CompileSblrDdlDropTypeDescriptor(
      context, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000002959"), 1, 1, 1);
  assert(!receipt_invalid.ok);
  assert(receipt_invalid.diagnostic.code == "SBLR.OPERAND_INVALID");
  assert(cancellation_probes.load() == 0);

  sblr::SblrDdlDropTypeDescriptorV1 seed;
  seed.body[0] = 1;
  seed.availability = 1;
  const auto encoded_seed =
      sblr::EncodeSblrDdlDropTypeDescriptorV1(seed, false);
  sblr::SblrDdlDropTypeDescriptorV1 canonical;
  std::string decode_detail;
  assert(!encoded_seed.empty());
  assert(sblr::DecodeSblrDdlDropTypeDescriptorV1(
      encoded_seed.data(), encoded_seed.size(), &canonical, &decode_detail,
      false));
  assert(!sblr::EncodeSblrDdlDropTypeDescriptorV1(canonical, true).empty());

  auto malformed_descriptor = canonical;
  malformed_descriptor.evidence[0] ^= 1;
  const auto malformed =
      api::ConsumeSblrDdlDropTypeDescriptor(context, malformed_descriptor);
  assert(!malformed.ok);
  ...
  const auto hidden =
      api::ConsumeSblrDdlDropTypeDescriptor(context, canonical);
  assert(!hidden.ok);
  ...
  unauthorized.trace_tags = {"private_ddl_drop_type"};
  const auto denied =
      api::ConsumeSblrDdlDropTypeDescriptor(unauthorized, canonical);
  assert(!denied.ok);
  assert(denied.diagnostic.code == "SECURITY.ACCESS_DENIED");
  assert(IsZero(denied.descriptor));
  assert(cancellation_probes.load() == 0);

  context.trace_tags = {"private_ddl_drop_type"};
