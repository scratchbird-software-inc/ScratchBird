#include "engine/internal_api/sblr_sec_alter_policy_coordinator.hpp"
#include "engine/internal_api/sblr_executor_availability_registry.hpp"

#include <chrono>
#include <cstdint>
#include <filesystem>
#include <string>

namespace {

scratchbird::engine::sblr::SblrSecAlterPolicyDescriptorV1 Descriptor() {
  scratchbird::engine::sblr::SblrSecAlterPolicyDescriptorV1 value;
  auto uuid = [](std::uint8_t seed) {
    scratchbird::engine::sblr::SecAlterPolicyUuid out{};
    for (std::size_t index = 0; index < out.size(); ++index) {
      out[index] = static_cast<std::uint8_t>(seed + index);
    }
    out[6] = static_cast<std::uint8_t>((out[6] & 0x0f) | 0x70);
    out[8] = static_cast<std::uint8_t>((out[8] & 0x3f) | 0x80);
    return out;
  };
  auto sha = [](std::uint8_t seed) {
    scratchbird::engine::sblr::SecAlterPolicySha out{};
    for (std::size_t index = 0; index < out.size(); ++index) {
      out[index] = static_cast<std::uint8_t>(seed + index);
    }
    return out;
  };
  value.receipt = uuid(1);
  value.occurrence = 1;
  value.policy_occurrence = 1;
  value.policy_uuid = uuid(20);
  value.expected_generation = 3;
  value.database_uuid = uuid(40);
  value.owning_transaction_uuid = uuid(60);
  value.owning_local_transaction_id = 7;
  value.statement_snapshot_uuid = uuid(80);
  value.catalog_epoch_uuid = uuid(100);
  value.catalog_generation = 11;
  value.security_context_uuid = uuid(120);
  value.security_generation = 13;
  value.policy_snapshot_uuid = uuid(140);
  value.policy_generation = 17;
  value.resource_grant_uuid = uuid(160);
  value.resource_generation = 19;
  value.principal_uuid = uuid(180);
  value.binding_uuid = uuid(200);
  value.recovery_uuid = uuid(220);
  value.binding_generation = 1;
  value.recovery_generation = 1;
  value.normalized_path_sha256 = sha(1);
  value.syntax_demand_sha256 = sha(40);
  value.authorization_evidence_sha256 = sha(80);
  value.frozen_policy_record_sha256 = sha(120);
  value.availability = 23;
  return value;
}

bool RequireSecurityPolicyShowAvailability() {
  namespace api = scratchbird::engine::internal_api;

  const auto nonce = std::chrono::steady_clock::now()
                         .time_since_epoch()
                         .count();
  api::EngineRequestContext context;
  context.database_path =
      (std::filesystem::temp_directory_path() /
       ("sb_security_policy_show_availability_" + std::to_string(nonce)))
          .string();
  context.database_uuid.canonical =
      "019d0000-0000-7000-8000-000000005823";
  context.security_context_present = true;
  context.trace_tags = {"right:SBLR_EXECUTOR_AVAILABILITY_ADMIN"};

  const api::SblrExecutorAvailabilityRowIdentity identity{
      api::kSblrSecurityPolicyShowExecutorId,
      api::kSblrSecurityPolicyShowOpcodeCode,
      api::kSblrSecurityPolicyShowOpcodeVersion,
      api::kSblrSecurityPolicyShowOperandDescriptorId,
      api::kSblrSecurityPolicyShowResultDescriptorId,
      api::kSblrSecurityPolicyShowResultDescriptorVersion};
  const auto registry_path = std::filesystem::path(
      context.database_path +
      ".sb.sblr_executor_availability_registry.v1.security_policy_show");

  const auto admitted = api::LoadSblrExecutorAvailabilitySnapshot(
      context, identity);
  if (!api::IsAdmittedExecutorAvailabilityIdentity(identity) ||
      !admitted.ok || !admitted.snapshot.installed ||
      admitted.snapshot.generation == 0 ||
      admitted.snapshot.row_identity_sha256 !=
          api::ComputeSblrExecutorAvailabilityRowIdentitySha256(identity) ||
      !std::filesystem::exists(registry_path)) {
    return false;
  }

  api::SblrExecutorAvailabilitySetRequest revoke;
  revoke.database_uuid = context.database_uuid.canonical;
  revoke.expected_snapshot_uuid = admitted.snapshot.snapshot_uuid;
  revoke.expected_generation = admitted.snapshot.generation;
  revoke.exact_row_identity = identity;
  revoke.requested_state = api::SblrExecutorAvailabilityState::revoked;
  revoke.reason_code = "CSC-TEST-005823";
  const auto revoked = api::SetSblrExecutorAvailability(context, revoke);
  api::SblrExecutorAvailabilitySnapshot current;
  const auto revalidated = api::RevalidateSblrExecutorAvailability(
      context, identity, admitted.snapshot, &current);
  const bool ok = revoked.ok && !revoked.snapshot.installed &&
                  revalidated.code ==
                      "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING" &&
                  !current.installed &&
                  current.generation == revoked.snapshot.generation;

  std::error_code cleanup_error;
  std::filesystem::remove(registry_path, cleanup_error);
  return ok;
}

}  // namespace

int main() {
  scratchbird::engine::internal_api::SblrSecAlterPolicyAuthorityInputV1 input;
  input.descriptor = Descriptor();
  const auto compiled = scratchbird::engine::internal_api::
      CompileSblrSecAlterPolicyDescriptor(input);
  return compiled.ok && RequireSecurityPolicyShowAvailability() ? 0 : 1;
}
