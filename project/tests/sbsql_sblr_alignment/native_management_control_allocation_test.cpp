...
struct Fixture {
 ...
 Fixture(unsigned profile){
  char name[]="/tmp/sb-management-control.XXXXXX";Check(mkdtemp(name),"isolated fixture directory");path=std::filesystem::path(name)/"node";
...
void Test(unsigned profile){
 ...
 Graph reuse(fixture);const auto occupied=reuse.before.front().header.page_number;auto replacement=Find(reuse.after,101);replacement.page_number=occupied;Find(reuse.after,occupied)=replacement;auto& source=Cover(reuse.after,101);source.states[101-source.first_page]=State::free;source.records.erase(std::remove_if(source.records.begin(),source.records.end(),[](const auto& r){return r.page_number==101;}),source.records.end());reuse.plan.target_checkpoint.page_number=occupied;reuse.Refresh();Check(reuse.CheckGraph()!=CE::none,"new checkpoint cannot overwrite a retained allocation map");
 Graph unendorsed(fixture);auto wrong_head=*db::DecodeNativeCheckpointRoot(unendorsed.target_bytes).root;wrong_head.roots.back().sha256[0]^=1;const auto changed_head=db::EncodeNativeCheckpointRoot(wrong_head);Check(changed_head.ok(),"valid checkpoint with wrong plan endorsement");unendorsed.target_bytes=changed_head.bytes;Check(unendorsed.CheckGraph()!=CE::none,"role16 binds the full plan image independently of graph projection");
 for(unsigned fault=0;fault<15;++fault){Graph bad(fixture);switch(fault){
  case 0:Find(bad.after,100).creator_operation_uuid=Id(333);break;
  case 1:Find(bad.after,100).allocation_uuid=bad.before.front().records.front().allocation_uuid;break;
  case 2:Find(bad.after,100).owner_uuid=Id(900);break;
  case 3:Find(bad.after,100).page_generation=3;break;
  case 4:Find(bad.after,100).page_uuid=Id(901);break;
  case 5:Find(bad.after,100).page_type=0x300;break;
  case 6:Find(bad.after,0).owner_uuid=Id(902);break;
  case 7:Cover(bad.after,220).states[220-Cover(bad.after,220).first_page]=State::quarantined;break;
  case 8:for(auto& m:bad.after)m.capacity_generation++;break;
  case 9:for(auto& m:bad.after)m.creator_operation_uuid=Id(333);break;
  case 10:for(auto& m:bad.after)m.map_generation=3;break;
  case 11:bad.target.selected_local_transaction_id=bad.target.stable_local_transaction_id=bad.target.local_durable_transaction_id=2;break;
  case 12:bad.target.roots[1].sha256[0]^=1;break;
  case 13:{auto& m=Cover(bad.after,100);m.states[100-m.first_page]=State::quarantined;m.records.erase(std::remove_if(m.records.begin(),m.records.end(),[](const auto& r){return r.page_number==100;}),m.records.end());break;}
  case 14:Find(bad.after,100).allocation_uuid=Find(bad.after,101).allocation_uuid;break;
 }bad.Refresh();Check(bad.CheckGraph()!=CE::none,"resealed invalid control allocation delta");}
 for(unsigned group=0;group<6;++group){Graph bad(fixture);Bytes* raw=group==0?&bad.base_bytes:group==1?&bad.target_bytes:group==2?&bad.plan_bytes:group==3?&bad.extent.front():group==4?&bad.before_bytes.front():&bad.after_bytes.front();raw->back()^=1;Check(bad.CheckGraph()!=CE::none,"every input family integrity");}
 counting=true;allocations=0;const auto result=graph.CheckGraph();counting=false;Check(result==CE::none,"allocation baseline");const auto sites=allocations;
 for(unsigned long at=0;at<sites;++at){allocation_budget=at;const auto e=graph.CheckGraph();allocation_budget=-1;Check(e==CE::resource_exhausted,"every required allocation failure");}
 hash_counting=true;hash_seen=0;Check(graph.CheckGraph()==CE::none,"hash baseline");hash_counting=false;const auto hashes=hash_seen;
 for(unsigned mode=1;mode<=5;++mode)for(unsigned at=1;at<=hashes;++at){hash_fault=mode;hash_target=at;hash_seen=0;hash_active=false;const auto e=graph.CheckGraph();const bool consumed=!hash_fault;hash_fault=0;Check(consumed&&e==CE::hash_failure,"all hashes preserve operational failure");}
 reads=writes=syncs=0;io_counting=true;Check(graph.CheckGraph()==CE::none,"no I/O validation");io_counting=false;Check(!reads&&!writes&&!syncs,"image verifier performs no I/O");
 std::cout<<"control allocation allocations="<<sites<<" hashes="<<hashes<<" bytes="<<graph.Budget()<<"\n";
}
}
int main(){try{for(unsigned p=0;p<5;++p)Test(p);std::cout<<"PASS management control allocation checks="<<checks<<" image_delta_only=true not_publication_authority_or_SQL_E2E=true\n";return 0;}catch(const std::exception& e){allocation_budget=-1;hash_fault=0;io_counting=false;std::cerr<<"FAIL management control allocation checks="<<checks<<" "<<e.what()<<'\n';return 1;}}
