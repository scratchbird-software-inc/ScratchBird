...
int main(){
  ...
  c.statement_metadata_snapshot_engine_owned=true; c.statement_uuid=scratchbird::tests::FixtureUuid(0xc008, 1);
  c.trace_tags={"private_ddl_drop_macro_binder"};
  auto x=CompileSblrDdlDropMacroDescriptor(c,c.statement_uuid,1,1,1); assert(x.ok);
