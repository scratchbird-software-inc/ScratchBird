...
struct Fixture {
 ...
 Fixture(unsigned profile){
  char name[]="/tmp/sb-management-install.XXXXXX";Check(mkdtemp(name),"isolated fixture directory");path=std::filesystem::path(name)/"node";
...
using PE=db::NativePublicationError;using Pages=std::vector<Bytes>;
...
void Test(unsigned profile){
 Fixture f(profile);const auto prior=db::InspectNativePublicationGenerationOnOpenDevices(Id(1),f.devices,Id(2),f.budget);Check(prior.ok(),"actual origin");
 db::NativePublicationIntent intent;intent.initiator_uuid=Id(310);intent.request_context_uuid=Id(311);intent.policy_snapshot_uuid=Id(312);intent.initiator_kind=4;intent.normalized_request_sha256.fill(13);
 auto reservation=db::ReserveNativePublicationGenerationOnOpenDevices(Id(1),f.devices,Id(2),*prior.snapshot,Id(300),f.budget,&intent);Check(reservation.ok(),"actual intent lease");const auto original=reservation.lease->snapshot();const auto w=original.watermark;const auto bare=f.Read(0,64);
 auto o=records::Record(2*(f.size-384)+17);o.database_uuid=Id(1);o.bootstrap_uuid=w.bootstrap_uuid;o.uuid=Id(333);o.initiator_uuid=intent.initiator_uuid;o.initiator_kind=intent.initiator_kind;o.request_context_uuid=intent.request_context_uuid;o.policy_snapshot_uuid=intent.policy_snapshot_uuid;o.normalized_request_sha256=intent.normalized_request_sha256;for(auto& s:o.steps)s.operation_uuid=o.uuid;
 std::vector<d::NativeCommonPageHeader> headers;for(unsigned i=0;i<3;++i)headers.push_back({static_cast<u32>(f.size),0x500,Id(1),Id(2),Id(6100+i),48+i,w.watermark,0,w.header.page_size_profile_uuid});
 const auto extent=db::EncodeNativeManagementExtent(o,Id(5000),headers,f.budget);Check(extent.ok()&&extent.pages==records::ExtentOracle(o,headers),"independent operation pages");const auto pages=extent.pages;
 db::NativePublicationPlan p;p.header={static_cast<u32>(f.size),0x500,Id(1),Id(2),Id(301),30,w.watermark,0,w.header.page_size_profile_uuid};p.object_uuid=Id(302);p.bootstrap_uuid=w.bootstrap_uuid;p.timeline_uuid=w.timeline_uuid;p.operation_uuid=w.operation_uuid;p.intent=intent;p.security_snapshot_uuid=o.security_snapshot_uuid;p.reservation_state_sha256=original.state_sha256;p.base_checkpoint_sha256=w.base_checkpoint_sha256;p.reserved_generation=w.watermark;p.base_checkpoint_generation=w.base_checkpoint_generation;p.base_root_set_generation=w.base_root_set_generation;p.target_root_set_generation=w.watermark;p.base_checkpoint=w.base_checkpoint;p.target_checkpoint={Id(2),31,w.watermark,w.header.page_size_profile_uuid};p.base_checkpoint_object_uuid=p.target_checkpoint_object_uuid=w.base_checkpoint_object_uuid;p.catalog_generation=0;p.configuration_generation=1;p.security_generation=2;p.management_extent=extent.root;
 const auto old=db::DecodeNativeCheckpointRoot(f.Read(w.base_checkpoint.page_number));Check(old.ok(),"actual checkpoint");auto cp=*old.root;cp.header.page_uuid=Id(304);const auto target=Finish(p,cp),plan_bytes=Oracle(p);const auto decoded=db::DecodeNativePublicationPlan(plan_bytes);Check(decoded.ok()&&decoded.plan->management_extent==p.management_extent&&decoded.plan->generation_guard_flags==7&&decoded.plan->catalog_generation==0,"V2 independent bytes preserve present zero");
 const auto install=[&]{return db::InstallNativeManagementPublicationOnLease(*reservation.lease,p,target,pages,f.budget);};
 const auto restore=[&]{Reset();reservation.lease.reset();const auto wr=f.device.WriteAt(0,bare.data(),bare.size());Check(wr.ok()&&f.device.Sync().ok(),"restore owned isolated origin");
   const auto inspected=db::InspectNativePublicationGenerationOnOpenDevices(Id(1),f.devices,Id(2),f.budget);Check(inspected.ok(),"inspect restored origin");reservation=db::ResumeNativePublicationGenerationOnOpenDevices(Id(1),f.devices,Id(2),*inspected.snapshot,p.operation_uuid,intent,f.budget);Check(reservation.ok(),"resume exact origin");};
 const auto resume=[&]{Reset();reservation.lease.reset();const auto inspected=db::RecoverNativePublicationGenerationOnOpenDevices(Id(1),f.devices,Id(2),f.budget);Check(inspected.ok(),"explicit actual metadata recovery");reservation=db::ResumeNativePublicationGenerationOnOpenDevices(Id(1),f.devices,Id(2),*inspected.snapshot,p.operation_uuid,intent,f.budget);Check(reservation.ok(),"resume anchored operation");};
 const auto verify=[&]{const auto all=f.Read(0,64);auto expected=bare;
   for(u64 page:{w.header.page_number,w.header.page_number+1})std::copy_n(all.begin()+page*f.size,f.size,expected.begin()+page*f.size);
   std::copy(plan_bytes.begin(),plan_bytes.end(),expected.begin()+30*f.size);for(unsigned i=0;i<3;++i)std::copy(pages[i].begin(),pages[i].end(),expected.begin()+(48+i)*f.size);
   Check(all==expected,"only exact plan extent and duplex slots changed");for(u64 page:{w.header.page_number,w.header.page_number+1}){const auto wm=db::DecodeNativePublicationWatermark(f.Read(page));Check(wm.ok()&&wm.state->publication_plan&&wm.state->publication_plan->sha256==Sha(plan_bytes),"both durable anchors bind full V2 plan");}
   const auto record=db::ReadNativeManagementExtentFromOpenDevice(f.devices[0],*p.management_extent,Id(1),p.bootstrap_uuid,f.budget);Check(record.ok()&&*record.record==o,"actual complete installed record");Check(reservation.lease->snapshot().selection.checkpoint_generation==1&&reservation.lease->snapshot().watermark.watermark==2,"installation does not select history or activate transaction");};
 if(allocation_shard>=0){counting=true;allocations=0;const auto baseline=install();counting=false;const auto sites=allocations;Check(baseline.ok(),"allocation baseline");restore();
  for(unsigned long at=allocation_shard;at<sites;at+=4){const auto lost=f.device.failed_io_latency_observations();Reset();io_counting=true;allocation_budget=at;const auto result=install();allocation_budget=-1;io_counting=false;
   if(result.ok()){Check(f.device.failed_io_latency_observations()==lost+1,"only explicit telemetry loss permits success");verify();restore();}
   else{if(result.error!=PE::resource_exhausted||writes)std::cerr<<"allocation at="<<at<<" sites="<<sites<<" error="<<unsigned(result.error)<<" writes="<<writes<<'\n';Check(result.error==PE::resource_exhausted&&!result.snapshot&&writes==0,"all required allocation faults precede writes");Check(f.Read(0,64)==bare&&reservation.lease->snapshot().state_sha256==original.state_sha256,"allocation failure preserves disk and lease");}}
  std::cout<<"installation allocations="<<sites<<" shard="<<allocation_shard<<"\n";return;
 }
 Reset();io_counting=true;hash_counting=true;hash_seen=0;const auto result=install();hash_counting=false;io_counting=false;const auto read_sites=reads,write_sites=writes,sync_sites=syncs,hash_sites=hash_seen;
 Check(result.ok()&&write_sites==6&&sync_sites==5,"actual anchor plan and three-page writes");verify();
 Reset();io_counting=true;Check(install().ok(),"same lease retry");io_counting=false;Check(writes==0&&syncs==5,"retry syncs without rewriting");verify();
 // A failed already-anchored retry must poison the lease even if its hash is unchanged.
 Reset();read_fault=read_sites;io_counting=true;const auto interrupted=install();io_counting=false;read_fault=0;Check(!interrupted.ok()&&!interrupted.snapshot,"already anchored retry interruption");Check(install().error==PE::stale_base,"ambiguous lease cannot be reused even with unchanged anchor");resume();Check(install().ok(),"explicit resumed retry");verify();
 reservation.lease.reset();Check(f.device.Close().ok()&&f.device.Open(f.path.string(),d::FileOpenMode::open_existing).ok(),"owned reopen");resume();Check(install().ok(),"reopened retry");verify();
 restore();Reset();io_counting=true;const auto old_api=db::InstallNativePublicationPlanOnLease(*reservation.lease,p,target,f.budget);io_counting=false;Check(old_api.error==PE::invalid_request&&writes==0,"V1 API cannot partly install V2");
 for(unsigned i=0;i<12;++i){auto changed=p;switch(i){case 0:changed.intent.initiator_uuid=Id(88);break;case 1:changed.intent.request_context_uuid=Id(88);break;case 2:changed.intent.policy_snapshot_uuid=Id(88);break;case 3:changed.intent.initiator_kind=1;break;case 4:changed.intent.normalized_request_sha256[0]^=1;break;case 5:changed.security_snapshot_uuid=Id(88);break;case 6:changed.catalog_generation=1;break;case 7:changed.configuration_generation=2;break;case 8:changed.security_generation=3;break;case 9:changed.generation_guard_flags=6;break;case 10:changed.management_extent->revision++;break;case 11:changed.header.page_uuid=headers[0].page_uuid;break;}Check(db::BindNativePublicationPlanToManagementExtent(changed,pages,f.budget)!=E::none,"every provenance field exactly bound");}
 for(std::size_t at:{136u,138u,140u,720u,724u,735u,736u,784u,800u,816u,824u,828u,832u,864u,896u,1023u,1024u}){auto bad=plan_bytes;bad[at]^=0x80;PlanSeal(bad);const auto image=db::DecodeNativePublicationPlan(bad);if(image.ok())Check(db::BindNativePublicationPlanToManagementExtent(*image.plan,pages,f.budget)!=E::none,"resealed changed root cannot bind");}
 for(u64 page:{w.header.page_number,u64{63}}){auto h=headers;for(unsigned i=0;i<3;++i)h[i].page_number=page+i;const auto e=db::EncodeNativeManagementExtent(o,Id(5000),h,f.budget);Check(e.ok(),"bad physical range still encoded");auto changed=p;changed.management_extent=e.root;const auto graph=Finish(changed,cp);Reset();io_counting=true;const auto r=db::InstallNativeManagementPublicationOnLease(*reservation.lease,changed,graph,e.pages,f.budget);io_counting=false;Check(r.error==PE::allocation_mismatch&&writes==0,"actual allocated or beyond-file range refuses before writes");}
 Bytes junk(f.size,0x5a);const auto wr=f.device.WriteAt(49*f.size,junk.data(),junk.size());Check(wr.ok()&&f.device.Sync().ok(),"unowned nonzero fixture");Reset();io_counting=true;const auto unowned=install();io_counting=false;Check(unowned.error==PE::preimage_changed&&writes==0,"cannot overwrite unanchored nonzero extent");restore();
 auto cluster=o;cluster.scope=db::NativeManagementScope::cluster;cluster.cluster_uuid=Id(334);cluster.generation_guards[3]=1;const auto ce=db::EncodeNativeManagementExtent(cluster,Id(5000),headers,f.budget);Check(ce.ok(),"cluster record remains representable");auto cluster_plan=p;cluster_plan.management_extent=ce.root;const auto cluster_graph=Finish(cluster_plan,cp);Reset();io_counting=true;const auto cluster_refusal=db::InstallNativeManagementPublicationOnLease(*reservation.lease,cluster_plan,cluster_graph,ce.pages,f.budget);io_counting=false;Check(cluster_refusal.error==PE::cluster_requires_authority&&!cluster_refusal.snapshot&&writes==0,"local installation cannot substitute for cluster authority");
 if(profile==0){
  for(unsigned kind=0;kind<3;++kind)for(unsigned at=1;at<=(kind==0?read_sites:kind==1?write_sites:sync_sites);++at){restore();Reset();(kind==0?read_fault:kind==1?write_fault:sync_fault)=at;io_counting=true;const auto failed=install();io_counting=false;Check(!failed.ok()&&!failed.snapshot&&reservation.lease->snapshot().state_sha256==original.state_sha256,"every I/O fault withholds snapshot");resume();Check(install().ok(),"I/O recover resume retry");verify();}
  restore();for(unsigned mode=1;mode<=5;++mode)for(unsigned at=1;at<=hash_sites;++at){Reset();io_counting=true;hash_fault=mode;hash_target=at;hash_seen=0;hash_active=false;const auto failed=install();const bool consumed=!hash_fault;hash_fault=0;io_counting=false;Check(consumed&&failed.error==PE::hash_failure&&!failed.snapshot&&writes==0,"every hash failure precedes writes");Check(f.Read(0,64)==bare,"hash failure preserves origin");}
  for(unsigned at=read_sites-3;at<=read_sites;++at){restore();Reset();corrupt_read=at;io_counting=true;const auto bad_read=install();io_counting=false;corrupt_read=0;Check(bad_read.error==(at==read_sites-3?PE::preimage_changed:PE::readback_mismatch)&&!bad_read.snapshot,"every final extent readback and preimage must match complete bytes");Check(install().error==PE::stale_base,"comparison failure poisons lease");resume();Check(install().ok(),"readback failure explicit resumed repair");verify();}
  for(unsigned at=3;at<=6;++at)for(std::size_t prefix:{1u,384u,1024u}){restore();Reset();write_fault=at;torn_bytes=prefix;io_counting=true;const auto failed=install();io_counting=false;Check(!failed.ok()&&!failed.snapshot,"torn plan/extent withholds result");resume();Check(install().ok(),"repair exact owned torn extent");verify();}
  for(unsigned at:{3u,4u,6u}){restore();reservation.lease.reset();Check(f.device.Close().ok(),"close before process loss");const auto child=fork();Check(child>=0,"fork installer");if(!child){d::FileDevice own;if(!own.Open(f.path.string(),d::FileOpenMode::open_existing).ok())_exit(80);std::vector<d::NativeFilespaceDevice> files{{Id(2),p.header.page_size_profile_uuid,&own}};const auto inspected=db::InspectNativePublicationGenerationOnOpenDevices(Id(1),files,Id(2),f.budget);if(!inspected.ok())_exit(81);auto held=db::ResumeNativePublicationGenerationOnOpenDevices(Id(1),files,Id(2),*inspected.snapshot,p.operation_uuid,intent,f.budget);if(!held.ok())_exit(82);Reset();kill_write=at;io_counting=true;const auto ignored=db::InstallNativeManagementPublicationOnLease(*held.lease,p,target,pages,f.budget);(void)ignored;_exit(83);}
   int status=0;Check(waitpid(child,&status,0)==child&&WIFEXITED(status)&&WEXITSTATUS(status)==86,"actual process loss boundary");Check(f.device.Open(f.path.string(),d::FileOpenMode::open_existing).ok(),"parent reopen");resume();Check(!db::ReadNativeManagementExtentFromOpenDevice(f.devices[0],*p.management_extent,Id(1),p.bootstrap_uuid,f.budget).ok(),"metadata recovery does not invent complete extent");Check(install().ok(),"process loss exact resumed installation");verify();}
 }
 // Created and before-authorization terminal failure retain absence, not invented guards.
 for(unsigned state:{1u,15u}){restore();auto early=records::Example(state);early.database_uuid=o.database_uuid;early.bootstrap_uuid=o.bootstrap_uuid;early.uuid=o.uuid;early.initiator_uuid=o.initiator_uuid;early.initiator_kind=o.initiator_kind;early.request_context_uuid=o.request_context_uuid;early.policy_snapshot_uuid=o.policy_snapshot_uuid;early.normalized_request_sha256=o.normalized_request_sha256;early.security_snapshot_uuid=early.phase_uuid=early.resource_plan_uuid=early.lock_plan_uuid={};early.generation_guards={};
  auto h=headers;h.resize(1);const auto e=db::EncodeNativeManagementExtent(early,Id(5000),h,f.budget);Check(e.ok(),"early failure extent");auto ep=p;ep.security_snapshot_uuid={};ep.generation_guard_flags=0;ep.catalog_generation=ep.configuration_generation=ep.security_generation=0;ep.management_extent=e.root;const auto graph=Finish(ep,cp);const auto stored=db::InstallNativeManagementPublicationOnLease(*reservation.lease,ep,graph,e.pages,f.budget);Check(stored.ok(),"preauthorization state persists without fabricated security");const auto read=db::ReadNativeManagementExtentFromOpenDevice(f.devices[0],*ep.management_extent,Id(1),p.bootstrap_uuid,f.budget);Check(read.ok()&&*read.record==early,"early state actual roundtrip");}
 std::cout<<"installation reads="<<read_sites<<" writes="<<write_sites<<" syncs="<<sync_sites<<" hashes="<<hash_sites<<" profile="<<profile<<"\n";
}
}
int main(int argc,char** argv){try{if(argc==3&&std::string_view(argv[1])=="--allocations"){allocation_shard=std::stoi(argv[2]);Check(allocation_shard>=0&&allocation_shard<4,"shard range");}else Check(argc==1,"arguments");for(unsigned p=0;p<(allocation_shard<0?5u:1u);++p)Test(p);std::cout<<"PASS native management installation checks="<<checks<<" installed_unselected_only=true not_SQL_E2E=true\n";return 0;}catch(const std::exception& e){allocation_budget=-1;hash_fault=0;io_counting=false;std::cerr<<"FAIL management installation checks="<<checks<<" "<<e.what()<<'\n';return 1;}}
