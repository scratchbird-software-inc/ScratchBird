...
extern "C" int __wrap_EVP_PBE_scrypt(const char* password,std::size_t password_size,const unsigned char* salt,std::size_t salt_size,std::uint64_t n,std::uint64_t r,std::uint64_t p,std::uint64_t maxmem,unsigned char* out,std::size_t size) {
  if(scrypt_watch){scrypt_scratch=out;scrypt_scratch_size=size;}
...
void CryptoScrypt() {
  ...
  const auto watch=[] {scrypt_watch=true;scrypt_cleanses=0;scrypt_cleared=false;scrypt_scratch=nullptr;};
  ...
  for(const auto& v:known) {
    ...
    Check(scrypt_scratch==nullptr&&scrypt_cleanses==1&&scrypt_cleared,"scrypt success clears full function-owned derived-key scratch");
  }
  ...
  // product share a digest backend; the RFC vectors supply algorithm KATs.
  ...
  for(const auto& v:std::array<Extent,7>{{
    ...
    const auto result=f::DispatchCryptoHashFunction(request);Check(binary(result,v.count),"scrypt publishes requested uint16 output length including above128 bytes");
  ...
  // High-cost combinations are admission/failure tests only, not successful
  // execution evidence. Interposition prevents allocating expensive work here.
  for(const auto& tuple:std::array<std::array<std::uint64_t,4>,4>{{{32768,1,1,64},{1ULL<<31,2,1,64},{1ULL<<63,4,1,64},{2,1,(1ULL<<30)-1,65535}}}) {
    make("",{},tuple[0],tuple[1],tuple[2],tuple[3]);arm(0,13);
    Check(refuse(f::DispatchCryptoHashFunction(request),"CRYPTO.PROFILE.UNAVAILABLE")&&scrypt_interceptions==1&&scrypt_parameters==tuple,"structurally valid scrypt parameters reach provider without narrowing or invented defaults");disarm();
    Check(scrypt_scratch==nullptr&&scrypt_cleared,"failed scrypt provider clears partial derived key");
  }
  for(int failure:{0,-1})for(unsigned prefix:{0u,1u,13u,64u}) {
    request.arguments=valid;arm(failure,prefix);
    Check(refuse(f::DispatchCryptoHashFunction(request),"CRYPTO.PROFILE.UNAVAILABLE"),"scrypt partial provider failure never publishes bytes");disarm();
    Check(scrypt_scratch==nullptr&&scrypt_cleared&&scrypt_cleanses==1,"scrypt partial failure clears complete key allocation");
  }
  for(bool failure:{false,true}) {
    request.arguments=valid;bool completed=false;unsigned faults=0;
    for(long budget=0;budget<100;++budget) {
      if(failure)arm(0,13);else watch();fail_after=budget;
      try {const auto result=f::DispatchCryptoHashFunction(request);fail_after=-1;
        Check(failure?refuse(result,"CRYPTO.PROFILE.UNAVAILABLE"):(binary(result,64)&&result.result.scalar_values[0].binary_value==hex_bytes(known[0].hex)),"scrypt allocation sweep reaches truthful full known-answer or failed-provider result");completed=true;
      }catch(const std::bad_alloc&){fail_after=-1;++faults;}
      disarm();Check(scrypt_scratch==nullptr&&(scrypt_cleanses==0||scrypt_cleared),"scrypt allocation failure cannot leave uncleared derived-key scratch");
      Check(request.arguments[2].value.uint64_value==16,"scrypt allocation failure preserves caller parameters");
      if(completed)break;
    }
    allocation_faults+=faults;Check(completed&&faults>0,"scrypt success/failure allocations exercised through publication");
  }
}
