...
[[noreturn]] void Fail(const char* message) {
  std::cerr << "CSC-TEST-003611: " << message << '\n';
...
struct Fixture {
  ...
  ~Fixture() {
    ...
    std::filesystem::remove(
        database_path +
            ".sb.sblr_executor_availability_registry.v1.catalog_introspect",
        ignored);
  }
};
...
Fixture MakeFixture() {
  ...
  fixture.database_path =
      (std::filesystem::temp_directory_path() /
       ("sb_catalog_introspect_availability_" +
...
api::EngineRequestContext Context(const Fixture& fixture,
  ...
  context.database_uuid = scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003611");
...
api::SblrExecutorAvailabilityRowIdentity Identity() {
  return {api::kSblrCatalogIntrospectExecutorId,
...
int main() {
  ...
  auto admission_context = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003612"));
  ...
  Require(admitted.ok && admitted.cohort != nullptr &&
          ...
          "catalog-introspect admission cohort was not installed");
  ...
  revoke.reason_code = "CSC-TEST-003611";
  const auto revoked =
  ...
  Require(revoked.ok && !revoked.snapshot.installed &&
          ...
          "catalog-introspect executor revocation was not published");
  ...
  Require(diagnostic.code == "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING" &&
              ...
              !current.installed,
          "pinned cohort hid current catalog-introspect revocation");

  auto next_statement = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003613"));
  ...
  Require(observed.ok && observed.cohort != nullptr &&
              ...
              observed.rows.front().snapshot.availability_state ==
                  api::SblrExecutorAvailabilityState::revoked,
          "next statement did not observe revoked catalog-introspect authority");
  return EXIT_SUCCESS;
}
