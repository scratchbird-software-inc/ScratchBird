...
[[noreturn]] void Fail(const char* message) {
  std::cerr << "CSC-TEST-003627: " << message << '\n';
...
struct Fixture {
  ...
  ~Fixture() {
    ...
    std::filesystem::remove(
        database_path +
            ".sb.sblr_executor_availability_registry.v1.parse_text",
        ignored);
  }
};
...
api::EngineRequestContext Context(const Fixture& fixture,
  ...
  context.database_uuid = scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003627");
...
api::SblrExecutorAvailabilityRowIdentity Identity() {
  return {api::kSblrParseTextExecutorId,
          api::kSblrParseTextOpcodeCode,
          api::kSblrParseTextOpcodeVersion,
          api::kSblrParseTextOperandDescriptorId,
          api::kSblrParseTextResultDescriptorId,
          api::kSblrParseTextResultDescriptorVersion};
}
...
int main() {
  ...
  auto statement = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003628"));
  ...
  Require(admitted.ok && admitted.cohort != nullptr &&
          ...
          "parse-text admission cohort was not installed");
  ...
  revoke.reason_code = "CSC-TEST-003627";
  ...
  Require(revoked.ok && !revoked.snapshot.installed &&
          ...
          "parse-text executor revocation was not published");
  ...
  Require(diagnostic.code == "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING" &&
              ...
              !current.installed,
          "pinned cohort hid current parse-text revocation");

  auto next_statement = Context(
      fixture, scratchbird::tests::FixtureUuidLiteral("019d0000-0000-7000-8000-000000003629"));
  ...
  Require(observed.ok && observed.cohort != nullptr &&
              ...
              observed.rows.front().snapshot.availability_state ==
                  api::SblrExecutorAvailabilityState::revoked,
          "next statement did not observe revoked parse-text authority");
  return EXIT_SUCCESS;
}
