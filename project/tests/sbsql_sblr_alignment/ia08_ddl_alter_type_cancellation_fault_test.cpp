...
int main() {
  ...
  sblr::SblrDdlAlterTypeDescriptorV1 seed;
  seed.body[0] = 1;
  seed.availability = 1;
  const auto encoded_seed =
      sblr::EncodeSblrDdlAlterTypeDescriptorV1(seed, false);
  sblr::SblrDdlAlterTypeDescriptorV1 canonical;
  std::string decode_detail;
  assert(!encoded_seed.empty());
  assert(sblr::DecodeSblrDdlAlterTypeDescriptorV1(
      encoded_seed.data(), encoded_seed.size(), &canonical, &decode_detail,
      false));
  assert(!sblr::EncodeSblrDdlAlterTypeDescriptorV1(canonical, true).empty());

  auto malformed_descriptor = canonical;
  malformed_descriptor.evidence[0] ^= 1;
  const auto malformed = api::ConsumeSblrDdlAlterTypeDescriptor(
  ...
  const auto hidden =
      api::ConsumeSblrDdlAlterTypeDescriptor(context, canonical);
  assert(!hidden.ok);
  ...
  unauthorized.trace_tags = {"private_ddl_alter_type"};
  const auto denied =
      api::ConsumeSblrDdlAlterTypeDescriptor(unauthorized, canonical);
  assert(!denied.ok);
  assert(denied.diagnostic.code == "SECURITY.ACCESS_DENIED");
  assert(IsZero(denied.descriptor));
  assert(cancellation_probes.load() == 0);

  context.trace_tags = {"private_ddl_alter_type"};
