...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="sb_firebird_norm_") as temp:
        ...
        sample = "\n".join(
            [
                f"RAW_PATH={repo_root}/build/reference/firebird-5.0.4-release-src/out.log",
                f"TEMP_PATH={temp_root}/case_017/out.log",
                f"UNTAGGED_REPO_PATH {repo_root}/build/reference/firebird-5.0.4-release-src/out.log",
        ...
        normalized = normalize_firebird_reference_output(
            sample,
            repo_root=repo_root,
            temp_root=temp_root,
    ...
    print("validated Firebird reference-native result normalization policy and seed normalizer")
    return 0
