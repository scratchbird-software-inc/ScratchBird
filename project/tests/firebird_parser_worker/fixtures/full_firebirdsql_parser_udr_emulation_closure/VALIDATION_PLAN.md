...
| L0D reference-native regression acquisition | Prove Firebird tools and original regression inputs are located, hashed, imported, and classified; missing roots block closure. |
...
| L0H reference-native harness policy | Prove reference tools run in sandboxed tiered CTest lanes with deterministic result normalization and complete failure inventory. |
...
| L6 reference-native tools | Firebird-built `isql` and service tools drive ScratchBird Firebird endpoint as test-only clients. |
| L6A original reference regression replay | Original Firebird regression cases replay through ScratchBird or block closure until acquired. |
...
| L8 hardening | No third-party Firebird runtime libraries, no cross-dialect linkage, no raw SQL to engine, no reference authority import, no resource abuse. |
| L9 final E2E | Every row has parser, UDR if applicable, SBLR, engine, diagnostic, reference-native replay, implementation, or emulation evidence. |
...
- UDR parity test.
- Engine E2E test.
- Reference-native tool replay test.
...
## Reference-Native Replay Requirements

Reference-native replay must:

- Build Firebird tools only in a test-only external build area.
- Preserve reference tool stdout, stderr, exit status, status vectors, and normalized result output.
- Never link reference tools or Firebird client libraries into ScratchBird runtime, parser worker, or UDR products.
- Run reference tools in isolated temporary directories with loopback-only endpoints, no system install, no external network, strict timeouts, and deterministic cleanup.
...
Failures must be collected as a complete inventory before debugging. A failed reference-native regression may close only when:
