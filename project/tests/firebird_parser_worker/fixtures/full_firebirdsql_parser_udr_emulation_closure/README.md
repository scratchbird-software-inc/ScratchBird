...
Status: draft
Created: 2026-05-08
Owner: ScratchBird donor parser/UDR implementation coordinator
Search key: `FULL-FIREBIRDSQL-PARSER-UDR-EMULATION-IMPLEMENTATION-CLOSURE`

## Purpose

Implement the FirebirdSQL donor compatibility package as an independent dialect bundle:
...
This is not an SBSQL alias layer. FirebirdSQL must not depend on SBSQL, PostgreSQL, MySQL, or any other donor parser/UDR package. The external parser worker and trusted parser-support UDR may share Firebird-owned dialect/wire/support libraries so same-dialect behavior cannot diverge.
...
- CTest coverage generated from exact Firebird surface rows plus donor-native tool and original donor regression replay.

## Boundary Rules

- Reusing SBSQL parser or UDR code.
- Linking ScratchBird runtime products against Firebird donor tools or third-party Firebird client libraries.
...
- Allowing donor SQL/API/BLR frames to enter `sb_engine` as executable authority.
...
| `public_contract_snapshot` through `appendix-donor-firebird-exact-extraction-slice-72-completion-closure-and-conformance-summary-profile.md` | Firebird 5.0.4 exact extraction closure inputs. |
| `public_contract_snapshot` | Exact row model for valid donor syntax, invalid input, API, protocol, metadata, and admin operations. |
...
| `public_contract_snapshot` | Donor environment installer ABI, idempotency, UUID, and version drift behavior. |
| `public_contract_snapshot` | Firebird status-vector and diagnostic rendering contract. |
| `project/tests/donor_regression/donor_release_acquisition/firebird/5.0.4/source` | Local Firebird 5.0.4 source used as read-only reference evidence. |
| `project/tests/donor_regression/donor_catalog_seeds/firebird/` | Firebird catalog seed, builtin inventory, behavior mapping, and catalog overlay inputs. |
| `docs/reference/donor_reference_packets/emulation_1_to_1_engine_reference_packets_2026-04-02/firebird/` | Firebird datatype, index, catalog, and source authority reference packets. |
...
- Firebird source code is locally available under `project/tests/donor_regression/donor_release_acquisition/firebird/5.0.4/source`.
...
- The release evidence packet currently marks upstream regression roots as unresolved in `project/tests/donor_regression/donor_release_acquisition/firebird/5.0.4/regression/SOURCE_POINTERS.md`; this is a P0 blocker for donor-native regression completeness.
...
- Donor-native tools may be built and used only by the CTest harness, never linked into ScratchBird runtime products.
- Donor-native tools must run in a sandboxed test area with loopback-only endpoints, strict timeouts, no system install, no external network, preserved logs, and deterministic cleanup.
...
- Firebird donor-native tool and original regression replay manifest.
- Firebird donor-native result normalization and operational failure inventory policy.
...
P0 source/surface/regression acquisition
  ...
  -> P0D donor-native regression acquisition
  ...
  -> P0I donor-tool sandbox result-normalization CTest-tier and failure-inventory policy
  ...
  -> P15A donor-native tool build and smoke
  -> P15B original donor regression replay
  -> P15C differential donor oracle lane
  -> P15D donor-native result normalization and operational failure inventory
  -> P16 diagnostics/status-vector rendering
  -> P17 hardening package isolation and no-authority-drift gates
  -> P17A Firebird SQL BLR wire buffer service and donor-tool fuzzing
  -> P17B runtime absence disabled incompatible and policy-denied package tests
  -> P18 final generated E2E and zero-open audit
```

## Donor-Native Regression Requirement

The CTest suite must include a donor-native regression lane. This lane builds selected Firebird tools from the local reference source in an isolated test-only build area, points those tools at the ScratchBird Firebird parser/listener endpoint, and replays donor-original tests where available.

The donor-native lane must classify every test case as one of:
...
The lane must fail if a donor-visible surface has no matching generated test, donor-native replay test, differential oracle, or emulation/diagnostic rule.
...
- Implementation continues until all slices pass their acceptance gates and final zero-open audit, or until a human decision is required for an authority conflict, missing donor regression acquisition source, destructive operation outside approved boundaries, security/policy contradiction, or incompatible contract requirement.
...
- No Firebird package links against SBSQL or another donor parser/UDR.
- ScratchBird core still builds and runs without Firebird installed.
- Original donor tool/regression lanes are passing or blocked with a mandatory acquisition gap that prevents closure.
