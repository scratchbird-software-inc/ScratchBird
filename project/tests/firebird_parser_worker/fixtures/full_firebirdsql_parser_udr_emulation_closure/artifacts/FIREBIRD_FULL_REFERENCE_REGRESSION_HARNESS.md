# Firebird Full Reference Regression Harness
...
full reference QA replay does not silently enter smoke runs. The labels retain the
Firebird reference-native and original-regression families for explicit CTest label
selection.
...
template-fragment detection, trace-only Python SQL capture, reference QA
...
connection-info, reference utility, and environment surfaces, the result is:

- `1,949` parser-probe passes
- `0` failure inventory rows
- `0` parser-probe rejection rows
- `224` reference-marked no-op rows admitted as Firebird 5 Linux profile skips
