...
| Donor native | `firebird_donor_native` | Firebird-built tools and original regression replay. |
...
Default developer CTest runs may use fast or standard labels. Exhaustive, donor-native, differential-oracle, and soak labels must be explicit so long-running donor regression jobs do not run accidentally.
