...
"""P6 reference compatibility profile and regression-import gate.

This gate proves that reference compatibility admission is data-driven, release
packet backed, and constrained to ScratchBird engine authority. Reference tools and
...
def load_profiles(repo_root: pathlib.Path) -> list[dict[str, str]]:
    path = repo_root / "project/src/parsers/compatibility/CompatibilityProfileManifest.csv"
    rows = read_csv(path)
...
def load_udr_policies(repo_root: pathlib.Path) -> dict[str, dict[str, str]]:
    path = repo_root / "project/src/udr/packages/compatibility/CompatibilityUdrBridgePolicyManifest.csv"
    rows = read_csv(path)
...
def validate_seed_manifest(repo_root: pathlib.Path, row: dict[str, str]) -> None:
    ...
    if manifest.get("reference_family") != row["seed_manifest_family"]:
        raise AssertionError(f"{row['family_id']}: seed reference_family mismatch")
...
def validate_true_reference(repo_root: pathlib.Path, row: dict[str, str]) -> None:
    ...
    if row["runtime_seed_authority"] != "reference_catalog_seed_manifest":
    ...
    if row["capability_reference_policy"] != "not_capability_reference":
        raise AssertionError(f"{family}: true reference must not use capability-reference policy")
    ...
    require_labels(
        row,
        (
            "reference_core_framework_gate",
            "reference_seed_gate",
            "reference_datatype_gate",
            "reference_index_translation_gate",
...
def validate_profile_variant(repo_root: pathlib.Path, row: dict[str, str]) -> None:
    ...
    if owner not in TRUE_REFERENCES:
        raise AssertionError(f"{family}: profile variant owner is not a true reference")
    if family in TRUE_REFERENCES:
        raise AssertionError(f"{family}: profile variant must not be a true reference")
    ...
    if row["parser_module"] != f"project/src/parsers/compatibility/{owner}":
    ...
    if row["seed_manifest_family"] != owner:
        raise AssertionError(f"{family}: profile variant seed family must be owner reference")
    if row["seed_manifest_path"] != (
        "project/tests/reference_regression/reference_catalog_seeds/mysql_lts/mysql_lts_beta2_full_seed_manifest.yaml"
    ):
...
def validate_udr_policy(rows: list[dict[str, str]], policies: dict[str, dict[str, str]]) -> None:
    ...
    if set(policies) != expected:
        raise AssertionError("UDR policy families do not match reference profile families")
    variant_families = {
        row["family_id"]
        for row in rows
        if row["profile_class"] == "release_profile_variant"
...
def validate_target_evidence(repo_root: pathlib.Path) -> None:
    target_manifest = repo_root / (
        "project/tests/reference_regression/fixtures/public_single_node_closure/"
    ...
    for row in p6:
        if "artifacts/P6_REFERENCE_COMPATIBILITY_CLOSURE_EVIDENCE.md" not in row["evidence_artifacts"]:
...
def validate_registry_rollups(repo_root: pathlib.Path) -> None:
    ...
    for rel in registries:
        if rel == PUBLIC_CONTRACT_SENTINEL:
            continue
        path = require_file(repo_root, rel, "reference registry rollup")
        text = path.read_text(encoding="utf-8")
        for family in TRUE_REFERENCES:
...
def validate_profiles(repo_root: pathlib.Path) -> list[dict[str, str]]:
    ...
    if set(RELEASE_PROFILE_VARIANTS) & set(TRUE_REFERENCES):
        raise AssertionError("release/profile variants must not be listed as true references")
...
def main() -> int:
    ...
    try:
        ...
        print(f"P6_REFERENCE_COMPATIBILITY_GATE=failed: {exc}", file=sys.stderr)
        return 1

    digest = hashlib.sha256(args.mode.encode("utf-8")).hexdigest()[:12]
    print(f"P6_REFERENCE_COMPATIBILITY_GATE=passed mode={args.mode} evidence={digest}")
    return 0
