...
path = Path('project/tests/reference_regression/fixtures/public_single_node_closure/public_proof/artifacts/TARGET_GAPS.csv')
...
python3 - <<'PY'
from pathlib import Path
root = Path('project/tests/reference_regression/fixtures/public_single_node_closure/public_proof')
required = [
...
3. Run the MGA policy gate before declaring any transaction-sensitive parser,
   storage, driver, reference, or benchmark path compliant.

```sh
python3 ${PUBLIC_TOOL_ROOT}/skills/scratchbird-mga-transaction-authority/scripts/mga_policy_gate.py --repo . project/src project/tests project/tests/reference_regression/fixtures/public_single_node_closure/public_proof
```
...
5. Run full-route and reference regression gates.

```sh
ctest --test-dir build -L public_single_node_full_route_gate --output-on-failure
ctest --test-dir build -L reference_original_regression_gate --output-on-failure
...
- Reference tools may be compatibility oracles only; they must not execute storage
