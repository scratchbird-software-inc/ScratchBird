...
Search key: SB_REFERENCE_REFERENCE_MARIADB_BUILTIN_MAPPING_STATUS

The MariaDB built-in inventory is a first-pass behavior inventory. It is not yet a complete signature table. The MariaDB reference profile must not be marked implementation-ready until every built-in and SQL-mode-sensitive construct has an exact mapping row.
...
| Metadata/session functions | Reference projection over ScratchBird session, transaction, security, catalog, and metrics state. |
...
|---|---|
| Exact signature missing | REFERENCE.MARIADB.BUILTIN.SIGNATURE_MISSING |
| SQL mode behavior unspecified | REFERENCE.MARIADB.BUILTIN.SQL_MODE_UNSPECIFIED |
| C++ UDR entry point missing | REFERENCE.MARIADB.BUILTIN.UDR_ENTRY_MISSING |
| Raw secret exposure attempted | REFERENCE.MARIADB.BUILTIN.RAW_SECRET_FORBIDDEN |
| UUID v7 called without required cluster-time authority | REFERENCE.MARIADB.UUID.CLUSTER_TIME_REQUIRED |
| Unsupported plugin datatype behavior | REFERENCE.MARIADB.TYPE.UNSUPPORTED |
