...
"""Fail when reference parser test metadata depends on out-of-tree assets."""
            ...
            for column, value in row.items():
                ...
                for item in split_path_items(value):
                    if item in {"", "forbidden", "no_tool_recorded", "not_applicable"}:
                        continue
                    if item.startswith(("reference.", "scratchbird.", "SB", "normalized_")):
                        continue
...
def main() -> int:
    ...
    reference_root = repo / REFERENCE_TEST_PREFIX
    if not reference_root.is_dir():
        print(f"missing reference test root: {reference_root}", file=sys.stderr)
        return 1

    failures: list[str] = []
    for path in iter_owned_files(reference_root):
    ...
    print("reference_test_tree_self_contained_gate: PASS")
    return 0
