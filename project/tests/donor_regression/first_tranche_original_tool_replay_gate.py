...
"""Stage original donor tools and replay first-tranche donor SQL parser cases.

This is a regular CTest gate.  It intentionally keeps donor tools as
test-only artifacts under project/tests while preserving ScratchBird authority:
donor tools and donor SQL fragments provide parser-facing evidence only; they
...
PUBLIC_DONOR_REFERENCE_NOT_PACKAGED = "public_repo_donor_reference_not_packaged"

MYSQL_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
POSTGRESQL_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
FIREBIRD_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
SQLITE_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
MARIADB_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
DUCKDB_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
CLICKHOUSE_SOURCE_REL = (
    PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
)
TIDB_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
VITESS_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
...
CASSANDRA_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
MONGODB_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
REDIS_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
OPENSEARCH_SQL_PPL_SOURCE_REL = (
    PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
)
OPENSEARCH_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
NEO4J_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
INFLUXDB_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
MILVUS_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
DOLT_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
APACHE_IGNITE_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
TIKV_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
FOUNDATIONDB_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
IMMUDB_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
XTDB_SOURCE_REL = PUBLIC_DONOR_REFERENCE_NOT_PACKAGED
...
class ReplayCase:
    ...
    provenance: str = "donor_release_packet_sql"
...
def ensure_mysql_tools(repo_root: pathlib.Path,
    ...
    build_dir = build_root / "donor" / "mysql-8.4.8-tools"
    ...
    if result.returncode != 0:
        raise AssertionError(f"MySQL donor tool configure failed:\n{result.stdout}")
    ...
    for target in ("mysqltest", "mysql"):
        ...
        if result.returncode != 0:
            raise AssertionError(f"MySQL donor tool build failed for {target}:\n{result.stdout}")

    if not mysql.exists() or not mysqltest.exists():
        raise AssertionError("MySQL donor tool build completed without mysql/mysqltest binaries")
    return mysql, mysqltest


def ensure_firebird_isql(repo_root: pathlib.Path,
                         timeout_seconds: int) -> pathlib.Path:
    donor_root = repo_root / "build" / "donor" / "firebird-5.0.4-release-src"
    if not donor_root.exists():
        source_root = repo_root / FIREBIRD_SOURCE_REL
        if not source_root.is_dir():
            raise AssertionError(f"Firebird donor source packet missing: {source_root}")
        donor_root.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(source_root, donor_root)
    manifest = (
        repo_root
    ...
    gate = repo_root / "project/tests/firebird_parser_worker/firebird_donor_tool_gate.py"
    result = run_command(
        [
            sys.executable,
            str(gate),
    ...
    if result.returncode != 0:
        raise AssertionError(f"Firebird donor tool build gate failed:\n{result.stdout}")
    isql = donor_root / "gen" / "Release" / "firebird" / "bin" / "isql"
    if not isql.exists():
...
def ensure_sqlite_cli(repo_root: pathlib.Path,
    ...
    build_dir = build_root / "donor" / "sqlite-3.53.0-tools"
    staged_source = build_root / "donor" / "sqlite-3.53.0-source"
    ...
    if result.returncode != 0:
        raise AssertionError(f"SQLite donor tool configure failed:\n{result.stdout}")
    result = run_command(
        ["make", "-j4", "sqlite3"],
        cwd=build_dir,
        timeout_seconds=timeout_seconds,
    ...
    if result.returncode != 0:
        raise AssertionError(f"SQLite donor tool build failed:\n{result.stdout}")
    if not sqlite3.exists():
        raise AssertionError(f"SQLite donor tool build completed without sqlite3: {sqlite3}")
    return sqlite3
...
def executable_staged_path(repo_root: pathlib.Path, staged_rel: str) -> pathlib.Path:
    ...
    ensure_under(staged, repo_root / "project/tests/donor_regression")
    return staged
...
def copy_tool(spec: ToolSpec, repo_root: pathlib.Path) -> dict[str, object]:
    ...
    if not spec.source.exists():
        raise AssertionError(f"{spec.tool_id} packaged public tool missing: {spec.source}")
...
def build_tool_specs(repo_root: pathlib.Path,
    ...
    return [
        ToolSpec(
            "firebird",
            "firebird-isql",
            repo_root / "project/tests/donor_regression/firebird/native_tool_harness/tools/isql",
        ...
        ToolSpec(
            "mysql",
            "mysql-client",
            repo_root / "project/tests/donor_regression/mysql/native_tool_harness/tools/mysql",
        ...
        ToolSpec(
            "mysql",
            "mysqltest",
            repo_root / "project/tests/donor_regression/mysql/native_tool_harness/tools/mysqltest",
        ...
        ToolSpec(
            "postgresql",
            "pg-regress",
            repo_root / "project/tests/donor_regression/postgresql/native_tool_harness/tools/pg_regress",
        ...
        ToolSpec(
            ...
            repo_root / "project/tests/donor_regression/postgresql/native_tool_harness/tools/pg_isolation_regress",
        ...
        ToolSpec(
            "postgresql",
            "psql",
            repo_root / "project/tests/donor_regression/postgresql/native_tool_harness/tools/psql",
        ...
        ToolSpec(
            "postgresql",
            "pgbench",
            repo_root / "project/tests/donor_regression/postgresql/native_tool_harness/tools/pgbench",
        ...
        ToolSpec(
            "sqlite",
            "sqlite3-cli",
            repo_root / "project/tests/donor_regression/sqlite/native_tool_harness/tools/sqlite3",
...
def replay_cases() -> list[ReplayCase]:
    return [
        ...
        ReplayCase(
            ...
            "PRAGMA",
            "parser_support_udr",
            provenance="donor_release_packet_regression_launcher",
        ),
        ReplayCase(
            "mariadb",
...
def assert_source_fragment(repo_root: pathlib.Path, case: ReplayCase) -> dict[str, object]:
    if case.source_rel.startswith(PUBLIC_DONOR_REFERENCE_NOT_PACKAGED):
        return {
            "donor_reference_publication": "not_packaged_in_public_repo",
...
def assert_procedural_evidence(envelope: dict[str, object],
    ...
    assert_field(retention, "evidence_contract",
                 "donor_procedural_body_source_retention.v1", case.case_id)
    ...
    assert_field(functional, "evidence_contract",
                 "donor_procedural_functional_encoding_source_span_uuid_binding.v1",
                 case.case_id)
    ...
    if firebird_case:
        ...
        assert_field(firebird_encoding, "runtime_equivalence_status",
                     "pending_donor_native_psql_replay", case.case_id)
    for key in (
...
def assert_datatype_evidence(envelope: dict[str, object],
    ...
    assert_field(descriptor, "evidence_contract",
                 "donor_datatype_descriptor_evidence.v1", case.case_id)
    ...
    assert_field(profile, "evidence_contract",
                 "donor_datatype_profile_family_detection.v1", case.case_id)
    ...
    assert_field(profile, "runtime_equivalence_status",
                 "pending_donor_native_exactness_replay", case.case_id)
...
def run_replay_case(repo_root: pathlib.Path,
    ...
    if envelope.get("donor_engine_sql_executed") is not False:
        raise AssertionError(f"{case.case_id} claimed donor SQL execution")
...
def main(argv: list[str]) -> int:
    ...
    evidence = {
        "gate": "donor_sql_first_tranche_original_tool_replay_gate",
        "regular_ctest_gate": True,
        "tool_staging_root": "project/tests/donor_regression/*/native_tool_harness/tools",
        "donor_tools_are_storage_authority": False,
        "donor_tools_are_transaction_authority": False,
        "parser_authority_rule": (
            "native donor tools and donor regression fragments drive parser "
        ...
        "runtime_enterprise_blocker_reason": (
            ...
            "procedural bodies, exact datatypes, donor semantic defaults, and "
            "donor-native runtime equivalence are fully implemented and proven"
        ),
        "replay_results": replay_results,
    }
    write_evidence(args.evidence_file, evidence)
    print(
        "donor_sql_first_tranche_original_tool_replay_gate=passed "
...
if __name__ == "__main__":
    try:
        ...
        print(f"donor_sql_first_tranche_original_tool_replay_gate: {exc}",
              file=sys.stderr)
        raise SystemExit(1)
