...
Search key: SB_REFERENCE_DONOR_SQLSERVER_2022_AZURE_BUILTIN_MAPPING_STATUS
...
This file records the implementation status for SQL Server built-in behavior copied into the private reference tree. It is not a completed function implementation table. It exists so implementation work can distinguish authoritative donor behavior groups from generated inventory defects.
...
| System/session | @@ERROR, @@ROWCOUNT, @@TRANCOUNT, XACT_STATE, SESSION_CONTEXT | Donor session-state object backed by ScratchBird transaction/session diagnostics | Inventory copied; exact state-transition rules still required |
| Metadata | OBJECT_ID, DB_NAME, SERVERPROPERTY, TYPEPROPERTY, STATS_DATE | Donor catalog projection over UUID-backed ScratchBird catalog and metrics | Inventory copied; exact visibility rules still required |
| Security | HAS_PERMS_BY_NAME, IS_MEMBER, SUSER_SNAME, ORIGINAL_LOGIN | Donor security projection over ScratchBird security tree and external-auth cache | Inventory copied; exact principal visibility and denial behavior still required |
| Procedure compatibility | sp_helplanguage | C++ UDR compatibility procedure returning donor-shaped rowsets | Inventory copied; exact default rowset still required |
...
|---|---|
| donor_name | Exact donor-visible token or grammar production. |
| donor_kind | scalar_function, aggregate_function, window_function, table_function, procedure, session_variable, grammar_construct, or metadata_construct. |
...
| return_signature | Return datatype, nullability, precision/scale/length, rowset shape if table-returning, and donor-specific type aliases. |
...
| conformance_tests | Positive, negative, null, collation, overflow, security, transaction, and donor-visibility tests. |

## Diagnostics

| Condition | Diagnostic vector |
|---|---|
| Generated token treated as a function | DONOR.SQLSERVER.BUILTIN.GENERATED_TOKEN_FORBIDDEN |
| Missing exact signature | DONOR.SQLSERVER.BUILTIN.SIGNATURE_MISSING |
| Missing return shape | DONOR.SQLSERVER.BUILTIN.RETURN_SHAPE_MISSING |
| Missing session-state binding | DONOR.SQLSERVER.BUILTIN.SESSION_STATE_UNBOUND |
| Missing metadata visibility rule | DONOR.SQLSERVER.BUILTIN.VISIBILITY_RULE_MISSING |
| Unsupported SQL Server behavior called | DONOR.SQLSERVER.BUILTIN.UNSUPPORTED_BEHAVIOR |
| Raw secret or credential exposure attempted | DONOR.SQLSERVER.BUILTIN.RAW_SECRET_FORBIDDEN |
