...
"""Generate and validate donor parser implementation-start evidence manifests.
...
DPGEC = PRIVATE_EXECUTION_PLAN_ROOT / "donor-parser-implementation-gate-evidence-closure"
DRP = PRIVATE_EXECUTION_PLAN_ROOT / "donor-parser-regression-policy-extraction-closure"
GENERATION_BASELINE = "2026-06-05T00:00:00Z"
DOT_GIT = "." + "git"
DOT_GITHUB = "." + "github"

EVIDENCE_FILES = {
    "donor_regression_inventory": "upstream_manifest.csv",
...
PER_DONOR_STATUS = {
    "DONOR_REGRESSION_TEST_EXTRACTION_MATRIX.csv": "manifest_generated_pending_replay",
    ...
    "DONOR_REGRESSION_PERFORMANCE_SOURCE_LOCATOR.csv": "source_locator_verified_manifest_generated",
}

ABI_SURFACES = (
    ("setup_database", "Role C setup installs donor catalog projection policy metadata and emulation root."),
    ("drop_database", "Role C teardown removes emulation metadata only after engine authorization."),
    ("add_user", "Security management projects donor principal metadata without parser-owned authentication."),
    ("drop_user", "Security management removes donor principal projection after engine authorization."),
    ("grant_privilege", "Security management maps donor privilege syntax to object-scoped SB authorization."),
    ("revoke_privilege", "Security management removes donor privilege projection with audit evidence."),
    ("catalog_seed_projection", "Catalog management seeds donor-visible catalog rows from SB catalog authority."),
    ("database_presentation", "Presentation management maps donor database/root naming to recursive SB schema roots."),
    ...
    ("replication_status", "Operational route reports donor-compatible replication or ETL status."),
    ("support_bundle", "Support route emits redacted diagnostics and retained evidence metadata."),
    ("external_authority_refusal", "Refusal route denies donor commands that attempt real physical authority outside SBsql."),
)
...
def scrub_generated_project_paths(repo: pathlib.Path) -> None:
    root = repo / "project/tests/donor_regression"
    replacements = (
        (str(repo) + "/", ""),
...
def generate_upstream_manifest(
    ...
    if rels and len(rels) != len(abss):
        raise AssertionError(f"{donor['donor_id']}: source relative/absolute path count mismatch")
        ...
        rows.append(
            {
                ...
                "suite_id": f"{donor['donor_id'].upper()}-SUITE-{index:03d}",
                ...
                "extraction_destination": donor["project_tests_root"],
...
def generate_native_harness_manifest(
    ...
    for index, tool in enumerate(tool_paths, start=1):
        ...
        rows.append(
            {
                "donor_id": donor["donor_id"],
                "harness_id": f"{donor['donor_id'].upper()}-NATIVE-TOOL-{index:03d}",
                ...
                "required_endpoint_env": "SCRATCHBIRD_DONOR_ENDPOINT;SCRATCHBIRD_DONOR_AUTH_PACKET;SCRATCHBIRD_DONOR_RESULT_DIR",
    ...
    if not rows:
        rows.append(
            {
                "donor_id": donor["donor_id"],
                "harness_id": f"{donor['donor_id'].upper()}-NATIVE-TOOL-NONE",
                "native_tool_family": native.get("native_tool_family", "no donor-native tool recorded"),
                ...
                "required_endpoint_env": "SCRATCHBIRD_DONOR_ENDPOINT;SCRATCHBIRD_DONOR_AUTH_PACKET;SCRATCHBIRD_DONOR_RESULT_DIR",
                "required_output": "normalized_native_replay_results.json;native_replay_support_bundle.json",
                "parser_authority_rule": "client replay harness required even when donor has no standalone tool",
                "status": "manifest_generated_pending_native_replay",
            }
        )
    return rows
...
def write_readme(path: pathlib.Path, donor: dict[str, str]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        "\n".join(
            [
                f"# {donor['display_name']} Donor Parser Start Evidence",
                "",
                f"Donor id: `{donor['donor_id']}`",
                "",
                "This directory is generated from the donor parser readiness and",
...
def write_small_manifest(path: pathlib.Path, donor: dict[str, str], family: str, status: str) -> None:
    write_csv(
        path,
        ["donor_id", "evidence_family", "required_artifact", "authority_rule", "status"],
        [
            {
                "donor_id": donor["donor_id"],
...
def generate_for_donor(
    ...
    root = repo / donor["project_tests_root"]
    execution_plan = repo / donor["donor_execution_plan"]
    root.mkdir(parents=True, exist_ok=True)
    write_readme(root / "README.md", donor)

    upstream_rows = generate_upstream_manifest(repo, donor, source, native)
    write_csv(
    ...
    if donor["donor_id"] == "opensearch_sql_ppl":
        exclusion_rows.append(
            {
                ...
                "owner": "donor regression extraction controller",
    ...
    write_csv(
        ...
        ["donor_id", "exclusion_id", "scope", "reason_code", "owner", "acceptance_rule", "status"],
        exclusion_rows,
    )

    native_rows = generate_native_harness_manifest(repo, donor, native)
    native_dir = root / "native_tool_harness"
    native_dir.mkdir(parents=True, exist_ok=True)
    (native_dir / "README.md").write_text(
        "\n".join(
            [
                f"# {donor['display_name']} Native Tool Harness",
                "",
    ...
    extra = {
        "donor_id": donor["donor_id"],
        "display_name": donor["display_name"],
        "project_tests_root": donor["project_tests_root"],
    }
    ...
    write_csv(
        ...
        if policy
        else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    write_csv(
        ...
        list(catalog[0]) if catalog else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    write_csv(
        ...
        list(migration_rows[0]) if migration_rows else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    write_csv(
        ...
        list(performance_rows[0]) if performance_rows else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    write_csv(
        ...
        list(wire[0]) if wire else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    write_csv(
        ...
        list(resource[0]) if resource else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    abi_rows = [
        {
            ...
            "package_naming_rule": "SB<enginename> donor package only; SBsql is excluded",
            "parser_rule": "parser emits SBLR management request and cannot perform authority",
            "engine_rule": "engine authorizes and invokes registered donor sbup package routine",
    ...
    write_csv(
        ...
        list(release[0]) if release else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    write_csv(
        ...
        list(version[0]) if version else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    write_csv(
        ...
        list(variance[0]) if variance else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    write_csv(
        ...
        list(shared[0]) if shared else ["donor_id", "display_name", "project_tests_root", "status"],
    ...
    write_csv(
        ...
        list(completion[0]) if completion else ["donor_id", "display_name", "project_tests_root", "status"],
...
def update_source_matrices(repo: pathlib.Path) -> tuple[list[dict[str, str]], list[dict[str, str]], list[dict[str, str]]]:
    source_path = repo / DRP / "DONOR_REGRESSION_SOURCE_LOCATION_MATRIX.csv"
    native_path = repo / DRP / "DONOR_NATIVE_TEST_TOOL_INDEX.csv"
    perf_path = repo / DRP / "DONOR_PERFORMANCE_SUITE_SOURCE_MATRIX.csv"
...
def update_execution_plan_statuses(repo: pathlib.Path) -> None:
    ...
    update_gate_statuses(
        repo / DRP / "ACCEPTANCE_GATES.csv",
        {
            ...
            "DRP-GATE-008": "blocked_pending_final_no_open_donor_audit",
...
def update_blockers(repo: pathlib.Path) -> None:
    ...
    updates = {
        "DPGEC-BLOCKER-002": (
            "Donor-native regression and native replay manifests now exist in project/tests for every donor parser package; actual extraction replay execution fixtures goldens and accepted normalized results remain pending.",
            "project/tests/donor_regression/<donor>/upstream_manifest.csv and native_tool_harness/native_tool_harness_manifest.csv generated for all 25 donors.",
            "Per-donor native replay execution results fixtures goldens reason-coded final exclusions and support bundles accepted by QA.",
        ),
        "DPGEC-BLOCKER-003": (
            ...
            "project/tests/donor_regression/<donor>/security_operations/security_policy_manifest.csv and catalog_policy/catalog_policy_manifest.csv generated for all 25 donors.",
        ...
        "DPGEC-BLOCKER-004": (
            ...
            "project/tests/donor_regression/<donor>/operations_migration/migration_policy_manifest.csv and performance/performance_baseline_manifest.csv generated for all 25 donors.",
            "Per-donor cutover barrier proof and comparable original donor steady emulation and active live-migration performance reports.",
        ),
        "DPGEC-BLOCKER-005": (
            ...
            "project/tests/donor_regression/<donor>/wire_transcripts resource_limits compatibility compatibility_variance cross_dialect and release_evidence manifests generated for all 25 donors.",
        ...
        "DPGEC-BLOCKER-006": (
            "Per-donor parser-support UDR ABI manifests now exist, but route privilege idempotency crash/recovery and exact-refusal tests are not executed.",
            "project/tests/donor_regression/<donor>/management_package_abi/management_package_abi_manifest.csv generated for all 25 donors.",
...
def write_validation_reports(repo: pathlib.Path, totals: dict[str, int]) -> None:
    summary = {
        ...
        "donor_rows": len(read_csv(repo / DPGEC / "DONOR_GATE_EVIDENCE_LEDGER.csv")),
        ...
        "source_status_counts": status_counts(read_csv(repo / DRP / "DONOR_REGRESSION_SOURCE_LOCATION_MATRIX.csv"), "source_status"),
    }
    ...
    report = [
        "# Donor Parser Gate Evidence Validation Report",
        "",
        "Search key: `DONOR-PARSER-GATE-EVIDENCE-VALIDATION-REPORT`",
        "",
        ...
        "",
        "The project-test start manifests now exist for every donor parser package.",
        "This closes the previous missing-manifest blocker, but it does not grant",
        "parser implementation start. Actual donor-native replay, executable policy",
        ...
        "## Current Counts",
        "",
        f"- Donor parser ledger rows: {summary['donor_rows']}.",
        ...
        "- Parser-support UDR ABI conformance must execute for every donor.",
    ...
    handoff = [
        "# Donor Parser Implementation Start Handoff Packet",
        "",
        "Search key: `DONOR-PARSER-IMPLEMENTATION-START-HANDOFF-PACKET`",
        "",
        f"Generated: {GENERATION_BASELINE}",
        "",
        "## Handoff Decision",
        ...
        "`no_go_pending_evidence_execution_and_gold_signoff`",
        "",
        "Parser implementation agents must not start donor parser code from this package.",
        ...
        "",
        "- Donor parser ledger inventory: 25 parser rows.",
        "- Donor source locator matrix: verified for all 25 parser rows.",
        "- Per-donor project-test start manifests: generated under `project/tests/donor_regression/`.",
        "- Live migration support class selection: recorded for every donor parser package.",
        ...
        "remain blocked until each donor implementation execution_plan regenerates integrated",
    ...
    go_no_go = [
        "# Donor Parser Implementation Gate Evidence Go/No-Go",
        "",
        "Search key: `DONOR-PARSER-IMPLEMENTATION-GATE-EVIDENCE-GO-NO-GO`",
        "",
        ...
        "- `DONOR_GATE_EVIDENCE_LEDGER.csv` marks every donor `implementation_start_go`;",
...
def write_drp_go_no_go(repo: pathlib.Path) -> None:
    text = [
        "# Donor Parser Regression and Policy Extraction Go/No-Go",
        "",
        "Search key: `DONOR-PARSER-REGRESSION-POLICY-GO-NO-GO`",
        "",
        ...
        "been generated under `project/tests/donor_regression/` for every donor",
        "parser package.",
        "",
        "## Remaining No-Go Conditions",
        "",
        "- Parser implementation code is changed under this execution_plan.",
        "- Donor source trees are modified or added to git.",
        "- Native replay execution results are missing.",
        "- Donor regression tests are silently skipped without reason codes.",
        "- Security or operational policy fixtures are not executable.",
        "- Performance reports do not compare original donor, steady emulation, and active migration.",
        "- Any donor parser completion relies on stubs, skeletons, fake pass fixtures, deferred implementation markers, temporary unsupported markers, generated-only artifacts, or minimal implementation claims.",
        "",
        "## Decision",
        "",
        "Preparation manifest generation is complete. Parser implementation remains",
        "blocked until listener/engine gold, executed donor regression evidence,",
...
def generate(repo: pathlib.Path) -> dict[str, int]:
    ...
    ledger = read_csv(repo / DPGEC / "DONOR_GATE_EVIDENCE_LEDGER.csv")
    ...
    for donor in ledger:
        donor_id = donor["donor_id"]
        if donor_id not in source_by_id:
            raise AssertionError(f"{donor_id}: missing source matrix row")
        if donor_id not in native_by_id:
            raise AssertionError(f"{donor_id}: missing native tool matrix row")
        if donor_id not in performance_by_id:
            raise AssertionError(f"{donor_id}: missing performance source matrix row")
        stats = generate_for_donor(repo, donor, source_by_id[donor_id], native_by_id[donor_id], performance_by_id[donor_id])
        totals["missing_sources"] += stats["missing_sources"]
        totals["missing_tools"] += stats["missing_tools"]

        for rel in EVIDENCE_FILES.values():
            if (repo / donor["project_tests_root"] / rel).is_file():
                totals["manifest_files"] += 1
            else:
                totals["missing_manifest_files"] += 1

        execution_plan = repo / donor["donor_execution_plan"]
        for filename, status in PER_DONOR_STATUS.items():
...
def validate(repo: pathlib.Path) -> None:
    ledger = read_csv(repo / DPGEC / "DONOR_GATE_EVIDENCE_LEDGER.csv")
    if len(ledger) != 25:
        raise AssertionError(f"expected 25 donor ledger rows, found {len(ledger)}")
    ...
    for donor in ledger:
        ...
        for family, rel in EVIDENCE_FILES.items():
            path = root / rel
            if not path.is_file():
                missing.append(f"{donor['donor_id']}:{family}:{rel}")
                continue
            rows = read_csv(path)
            if not rows:
                bad_status.append(f"{donor['donor_id']}:{family}:empty")
                continue
            ...
            if not any(
                ...
                bad_status.append(f"{donor['donor_id']}:{family}:{sorted(statuses)}")
        ...
        if missing_sources:
            bad_status.append(f"{donor['donor_id']}:missing_source_entries:{','.join(missing_sources)}")
        for path in root.rglob("*"):
            if not path.is_file() or path.suffix not in {".csv", ".md", ".json"}:
                continue
            text = path.read_text(encoding="utf-8")
    ...
    if missing:
        raise AssertionError("missing generated donor evidence manifests: " + "; ".join(missing[:20]))
    if bad_status:
        raise AssertionError("invalid generated donor evidence status: " + "; ".join(bad_status[:20]))
    if local_path_bleed:
        raise AssertionError("generated donor evidence leaks local source paths: " + "; ".join(local_path_bleed[:20]))
    if vcs_metadata_bleed:
        raise AssertionError("generated donor evidence leaks VCS metadata references: " + "; ".join(vcs_metadata_bleed[:20]))
...
def main(argv: list[str]) -> int:
    ...
    if args.check:
        validate(repo)
        print("donor_parser_start_evidence_manifests=pass")
        return 0

    totals = generate(repo)
    validate(repo)
    print(
        "donor_parser_start_evidence_manifests=generated "
