...
--
-- Test unique index inference with operator class contracts and
-- named collations
--
...
-- Neither collation nor operator class contracts are required --
...
-- collation.
--
-- Works because every attribute in inference contract needs to be
...
-- opclass/collation contract.
--
-- The implementation is liberal in accepting inference contracts on the
...
-- Fails (no unique index inference contract, required for do update variant):
...
-- contract will do the right thing, though.
