...
"""Validate the donor parser implementation-start gate evidence controller.

This is a no-go enforcement gate for the current evidence state. It proves that
the central donor parser gate package is explicit, source-independent, and
...
EXECUTION_PLAN = EXECUTION_PLAN_ROOT / "donor-parser-implementation-gate-evidence-closure"
REGRESSION_PLAN = EXECUTION_PLAN_ROOT / "donor-parser-regression-policy-extraction-closure"
...
def validate_no_go_controller(repo: pathlib.Path) -> None:
    ...
    ledger = read_csv(repo, EXECUTION_PLAN / "DONOR_GATE_EVIDENCE_LEDGER.csv")
    if len(ledger) != 25:
        raise AssertionError(f"donor ledger must have 25 parser rows, found {len(ledger)}")
    bad_ledger = [
        row["donor_id"]
        for row in ledger
    ...
    require_file(repo, pathlib.Path("project/tests/donor_regression/generate_donor_parser_start_evidence.py"))
    ...
    for row in ledger:
        ...
        for family, rel in EVIDENCE_FILES.items():
            ...
            try:
                ...
                missing_evidence.append(f"{donor_id}:{family}:{evidence_rel.as_posix()}")
                continue
            ...
            if not any(is_generated_or_proof_closed_status(status) for status in statuses):
                bad_evidence_status.append(f"{donor_id}:{family}:{','.join(sorted(statuses))}")
    if missing_evidence:
        raise AssertionError(
            "missing generated donor parser start evidence manifests: "
            + "; ".join(missing_evidence[:20])
        )
    if bad_evidence_status:
        raise AssertionError(
            "invalid donor parser start evidence manifest statuses: "
    ...
    sources = read_csv(repo, REGRESSION_PLAN / "DONOR_REGRESSION_SOURCE_LOCATION_MATRIX.csv")
    if len(sources) != 25:
    ...
    if any(row.get("source_status") != "verified_present" for row in sources):
        not_verified = [row["donor_id"] for row in sources if row.get("source_status") != "verified_present"]
        raise AssertionError(f"unverified donor source locators: {', '.join(not_verified)}")

    for rel in (
        REGRESSION_PLAN / "DONOR_NATIVE_TEST_TOOL_INDEX.csv",
        REGRESSION_PLAN / "PROJECT_TEST_DESTINATION_MATRIX.csv",
    ):
        rows = read_csv(repo, rel)
        values = unique_values(rows, "donor_id")
        if len(values) != 25:
            raise AssertionError(f"{rel}: expected 25 distinct donor_id values, found {len(values)}")
        for donor_id in ("opensearch_rest", "opensearch_sql_ppl"):
            if donor_id not in values:
                raise AssertionError(f"{rel}: missing normalized {donor_id} row")
...
def main(argv: list[str]) -> int:
    ...
    print("donor_parser_gate_evidence_no_go=pass")
    return 0
