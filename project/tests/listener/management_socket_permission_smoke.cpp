...
std::filesystem::path MakeTempDir() {
  std::string tmpl = "/tmp/sb_lmsp.XXXXXX";
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: sb_listener_management_socket_permission_smoke <sb_listener> <sb_parser_dummy>\n";
    return EXIT_FAILURE;
  }
  ...
  if (pid == 0) {
    ...
    ::execl(listener.c_str(),
            ...
            "--database-selector=dev_bootstrap_path:/tmp/sb_lmsp.sbdb",
            "--server-endpoint=unix:/tmp/sb_lmsp.sbps.sock",
            parser_arg.c_str(),
            control_arg.c_str(),
            runtime_arg.c_str(),
  ...
  Require(::chmod(management_socket.c_str(), 0000) == 0, "could not chmod management socket");
  const int denied_fd = ConnectUnix(management_socket);
  if (denied_fd >= 0) {
    ::close(denied_fd);
    cleanup();
    std::cerr << "management socket remained connectable after chmod 0000\n";
    return EXIT_FAILURE;
  }
  ...
  if (client_fd < 0) {
    cleanup();
    std::cerr << "listener stopped accepting clients after management socket chmod\n";
    return EXIT_FAILURE;
  }
  std::string line;
  if (!ReadLineWithTimeout(client_fd, &line, 2000) || line != "ScratchBird dummy parser ready") {
    ::close(client_fd);
    cleanup();
    std::cerr << "client greeting failed after management socket chmod: " << line << '\n';
    return EXIT_FAILURE;
  }
  if (!WriteAll(client_fd, "management-socket-permission\n") || !ReadLineWithTimeout(client_fd, &line, 1000) ||
      line != "management-socket-permission") {
    ::close(client_fd);
    cleanup();
    std::cerr << "client echo failed after management socket chmod: " << line << '\n';
    return EXIT_FAILURE;
  }
  ::close(client_fd);
  cleanup();
  std::cout << "management_socket_permission_smoke=passed work=" << work << '\n';
  return EXIT_SUCCESS;
}
