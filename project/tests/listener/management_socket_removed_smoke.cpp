...
std::filesystem::path MakeTempDir() {
  std::string tmpl = "/tmp/sb_lmsr.XXXXXX";
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: sb_listener_management_socket_removed_smoke <sb_listener> <sb_parser_dummy>\n";
    return EXIT_FAILURE;
  }
  ...
  if (pid == 0) {
    ...
    ::execl(listener.c_str(),
            ...
            "--database-selector=dev_bootstrap_path:/tmp/sb_listener_management_socket_removed.sbdb",
            "--server-endpoint=unix:/tmp/sb_listener_management_socket_removed.sbps.sock",
            parser_arg.c_str(),
            control_arg.c_str(),
            runtime_arg.c_str(),
  ...
  std::error_code ec;
  std::filesystem::remove(management_socket, ec);
  if (ec) {
    cleanup();
    std::cerr << "could not remove management socket: " << ec.message() << '\n';
    return EXIT_FAILURE;
  }
  const int removed_probe = ConnectUnix(management_socket);
  if (removed_probe >= 0) {
    ::close(removed_probe);
    cleanup();
    std::cerr << "removed management socket remained connectable\n";
    return EXIT_FAILURE;
  }
  ...
  if (client_fd < 0) {
    cleanup();
    std::cerr << "listener stopped accepting clients after management socket removal\n";
    return EXIT_FAILURE;
  }
  std::string line;
  if (!ReadLineWithTimeout(client_fd, &line, 2000) || line != "ScratchBird dummy parser ready") {
    ::close(client_fd);
    cleanup();
    std::cerr << "client greeting failed after management socket removal: " << line << '\n';
    return EXIT_FAILURE;
  }
  if (!WriteAll(client_fd, "management-socket-removed\n") || !ReadLineWithTimeout(client_fd, &line, 1000) ||
      line != "management-socket-removed") {
    ::close(client_fd);
    cleanup();
    std::cerr << "client echo failed after management socket removal: " << line << '\n';
    return EXIT_FAILURE;
  }
  ::close(client_fd);
  cleanup();
  std::cout << "management_socket_removed_smoke=passed work=" << work << '\n';
  return EXIT_SUCCESS;
}
