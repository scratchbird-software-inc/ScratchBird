...
std::filesystem::path MakeTempDir() {
  std::string tmpl = "/tmp/sb_listener_manager_active.XXXXXX";
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: sb_listener_manager_active_status_smoke <sb_listener> <sb_parser_dummy>\n";
    return EXIT_FAILURE;
  }
  ...
  if (pid == 0) {
    ...
    ::execl(listener.c_str(),
            ...
            "--database-selector=dev_bootstrap_path:/tmp/sb_listener_manager_active.sbdb",
            "--server-endpoint=unix:/tmp/sb_listener_manager_active.sbps.sock",
            parser_arg.c_str(),
            control_arg.c_str(),
            runtime_arg.c_str(),
  ...
  bool saw_active_status = false;
  std::string status_body;
  for (int i = 0; i < 60; ++i) {
    auto status = scratchbird::manager::node::SendListenerManagementCommand(
        management_socket.string(), "STATUS", static_cast<std::uint64_t>(10 + i), 1000);
    if (status.ok) {
      status_body = status.response_text;
      if (status_body.find("\"handoff_complete_total\":1") != std::string::npos &&
          status_body.find("\"sys.metrics.listener.network.per_client_active_sessions\":1") != std::string::npos) {
  ...
  if (!saw_active_status) {
    ::close(client_fd);
    cleanup();
    std::cerr << "manager STATUS did not observe active parser-owned client; last_status=" << status_body << '\n';
    return EXIT_FAILURE;
  }

  std::string line;
  if (!ReadLineWithTimeout(client_fd, &line, 2500) || line != "ScratchBird dummy parser ready") {
    ::close(client_fd);
    cleanup();
    std::cerr << "active client did not receive dummy parser greeting: " << line << '\n';
    return EXIT_FAILURE;
  }

  ::close(client_fd);
  cleanup();
  std::cout << "manager_active_status_smoke=passed work=" << work << '\n';
  return EXIT_SUCCESS;
}
