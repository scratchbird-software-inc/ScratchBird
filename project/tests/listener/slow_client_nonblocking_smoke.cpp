...
std::filesystem::path MakeTempDir() {
  std::string tmpl = "/tmp/sb_listener_slow_client.XXXXXX";
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: sb_listener_slow_client_nonblocking_smoke <sb_listener> <sb_parser_dummy>\n";
    return EXIT_FAILURE;
  }
  ...
  if (pid == 0) {
    ...
    ::execl(listener.c_str(),
            ...
            "--database-selector=dev_bootstrap_path:/tmp/sb_listener_slow_client.sbdb",
            "--server-endpoint=unix:/tmp/sb_listener_slow_client.sbps.sock",
            parser_arg.c_str(),
            control_arg.c_str(),
            runtime_arg.c_str(),
  ...
  if (second_fd < 0) {
    ::close(first_fd);
    cleanup();
    std::cerr << "listener did not accept second connection while first was slow\n";
    return EXIT_FAILURE;
  }

  std::string line;
  if (!ReadLineWithTimeout(second_fd, &line, 2500) || line != "ScratchBird dummy parser ready") {
    ::close(first_fd);
    ::close(second_fd);
    cleanup();
    std::cerr << "second connection was blocked by slow first client; greeting=" << line << '\n';
    return EXIT_FAILURE;
  }
  if (!WriteAll(second_fd, "second-client\n") || !ReadLineWithTimeout(second_fd, &line, 1000) ||
      line != "second-client") {
    ::close(first_fd);
    ::close(second_fd);
    cleanup();
    std::cerr << "second connection echo failed: " << line << '\n';
    return EXIT_FAILURE;
  }
  ...
  ::close(second_fd);
  cleanup();
  std::cout << "slow_client_nonblocking_smoke=passed work=" << work << '\n';
  return EXIT_SUCCESS;
}
