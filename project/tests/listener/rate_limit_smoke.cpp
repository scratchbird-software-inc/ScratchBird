...
std::filesystem::path MakeTempDir() {
  std::string tmpl = "/tmp/sb_listener_rate_limit_smoke.XXXXXX";
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: sb_listener_rate_limit_smoke <sb_listener> <sb_parser_dummy>\n";
    return EXIT_FAILURE;
  }
  ...
  if (pid == 0) {
    ...
    ::execl(listener.c_str(),
            ...
            "--database-selector=dev_bootstrap_path:/tmp/sb_listener_rate_limit_smoke.sbdb",
            "--server-endpoint=unix:/tmp/sb_listener_rate_limit_smoke.sbps.sock",
            parser_arg.c_str(),
            control_arg.c_str(),
            runtime_arg.c_str(),
            ...
            "--warm-pool-min=2",
            "--warm-pool-max=2",
            "--accept-rate-limit-per-second=1",
            "--accept-rate-limit-burst=1",
            nullptr);
    _exit(127);
  }
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    if (::waitpid(pid, &status, WNOHANG) == pid) {
      std::cerr << "listener exited before accepting connections\n";
      return EXIT_FAILURE;
    }
  ...
  std::string line;
  if (!ReadLineWithTimeout(first_fd, &line, 3000) || line != "ScratchBird dummy parser ready") {
    cleanup();
    std::cerr << "first connection did not reach dummy parser: " << line << '\n';
    return EXIT_FAILURE;
  }

  const int second_fd = ConnectLoopback(port);
  if (second_fd < 0) {
    cleanup();
    std::cerr << "second connection could not connect to listener\n";
    return EXIT_FAILURE;
  }
  std::string second_line;
  const bool second_got_line = ReadLineWithTimeout(second_fd, &second_line, 700);
  ::close(second_fd);
  ::close(first_fd);
  if (second_got_line && second_line == "ScratchBird dummy parser ready") {
    cleanup();
    std::cerr << "second connection bypassed rate limiting\n";
    return EXIT_FAILURE;
  }

  cleanup();
  std::cout << "listener_rate_limit_smoke=passed work=" << work << '\n';
  return EXIT_SUCCESS;
}
