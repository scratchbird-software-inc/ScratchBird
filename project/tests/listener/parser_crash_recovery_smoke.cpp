...
std::filesystem::path MakeTempDir() {
  std::string tmpl = "/tmp/sb_lpc.XXXXXX";
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: sb_listener_parser_crash_recovery_smoke <sb_listener> <sb_parser_dummy>\n";
    return EXIT_FAILURE;
  }
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    if (::waitpid(pid, &status, WNOHANG) == pid) {
      std::cerr << "listener exited before first handoff\n";
      return EXIT_FAILURE;
    }
  ...
  if (first_fd < 0) {
    cleanup();
    std::cerr << "listener did not accept first client\n";
    return EXIT_FAILURE;
  }

  std::string line;
  if (ReadLineWithTimeout(first_fd, &line, 1200)) {
    ::close(first_fd);
    cleanup();
    std::cerr << "first crash-once handoff unexpectedly produced greeting: " << line << '\n';
    return EXIT_FAILURE;
  }
  ::close(first_fd);
  ...
  for (int i = 0; i < 30 && !recovered; ++i) {
    ...
    if (fd >= 0) {
      if (ReadLineWithTimeout(fd, &line, 1500) && line == "ScratchBird dummy parser ready") {
        if (WriteAll(fd, "after-parser-crash\n") &&
            ReadLineWithTimeout(fd, &line, 1000) &&
            line == "after-parser-crash") {
          recovered = true;
        }
      }
      last_line = line;
  ...
  if (!recovered) {
    cleanup();
    std::cerr << "listener did not recover a healthy parser after crash-once handoff; last_line=" << last_line << '\n';
    return EXIT_FAILURE;
  }

  cleanup();
  std::cout << "parser_crash_recovery_smoke=passed work=" << work << '\n';
  return EXIT_SUCCESS;
}
