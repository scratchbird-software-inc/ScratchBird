...
std::filesystem::path MakeTempDir() {
  std::string tmpl = "/tmp/sb_lpex.XXXXXX";
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: sb_listener_parser_pool_exhaustion_smoke <sb_listener> <sb_parser_dummy>\n";
    return EXIT_FAILURE;
  }
  ...
  if (pid == 0) {
    ...
    ::execl(listener.c_str(),
            ...
            "--database-selector=dev_bootstrap_path:/tmp/sb_lpex.sbdb",
            "--server-endpoint=unix:/tmp/sb_lpex.sbps.sock",
            parser_arg.c_str(),
            control_arg.c_str(),
            runtime_arg.c_str(),
  ...
  if (second_fd >= 0) {
    ...
    if (got_line && line == "ScratchBird dummy parser ready") {
      ::close(first_fd);
      cleanup();
      std::cerr << "parser pool exhaustion admitted a second client unexpectedly\n";
      return EXIT_FAILURE;
    }
  }
  ...
  Require(first_ready && first_line == "ScratchBird dummy parser ready",
          "first slow client should still be served by the only worker");
  std::cout << "parser_pool_exhaustion_smoke=passed work=" << work << '\n';
  return EXIT_SUCCESS;
}
