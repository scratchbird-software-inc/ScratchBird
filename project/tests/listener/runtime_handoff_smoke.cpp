...
std::filesystem::path MakeTempDir() {
  std::string tmpl = "/tmp/sb_listener_runtime_smoke.XXXXXX";
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: sb_listener_runtime_handoff_smoke <sb_listener> <sb_parser_dummy>\n";
    return EXIT_FAILURE;
  }
  ...
  if (pid == 0) {
    ...
    ::execl(listener.c_str(),
            ...
            "--database-selector=dev_bootstrap_path:/tmp/sb_listener_runtime_smoke.sbdb",
            "--server-endpoint=unix:/tmp/sb_listener_runtime_smoke.sbps.sock",
            parser_arg.c_str(),
            control_arg.c_str(),
            runtime_arg.c_str(),
  ...
  if (pid <= 0) {
    std::cerr << "could not fork listener\n";
    return EXIT_FAILURE;
  }
  ...
  if (fd < 0) {
    cleanup();
    std::cerr << "listener did not accept connections\n";
    return EXIT_FAILURE;
  }
  ...
  if (!WriteAll(fd, "listener-runtime-smoke\n") || !ReadLine(fd, &line) ||
      line != "listener-runtime-smoke") {
    cleanup();
  ...
  ::close(fd);
  cleanup();
  std::cout << "runtime_handoff_smoke=passed work=" << work << '\n';
  return EXIT_SUCCESS;
}
