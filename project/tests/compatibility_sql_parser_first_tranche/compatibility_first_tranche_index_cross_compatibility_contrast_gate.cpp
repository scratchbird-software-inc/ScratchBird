...
bool ExpectIndexDescriptor(std::string_view payload,
  ...
  ok &= ExpectField(payload, "descriptor_resolution", "uuid_required", label);
  ok &= ExpectField(payload, "compatibility_profile_uuid", expected.compatibility_profile_uuid,
                    label);
...
bool ExpectCrossCompatibilityDistinctFields(
  ...
  distinct("compatibility_profile_uuid", firebird.compatibility_profile_uuid,
           postgresql.compatibility_profile_uuid);
...
int main() {
  bool ok = true;
  ok &= ExpectCrossCompatibilityDistinctFields(kFirebirdUniqueIndex,
