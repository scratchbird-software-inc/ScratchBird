...
"""FPR-P5 compatibility parser dialect-isolation proof gate."""
...
def parser_package(compatibility: str) -> str:
    return f"project/src/parsers/compatibility/{compatibility}"


def udr_package(compatibility: str) -> str:
    return f"project/src/udr/sbu_{compatibility}_parser_support"
        ...
        for line_no, line in enumerate(text.splitlines(), start=1):
            for match in pattern.finditer(line):
                ...
                violations.append({
                    ...
                    "line": line_no,
                    "forbidden_compatibility_token": hit,
                })
    return violations
...
def validate_matrix(
    ...
    for row in rows:
        ...
        require(row["compatibility"] in DIALECTS, f"unknown compatibility in P5 matrix: {row['compatibility']}")
        require(row["parser_package"] == parser_package(row["compatibility"]),
                f"parser package mismatch: {row['guard_id']}")
        require(row["parser_support_udr"] == udr_package(row["compatibility"]),
                f"UDR package mismatch: {row['guard_id']}")
        require(row["manifest_profile_class"] == "compatibility_emulation",
                f"wrong manifest profile class: {row['guard_id']}")
        require(row["test_destination"] ==
                "project/tests/compatibility_sql_parser_first_tranche/parser_dialect_isolation_guard.py",
    ...
    for guard_type in EXPECTED_MATRIX_COUNTS:
        compatibilitys = {row["compatibility"] for row in grouped[guard_type]}
        require(compatibilitys == set(DIALECTS), f"{guard_type} compatibility coverage drift")
    ...
    for compatibility in DIALECTS:
        require(compatibility in manifest, f"manifest lacks compatibility profile: {compatibility}")
        require(manifest[compatibility]["parser_cross_dialect_dependency"] == "false",
                f"manifest cross-dialect dependency enabled: {compatibility}")
        require((repo_root / parser_package(compatibility)).is_dir(),
                f"parser package missing: {compatibility}")
        require((repo_root / udr_package(compatibility)).is_dir(),
                f"parser-support UDR missing: {compatibility}")

    runtime_rows = {row["compatibility"]: row for row in grouped["runtime_distinctive_probe"]}
    for compatibility, row in runtime_rows.items():
        require(row["probe_owner"] == compatibility, f"runtime probe owner mismatch: {row['guard_id']}")
        require(row["positive_probe"] == DISTINCTIVE_PROBES[compatibility],
                f"runtime probe text mismatch: {row['guard_id']}")
        require(row["expected_positive_token"] == EXPECTED_POSITIVE_TOKENS[compatibility],
...
def validate_source_boundaries(repo_root: pathlib.Path) -> dict[str, int]:
    ...
    for compatibility in DIALECTS:
        for root_kind, rel_path in (
            ("parser", parser_package(compatibility)),
            ("udr", udr_package(compatibility)),
        ):
            roots_checked += 1
            all_violations.extend(
                scan_source_root(repo_root, compatibility, root_kind, repo_root / rel_path)
            )
    if all_violations:
...
def require_positive_parse(
    ...
    binary = binary_path(build_root, compatibility)
    require(binary.exists(), f"parser binary missing: {binary}")
    code, stdout, stderr = run_parser(binary, probe)
    output = stdout + stderr
    require(code == 0, f"{compatibility} positive probe failed: {stderr}")
    for token in [
        "SBLRExecutionEnvelope.v3",
        f'"dialect":"{compatibility}"',
        expected_token,
        '"reference_engine_sql_executed":false',
        '"sql_text_included":false',
    ]:
        require(token in output, f"{compatibility} positive probe missing token {token}")
...
def validate_runtime(build_root: pathlib.Path) -> dict[str, int]:
    ...
    family_overlaps = 0
    for compatibility, probe in DISTINCTIVE_PROBES.items():
        require_positive_parse(
            build_root,
            compatibility,
            probe,
            EXPECTED_POSITIVE_TOKENS[compatibility],
        )
    ...
    for compatibility in DIALECTS:
        require_rejected_without_envelope(build_root, compatibility, "sbsql", SBSQL_NEGATIVE_PROBE)
...
def write_evidence(
    ...
    payload = {
        ...
        "cross_compatibility_detection_or_dispatch_allowed": False,
        "sbsql_surface_allowed_in_compatibility_parser": False,
    }
