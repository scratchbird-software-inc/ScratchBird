...
normalization, SBLR expansion, current compatibility parser packages, parser-support
...
EXPECTED_BASELINE_CTESTS = {
    "compatibility_sql_parser_first_tranche_probe",
    "sbu_compatibility_sql_parser_support_probe",
    "compatibility_sql_parser_first_tranche_manifest_gate",
    "compatibility_sql_parser_standalone_source_guard",
    "non_sql_compatibility_sblr_surface_gate",
    "compatibility_sql_parser_first_tranche_cli_evidence_gate",
    "compatibility_sql_first_tranche_original_tool_replay_gate",
}

EXPECTED_COUNTS = {
    "manifest_compatibility_emulation_profiles": 25,
    "manifest_capability_reference_profiles": 3,
    "actual_compatibility_parser_package_dirs": 25,
...
def registered_ctests(build_root: pathlib.Path) -> set[str]:
    ...
    if not ctest_file.exists():
        fail("compatibility SQL parser tranche CTest file is not configured")
...
def validate_scope_matrix(repo_root: pathlib.Path) -> dict[str, int]:
    ...
    for row in alias_rows:
        require(
            row["implementation_package_path"] == "project/src/parsers/compatibility/mysql",
    ...
    require(
        any(row["implementation_package_path"] ==
            "project/src/parsers/compatibility/apache_ignite" for row in apache_rows),
...
def validate_manifest_and_packages(repo_root: pathlib.Path) -> dict[str, int]:
    manifest = read_csv(
        repo_root,
        "project/src/parsers/compatibility/CompatibilityProfileManifest.csv",
    )
    compatibility_profiles = [row for row in manifest if row["profile_class"] == "compatibility_emulation"]
    capability_profiles = [
        row for row in manifest if row["profile_class"] == "capability_reference"
    ]
    require(len(compatibility_profiles) == 25, "manifest compatibility_emulation count drift")
    ...
    require(len(parser_dirs) == 25, "actual compatibility parser package count drift")
    ...
    for row in manifest:
        ...
        if fid == "mysql_lts":
            require(
                ...
                "mysql_lts manifest row must be release/profile metadata, not compatibility_emulation",
            )
            require(
                row["parser_module"] == "project/src/parsers/compatibility/mysql",
    ...
    return {
        "manifest_compatibility_emulation_profiles": len(compatibility_profiles),
        "manifest_capability_reference_profiles": len(capability_profiles),
        "actual_compatibility_parser_package_dirs": len(parser_dirs),
