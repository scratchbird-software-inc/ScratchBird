...
"""FPR-P2 cluster compatibility route remap proof gate."""
...
def main() -> int:
    ...
    require(len(map_rows) == EXPECTED_CLUSTER_ROWS, "cluster compatibility map row count drift")
    ...
    for row in matrix_rows:
        ...
        for column in ["compatibility", "compatibility_surface", "normalized_command", "sblr_route", "refusal_policy"]:
            require(row[column if column != "compatibility_surface" else "compatibility_surface"] == intake[column],
        ...
        require(row["compatibility_cluster_command"] == intake["compatibility_surface"],
                f"compatibility cluster command mismatch: {row['remap_id']}")
        require(row["compatibility_isolation"] == "own_compatibility_surface_only",
                f"compatibility isolation is not preserved: {row['remap_id']}")
        ...
        require("compatibility_execution_storage_transaction_finality_authority_forbidden" in
                proof["compatibility_authority_policy"],
                f"P7 compatibility authority policy drift: {row['remap_id']}")
        ...
        require("compatibility_cluster_execution_storage_transaction_finality_authority_forbidden" in
                row["compatibility_authority_policy"],
                f"compatibility authority policy missing: {row['remap_id']}")

    require(query_rows == EXPECTED_QUERY_COMPATIBILITY_ROWS, f"cluster query compatibility row count drift: {query_rows}")
