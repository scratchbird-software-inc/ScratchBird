...
using scratchbird::parser::compatibility::ParseStatement;
...
constexpr OperationPattern kPatterns[] = {
    {"SELECT", PatternMatch::kPrefix, "query", "common.query.select",
     ...
     "SBLR_COMPATIBILITY_COMMON_SELECT", "EngineSelect",
    ...
    {"CHANGE REPLICATION", PatternMatch::kPrefix, "replication",
     ...
     "common.udr.replication.change", "SBLR_COMPATIBILITY_COMMON_REPLICATION_ROUTE",
    ...
    {"LOAD DATA", PatternMatch::kPrefix, "bulk_io",
     ...
     "Server-local compatibility file access is refused by policy.", true, false},
    ...
    {"REPAIR", PatternMatch::kPrefix, "maintenance", "common.maintenance.repair",
     ...
     "Low-level repair and verify commands are unsupported from compatibility syntax.",
     true, false},
};
...
const DialectProfile& TestProfile() {
  static const DialectProfile profile{
      ...
      "test_only",
      "COMMON",
      "sblr.compatibility.common.enterprise_readiness_test.v1",
      kPatterns,
      kNoSurfaces,
      kNoSurfaces,
      kNoSurfaces,
...
bool ExpectReadinessSection(std::string_view envelope) {
  ...
  constexpr std::string_view kCompletedStatusFields[] = {
      ...
      "compatibility_native_regression_status",
