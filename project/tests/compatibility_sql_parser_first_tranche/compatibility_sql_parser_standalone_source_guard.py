...
"""Guard compatibility parser modules against cross-dialect implementation drift."""
...
def source_roots(repo_root: Path, dialect: str) -> list[tuple[str, Path]]:
    parser_root = repo_root / "project/src/parsers/compatibility" / dialect
...
def main() -> int:
    ...
    evidence = {
        "gate": "compatibility_sql_parser_standalone_source_guard",
        "status": "passed",
        "dialects": list(DIALECTS),
        "source_roots_checked": ["project/src/parsers/compatibility/<dialect>",
...
if __name__ == "__main__":
    try:
        ...
        print(f"compatibility_sql_parser_standalone_source_guard: {exc}", file=sys.stderr)
        raise SystemExit(1)
