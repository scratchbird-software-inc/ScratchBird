...
"""Truth gate for compatibility procedural-body and datatype enterprise readiness.
...
REQUIRED_FILES = (
    ...
    "COMPATIBILITY_NATIVE_PROCEDURAL_REGRESSION_MATRIX.csv",
...
REQUIRED_COMPATIBILITY_OPERATION_AREAS = {
    "procedural_api_functional_encoding",
    "exact_datatype_semantics",
    "observable_result_equivalence",
    "compatibility_semantic_defaults",
    "compatibility_native_regression_ctest",
...
SOURCE_EVIDENCE = {
    ...
    "firebird_routine_route_classifier": {
        "path": pathlib.Path("project/src/parsers/compatibility/firebird/firebird_dialect.cpp"),
        "needles": (
    ...
    "first_tranche_route_probe": {
        "path": pathlib.Path(
            ...
            "compatibility_sql_parser_first_tranche_probe.cpp"
        ),
        "needles": (
...
def compatibility_rows(rows: list[dict[str, str]], path: pathlib.Path) -> dict[str, dict[str, str]]:
    by_compatibility = {row["compatibility"]: row for row in rows}
    if len(by_compatibility) != len(rows):
        raise AssertionError(f"{path}: duplicate compatibility rows")
    expected = set(EXPECTED_COMPATIBILITYS)
    actual = set(by_compatibility)
    if actual != expected:
        raise AssertionError(
            f"{path}: compatibility set mismatch missing={sorted(expected - actual)} "
...
def validate_procedural_matrix(path: pathlib.Path) -> dict[str, object]:
    ...
    by_compatibility = compatibility_rows(rows, path)
    invalid: list[str] = []
    for compatibility, row in sorted(by_compatibility.items()):
        ...
        if current not in BLOCKER_STATUSES and current != COMPLETION_STATUS:
            invalid.append(f"{compatibility}: unexpected current status {current}")
        if required != COMPLETION_STATUS:
            invalid.append(f"{compatibility}: required status is not {COMPLETION_STATUS}")
        if "executable SBLR" not in encoding:
            invalid.append(f"{compatibility}: required encoding does not require executable SBLR")
        if "UUID-bound AST" not in encoding:
            invalid.append(f"{compatibility}: required encoding does not require UUID binding")
        if not row["proof_destination"].startswith("project/tests/"):
            invalid.append(f"{compatibility}: proof destination is outside project/tests")
    if invalid:
...
def validate_datatype_matrix(path: pathlib.Path) -> dict[str, object]:
    ...
    for compatibility, row in sorted(by_compatibility.items()):
        ...
        if "generic text" not in shortcut:
            invalid.append(f"{compatibility}: forbidden shortcuts do not ban generic text fallback")
        if not row["proof_destination"].startswith("project/tests/"):
            invalid.append(f"{compatibility}: proof destination is outside project/tests")
    if invalid:
        raise AssertionError(f"{path}: " + "; ".join(invalid))
...
def validate_semantic_defaults_matrix(path: pathlib.Path) -> dict[str, object]:
    ...
    for row in rows:
        ...
        if "compatibility" not in row["semantic_defaults_required"].lower():
            invalid.append(f"{row_id}: semantic defaults do not mention compatibility behavior")
        if (
        ...
        if "compatibility_profile_uuid" not in row["sblr_descriptor_fields_required"]:
            invalid.append(f"{row_id}: descriptor fields do not require compatibility_profile_uuid")
...
def validate_compatibility_operation_execution_plans(repo_root: pathlib.Path) -> dict[str, object]:
    roots = sorted(
        (repo_root / "public_release_evidence").glob("compatibility-parser-*implementation-readiness")
    )
    full_firebird = (
    ...
    for root in roots:
        ...
        missing = sorted(REQUIRED_COMPATIBILITY_OPERATION_AREAS - areas)
        extra = sorted(areas - REQUIRED_COMPATIBILITY_OPERATION_AREAS)
        if missing:
        ...
        required_readme = (
            ...
            "compatibility-profiled semantic defaults",
...
def validate_readme(path: pathlib.Path) -> dict[str, object]:
    ...
    required_phrases = (
        ...
        "Every compatibility parser must preserve compatibility semantic defaults",
        "observable equivalence with the original legacy compatibility engine",
        "generic ScratchBird descriptor",
        "No compatibility parser may execute compatibility SQL as storage or transaction authority.",
        "No original compatibility source may become execution authority",
        "file presence",
        "executable SBLR",
        "JIT",
        "AOT",
...
def build_evidence(repo_root: pathlib.Path, strict_release: bool) -> dict[str, object]:
    ...
    matrices = {
        ...
        "semantic_defaults": validate_semantic_defaults_matrix(
            execution_plan / "COMPATIBILITY_SEMANTIC_DEFAULTS_MATRIX.csv"
        ),
        ...
        "native_regression": validate_status_matrix(
            execution_plan / "COMPATIBILITY_NATIVE_PROCEDURAL_REGRESSION_MATRIX.csv", "proof_id"
        ),
        ...
        "compatibility_operation_execution-plans": validate_compatibility_operation_execution_plans(repo_root),
    }
    ...
    return {
        "gate": "compatibility_procedural_datatype_enterprise_truth_gate",
        "strict_release": strict_release,
        "execution_plan": str(EXECUTION_PLAN_REL),
        "compatibility_count": len(EXPECTED_COMPATIBILITYS),
        ...
        "compatibility_sql_is_storage_or_transaction_authority": False,
        ...
        "requires_compatibility_semantic_defaults": True,
        "requires_no_generic_defaults": True,
        "requires_all_compatibility_execution-plans_updated": True,
        "requires_project_tests_proof": True,
        "requires_compatibility_native_regression": True,
...
def main(argv: list[str]) -> int:
    ...
    print(
        "compatibility_procedural_datatype_enterprise_truth_gate="
...
if __name__ == "__main__":
    try:
        ...
        print(f"compatibility_procedural_datatype_enterprise_truth_gate: {exc}",
              file=sys.stderr)
        raise SystemExit(1)
