...
std::filesystem::path MakeTempDir() {
  std::string tmpl = "/tmp/sb_sbp_sbsql_handoff.XXXXXX";
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: sbp_sbsql_handoff_smoke <sb_listener> <sbp_sbsql>\n";
    return EXIT_FAILURE;
  }
  ...
  if (pid == 0) {
    ...
    ::execl(listener.c_str(),
            ...
            "--database-selector=dev_bootstrap_path:/tmp/sb_sbp_sbsql_handoff.sbdb",
            "--server-endpoint=unix:/tmp/sb_sbp_sbsql_handoff.sbps.sock",
            parser_arg.c_str(),
            control_arg.c_str(),
            runtime_arg.c_str(),
  ...
  if (!ReadLine(fd, &line) || line != "ScratchBird SBSQL parser ready") {
    cleanup();
    std::cerr << "unexpected sbp_sbsql greeting: " << line << '\n';
    return EXIT_FAILURE;
  }
  if (!WriteAll(fd, "PING\n") || !ReadLine(fd, &line) || line != "OK PONG") {
    cleanup();
    std::cerr << "unexpected PING response: " << line << '\n';
    return EXIT_FAILURE;
  }
  ::close(fd);
  cleanup();
  std::cout << "sbp_sbsql_handoff_smoke=passed work=" << work << '\n';
  return EXIT_SUCCESS;
}
