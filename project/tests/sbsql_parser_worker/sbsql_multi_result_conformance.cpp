...
ServerSessionRegistry MakeRegistry(
  ...
  Require(session_uuid != nullptr, "multi-result session output is required");
...
const ServerCursorRecord& FindCursor(
  ...
  Require(found != registry.cursors_by_uuid.end(),
          "multi-result cursor fixture is missing");
  return found->second;
}

std::array<std::uint8_t, 16> InstallMultiResultCursor(
    ServerSessionRegistry* registry,
    const std::array<std::uint8_t, 16>& session_uuid,
    std::uint64_t result_sets) {
  Require(registry != nullptr && result_sets != 0,
          "multi-result cursor fixture is invalid");
  ...
  cursor.multi_result_kind = "multi_result_sequence";
  cursor.multi_result_count = result_sets;
  cursor.total_row_count = result_sets * 2 + 1;
  ...
  Require(digest.ok(), "multi-result descriptor binding digest failed");
...
int main() {
  ...
  const auto cursor_uuid =
      InstallMultiResultCursor(&registry, session_uuid, 3);
  ...
  const auto fetch2 = scratchbird::server::HandleFetch(
      &registry, FetchFrame(session_uuid, FindCursor(registry, cursor_uuid), 4));
  const auto payload2 =
      scratchbird::server::DecodeFetchResultForTest(fetch2.payload);
  Require(fetch2.accepted && payload2.has_value() &&
              payload2->row_count == 4 && !payload2->end_of_cursor,
