...
scratchbird::server::HostedEngineState MakeEngineState() {
  ...
  database.database_path = "/tmp/sb_multi_result_conformance.sbdb";
  database.database_uuid = "019e05df-f010-7000-8000-000000000016";
...
scratchbird::server::ServerSessionRegistry MakeRegistry(
  ...
  session.database_path = "/tmp/sb_multi_result_conformance.sbdb";
  session.database_uuid = "019e05df-f010-7000-8000-000000000016";
...
int main() {
  ...
  const auto execute = scratchbird::server::HandleExecuteSblr(
      &registry, engine_state, ExecuteFrame(session_uuid, MultiResultEnvelope(3)));
  Require(execute.accepted, "multi-result execute was rejected");
  const auto cursor_uuid = scratchbird::server::DecodeCursorUuidForTest(execute.payload);
  Require(cursor_uuid.has_value(), "multi-result execute did not return cursor UUID");
  ...
  Require(fetch1.accepted && payload1.has_value() && payload1->row_count == 2 && !payload1->end_of_cursor,
          "first multi-result fetch did not return metadata plus command tag");
  Require(Contains(payload1->row_packet, "\"event\":\"result_set_metadata\"") &&
              Contains(payload1->row_packet, "\"ordinal\":0") &&
              Contains(payload1->row_packet, "\"event\":\"command_tag\"") &&
              Contains(payload1->row_packet, "\"tag\":\"SELECT 1\""),
          "first multi-result fetch missing result-set metadata or command tag");

  const auto fetch2 = scratchbird::server::HandleFetch(&registry, FetchFrame(session_uuid, *cursor_uuid, 4));
  const auto payload2 = scratchbird::server::DecodeFetchResultForTest(fetch2.payload);
  Require(fetch2.accepted && payload2.has_value() && payload2->row_count == 4 && !payload2->end_of_cursor,
          "second multi-result fetch did not return middle result sets");
  Require(Contains(payload2->row_packet, "\"ordinal\":1") &&
              Contains(payload2->row_packet, "\"tag\":\"SELECT 2\"") &&
              Contains(payload2->row_packet, "\"ordinal\":2"),
          "second multi-result fetch missing middle result-set sequence");
  ...
  Require(fetch3.accepted && payload3.has_value() && payload3->row_count == 1 && payload3->end_of_cursor,
          "final multi-result fetch did not return finality");
  Require(Contains(payload3->row_packet, "\"event\":\"multi_result_finality\"") &&
              Contains(payload3->row_packet, "\"result_sets\":3") &&
              Contains(payload3->detail, "\"end_of_cursor\":true"),
          "final multi-result fetch missing finality or cursor metadata");

  std::cout << "sbsql_multi_result_conformance=passed\n";
  return EXIT_SUCCESS;
}
