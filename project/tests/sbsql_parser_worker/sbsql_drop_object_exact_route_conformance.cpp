...
void RequireRegistryRow(std::string_view surface_id,
  ...
  Require(row != nullptr, "DROP object generated registry row missing");
  Require(row->canonical_name == canonical_name &&
              row->surface_kind == surface_kind && row->family == family &&
              row->source_status == "native_now" &&
              row->cluster_scope == "noncluster_or_profile_scoped" &&
              row->sblr_operation_family == sblr_family,
          "DROP object generated registry row drifted");
}

void RequireRegistryEvidence() {
  RequireRegistryRow("SBSQL-40CAFAB37942", "drop_object",
                     "canonical_surface", "ddl_catalog", kFamily);
  RequireRegistryRow("SBSQL-CFFCCDEF6AC4", "drop_object_stmt",
...
void RequireExactRefusal(const DropCase& drop_case) {
  ...
  Require(artifacts.envelope.operation_id == "engine.op.diagnostic_refusal" &&
          ...
          "DROP object exact refusal tuple drifted");
  Require(artifacts.envelope.result_shape_key == "diagnostic_vector.v1" &&
          ...
          "DROP object refusal metadata drifted");
  Require(!artifacts.envelope.parser_executes_sql &&
          ...
          "DROP object refusal retained executable authority");
  Require(HasValue(artifacts.envelope.required_authority_steps,
          ...
          "DROP object refusal authority steps drifted");
  Require(artifacts.envelope.messages.diagnostics.size() == 1,
          "DROP object did not emit one exact diagnostic");
