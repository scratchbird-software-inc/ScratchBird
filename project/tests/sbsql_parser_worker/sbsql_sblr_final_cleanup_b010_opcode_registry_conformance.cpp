...
constexpr std::array<OpcodeRow, 50> kRows{{
    ...
    {"engine.op.read_metrics", "SBLR_READ_METRICS", "diagnostics-and-metrics", sblr::SblrOpcodeCategory::observability, sblr::SblrOpcodeSupport::implemented, sblr::SblrOpcodeTransactionEffect::read, sblr::SblrOpcodeSecurityClass::authenticated, false, false},
...
void RequireClusterProviderBoundary(const OpcodeRow& row) {
  ...
  request.context.database_uuid = scratchbird::tests::FixtureUuid(1208, 4501);
  request.context.session_uuid = scratchbird::tests::FixtureUuid(1208, 4502);
  request.context.principal_uuid = scratchbird::tests::FixtureUuid(1208, 4503);
