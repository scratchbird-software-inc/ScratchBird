...
bool ValidateGroupingSetsBooleanHavingParserBindingLoweringAndDispatch() {
  ...
  passed &= Require(
      sum_only_ast.native_relational.recognized() &&
          ...
          HasParserDiagnostic(refused_lowering.messages,
                              "SBLR.PLAN_TREE.INVALID_HANDLE"),
      "grouping-set HAVING syntax, form, metadata, semantic, or ordinal drift did not fail closed");
  return passed;
}
...
bool ValidateRollupBooleanHavingParserBindingLoweringAndDispatch() {
  ...
  passed &= Require(
      sum_only_ast.native_relational.recognized() &&
          ...
          HasParserDiagnostic(refused_lowering.messages,
                              "SBLR.PLAN_TREE.INVALID_HANDLE"),
      "ROLLUP HAVING syntax, form, metadata, semantic, or lowering drift did not fail closed");
  return passed;
}
...
bool ValidateCubeBooleanHavingParserBindingLoweringAndDispatch() {
  ...
  passed &= Require(
      sum_only_ast.native_relational.recognized() &&
          ...
          HasParserDiagnostic(refused_lowering.messages,
                              "SBLR.PLAN_TREE.INVALID_HANDLE"),
      "CUBE HAVING syntax, metadata, semantic, or lowering drift did not fail closed");
  return passed;
}
