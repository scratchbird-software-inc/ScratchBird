...
std::string ContextUuid(const unsigned value) {
  ...
  std::snprintf(buffer, sizeof(buffer),
                "019f0000-0000-7650-8000-%012u", value);
  return buffer;
}
...
api::EngineDescriptor Descriptor() {
  api::EngineDescriptor descriptor;
  descriptor.descriptor_uuid = scratchbird::tests::FixtureUuidLiteral("019f0000-0000-7200-8000-000000007201");
...
exec::CanonicalDescriptorLimitRequest Request() {
  ...
  UpgradeToCanonicalStatementContext(&request.physical_dag, 7200);
...
bool ValidatePhysicalLimit() {
  ...
  passed &= Require(
      ...
      "typed physical limit node or statement context was not executable");
  ...
  passed &= Require(
      result.diagnostic.ok && result.output_batch.rows.empty() &&
          exec::PhysicalMgaStatementContextEqual(
              result.mga_statement_context,
              request.physical_dag.mga_statement_context),
      "maximum OFFSET overflowed into visible rows or lost statement context");
  ...
  passed &= Require(
      result.diagnostic.ok &&
          ...
          result.output_batch.rows[0].values[0].encoded_value == "4.00" &&
          exec::PhysicalMgaStatementContextEqual(
              result.mga_statement_context,
              request.physical_dag.mga_statement_context),
      "maximum LIMIT overflowed arithmetic or lost statement context");
  ...
  passed &= Require(
      result.diagnostic.ok && result.output_batch.rows.empty() &&
          exec::PhysicalMgaStatementContextEqual(
              result.mga_statement_context,
              request.physical_dag.mga_statement_context),
      "LIMIT zero returned data or lost statement context");
  ...
  passed &= ValidateStatementContextMatrix(
      Request(), [](const auto& candidate) {
        return exec::ExecuteCanonicalDescriptorLimit(candidate);
      });
  return passed;
}
