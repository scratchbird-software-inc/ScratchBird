...
| Donor alias coverage | `DONOR_ALIAS_TO_SBSQL_SURFACE_MATRIX.csv` retains 312 rows. |
...
`MANIFEST.yaml` also contains pre-existing donor exact-extraction entries that are outside this parser/UDR/SBSQL closure execution_plan. The runnable FSPE-013 gate intentionally validates manifest invariants and parser-v3/SBSQL canonicalization authority paths, while treating unrelated donor-manifest repair as a separate contract-corpus maintenance concern.
