...
"""SBSFC-030 temporal formatting and epoch helper fixture gate."""
...
FIXTURES = "project/tests/sbsql_parser_worker/generated/full_surface/SBSFC_030_TEMPORAL_FORMAT_EPOCH_FIXTURES.csv"

TARGETS = {
    "sb.temporal.day_name": {"SBSQL-F6C685816805", "SBSQL-308BCDB4E875"},
    "sb.temporal.epoch": {"SBSQL-1B190CAA8EB4", "SBSQL-EF179A79677B"},
    "sb.temporal.from_unixtime": {"SBSQL-819EAB680D05", "SBSQL-E6D846B78614"},
    "sb.temporal.month_name": {"SBSQL-924B70AB5641", "SBSQL-EBE863C39BD6"},
    "sb.temporal.months_between": {"SBSQL-A1FB79D234A0", "SBSQL-A66313AFAD59"},
    "sb.temporal.next_day": {"SBSQL-3D0D5DEFD6B5", "SBSQL-A48CE29C1EF8"},
    "sb.scalar.to_char": {"SBSQL-D9E42211F7B8", "SBSQL-7C1E9B3A101C"},
    "sb.scalar.to_date": {"SBSQL-B4E283C07390", "SBSQL-8457926FEA71"},
    "sb.scalar.to_number": {"SBSQL-9AFD6269AD11"},
    "sb.scalar.to_timestamp": {"SBSQL-9B20B713B248", "SBSQL-CDD43912803F"},
}
...
def main() -> int:
    ...
    registered_ids = set(re.findall(r'"(sb\.(?:temporal|scalar)\.(?:day_name|epoch|from_unixtime|month_name|months_between|next_day|to_char|to_date|to_number|to_timestamp))"', seed_text))
    ...
    for row in fixtures:
        ...
        if case_kind in {"arity_error", "domain_error", "invalid_input", "type_error"}:
    ...
    for canonical, surface_ids in sorted(TARGETS.items()):
        ...
        if canonical != "sb.scalar.to_number" and refusal_by_builtin[canonical] == 0:
    ...
    print(
        "sbsql_sbsfc_030_temporal_format_epoch_fixture_gate "
        f"fixtures={len(fixtures)} canonicals={len(TARGETS)} errors={len(errors)}"
    )
    if errors:
        print("sbsql_sbsfc_030_temporal_format_epoch_fixture_gate=failed", file=sys.stderr)
        for error in errors[:30]:
            print(f"  {error}", file=sys.stderr)
        return 1
    print("sbsql_sbsfc_030_temporal_format_epoch_fixture_gate=passed")
    return 0
