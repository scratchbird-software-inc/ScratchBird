...
"""SBSFC-017 regex/text-pattern scalar fixture gate."""
...
FIXTURES = "project/tests/sbsql_parser_worker/generated/full_surface/SBSFC_017_REGEX_TEXT_PATTERN_FIXTURES.csv"

TARGETS = {
    "sb.scalar.regexp_count": {"SBSQL-A0A8A427C5EC", "SBSQL-5A2E7551B74D"},
    "sb.scalar.occurrences_regex": {"SBSQL-9E51B994A03C", "SBSQL-5F642BC24E5F"},
    "sb.scalar.position_regex": {"SBSQL-FEC27B990B29", "SBSQL-0FF0127F4755"},
    "sb.scalar.regexp_match": {"SBSQL-642462C48322", "SBSQL-E8DA01F5F35C"},
    "sb.scalar.regexp_matches": {"SBSQL-7DE0B158322A", "SBSQL-D13340E9CF67"},
    "sb.scalar.regexp_replace": {"SBSQL-B6043EBD18DA", "SBSQL-6FB5567A4945"},
    "sb.scalar.regexp_split_to_array": {"SBSQL-E9A74ECD791A", "SBSQL-0714D2A0949C"},
    "sb.scalar.regexp_split_to_table": {"SBSQL-3053BF29742E", "SBSQL-B684AF9349FE"},
    "sb.scalar.regexp_substr": {"SBSQL-0E0495604E37", "SBSQL-3EB1ACE54376"},
    "sb.scalar.substring_regex": {"SBSQL-AB367C935012", "SBSQL-EA11A0912D91"},
    "sb.scalar.translate_regex": {"SBSQL-D14DD562957B", "SBSQL-76BE7E8C82E9"},
}
...
def main() -> int:
    ...
    registered_ids = set(
        re.findall(
            r'"(sb\.scalar\.(?:regexp_[\w_]+|occurrences_regex|position_regex|substring_regex|translate_regex))"',
            seed_text,
        )
    )
    ...
    for row in fixtures:
        ...
        elif case_kind in {"arity_error", "domain_error", "type_error"}:
    ...
    print(
        "sbsql_sbsfc_017_regex_text_pattern_fixture_gate "
        f"fixtures={len(fixtures)} canonicals={len(TARGETS)} errors={len(errors)}"
    )
    if errors:
        print("sbsql_sbsfc_017_regex_text_pattern_fixture_gate=failed", file=sys.stderr)
        for error in errors[:30]:
            print(f"  {error}", file=sys.stderr)
        return 1
    print("sbsql_sbsfc_017_regex_text_pattern_fixture_gate=passed")
    return 0
