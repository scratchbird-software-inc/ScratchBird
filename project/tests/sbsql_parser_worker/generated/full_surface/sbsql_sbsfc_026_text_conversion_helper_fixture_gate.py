...
"""SBSFC-026R-K text/conversion helper scalar fixture gate."""
...
TARGETS = {
    "sb.scalar.convert": {"SBSQL-138613D5F135", "SBSQL-22879CAE432D"},
    "sb.scalar.array_to_string": {"SBSQL-312B2C476C92", "SBSQL-2DAD0F0E4386"},
    "sb.scalar.to_ascii": {"SBSQL-300B227B2DB3", "SBSQL-A2B087795B49"},
    "sb.scalar.format": {"SBSQL-3E2297C02DE8", "SBSQL-DF7403739D12"},
}
...
def main() -> int:
    ...
    registered_ids = set(
        re.findall(r'"(sb\.scalar\.(?:convert|array_to_string|to_ascii|format))"', seed_text)
    )
    ...
    print(
        "sbsql_sbsfc_026_text_conversion_helper_fixture_gate "
        f"fixtures={len(fixtures)} canonicals={len(TARGETS)} errors={len(errors)}"
    )
    if errors:
        print("sbsql_sbsfc_026_text_conversion_helper_fixture_gate=failed", file=sys.stderr)
        for error in errors[:40]:
            print(f"  {error}", file=sys.stderr)
        return 1
    print("sbsql_sbsfc_026_text_conversion_helper_fixture_gate=passed")
    return 0
