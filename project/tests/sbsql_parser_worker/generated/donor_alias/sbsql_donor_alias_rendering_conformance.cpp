...
void ValidateCommands(const CsvTable& commands, Harness* harness) {
  ...
  for (const auto command_name :
       {"sbsql_donor_alias_mapping_conformance",
    ...
    harness->Check(Contains(Field(row, "executable_or_contract"),
                            "sbsql_donor_alias_rendering_conformance"),
                   std::string(command_name) + " command does not select donor alias gate");
    harness->Check(Contains(Field(row, "evidence_artifact"),
                            "DONOR_ALIAS_RENDERING_REPORT.md") &&
...
void ValidateReport(const std::filesystem::path& artifact_root, Harness* harness) {
  const auto report = ReadText(artifact_root / "DONOR_ALIAS_RENDERING_REPORT.md");
  for (const auto token :
       {"Status: complete", "312", "Donor Profiles", "Rendering Policy",
        "DONOR_ALIAS_RENDERING_FIXTURES.csv", "message vector",
        "engine authority", "sbsql_donor_alias_rendering_conformance"}) {
    harness->Check(Contains(report, token),
                   std::string("DONOR_ALIAS_RENDERING_REPORT.md missing token ") + token);
  }
}
...
void ValidateFixturePolicy(const CsvTable& fixture_policy, Harness* harness) {
  ...
  for (const auto& row : fixture_policy.rows) {
    ...
    harness->Check(StartsWith(Field(row, "fixture_root"),
                   ...
                   alias_kind + " fixture root is not durable donor_alias project path");
    harness->Check(Field(row, "ctest_label") == "sbsql_donor_alias_rendering_conformance",
    ...
    harness->Check(StartsWith(Field(row, "command_tag_policy"), "render_donor_"),
                   alias_kind + " command tag policy is not donor-rendered");
    harness->Check(Contains(Field(row, "warning_error_policy"),
                            "donor_message_vector"),
                   alias_kind + " warning/error policy does not use donor message vectors");
    harness->Check(Contains(Field(row, "catalog_shape_policy"),
                            "donor_catalog_projection_or_exact_refusal"),
...
void ValidateDonorMatrices(const CsvTable& canonical_aliases,
  ...
  const auto canonical_by_surface = IndexUnique(canonical_aliases, "donor_surface", harness);
  const auto backlog_by_surface = IndexUnique(backlog_aliases, "donor_surface", harness);
  ...
  harness->Check(canonical_aliases.rows.size() == 312,
                 "canonical donor alias matrix must contain 312 rows");
  harness->Check(backlog_aliases.rows.size() == canonical_aliases.rows.size(),
                 "donor alias backlog/canonical row count mismatch");

  std::map<std::string, std::size_t> donor_counts;
  std::map<std::string, std::size_t> alias_kind_counts;

  for (const auto& [donor_surface, canonical] : canonical_by_surface) {
    const auto backlog_it = backlog_by_surface.find(donor_surface);
    harness->Check(backlog_it != backlog_by_surface.end(),
                   donor_surface + " missing from donor alias backlog");
    ...
    for (const auto column :
      ...
      harness->Check(Field(*canonical, column) == Field(backlog, column),
                     donor_surface + " mismatch for " + std::string(column));
    }

    const std::string donor(Field(*canonical, "donor"));
    const std::string alias_kind(Field(*canonical, "alias_kind"));
    ++donor_counts[donor];
    ++alias_kind_counts[alias_kind];

    harness->Check(StartsWith(donor_surface, donor + ":"),
                   donor_surface + " does not start with donor profile");
    harness->Check(policy_by_alias_kind.contains(alias_kind),
                   donor_surface + " has no rendering fixture policy");
    harness->Check(Field(backlog, "mapping_status") ==
                       "mapped_by_profile_or_refused_with_exact_diagnostic",
                   donor_surface + " mapping status is not exact mapped/refused policy");
    harness->Check(!Field(backlog, "native_sbsql_surface").empty(),
                   donor_surface + " missing native SBSQL surface");
    harness->Check(StartsWith(Field(backlog, "sblr_operation_family"), "sblr."),
                   donor_surface + " missing SBLR operation family");
    harness->Check(Contains(Field(backlog, "parser_owned_behavior"), "donor syntax") &&
                       Contains(Field(backlog, "parser_owned_behavior"),
                                "donor error rendering"),
                   donor_surface + " parser-owned behavior lacks donor rendering");
    harness->Check(Contains(Field(backlog, "engine_owned_behavior"), "UUID authority") &&
                       Contains(Field(backlog, "engine_owned_behavior"),
                                "SBLR execution"),
                   donor_surface + " engine-owned behavior does not preserve authority");
    harness->Check(Field(backlog, "owner_lane") ==
                       "donor conformance/parser profile worker",
                   donor_surface + " owner lane mismatch");
    harness->Check(Contains(Field(backlog, "target_file_group"),
                            "project/tests/sbsql_parser_worker"),
                   donor_surface + " target file group lacks generated test path");
    harness->Check(Field(backlog, "diagnostic_target") ==
                       "donor_profile_message_vector_rendering",
                   donor_surface + " diagnostic target mismatch");
    harness->Check(StartsWith(Field(backlog, "validation_fixture_id"), "SBSQL-DONOR-"),
                   donor_surface + " fixture id is not donor-prefixed");
    harness->Check(Contains(Field(backlog, "final_acceptance_rule"),
                            "donor_rendering_test"),
                   donor_surface + " final acceptance lacks donor rendering test");
    harness->Check(Field(backlog, "closure_action") ==
                       "map_to_native_behavior_or_exact_canonical_refusal",
                   donor_surface + " closure action mismatch");
    harness->Check(!Field(backlog, "status").empty(),
                   donor_surface + " missing status");
  }

  harness->Check(donor_counts.size() == 24, "donor profile count must be 24");
  harness->Check(alias_kind_counts.size() == 13, "alias kind count must be 13");
  for (const auto& [donor, count] : donor_counts) {
    harness->Check(count == 13, donor + " must have 13 alias kinds");
  }
  for (const auto& [alias_kind, count] : alias_kind_counts) {
    harness->Check(count == 24, alias_kind + " must cover 24 donor profiles");
  }
}
...
int main(int argc, char** argv) {
  if (argc != 4) {
    std::cerr << "usage: sbp_sbsql_donor_alias_rendering_conformance "
  ...
  try {
    ...
    if (!harness.ok) {
      std::cerr << "FSPE-011B donor alias rendering conformance failed with "
                << harness.failures << " failure(s)\n";
      return 1;
    }

    std::cout << "FSPE-011B donor alias rendering conformance passed: donor_aliases=312 "
                 "donor_profiles=24 alias_kinds=13 fixture_policies="
              << fixture_policy.rows.size() << '\n';
  } catch (const std::exception& ex) {
    std::cerr << "FSPE-011B donor alias rendering conformance failed: "
              << ex.what() << '\n';
    return 1;
  }
