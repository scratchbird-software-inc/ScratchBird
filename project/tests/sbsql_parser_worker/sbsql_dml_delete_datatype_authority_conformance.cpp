...
api::EngineRequestContext Context(const fixture::Fixture& f, std::string phase) {
  ...
  const auto native_uuid = api::datatype_operator_projection::UuidValue;
  c.database_path = "/component/no_database_access";
  c.database_uuid = native_uuid(fixture::Uuid(80));
  c.session_uuid = native_uuid(fixture::Uuid(81));
  c.principal_uuid = native_uuid(fixture::Uuid(82));
  c.transaction_uuid = native_uuid(f.descriptor.owning_transaction_uuid);
  c.local_transaction_id = f.descriptor.owning_local_transaction_id;
  c.statement_receipt_uuid = native_uuid(f.descriptor.authenticated_statement_receipt_uuid);
  c.statement_snapshot_uuid = native_uuid(f.descriptor.statement_snapshot_uuid);
  c.statement_snapshot_generation = 1;
  c.statement_metadata_snapshot_uuid = native_uuid(f.descriptor.catalog_snapshot_uuid);
  c.catalog_generation_id = f.descriptor.catalog_generation;
  c.datatype_catalog_snapshot_uuid = native_uuid(wire::kTypedUpdateDatatypeSnapshotUuid);
  ...
  c.authorization_context.authority_uuid = native_uuid(f.descriptor.security_context_uuid);
