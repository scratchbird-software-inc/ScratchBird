...
api::CrudTableRecord Table(const Fixture& fixture,
  ...
  Require(api::EncodeCatalogColumnMetadata(key, &key_metadata),
          "ODF-042 binary primary key metadata encoding failed");
...
Fixture MakeFixture(std::string name, platform::u64 salt) {
  ...
  fixture.session = std::make_shared<scratchbird::tests::FixtureEngineSession>(
      BaseContext(fixture, "odf042-session"));
  return fixture;
}
...
void AuthorityMissingFailsClosed() {
  ...
  auto request = ImportRequest(fixture, context, Rows("authority", 2),
                               PreallocationOptions(8, 8));
  request.context.transaction_uuid.bytes[8] = 0; // Corrupt an actually issued identity.
  const auto refused = api::EngineExecuteImportRows(request);
...
int main() {
  ...
  policy.policy_name = "odf042_statement_fixture";
  Require(scratchbird::core::memory::ConfigureDefaultMemoryManagerForFixture(
              policy, "odf042_statement_fixture").ok(),
          "ODF-042 memory policy configuration failed");
