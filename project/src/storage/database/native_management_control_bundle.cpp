...
u32 Shape(const Root& r,const Uuid& database,const Uuid& bootstrap,u64 budget,bool digests=true){
  ...
  Require(r.map_count&&r.map_count<=limit/size&&r.first.page_number&&r.first.page_generation,E::invalid_extent);
  const u64 bytes=r.map_count*size;Require(r.page_count==bytes/capacity+(bytes%capacity!=0),E::invalid_extent);
...
u64 Maps(const Pages& images,const std::vector<Header>& headers,const Root& root,const Uuid& db,u32 size){
  Require(images.size()==root.map_count&&headers.size()==root.page_count,E::invalid_allocation);
...
NativeManagementControlBundleRead Decode(const Pages& pages,const Root& r,const Uuid& db,const Uuid& bootstrap,u64 budget){
  const auto size=Shape(r,db,bootstrap,budget),capacity=size-384;Require(pages.size()==r.page_count,E::invalid_extent);const u64 total=r.map_count*size;
  ...
  for(std::size_t i=0;i<pages.size();++i){const auto& b=pages[i];Require(b.size()==size,E::invalid_header);const auto h=disk::DecodeNativeCommonPageHeader(b.data(),128);Require(h.ok(),E::invalid_header);Common(*h.header,r,db,bootstrap,size,i,ids);headers.push_back(*h.header);
    ...
    Require(std::string_view(reinterpret_cast<const char*>(f),8)=="SBMCB001"&&LoadLittle16(f+8)==1&&LoadLittle16(f+10)==256&&LoadLittle32(f+12)==384+length&&LoadLittle64(f+64)==r.map_count&&LoadLittle64(f+72)==total&&LoadLittle64(f+80)==offset&&LoadLittle64(f+88)==i&&LoadLittle64(f+96)==r.page_count&&LoadLittle64(f+104)==r.first.page_number&&LoadLittle32(f+144)==length&&Zero(f+148,12)&&Zero(f+224,32)&&Zero(b.data()+384+length,b.size()-384-length),E::invalid_header);
  ...
  Require(Hash(payload)==r.aggregate_sha256,E::invalid_integrity);Pages images;images.reserve(static_cast<std::size_t>(r.map_count));
  for(std::size_t at=0;at<payload.size();at+=size)images.emplace_back(payload.begin()+at,payload.begin()+at+size);
  const auto total_pages=Maps(images,headers,r,db,size);return {E::none,std::move(images),std::move(headers),total_pages};
}
} // namespace
...
NativeManagementControlBundleImage EncodeNativeManagementControlBundle(const Pages& maps,const Uuid& db,const Uuid& bootstrap,const Uuid& object,const Uuid& attempt,const std::vector<Header>& headers,u64 budget) noexcept {
  try{
    Require(!headers.empty()&&!maps.empty(),E::invalid_request);Root root;root.first=Self(headers.front());root.object_uuid=object;root.operation_uuid=attempt;root.map_count=maps.size();root.page_count=headers.size();
    const auto size=Shape(root,db,bootstrap,budget,false),capacity=size-384;std::set<Uuid> ids;for(std::size_t i=0;i<headers.size();++i)Common(headers[i],root,db,bootstrap,size,i,ids);Maps(maps,headers,root,db,size);
    Bytes payload;payload.reserve(static_cast<std::size_t>(root.map_count*size));for(const auto& b:maps)payload.insert(payload.end(),b.begin(),b.end());root.aggregate_sha256=Hash(payload);
    ...
    for(std::size_t left=pages.size();left;--left){const auto i=left-1;auto& b=pages[i];b.resize(size);const auto header=disk::EncodeNativeCommonPageHeader(headers[i]);Require(header.ok(),E::invalid_header);std::copy(header.bytes->begin(),header.bytes->end(),b.begin());auto* f=b.data()+128;const u64 offset=u64{i}*capacity;const u32 length=static_cast<u32>(std::min<u64>(capacity,payload.size()-offset));
      std::copy_n("SBMCB001",8,f);StoreLittle16(f+8,1);StoreLittle16(f+10,256);StoreLittle32(f+12,384+length);Put(f+16,object);Put(f+32,bootstrap);Put(f+48,attempt);StoreLittle64(f+64,root.map_count);StoreLittle64(f+72,payload.size());StoreLittle64(f+80,offset);StoreLittle64(f+88,i);StoreLittle64(f+96,root.page_count);StoreLittle64(f+104,root.first.page_number);std::copy(root.aggregate_sha256.begin(),root.aggregate_sha256.end(),f+112);StoreLittle32(f+144,length);std::copy(next.begin(),next.end(),f+160);std::copy_n(payload.begin()+offset,length,b.begin()+384);
