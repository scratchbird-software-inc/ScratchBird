...
std::vector<BootstrapRecordSeed> DefaultRegistryPolicyRecords() {
  static constexpr std::array<DefaultPolicySeed, 58> kPolicies = {{
      ...
      {"parser.package_admission", "registered_packages_only_v1", "enabled", "policy_defined", "unregistered_parser,sbsql_profile,donor_profile,parser_auth_authority"},
      ...
      {"sequence.generator_cache", "bounded_nonfinality_cache_v1", "enabled", "policy_defined", "default_cache_size,cache_not_transaction_finality,crash_gaps_allowed,donor_mapping_requires_profile"},
      ...
      {"diagnostics.message_vector", "canonical_redacted_v1", "enabled", "no_override", "raw_strings_forbidden,redaction_required,donor_mapping_generic_on_failure,correlation_id_required"},
      ...
      {"donor.emulation_profile", "strict_not_authority_v1", "enabled", "no_override", "donor_sql_exec_inside_engine,unsupported_donor_feature,donor_catalog_overlay_not_authority,cross_dialect_dependency"},
...
CatalogRowsBuildResult BuildCreateCatalogRows(const DatabaseCreateConfig& config,
  ...
  for (const auto& descriptor : BuiltinDatatypeDescriptors()) {
    const std::string datatype_payload = KeyValuePayload({{"type_id", std::to_string(static_cast<u32>(descriptor.type_id))},
                                                          ...
                                                          {"donor_name_is_alias_only", descriptor.donor_name_is_alias_only ? "1" : "0"}});
    typed = AddTypedCatalogRecord(&result.rows,
