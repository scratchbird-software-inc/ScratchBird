...
MetricLabelDescriptor UuidLabel(std::string key, bool required = true, bool sensitive = false) {
  return {std::move(key), required, sensitive, MetricLabelType::system_uuid};
}

std::vector<MetricLabelDescriptor> CommonLabels() {
  return {UuidLabel("cluster_uuid"),
          UuidLabel("node_uuid"),
          UuidLabel("external_provider_uuid"),
          Label("provider_epoch"),
          UuidLabel("authority_provenance_uuid"),
          Label("result")};
}
