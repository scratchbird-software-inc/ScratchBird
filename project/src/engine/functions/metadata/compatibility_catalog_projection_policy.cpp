...
constexpr std::string_view kSeedManifestSchema =
    "scratchbird.compatibility.catalog_seed_manifest.v1";
constexpr std::string_view kRedacted = "[redacted]";

constexpr auto kManifestDefs = std::to_array<ManifestDef>({
    {"apache_ignite", "compatibility.catalog.seed_manifest.apache_ignite.v1"},
    {"cassandra", "compatibility.catalog.seed_manifest.cassandra.v1"},
    {"clickhouse", "compatibility.catalog.seed_manifest.clickhouse.v1"},
    {"cockroachdb", "compatibility.catalog.seed_manifest.cockroachdb.v1"},
    {"dolt", "compatibility.catalog.seed_manifest.dolt.v1"},
    {"firebird", "compatibility.catalog.seed_manifest.firebird.v1"},
    {"foundationdb", "compatibility.catalog.seed_manifest.foundationdb.v1"},
    {"immudb", "compatibility.catalog.seed_manifest.immudb.v1"},
    {"mariadb", "compatibility.catalog.seed_manifest.mariadb.v1"},
    {"milvus", "compatibility.catalog.seed_manifest.milvus.v1"},
    {"mysql", "compatibility.catalog.seed_manifest.mysql.v1"},
    {"neo4j", "compatibility.catalog.seed_manifest.neo4j.v1"},
    {"opensearch", "compatibility.catalog.seed_manifest.opensearch.v1"},
    {"redis", "compatibility.catalog.seed_manifest.redis.v1"},
    {"sqlite", "compatibility.catalog.seed_manifest.sqlite.v1"},
    {"tidb", "compatibility.catalog.seed_manifest.tidb.v1"},
    {"vitess", "compatibility.catalog.seed_manifest.vitess.v1"},
    {"xtdb", "compatibility.catalog.seed_manifest.xtdb.v1"},
    {"yugabytedb", "compatibility.catalog.seed_manifest.yugabytedb.v1"},
});

void AddEvidence(CompatibilityCatalogProjectionResult* result,
                 std::string key,
                 std::string value) {
...
CompatibilityCatalogProjectionSeedRow BuildSeedRow(
    const CompatibilityCatalogProjectionRequest& request,
    const CompatibilityCatalogSeedManifestDescriptor& manifest,
    const CompatibilityCatalogProjectionContract& contract) {
  CompatibilityCatalogProjectionSeedRow row;
  row.row_key = std::string(request.engine_id);
  row.row_key.push_back(':');
  row.row_key.append(request.inventory_id);
  row.external_engine_id = std::string(request.engine_id);
  ...
  row.clean_database_state = "present_from_compatibility_catalog_seed_manifest";
  row.visible = true;
  row.metadata_redacted = !request.metadata_visible;
  row.engine_owned = contract.engine_owned;
  row.parser_mutable = false;
  row.external_mutable = false;
  return row;
}

CompatibilityCatalogProjectionResult FailClosed(std::string diagnostic_code,
                                        std::string route_contract_id) {
  CompatibilityCatalogProjectionResult result;
  ...
  AddEvidence(&result, "external_execution_authority", "false");
...
std::optional<CompatibilityCatalogSeedManifestDescriptor> ResolveCompatibilityCatalogSeedManifest(
  ...
  return CompatibilityCatalogSeedManifestDescriptor{
      it->engine_id,
      it->manifest_id,
...
std::optional<CompatibilityCatalogProjectionContract> ResolveCompatibilityCatalogProjectionContract(
  ...
  if (sb_normalized_target == "SB.ADMIN.PLUGIN_STATUS") {
    return CompatibilityCatalogProjectionContract{
        CompatibilityCatalogProjectionTarget::kAdminPluginStatus,
        "SB.ADMIN.PLUGIN_STATUS",
        "compatibility_catalog_projection.admin_plugin_status.seed_rowset.v1",
        "sys.catalog.compatibility_admin_plugin_status_seed.v1",
        "sys.catalog.compatibility_extension_status",
        "sys.catalog.compat.compatibility_admin_plugin_status",
        "metadata_visible_or_admin_observability_right",
        "redact_compatibility_catalog_detail_without_metadata_visibility",
        "clean_database_catalog_projection.plugin_status.v1",
        "derive_clean_database_rows_from_compatibility_catalog_seed_manifest",
        true,
        true,
        true,
        false,
  ...
  if (sb_normalized_target == "SB.EXTENSION.LIST") {
    return CompatibilityCatalogProjectionContract{
        CompatibilityCatalogProjectionTarget::kExtensionList,
        "SB.EXTENSION.LIST",
        "compatibility_catalog_projection.extension_list.seed_rowset.v1",
        "sys.catalog.compatibility_extension_list_seed.v1",
        "sys.catalog.compatibility_extension_registry",
        "sys.catalog.compat.compatibility_extension_list",
  ...
  if (sb_normalized_target == "SB.EXTENSION.ITEMS") {
    return CompatibilityCatalogProjectionContract{
        CompatibilityCatalogProjectionTarget::kExtensionItems,
        "SB.EXTENSION.ITEMS",
        "compatibility_catalog_projection.extension_items.seed_rowset.v1",
        "sys.catalog.compatibility_extension_item_seed.v1",
        "sys.catalog.compatibility_extension_items",
        "sys.catalog.compat.compatibility_extension_items",
        "metadata_visible_or_extension_catalog_right",
...
CompatibilityCatalogProjectionResult EvaluateCompatibilityCatalogProjection(
  ...
  if (request.implementation_decision != "catalog_projection_only") {
    return FailClosed("SB.COMPATIBILITY_CATALOG_PROJECTION.NOT_CATALOG_PROJECTION",
                      "compatibility_catalog_projection.not_catalog_projection.fail_closed.v1");
  }

  auto manifest = ResolveCompatibilityCatalogSeedManifest(request.engine_id);
  if (!manifest.has_value()) {
    return FailClosed("SB.COMPATIBILITY_CATALOG_PROJECTION.UNKNOWN_MANIFEST",
                      "compatibility_catalog_projection.unknown_manifest.fail_closed.v1");
  }

  auto contract = ResolveCompatibilityCatalogProjectionContract(request.sb_normalized_target);
  if (!contract.has_value()) {
    return FailClosed("SB.COMPATIBILITY_CATALOG_PROJECTION.UNKNOWN_TARGET",
                      "compatibility_catalog_projection.unknown_target.fail_closed.v1");
  }
  ...
  result.external_execution_authority = false;
  result.sblr_execution_authority = false;
  result.diagnostic_code = "SB.COMPATIBILITY_CATALOG_PROJECTION.READY";
