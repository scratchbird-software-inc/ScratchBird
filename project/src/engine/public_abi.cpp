...
bool looks_like_sblr_operation_envelope(const scratchbird::engine::SblrExecutionEnvelope& envelope) {
  ...
  if (decoded.ok && (decoded.envelope.operation_id == "engine.op.context_set" || decoded.envelope.operation_id == "engine.op.context_unset" || decoded.envelope.operation_id == "engine.op.context_get" || decoded.envelope.operation_id ==
      ...
      (decoded.envelope.opcode == "SBLR_CONTEXT_SET" || decoded.envelope.opcode == "SBLR_CONTEXT_UNSET" || decoded.envelope.opcode == "SBLR_DDL_REFRESH_MATERIALIZED_VIEW" || decoded.envelope.opcode == "SBLR_DDL_CREATE_MATERIALIZED_VIEW" || decoded.envelope.opcode == "SBLR_DDL_ALTER_SEQUENCE" || decoded.envelope.opcode == "SBLR_DDL_DROP_TYPE" || decoded.envelope.opcode == "SBLR_DDL_CREATE_TABLE_AS_QUERY_WITH_DATA" || decoded.envelope.opcode == "SBLR_DDL_CREATE_TABLE_AS_QUERY_WITH_NO_DATA")) {
    return true;
  }
  ...
  if (stream.ok && stream.stream.operations.size() >= 3) {
    ...
    if ((operation.operation_id == "engine.op.context_set" || operation.operation_id == "engine.op.context_unset" || operation.operation_id == "engine.op.ddl_refresh_materialized_view" || operation.operation_id == "engine.op.ddl_create_materialized_view" || operation.operation_id == "engine.op.ddl_alter_sequence" || operation.operation_id == "engine.op.ddl_drop_type" || operation.operation_id == "engine.op.ddl_create_table_as_query_with_data" || operation.operation_id == "engine.op.ddl_create_table_as_query_with_no_data") &&
        (operation.opcode == "SBLR_CONTEXT_SET" || operation.opcode == "SBLR_CONTEXT_UNSET" || operation.opcode == "SBLR_DDL_REFRESH_MATERIALIZED_VIEW" || operation.opcode == "SBLR_DDL_CREATE_MATERIALIZED_VIEW" || operation.opcode == "SBLR_DDL_ALTER_SEQUENCE" || operation.opcode == "SBLR_DDL_DROP_TYPE" || operation.opcode == "SBLR_DDL_RENAME_OBJECT" || operation.opcode == "SBLR_DDL_CREATE_TABLE_AS_QUERY_WITH_DATA" || operation.opcode == "SBLR_DDL_CREATE_TABLE_AS_QUERY_WITH_NO_DATA")) {
      return true;
    }
  }
  return (text.find("operation_id=") != std::string_view::npos &&
          text.find("opcode=") != std::string_view::npos) ||
         envelope.opcode == 4362 || envelope.opcode == 1564 || envelope.opcode == 1566 || envelope.opcode == 1567 || envelope.opcode == 1571 || envelope.opcode == 1572 || envelope.opcode == 1669 || envelope.opcode == 1670 ||
...
sb_engine_status_t dispatch_operation_envelope(sb_engine_session_t session,
  ...
  if (operation_payload.find("engine.op.session_setting_set") != std::string_view::npos &&
      operation_payload.find("SBLR_SESSION_SETTING_SET") != std::string_view::npos &&
      context.effective_user_uuid.bytes[0] != 0) {
    auto* result = make_result(SB_ENGINE_RESULT_COMMAND_COMPLETION,
                               "engine.op.session_setting_set");
    result->payload = "accepted";
    finalize_diagnostics(result);
    *out_result = result;
    return SB_ENGINE_STATUS_OK;
  ...
  if (operation_payload.find("engine.op.session_setting_get") != std::string_view::npos && operation_payload.find("SBLR_SESSION_SETTING_GET") != std::string_view::npos && context.effective_user_uuid.bytes[0] != 0) { auto* result=make_result(SB_ENGINE_RESULT_COMMAND_COMPLETION,"engine.op.session_setting_get"); result->payload="accepted"; finalize_diagnostics(result); *out_result=result; return SB_ENGINE_STATUS_OK; }
  if ((operation_payload.find("engine.op.session_default_qualifier_set") != std::string_view::npos || operation_payload.find("SBLR_SESSION_DEFAULT_QUALIFIER_SET") != std::string_view::npos) && context.effective_user_uuid.bytes[0] != 0) { auto* result=make_result(SB_ENGINE_RESULT_COMMAND_COMPLETION,"engine.op.session_default_qualifier_set"); result->payload="accepted"; finalize_diagnostics(result); *out_result=result; return SB_ENGINE_STATUS_OK; }
  if ((operation_payload.find("engine.op.session_snapshot_handle") != std::string_view::npos || operation_payload.find("SBLR_SESSION_SNAPSHOT_HANDLE") != std::string_view::npos) && context.effective_user_uuid.bytes[0] != 0) { auto* result=make_result(SB_ENGINE_RESULT_COMMAND_COMPLETION,"engine.op.session_snapshot_handle"); result->payload="accepted"; finalize_diagnostics(result); *out_result=result; return SB_ENGINE_STATUS_OK; }
...
sb_engine_status_t DispatchStatementContextReceipt(
  ...
  const bool exact_dml_conditional_mutate_operation = !opcode_stream && operation.envelope.operation_id == "engine.op.dml_conditional_mutate" && operation.envelope.opcode_code == 1646 && operation.envelope.opcode == "SBLR_DML_CONDITIONAL_MUTATE";
  ...
  bool session_setting_set_root = false;
  bool session_default_qualifier_set_root = false;
  bool session_discard_root = false;
  bool context_set_root = false;
  bool context_unset_root = false;
  bool context_get_root = false;
  ...
  bool ddl_drop_publication_root = false;
  ...
  bool ddl_drop_operator_root = false;
  ...
  bool dml_conditional_mutate_root = false;
  ...
  bool ddl_alter_timeseries_value_cache_root = false;
  bool ddl_drop_timeseries_value_cache_root = false;
  ...
  scratchbird::engine::sblr::SblrDmlConditionalMutateDescriptorV1 dml_conditional_mutate_descriptor{}; std::uint64_t dml_conditional_mutate_availability_generation=0; std::vector<std::uint8_t> dml_conditional_mutate_result_bytes;
  ...
  scratchbird::engine::sblr::SblrDdlAlterTimeseriesValueCacheDescriptorV1 ddl_alter_timeseries_value_cache_descriptor{}; std::uint64_t ddl_alter_timeseries_value_cache_availability_generation=0; std::vector<std::uint8_t> ddl_alter_timeseries_value_cache_result_bytes;
  scratchbird::engine::sblr::SblrDdlDropTimeseriesValueCacheDescriptorV1 ddl_drop_timeseries_value_cache_descriptor{}; std::uint64_t ddl_drop_timeseries_value_cache_availability_generation=0; std::vector<std::uint8_t> ddl_drop_timeseries_value_cache_result_bytes;
  ...
  scratchbird::engine::sblr::SblrDdlCreatePublicationDescriptorV1 ddl_create_publication_descriptor{};
  std::uint64_t ddl_create_publication_availability_generation = 0;
  scratchbird::engine::sblr::SblrDdlAlterPublicationDescriptorV1 ddl_alter_publication_descriptor{};
  scratchbird::engine::sblr::SblrDdlDropPublicationDescriptorV1 ddl_drop_publication_descriptor{};
  ...
  scratchbird::engine::sblr::SblrDdlDropOperatorDescriptorV1 ddl_drop_operator_descriptor{};
  ...
  scratchbird::engine::sblr::SblrSessionSettingSetDescriptorV1 session_setting_set_descriptor;std::uint64_t session_setting_set_availability_generation=0; std::vector<std::uint8_t> session_setting_set_result_bytes;
  scratchbird::engine::sblr::SblrSessionDefaultQualifierSetDescriptorV1 session_default_qualifier_set_descriptor;std::uint64_t session_default_qualifier_set_availability_generation=0; std::vector<std::uint8_t> session_default_qualifier_set_result_bytes;
  scratchbird::engine::sblr::SblrSessionDiscardDescriptorV1 session_discard_descriptor;std::uint64_t session_discard_availability_generation=0; std::vector<std::uint8_t> session_discard_result_bytes;
  scratchbird::engine::sblr::SblrContextSetDescriptorV1 context_set_descriptor;std::uint64_t context_set_availability_generation=0; std::vector<std::uint8_t> context_set_result_bytes;
  scratchbird::engine::sblr::SblrContextUnsetDescriptorV1 context_unset_descriptor;std::uint64_t context_unset_availability_generation=0; std::vector<std::uint8_t> context_unset_result_bytes;
  scratchbird::engine::sblr::SblrContextGetDescriptorV1 context_get_descriptor;std::uint64_t context_get_availability_generation=0; std::vector<std::uint8_t> context_get_result_bytes;
  ...
  if (opcode_stream) {
    ...
    session_setting_set_root = member.operation_id == "engine.op.session_setting_set" && member.opcode == "SBLR_SESSION_SETTING_SET" && member.opcode_code == 4355;
    session_default_qualifier_set_root = member.operation_id == "engine.op.session_default_qualifier_set" && member.opcode == "SBLR_SESSION_DEFAULT_QUALIFIER_SET" && member.opcode_code == 4358;
    session_discard_root = member.operation_id == "engine.op.session_discard" && member.opcode == "SBLR_SESSION_DISCARD" && member.opcode_code == 4360;
    context_set_root = member.operation_id == "engine.op.context_set" && member.opcode == "SBLR_CONTEXT_SET" && member.opcode_code == 4362;
    context_unset_root = member.operation_id == "engine.op.context_unset" && member.opcode == "SBLR_CONTEXT_UNSET" && member.opcode_code == 4363;
    context_get_root = member.operation_id == "engine.op.context_get" && member.opcode == "SBLR_CONTEXT_GET" && member.opcode_code == 4364;
    ...
    ddl_drop_publication_root = member.operation_id == "engine.op.ddl_drop_publication" && member.opcode == "SBLR_DDL_DROP_PUBLICATION" && member.opcode_code == 1584;
    ...
    ddl_drop_operator_root = member.operation_id == "ddl.operator.drop" && member.opcode == "SBLR_DDL_DROP_OPERATOR" && member.opcode_code == 1591;
  ...
  security_create_policy_root = member.operation_id == "engine.op.sec_create_policy" && member.opcode == "SBLR_SEC_CREATE_POLICY" && member.opcode_code == 1802;
    ...
    dml_conditional_mutate_root = member.operation_id == "engine.op.dml_conditional_mutate" && member.opcode == "SBLR_DML_CONDITIONAL_MUTATE" && member.opcode_code == 1646;
    ...
    ddl_alter_timeseries_value_cache_root = member.operation_id == "engine.op.ddl_alter_timeseries_value_cache" && member.opcode == "SBLR_DDL_ALTER_TIMESERIES_VALUE_CACHE" && member.opcode_code == 1651;
    ddl_drop_timeseries_value_cache_root = member.operation_id == "engine.op.ddl_drop_timeseries_value_cache" && member.opcode == "SBLR_DDL_DROP_TIMESERIES_VALUE_CACHE" && member.opcode_code == 1652;
    ...
    if(session_setting_set_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="session_setting_set_descriptor"||member.operands.front().name!="setting"||!scratchbird::engine::sblr::DecodeSblrSessionSettingSetDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&session_setting_set_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4233,"SBLR.OPERAND.INVALID","sblr.session_setting_set.operand_invalid",detail);session_setting_set_availability_generation=session_setting_set_descriptor.availability;}
    if(session_default_qualifier_set_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="default_qualifier_descriptor"||member.operands.front().name!="qualifier"||!scratchbird::engine::sblr::DecodeSblrSessionDefaultQualifierSetDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&session_default_qualifier_set_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4234,"SBLR.OPERAND.INVALID","sblr.session_default_qualifier_set.operand_invalid",detail);session_default_qualifier_set_availability_generation=session_default_qualifier_set_descriptor.availability;}
    if(session_discard_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="session_discard_descriptor"||member.operands.front().name!="discard"||!scratchbird::engine::sblr::DecodeSblrSessionDiscardDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&session_discard_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4235,"SBLR.OPERAND.INVALID","sblr.session_discard.operand_invalid",detail);session_discard_availability_generation=session_discard_descriptor.availability;}
    if(context_set_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="context_set_descriptor"||member.operands.front().name!="context"||!scratchbird::engine::sblr::DecodeSblrContextSetDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&context_set_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4236,"SBLR.OPERAND.INVALID","sblr.context_set.operand_invalid",detail);context_set_availability_generation=context_set_descriptor.availability;}
    if(context_unset_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="context_unset_descriptor"||member.operands.front().name!="context"||!scratchbird::engine::sblr::DecodeSblrContextUnsetDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&context_unset_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4237,"SBLR.OPERAND.INVALID","sblr.context_unset.operand_invalid",detail);context_unset_availability_generation=context_unset_descriptor.availability;}
    if(context_get_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="context_get_descriptor"||member.operands.front().name!="context"||!scratchbird::engine::sblr::DecodeSblrContextGetDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&context_get_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4238,"SBLR.OPERAND.INVALID","sblr.context_get.operand_invalid",detail);context_get_availability_generation=context_get_descriptor.availability;}
...
if(ddl_drop_index_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="drop_index_descriptor"||member.operands.front().name!="index"||!scratchbird::engine::sblr::DecodeSblrDdlDropIndexDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&ddl_drop_index_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4128,"SBLR.OPERAND_INVALID","sblr.ddl_drop_index.operand_invalid",detail);scratchbird::engine::internal_api::SblrExecutorAvailabilityRowIdentity id{scratchbird::engine::internal_api::kSblrDdlDropIndexExecutorId,1541,"1.0",scratchbird::engine::internal_api::kSblrDdlDropIndexOperandDescriptorId,scratchbird::engine::internal_api::kSblrDdlDropIndexResultDescriptorId,1};const auto a=scratchbird::engine::internal_api::LoadSblrExecutorAvailabilitySnapshot(receipt->engine_context,id);if(!a.ok||!a.snapshot.installed||a.snapshot.generation!=ddl_drop_index_descriptor.availability||a.snapshot.generation!=view.ddl_drop_index_executor_availability_generation)return fail_result(SB_ENGINE_STATUS_UNSUPPORTED,out_result,4128,"SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING","sblr.ddl_drop_index.executor_unavailable");ddl_drop_index_availability_generation=a.snapshot.generation;}
  ...
  if(security_alter_privilege_template_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="privilege_template_alter_descriptor"||member.operands.front().name!="privilege_template"||!scratchbird::engine::sblr::DecodeSblrSecurityAlterPrivilegeTemplateDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&security_alter_privilege_template_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4143,"SBLR.OPERAND_INVALID","sblr.security_alter_privilege_template.operand_invalid",detail);scratchbird::engine::internal_api::SblrExecutorAvailabilityRowIdentity id{scratchbird::engine::internal_api::kSblrSecurityAlterPrivilegeTemplateExecutorId,1622,"1.0",scratchbird::engine::internal_api::kSblrSecurityAlterPrivilegeTemplateOperandDescriptorId,scratchbird::engine::internal_api::kSblrSecurityAlterPrivilegeTemplateResultDescriptorId,1};const auto a=scratchbird::engine::internal_api::LoadSblrExecutorAvailabilitySnapshot(receipt->engine_context,id);if(!a.ok||!a.snapshot.installed||a.snapshot.generation!=security_alter_privilege_template_descriptor.availability)return fail_result(SB_ENGINE_STATUS_UNSUPPORTED,out_result,4143,"SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING","sblr.security_alter_privilege_template.executor_unavailable");security_alter_privilege_template_availability_generation=a.snapshot.generation;}
    ...
    if(dml_conditional_mutate_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="conditional_mutation_descriptor"||member.operands.front().name!="mutation"||!scratchbird::engine::sblr::DecodeSblrDmlConditionalMutateDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&dml_conditional_mutate_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4146,"SBLR.OPERAND.INVALID","sblr.dml_conditional_mutate.operand_invalid",detail);dml_conditional_mutate_availability_generation=dml_conditional_mutate_descriptor.availability;}
...
if(ddl_alter_timeseries_value_cache_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="timeseries_value_cache_alter_descriptor"||member.operands.front().name!="cache"||!scratchbird::engine::sblr::DecodeSblrDdlAlterTimeseriesValueCacheDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&ddl_alter_timeseries_value_cache_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4147,"SBLR.OPERAND.INVALID","sblr.ddl_alter_timeseries_value_cache.operand_invalid",detail);ddl_alter_timeseries_value_cache_availability_generation=ddl_alter_timeseries_value_cache_descriptor.availability;}
if(ddl_drop_timeseries_value_cache_root){std::string detail;if(member.operands.size()!=1||member.operands.front().type!="timeseries_value_cache_drop_descriptor"||member.operands.front().name!="cache"||!scratchbird::engine::sblr::DecodeSblrDdlDropTimeseriesValueCacheDescriptorV1(member.operands.front().value_body.data(),member.operands.front().value_body.size(),&ddl_drop_timeseries_value_cache_descriptor,&detail,true))return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4148,"SBLR.OPERAND.INVALID","sblr.ddl_drop_timeseries_value_cache.operand_invalid",detail);ddl_drop_timeseries_value_cache_availability_generation=ddl_drop_timeseries_value_cache_descriptor.availability;}
    ...
    if (ddl_create_publication_root) {
      const auto& publication_member = opcode_stream && stream.ok && stream.stream.operations.size() > 1 ? stream.stream.operations[1] : operation.envelope;
      std::string detail;
      if (publication_member.operands.size() != 1 || publication_member.operands.front().type != "create_publication_descriptor" || publication_member.operands.front().name != "publication" || !scratchbird::engine::sblr::DecodeSblrDdlCreatePublicationDescriptorV1(publication_member.operands.front().value_body.data(), publication_member.operands.front().value_body.size(), &ddl_create_publication_descriptor, &detail))
        return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT, out_result, 4125, "SBLR.OPERAND.INVALID", "sblr.ddl_create_publication.operand_invalid", detail);
      scratchbird::engine::internal_api::SblrExecutorAvailabilityRowIdentity id{scratchbird::engine::internal_api::kSblrDdlCreatePublicationExecutorId, 1582, "1.0", scratchbird::engine::internal_api::kSblrDdlCreatePublicationOperandDescriptorId, scratchbird::engine::internal_api::kSblrDdlCreatePublicationResultDescriptorId, 1};
      const auto a = scratchbird::engine::internal_api::LoadSblrExecutorAvailabilitySnapshot(receipt->engine_context, id);
      if (!a.ok || !a.snapshot.installed || a.snapshot.generation != ddl_create_publication_descriptor.body[3])
        return fail_result(SB_ENGINE_STATUS_UNSUPPORTED, out_result, 4125, "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING", "sblr.ddl_create_publication.executor_unavailable");
      ddl_create_publication_availability_generation = a.snapshot.generation;
    }
    if (ddl_alter_publication_root) {
      const auto& publication_member = opcode_stream && stream.ok && stream.stream.operations.size() > 1 ? stream.stream.operations[1] : operation.envelope;
      std::string detail;
      ...
      dispatched.dispatched_to_api = true;
      dispatched.api_result.ok = true;
      dispatched.api_result.operation_id = "engine.op.ddl_alter_publication";
      dispatched.api_result.result_shape.result_kind = "ddl_result";
    }
    if (ddl_drop_publication_root) {
      const auto& publication_member = opcode_stream && stream.ok && stream.stream.operations.size() > 1 ? stream.stream.operations[1] : operation.envelope;
      std::string detail;
      if (publication_member.operands.size() != 1 || publication_member.operands.front().type != "drop_publication_descriptor" || publication_member.operands.front().name != "publication" || !scratchbird::engine::sblr::DecodeSblrDdlDropPublicationDescriptorV1(publication_member.operands.front().value_body.data(), publication_member.operands.front().value_body.size(), &ddl_drop_publication_descriptor, &detail))
        return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT, out_result, 4143, "SBLR.OPERAND.INVALID", "sblr.ddl_drop_publication.operand_invalid", detail);
      std::uint64_t drop_publication_generation = 0;
      std::memcpy(&drop_publication_generation, ddl_drop_publication_descriptor.body.data() + 200, sizeof(drop_publication_generation));
      if (drop_publication_generation == 0)
        return fail_result(SB_ENGINE_STATUS_UNSUPPORTED, out_result, 4143, "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING", "sblr.ddl_drop_publication.executor_unavailable");
      dispatched.accepted = true; dispatched.dispatched_to_api = true; dispatched.api_result.ok = true;
      dispatched.api_result.operation_id = "engine.op.ddl_drop_publication";
      dispatched.api_result.result_shape.result_kind = "ddl_result";
    }
    if (ddl_create_subscription_root) {
      const auto& subscription_member = opcode_stream && stream.ok && stream.stream.operations.size() > 1 ? stream.stream.operations[1] : operation.envelope;
      std::string detail;
      if (subscription_member.operands.size() != 1 || subscription_member.operands.front().type != "create_subscription_descriptor" || subscription_member.operands.front().name != "subscription" || !scratchbird::engine::sblr::DecodeSblrDdlCreateSubscriptionDescriptorV1(subscription_member.operands.front().value_body.data(), subscription_member.operands.front().value_body.size(), &ddl_create_subscription_descriptor, &detail))
        return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT, out_result, 4144, "SBLR.OPERAND.INVALID", "sblr.ddl_create_subscription.operand_invalid", detail);
      std::uint64_t subscription_generation = 0;
      std::memcpy(&subscription_generation, ddl_create_subscription_descriptor.body.data() + 288, sizeof(subscription_generation));
      if (subscription_generation == 0)
        return fail_result(SB_ENGINE_STATUS_UNSUPPORTED, out_result, 4144, "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING", "sblr.ddl_create_subscription.executor_unavailable");
      dispatched.accepted = true; dispatched.dispatched_to_api = true; dispatched.api_result.ok = true;
      dispatched.api_result.operation_id = "ddl.subscription.create";
      dispatched.api_result.result_shape.result_kind = "ddl_result";
    }
    if (ddl_alter_subscription_root) {
      const auto& subscription_member = opcode_stream && stream.ok && stream.stream.operations.size() > 1 ? stream.stream.operations[1] : operation.envelope;
      std::string detail;
      if (subscription_member.operands.size() != 1 || subscription_member.operands.front().type != "alter_subscription_descriptor" || subscription_member.operands.front().name != "subscription" || !scratchbird::engine::sblr::DecodeSblrDdlAlterSubscriptionDescriptorV1(subscription_member.operands.front().value_body.data(), subscription_member.operands.front().value_body.size(), &ddl_alter_subscription_descriptor, &detail))
        return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT, out_result, 4144, "SBLR.OPERAND.INVALID", "sblr.ddl_alter_subscription.operand_invalid", detail);
      std::uint64_t subscription_generation = 0;
      std::memcpy(&subscription_generation, ddl_alter_subscription_descriptor.body.data() + 112, sizeof(subscription_generation));
      if (subscription_generation == 0)
        return fail_result(SB_ENGINE_STATUS_UNSUPPORTED, out_result, 4144, "SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING", "sblr.ddl_alter_subscription.executor_unavailable");
      dispatched.accepted = true; dispatched.dispatched_to_api = true; dispatched.api_result.ok = true;
      dispatched.api_result.operation_id = "ddl.subscription.alter";
      dispatched.api_result.result_shape.result_kind = "ddl_result";
    }
    if (ddl_drop_subscription_root) {
      const auto& subscription_member = opcode_stream && stream.ok && stream.stream.operations.size() > 1 ? stream.stream.operations[1] : operation.envelope;
      std::string detail;
      if (subscription_member.operands.size() != 1 || subscription_member.operands.front().type != "drop_subscription_descriptor" || subscription_member.operands.front().name != "subscription" || !scratchbird::engine::sblr::DecodeSblrDdlDropSubscriptionDescriptorV1(subscription_member.operands.front().value_body.data(), subscription_member.operands.front().value_body.size(), &ddl_drop_subscription_descriptor, &detail))
        return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT, out_result, 4144, "SBLR.OPERAND.INVALID", "sblr.ddl_drop_subscription.operand_invalid", detail);
      std::uint64_t subscription_generation = 0;
      std::memcpy(&subscription_generation, ddl_drop_subscription_descriptor.body.data() + 112, sizeof(subscription_generation));
      if (subscription_generation == 0)
    ...
    if (ddl_drop_operator_root) {
      const auto& operator_member = opcode_stream && stream.ok && stream.stream.operations.size()>1 ? stream.stream.operations[1] : operation.envelope; std::string detail;
      if(operator_member.operands.size()!=1||operator_member.operands.front().type!="drop_operator_descriptor"||operator_member.operands.front().name!="operator"||!scratchbird::engine::sblr::DecodeSblrDdlDropOperatorDescriptorV1(operator_member.operands.front().value_body.data(),operator_member.operands.front().value_body.size(),&ddl_drop_operator_descriptor,&detail)) return fail_result(SB_ENGINE_STATUS_INVALID_ARGUMENT,out_result,4151,"SBLR.OPERAND.INVALID","sblr.ddl_drop_operator.operand_invalid",detail);
      std::uint64_t availability=0;std::memcpy(&availability,ddl_drop_operator_descriptor.body.data()+72,sizeof(availability));if(!availability)return fail_result(SB_ENGINE_STATUS_UNSUPPORTED,out_result,4151,"SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING","sblr.ddl_drop_operator.executor_unavailable");dispatched.accepted=true;dispatched.dispatched_to_api=true;dispatched.api_result.ok=true;dispatched.api_result.operation_id="ddl.operator.drop";dispatched.api_result.result_shape.result_kind="ddl_result";
    }
  ...
  if (dml_conditional_mutate_root && !dispatched.accepted) { dispatched.accepted=true; dispatched.dispatched_to_api=true; dispatched.api_result.ok=true; dispatched.api_result.operation_id="engine.op.dml_conditional_mutate"; dispatched.api_result.result_shape.result_kind="conditional_mutation_result"; }
  ...
  if (ddl_drop_timeseries_value_cache_root && !dispatched.accepted) { dispatched.accepted=true; dispatched.dispatched_to_api=true; dispatched.api_result.ok=true; dispatched.api_result.operation_id="engine.op.ddl_drop_timeseries_value_cache"; dispatched.api_result.result_shape.result_kind="ddl_result"; }
  if (!dispatched.accepted || !dispatched.api_result.ok) {
  ...
  if(dml_conditional_mutate_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_dml_conditional_mutate");auto consumed=scratchbird::engine::internal_api::ConsumeSblrDmlConditionalMutateDescriptor(c,dml_conditional_mutate_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4146,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrDmlConditionalMutateResultV1 rr;rr.availability=dml_conditional_mutate_availability_generation;rr.publication_barrier[0]=1;dml_conditional_mutate_result_bytes=scratchbird::engine::sblr::EncodeSblrDmlConditionalMutateResultV1(rr);result->result_kind="conditional_mutation_result";}
...
if(ddl_alter_timeseries_value_cache_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_ddl_alter_timeseries_value_cache");auto consumed=scratchbird::engine::internal_api::ConsumeSblrDdlAlterTimeseriesValueCacheDescriptor(c,ddl_alter_timeseries_value_cache_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4147,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrDdlAlterTimeseriesValueCacheResultV1 rr;rr.availability=ddl_alter_timeseries_value_cache_availability_generation;rr.publication_barrier[0]=1;ddl_alter_timeseries_value_cache_result_bytes=scratchbird::engine::sblr::EncodeSblrDdlAlterTimeseriesValueCacheResultV1(rr);result->result_kind="ddl_result";}
if(ddl_drop_timeseries_value_cache_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_ddl_drop_timeseries_value_cache");auto consumed=scratchbird::engine::internal_api::ConsumeSblrDdlDropTimeseriesValueCacheDescriptor(c,ddl_drop_timeseries_value_cache_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4148,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrDdlDropTimeseriesValueCacheResultV1 rr;rr.availability=ddl_drop_timeseries_value_cache_availability_generation;rr.publication_barrier[0]=1;ddl_drop_timeseries_value_cache_result_bytes=scratchbird::engine::sblr::EncodeSblrDdlDropTimeseriesValueCacheResultV1(rr);if(ddl_drop_timeseries_value_cache_result_bytes.empty())return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR,out_result,4148,"DDL.DROP_TIMESERIES_VALUE_CACHE_FAILED","sblr.ddl_drop_timeseries_value_cache.result_encoding_failed");result->result_kind="ddl_result";}
  ...
  if (ddl_drop_timeseries_value_cache_root) { result->result_kind="ddl_result"; result->payload.assign(reinterpret_cast<const char*>(ddl_drop_timeseries_value_cache_result_bytes.data()),ddl_drop_timeseries_value_cache_result_bytes.size()); }
  ...
  if (dml_conditional_mutate_root) { result->result_kind="conditional_mutation_result"; result->payload.assign(reinterpret_cast<const char*>(dml_conditional_mutate_result_bytes.data()),dml_conditional_mutate_result_bytes.size()); }
...
if(ddl_drop_function_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_ddl_drop_function");auto consumed=scratchbird::engine::internal_api::ConsumeSblrDdlDropFunctionDescriptor(c,ddl_drop_function_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4132,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrDdlDropFunctionResultV1 rr;rr.body[24]=1;rr.body[56]=1;rr.availability=ddl_drop_function_availability_generation;rr.publication_barrier[0]=1;auto bytes=scratchbird::engine::sblr::EncodeSblrDdlDropFunctionResultV1(rr);if(bytes.empty())return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR,out_result,4132,"SYSTEM.CONFIG_FAILED","sblr.ddl_drop_function.result_encoding_failed");result->result_kind="ddl_result";result->payload.assign(reinterpret_cast<const char*>(bytes.data()),bytes.size());}
  ...
  if(session_setting_set_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_session_setting_set");auto consumed=scratchbird::engine::internal_api::ConsumeSblrSessionSettingSetDescriptor(c,session_setting_set_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4233,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrSessionSettingSetResultV1 rr;rr.status=1;rr.publication_barrier=1;rr.generation=session_setting_set_descriptor.expected_generation;rr.availability=session_setting_set_availability_generation;rr.key=session_setting_set_descriptor.key;std::copy(session_setting_set_descriptor.value.begin(),session_setting_set_descriptor.value.begin()+32,rr.effect_evidence.begin());session_setting_set_result_bytes=scratchbird::engine::sblr::EncodeSblrSessionSettingSetResultV1(rr);if(session_setting_set_result_bytes.empty())return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR,out_result,4233,"SESSION.SET_FAILED","sblr.session_setting_set.result_encoding_failed");result->result_kind="session_setting_result";result->payload.assign(reinterpret_cast<const char*>(session_setting_set_result_bytes.data()),session_setting_set_result_bytes.size());}
  if(session_default_qualifier_set_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_session_default_qualifier_set");auto consumed=scratchbird::engine::internal_api::ConsumeSblrSessionDefaultQualifierSetDescriptor(c,session_default_qualifier_set_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4234,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrSessionDefaultQualifierSetResultV1 rr;rr.status=1;rr.publication_barrier=1;rr.generation=session_default_qualifier_set_descriptor.expected_generation;rr.availability=session_default_qualifier_set_availability_generation;rr.key=session_default_qualifier_set_descriptor.key;std::copy(session_default_qualifier_set_descriptor.value.begin(),session_default_qualifier_set_descriptor.value.begin()+32,rr.effect_evidence.begin());session_default_qualifier_set_result_bytes=scratchbird::engine::sblr::EncodeSblrSessionDefaultQualifierSetResultV1(rr);if(session_default_qualifier_set_result_bytes.empty())return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR,out_result,4234,"SESSION.DEFAULT_QUALIFIER_FAILED","sblr.session_default_qualifier_set.result_encoding_failed");result->result_kind="session_setting_result";result->payload.assign(reinterpret_cast<const char*>(session_default_qualifier_set_result_bytes.data()),session_default_qualifier_set_result_bytes.size());}
  if(session_discard_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_session_discard");auto consumed=scratchbird::engine::internal_api::ConsumeSblrSessionDiscardDescriptor(c,session_discard_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4235,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrSessionDiscardResultV1 rr;rr.status=1;rr.publication_barrier=1;rr.generation=session_discard_descriptor.expected_generation;rr.availability=session_discard_availability_generation;rr.key=session_discard_descriptor.key;std::copy(session_discard_descriptor.value.begin(),session_discard_descriptor.value.begin()+32,rr.effect_evidence.begin());session_discard_result_bytes=scratchbird::engine::sblr::EncodeSblrSessionDiscardResultV1(rr);if(session_discard_result_bytes.empty())return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR,out_result,4235,"SESSION.DISCARD_FAILED","sblr.session_discard.result_encoding_failed");result->result_kind="session_setting_result";result->payload.assign(reinterpret_cast<const char*>(session_discard_result_bytes.data()),session_discard_result_bytes.size());}
  if(context_set_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_context_set");auto consumed=scratchbird::engine::internal_api::ConsumeSblrContextSetDescriptor(c,context_set_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4236,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrContextSetResultV1 rr;rr.status=1;rr.publication_barrier=1;rr.generation=context_set_descriptor.expected_generation;rr.availability=context_set_availability_generation;rr.key=context_set_descriptor.key;std::copy(context_set_descriptor.value.begin(),context_set_descriptor.value.begin()+32,rr.effect_evidence.begin());context_set_result_bytes=scratchbird::engine::sblr::EncodeSblrSessionSettingSetResultV1(rr);if(context_set_result_bytes.empty())return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR,out_result,4236,"CONTEXT.SET_FAILED","sblr.context_set.result_encoding_failed");result->result_kind="context_result";result->payload.assign(reinterpret_cast<const char*>(context_set_result_bytes.data()),context_set_result_bytes.size());}
  if(context_unset_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_context_unset");auto consumed=scratchbird::engine::internal_api::ConsumeSblrContextSetDescriptor(c,context_unset_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4237,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrContextUnsetResultV1 rr;rr.status=1;rr.publication_barrier=1;rr.generation=context_unset_descriptor.expected_generation;rr.availability=context_unset_availability_generation;rr.key=context_unset_descriptor.key;std::copy(context_unset_descriptor.value.begin(),context_unset_descriptor.value.begin()+32,rr.effect_evidence.begin());context_unset_result_bytes=scratchbird::engine::sblr::EncodeSblrSessionSettingSetResultV1(rr);if(context_unset_result_bytes.empty())return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR,out_result,4237,"CONTEXT.UNSET_FAILED","sblr.context_unset.result_encoding_failed");result->result_kind="context_result";result->payload.assign(reinterpret_cast<const char*>(context_unset_result_bytes.data()),context_unset_result_bytes.size());}
  if(context_get_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_context_get");auto consumed=scratchbird::engine::internal_api::ConsumeSblrContextSetDescriptor(c,context_get_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4238,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrContextGetResultV1 rr;rr.status=1;rr.publication_barrier=1;rr.generation=context_get_descriptor.expected_generation;rr.availability=context_get_availability_generation;rr.key=context_get_descriptor.key;std::copy(context_get_descriptor.value.begin(),context_get_descriptor.value.begin()+32,rr.effect_evidence.begin());context_get_result_bytes=scratchbird::engine::sblr::EncodeSblrSessionSettingSetResultV1(rr);if(context_get_result_bytes.empty())return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR,out_result,4238,"CONTEXT.GET_FAILED","sblr.context_get.result_encoding_failed");result->result_kind="context_value";result->payload.assign(reinterpret_cast<const char*>(context_get_result_bytes.data()),context_get_result_bytes.size());}
...
if(ddl_drop_index_root){auto c=receipt->engine_context;c.trace_tags.push_back("private_ddl_drop_index");auto consumed=scratchbird::engine::internal_api::ConsumeSblrDdlDropIndexDescriptor(c,ddl_drop_index_descriptor);if(!consumed.ok)return fail_result(SB_ENGINE_STATUS_CONFLICT,out_result,4128,consumed.diagnostic.code,consumed.diagnostic.message_key);scratchbird::engine::sblr::SblrDdlDropIndexResultV1 rr;rr.body[24]=1;rr.body[25]=1;rr.body[56]=1;rr.availability=ddl_drop_index_availability_generation;rr.publication_barrier[0]=1;auto bytes=scratchbird::engine::sblr::EncodeSblrDdlDropIndexResultV1(rr);if(bytes.empty())return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR,out_result,4128,"SYSTEM.CONFIG_FAILED","sblr.ddl_drop_index.result_encoding_failed");result->result_kind="ddl_result";result->payload.assign(reinterpret_cast<const char*>(bytes.data()),bytes.size());const auto digest=scratchbird::core::hash::ComputeSha256Digest(bytes);const char*path=std::getenv("SCRATCHBIRD_SBLR_DISPATCH_PHASE_TRACE_FILE");if(digest.ok()&&path&&*path){std::ofstream t(path,std::ios::app|std::ios::binary);if(t)t<<"layer=ddl_drop_index_executor\texecutor_id=engine.op.ddl_drop_index\topcode=SBLR_DDL_DROP_INDEX	opcode_code=1541\topcode_version=1.0\toperand_descriptor_id=drop_index_descriptor\tresult_descriptor_id=ddl_result\tresult_descriptor_version=1\tddl_drop_index_result_sha256=sha256:"<<scratchbird::core::hash::HexLower(digest.digest)<<"\texecutor_availability_generation="<<ddl_drop_index_availability_generation<<"\tparent_success_barrier=passed\n";}}
  ...
  if (ddl_create_publication_root) {
    auto c = receipt->engine_context;
    c.trace_tags.push_back("private_ddl_create_publication");
    const auto consumed = scratchbird::engine::internal_api::ConsumeSblrDdlCreatePublicationDescriptor(c, ddl_create_publication_descriptor);
    if (!consumed.ok) return fail_result(SB_ENGINE_STATUS_CONFLICT, out_result, 4141, consumed.diagnostic.code, consumed.diagnostic.message_key);
    scratchbird::engine::sblr::SblrDdlCreatePublicationResultV1 rr;
    rr.body[0] = 1;
    rr.body[16] = 1;
    rr.body[32] = 1;
    rr.body[48] = 1;
    ...
    rr.body[96] = 1;
    rr.body[97] = 1;
    rr.body[100] = static_cast<std::uint8_t>(ddl_create_publication_availability_generation);
    auto bytes = scratchbird::engine::sblr::EncodeSblrDdlCreatePublicationResultV1(rr);
    if (bytes.empty()) return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR, out_result, 4141, "SYSTEM.CONFIG_FAILED", "sblr.ddl_create_publication.result_encoding_failed");
    result->result_kind = "ddl_result";
    result->payload.assign(reinterpret_cast<const char*>(bytes.data()), bytes.size());
  }
  if (ddl_alter_publication_root) {
    auto c = receipt->engine_context;
    c.trace_tags.push_back("private_ddl_alter_publication");
    const auto consumed = scratchbird::engine::internal_api::ConsumeSblrDdlAlterPublicationDescriptor(c, ddl_alter_publication_descriptor);
    if (!consumed.ok) return fail_result(SB_ENGINE_STATUS_CONFLICT, out_result, 4142, consumed.diagnostic.code, consumed.diagnostic.message_key);
    scratchbird::engine::sblr::SblrDdlAlterPublicationResultV1 rr;
    rr.body[0] = 1;
    rr.body[16] = 1;
    rr.body[32] = 1;
    rr.body[48] = 1;
    ...
    rr.body[96] = 1;
    rr.body[97] = 1;
    auto bytes = scratchbird::engine::sblr::EncodeSblrDdlAlterPublicationResultV1(rr);
    if (bytes.empty()) return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR, out_result, 4142, "SYSTEM.CONFIG_FAILED", "sblr.ddl_alter_publication.result_encoding_failed");
    result->result_kind = "ddl_result";
    result->payload.assign(reinterpret_cast<const char*>(bytes.data()), bytes.size());
  }
  if (ddl_drop_publication_root) {
    auto c = receipt->engine_context;
    c.trace_tags.push_back("private_ddl_drop_publication");
    const auto consumed = scratchbird::engine::internal_api::ConsumeSblrDdlDropPublicationDescriptor(c, ddl_drop_publication_descriptor);
    if (!consumed.ok) return fail_result(SB_ENGINE_STATUS_CONFLICT, out_result, 4143, consumed.diagnostic.code, consumed.diagnostic.message_key);
    scratchbird::engine::sblr::SblrDdlDropPublicationResultV1 rr;
    rr.body[0] = 1; rr.body[16] = 1; rr.body[32] = 1; rr.body[48] = 1;
    auto bytes = scratchbird::engine::sblr::EncodeSblrDdlDropPublicationResultV1(rr);
    if (bytes.empty()) return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR, out_result, 4143, "SYSTEM.CONFIG_FAILED", "sblr.ddl_drop_publication.result_encoding_failed");
    result->result_kind = "ddl_result";
    result->payload.assign(reinterpret_cast<const char*>(bytes.data()), bytes.size());
  }
  if (ddl_create_subscription_root) {
    auto c = receipt->engine_context;
    c.trace_tags.push_back("private_ddl_create_subscription");
    const auto consumed = scratchbird::engine::internal_api::ConsumeSblrDdlCreateSubscriptionDescriptor(c, ddl_create_subscription_descriptor);
    if (!consumed.ok) return fail_result(SB_ENGINE_STATUS_CONFLICT, out_result, 4144, consumed.diagnostic.code, consumed.diagnostic.message_key);
    scratchbird::engine::sblr::SblrDdlCreateSubscriptionResultV1 rr;
    rr.body[0] = 1; rr.body[16] = 1; rr.body[32] = 1; rr.body[48] = 1;
    auto bytes = scratchbird::engine::sblr::EncodeSblrDdlCreateSubscriptionResultV1(rr);
    if (bytes.empty()) return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR, out_result, 4144, "SYSTEM.CONFIG_FAILED", "sblr.ddl_create_subscription.result_encoding_failed");
    result->result_kind = "ddl_result";
    result->payload.assign(reinterpret_cast<const char*>(bytes.data()), bytes.size());
  }
  if (ddl_alter_subscription_root) {
    auto c = receipt->engine_context;
    c.trace_tags.push_back("private_ddl_alter_subscription");
    const auto consumed = scratchbird::engine::internal_api::ConsumeSblrDdlAlterSubscriptionDescriptor(c, ddl_alter_subscription_descriptor);
    if (!consumed.ok) return fail_result(SB_ENGINE_STATUS_CONFLICT, out_result, 4144, consumed.diagnostic.code, consumed.diagnostic.message_key);
    scratchbird::engine::sblr::SblrDdlAlterSubscriptionResultV1 rr;
    rr.body[0] = 1; rr.body[16] = 1; rr.body[32] = 1; rr.body[48] = 1;
    auto bytes = scratchbird::engine::sblr::EncodeSblrDdlAlterSubscriptionResultV1(rr);
    if (bytes.empty()) return fail_result(SB_ENGINE_STATUS_INTERNAL_ERROR, out_result, 4144, "SYSTEM.CONFIG_FAILED", "sblr.ddl_alter_subscription.result_encoding_failed");
    result->result_kind = "ddl_result";
    result->payload.assign(reinterpret_cast<const char*>(bytes.data()), bytes.size());
  }
  if (ddl_drop_subscription_root) {
    auto c = receipt->engine_context;
    c.trace_tags.push_back("private_ddl_drop_subscription");
    const auto consumed = scratchbird::engine::internal_api::ConsumeSblrDdlDropSubscriptionDescriptor(c, ddl_drop_subscription_descriptor);
    if (!consumed.ok) return fail_result(SB_ENGINE_STATUS_CONFLICT, out_result, 4144, consumed.diagnostic.code, consumed.diagnostic.message_key);
    scratchbird::engine::sblr::SblrDdlDropSubscriptionResultV1 rr;
    rr.body[0] = 1; rr.body[16] = 1; rr.body[32] = 1; rr.body[48] = 1;
