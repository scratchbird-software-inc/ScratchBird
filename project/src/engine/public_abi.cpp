...
bool looks_like_sblr_operation_envelope(const scratchbird::engine::SblrExecutionEnvelope& envelope) {
  ...
  if (decoded.ok && (decoded.envelope.operation_id == "engine.op.context_set" || decoded.envelope.operation_id ==
      ...
      (decoded.envelope.opcode == "SBLR_CONTEXT_SET" || decoded.envelope.opcode == "SBLR_DDL_REFRESH_MATERIALIZED_VIEW" || decoded.envelope.opcode == "SBLR_DDL_CREATE_MATERIALIZED_VIEW" || decoded.envelope.opcode == "SBLR_DDL_ALTER_SEQUENCE" || decoded.envelope.opcode == "SBLR_DDL_DROP_TYPE" || decoded.envelope.opcode == "SBLR_DDL_CREATE_TABLE_AS_QUERY_WITH_DATA" || decoded.envelope.opcode == "SBLR_DDL_CREATE_TABLE_AS_QUERY_WITH_NO_DATA")) {
    return true;
  }
  ...
  if (stream.ok && stream.stream.operations.size() >= 3) {
    ...
    if ((operation.operation_id == "engine.op.context_set" || operation.operation_id == "engine.op.ddl_refresh_materialized_view" || operation.operation_id == "engine.op.ddl_create_materialized_view" || operation.operation_id == "engine.op.ddl_alter_sequence" || operation.operation_id == "engine.op.ddl_drop_type" || operation.operation_id == "engine.op.ddl_create_table_as_query_with_data" || operation.operation_id == "engine.op.ddl_create_table_as_query_with_no_data") &&
        (operation.opcode == "SBLR_CONTEXT_SET" || operation.opcode == "SBLR_DDL_REFRESH_MATERIALIZED_VIEW" || operation.opcode == "SBLR_DDL_CREATE_MATERIALIZED_VIEW" || operation.opcode == "SBLR_DDL_ALTER_SEQUENCE" || operation.opcode == "SBLR_DDL_DROP_TYPE" || operation.opcode == "SBLR_DDL_RENAME_OBJECT" || operation.opcode == "SBLR_DDL_CREATE_TABLE_AS_QUERY_WITH_DATA" || operation.opcode == "SBLR_DDL_CREATE_TABLE_AS_QUERY_WITH_NO_DATA")) {
      return true;
    }
  }
