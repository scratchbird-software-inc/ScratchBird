...
std::string RowPrefix(const OptimizerReferenceComparisonArtifactRow& row) {
  if (!row.row_id.empty()) return row.row_id;
  if (!row.reference_engine.empty()) return row.reference_engine;
  return "unnamed_reference_comparison_row";
}

std::string ReportPrefix(const OptimizerReferenceComparisonReport& report) {
  return report.report_id.empty() ? "unnamed_reference_comparison_report"
                                  : report.report_id;
}

void RequireField(OptimizerReferenceComparisonValidation* validation,
                  bool present,
                  std::string field_name) {
  if (!present) validation->missing_fields.push_back(std::move(field_name));
}

void AddDiagnostic(OptimizerReferenceComparisonValidation* validation,
                   std::string diagnostic) {
  validation->diagnostics.push_back(std::move(diagnostic));
}

void AddRowDiagnostic(OptimizerReferenceComparisonValidation* validation,
...
bool HasAuthorityDrift(
    const OptimizerReferenceComparisonAuthorityFlags& authority) {
  return authority.transaction_finality_authority ||
         ...
         authority.reference_result_authority ||
         authority.reference_execution_authority ||
         authority.reference_storage_authority ||
...
bool TimingValid(const OptimizerReferenceTimingEvidence& timing) {
  return Positive(timing.scratchbird_p50_us) &&
         Positive(timing.scratchbird_p95_us) &&
         Positive(timing.scratchbird_p99_us) &&
         Positive(timing.reference_p50_us) &&
         Positive(timing.reference_p95_us) &&
         Positive(timing.reference_p99_us) &&
         timing.scratchbird_p50_us <= timing.scratchbird_p95_us &&
         timing.scratchbird_p95_us <= timing.scratchbird_p99_us &&
         timing.reference_p50_us <= timing.reference_p95_us &&
         timing.reference_p95_us <= timing.reference_p99_us;
}
...
bool BenchmarkEvidenceMatchesRow(
  ...
  return std::any_of(
      benchmark.reference_methodology.begin(),
      benchmark.reference_methodology.end(),
      [&](const OptimizerBenchmarkReferenceMethodologyEvidence& reference) {
        return reference.reference_engine == row.reference_engine &&
               reference.reference_version == row.reference_version &&
               reference.reference_native_method == row.reference_native_method &&
               reference.dataset_schema_mapping_digest ==
                   row.dataset_mapping_digest &&
               reference.workload_mapping_digest ==
                   row.workload_query_mapping_digest &&
               reference.route_equivalence_contract_hash ==
                   row.route_equivalence_contract_hash &&
               reference.reference_result_hash == row.reference_result_hash &&
               reference.comparable_status == "comparable" &&
               reference.reference_reference_only &&
               !reference.reference_as_authority &&
...
bool RowClaimBlockedExternal(
  ...
  return row.cluster_mode ==
             OptimizerReferenceComparisonClusterMode::kExternalProviderDelegated &&
         !row.external_cluster_provider_id.empty() &&
         row.cluster_claim_blocked &&
         !row.production_reference_comparison_claim;
}
...
ValidateOptimizerReferenceComparisonArtifactRow(
    const OptimizerReferenceComparisonArtifactRow& row) {
  OptimizerReferenceComparisonValidation validation;

  RequireField(&validation, !Empty(row.row_id), "row_id");
  RequireField(&validation, !Empty(row.reference_engine), "reference_engine");
  RequireField(&validation, !Empty(row.reference_version), "reference_version");
  RequireField(&validation,
               IsHashLike(row.reference_version_digest) &&
                   !PlaceholderDigest(row.reference_version_digest),
               "reference_version_digest");
  RequireField(&validation,
               !Empty(row.reference_specialty_class),
               "reference_specialty_class");
  RequireField(&validation,
               !Empty(row.reference_native_method),
               "reference_native_method");
  RequireField(&validation, !Empty(row.reference_run_id), "reference_run_id");
  RequireField(&validation,
               IsHashLike(row.reference_execution_artifact_digest) &&
                   !PlaceholderDigest(row.reference_execution_artifact_digest),
               "reference_execution_artifact_digest");
  RequireField(&validation,
  ...
  if (!row.trusted_provenance || !row.fresh || !row.redaction_applied ||
    ...
    AddRowDiagnostic(&validation, row,
                     "SB_OPT_REFERENCE_COMPARE.PROVENANCE_FRESHNESS_INVALID");
  }
  if (row.placeholder_runtime_evidence || row.synthetic_statistics ||
      row.local_default_statistics || row.policy_default_statistics) {
    AddRowDiagnostic(&validation, row,
                     "SB_OPT_REFERENCE_COMPARE.SYNTHETIC_OR_PLACEHOLDER");
  }
  if (HasAuthorityDrift(row.authority) || !row.reference_reference_only ||
      row.reference_as_authority || row.reference_result_authority ||
      row.reference_execution_authority ||
      row.reference_storage_or_finality_substitution ||
      row.reference_transaction_authority || row.reference_security_authority ||
      row.reference_recovery_authority || row.reference_optimizer_plan_authority ||
      row.benchmark_dominance_claimed) {
    AddRowDiagnostic(&validation, row,
                     "SB_OPT_REFERENCE_COMPARE.FORBIDDEN_AUTHORITY");
  }

  if (row.run_mode == OptimizerReferenceComparisonRunMode::kLiveExternalRun &&
      !row.live_external_engine_run_observed) {
    AddRowDiagnostic(&validation, row,
                     "SB_OPT_REFERENCE_COMPARE.LIVE_RUN_NOT_OBSERVED");
  }
  if (row.run_mode == OptimizerReferenceComparisonRunMode::kArtifactOnlyEvidence &&
      row.live_external_engine_run_observed) {
    AddRowDiagnostic(&validation, row,
                     "SB_OPT_REFERENCE_COMPARE.ARTIFACT_MODE_LIVE_OVERCLAIM");
  }
  if (row.comparable_status ==
      OptimizerReferenceComparisonStatus::kComparable) {
    if (!IsHashLike(row.reference_result_hash) ||
        PlaceholderDigest(row.reference_result_hash) ||
        row.reference_result_row_count == 0 || !TimingValid(row.timing)) {
      AddRowDiagnostic(&validation, row,
                       "SB_OPT_REFERENCE_COMPARE.COMPARABLE_RESULT_MISSING");
    }
    if (!row.non_comparable_blockers.empty() ||
        row.run_mode ==
            OptimizerReferenceComparisonRunMode::kNonComparableEvidence) {
      AddRowDiagnostic(&validation, row,
                       "SB_OPT_REFERENCE_COMPARE.COMPARABLE_BLOCKER_CONFLICT");
    }
    if (!row.ceic_051_benchmark_evidence_attached) {
      AddRowDiagnostic(&validation, row,
                       "SB_OPT_REFERENCE_COMPARE.CEIC051_EVIDENCE_REQUIRED");
    } else {
      ...
      if (!nested.ok ||
        ...
        AddRowDiagnostic(&validation, row,
                         "SB_OPT_REFERENCE_COMPARE.CEIC051_EVIDENCE_INVALID");
        ...
        AddRowDiagnostic(&validation, row,
                         "SB_OPT_REFERENCE_COMPARE.CEIC051_EVIDENCE_MISMATCH");
      }
    }
  } else {
    if (row.non_comparable_blockers.empty()) {
      AddRowDiagnostic(
          &validation,
          row,
          "SB_OPT_REFERENCE_COMPARE.NON_COMPARABLE_BLOCKER_REQUIRED");
    }
    if (row.run_mode !=
        OptimizerReferenceComparisonRunMode::kNonComparableEvidence) {
      AddRowDiagnostic(&validation, row,
                       "SB_OPT_REFERENCE_COMPARE.NON_COMPARABLE_MODE_REQUIRED");
    }
  }

  if (row.cluster_mode ==
          OptimizerReferenceComparisonClusterMode::kLocalClusterEvidence ||
      row.route_kind == "cluster") {
    AddRowDiagnostic(&validation, row,
                     "SB_OPT_REFERENCE_COMPARE.LOCAL_CLUSTER_FORBIDDEN");
    ...
    if (row.external_cluster_provider_id.empty() ||
      ...
      AddRowDiagnostic(
          &validation,
          row,
          "SB_OPT_REFERENCE_COMPARE.EXTERNAL_CLUSTER_CLAIM_BLOCK_REQUIRED");
    }
    if (row.ceic_051_benchmark_evidence_attached &&
        !BenchmarkEvidenceIsClaimBlockedExternal(row.benchmark_evidence)) {
      AddRowDiagnostic(
          &validation,
          row,
          "SB_OPT_REFERENCE_COMPARE.EXTERNAL_CLUSTER_NESTED_CLAIM_BLOCK_REQUIRED");
    }
  }
  ...
  validation.comparison_artifact_proven =
      validation.ok && row.production_reference_comparison_claim &&
      row.cluster_mode == OptimizerReferenceComparisonClusterMode::kNoCluster;
  validation.diagnostic_code =
      validation.ok
          ? "SB_OPT_REFERENCE_COMPARE.ROW_OK"
          : (validation.missing_fields.empty()
                 ? "SB_OPT_REFERENCE_COMPARE.ROW_INVALID"
                 : "SB_OPT_REFERENCE_COMPARE.MISSING_REQUIRED_FIELD");
  return validation;
}

OptimizerReferenceComparisonValidation ValidateOptimizerReferenceComparisonReport(
    const OptimizerReferenceComparisonReport& report) {
  OptimizerReferenceComparisonValidation validation;
  const auto prefix = ReportPrefix(report);

  RequireField(&validation,
               report.schema_id == kOptimizerLiveReferenceComparisonSchemaId,
               "schema_id");
  RequireField(&validation,
  ...
  if (!report.trusted_provenance || !report.fresh ||
    ...
    AddDiagnostic(&validation,
                  prefix + ":SB_OPT_REFERENCE_COMPARE.REPORT_PROVENANCE_INVALID");
  }
  if (HasAuthorityDrift(report.authority) ||
      report.benchmark_dominance_claimed) {
    AddDiagnostic(&validation,
                  prefix + ":SB_OPT_REFERENCE_COMPARE.REPORT_FORBIDDEN_AUTHORITY");
  }
  if (report.required_reference_engines.size() <
      kMinimumOptimizerReferenceComparisonEngines) {
    AddDiagnostic(&validation,
                  prefix + ":SB_OPT_REFERENCE_COMPARE.REQUIRED_REFERENCE_SET_TOO_SMALL");
  }
  if (report.required_route_kinds.empty()) {
    AddDiagnostic(&validation,
                  prefix + ":SB_OPT_REFERENCE_COMPARE.REQUIRED_ROUTES_MISSING");
  }
  if (report.rows.empty()) {
    AddDiagnostic(&validation,
                  prefix + ":SB_OPT_REFERENCE_COMPARE.ROWS_MISSING");
  }

  if (report.cluster_mode ==
      OptimizerReferenceComparisonClusterMode::kLocalClusterEvidence) {
    AddDiagnostic(&validation,
                  prefix + ":SB_OPT_REFERENCE_COMPARE.LOCAL_CLUSTER_FORBIDDEN");
  } else if (report.cluster_mode ==
             OptimizerReferenceComparisonClusterMode::kExternalProviderDelegated) {
    if (report.external_cluster_provider_id.empty() ||
        !report.cluster_claim_blocked ||
        report.production_reference_matrix_claim) {
      AddDiagnostic(
          &validation,
          prefix + ":SB_OPT_REFERENCE_COMPARE.EXTERNAL_CLUSTER_CLAIM_BLOCK_REQUIRED");
    }
  }

  std::set<std::string> seen_row_ids;
  std::set<std::string> proven_references;
  ...
  for (const auto& row : report.rows) {
    ...
    const auto row_validation =
        ValidateOptimizerReferenceComparisonArtifactRow(row);
    ...
    if (row_validation.comparison_artifact_proven) {
      proven_references.insert(row.reference_engine);
      proven_routes.insert(row.route_kind);
    }
  }

  for (const auto& reference_engine : report.required_reference_engines) {
    if (reference_engine.empty()) {
      AddDiagnostic(&validation,
                    prefix + ":SB_OPT_REFERENCE_COMPARE.EMPTY_REQUIRED_REFERENCE");
    } else if (proven_references.find(reference_engine) == proven_references.end()) {
      AddDiagnostic(&validation,
                    reference_engine +
                        ":SB_OPT_REFERENCE_COMPARE.MISSING_REQUIRED_REFERENCE");
    }
  }
  for (const auto& route_kind : report.required_route_kinds) {
    if (proven_routes.find(route_kind) == proven_routes.end()) {
      AddDiagnostic(&validation,
                    route_kind +
                        ":SB_OPT_REFERENCE_COMPARE.MISSING_REQUIRED_ROUTE");
    }
  }
  if (proven_references.size() < kMinimumOptimizerReferenceComparisonEngines) {
    AddDiagnostic(&validation,
                  prefix + ":SB_OPT_REFERENCE_COMPARE.REFERENCE_BREADTH_INSUFFICIENT");
  }
  if (saw_artifact_only && !report.evidence_only) {
    AddDiagnostic(&validation,
                  prefix + ":SB_OPT_REFERENCE_COMPARE.ARTIFACT_ONLY_OVERCLAIM");
  }

  validation.ok = validation.missing_fields.empty() &&
                  validation.diagnostics.empty();
  validation.comparison_artifact_proven =
      validation.ok && report.production_reference_matrix_claim &&
      report.cluster_mode == OptimizerReferenceComparisonClusterMode::kNoCluster;
  validation.diagnostic_code =
      validation.ok
          ? "SB_OPT_REFERENCE_COMPARE.REPORT_OK"
          : (validation.missing_fields.empty()
                 ? "SB_OPT_REFERENCE_COMPARE.REPORT_INVALID"
                 : "SB_OPT_REFERENCE_COMPARE.MISSING_REQUIRED_FIELD");
  return validation;
}
