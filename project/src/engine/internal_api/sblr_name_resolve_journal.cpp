...
constexpr std::string_view kRecordDomain =
    "ScratchBird.NameResolveJournalRecord.V1";
...
SblrNameResolveJournalHashV1 Hash(const std::uint8_t* bytes,
                                  std::size_t size) {
...
SblrNameResolveJournalHashV1 RecordHash(
...
std::string UuidText(const SblrNameResolveJournalUuidV1& value) {
...
bool KeyValid(const SblrNameResolveJournalKeyV1& key) {
  return NonZero(key.database_uuid) &&
         NonZero(key.statement_receipt_uuid) &&
         NonZero(key.resolution_uuid) && NonZero(key.descriptor_sha256) &&
         NonZero(key.canonical_name_sha256) && key.catalog_generation != 0 &&
         key.security_epoch != 0 && key.resource_epoch != 0;
}

bool SameKey(const SblrNameResolveJournalKeyV1& left,
             const SblrNameResolveJournalKeyV1& right) {
  return left.database_uuid == right.database_uuid &&
         left.statement_receipt_uuid == right.statement_receipt_uuid &&
         left.resolution_uuid == right.resolution_uuid &&
         left.descriptor_sha256 == right.descriptor_sha256 &&
         left.canonical_name_sha256 == right.canonical_name_sha256 &&
         left.namespace_generation == right.namespace_generation &&
...
bool HasAuthority(const EngineRequestContext& context,
  ...
  return context.security_context_present &&
         ...
         std::find(context.trace_tags.begin(), context.trace_tags.end(),
                   "private_name_resolve_journal") !=
             context.trace_tags.end();
}

std::string Path(const EngineRequestContext& context,
                 const SblrNameResolveJournalKeyV1& key) {
  return context.database_path + ".sb.sblr_name_resolve_journal.v1." +
         UuidText(key.resolution_uuid);
}

std::vector<std::uint8_t> Encode(
    const SblrNameResolveJournalSnapshotV1& snapshot) {
  if (!KeyValid(snapshot.key) || snapshot.journal_generation == 0 ||
      (snapshot.state != SblrNameResolveJournalStateV1::begun &&
       snapshot.state != SblrNameResolveJournalStateV1::published)) {
    return {};
  }
  if ((snapshot.state == SblrNameResolveJournalStateV1::begun &&
  ...
  std::vector<std::uint8_t> bytes{'S', 'B', 'N', 'J'};
  PutLe(&bytes, 1, 2);
  ...
  Put(&bytes, snapshot.key.resolution_uuid);
  Put(&bytes, snapshot.key.descriptor_sha256);
  Put(&bytes, snapshot.key.canonical_name_sha256);
  PutLe(&bytes, snapshot.key.namespace_generation, 8);
  ...
  PutLe(&bytes, snapshot.canonical_result_bytes.size(), 4);
  PutLe(&bytes, 0, 4);
  Put(&bytes, result_hash);
  bytes.insert(bytes.end(), 32, 0);
  bytes.insert(bytes.end(), 48, 0);
...
bool Decode(const std::vector<std::uint8_t>& bytes,
  ...
  if (snapshot == nullptr || bytes.size() < kHeaderBytes ||
      ...
      !std::equal(bytes.begin(), bytes.begin() + 4, "SBNJ") ||
      GetLe(bytes.data() + 4, 2) != 1 ||
      GetLe(bytes.data() + 6, 2) != kHeaderBytes ||
      GetLe(bytes.data() + 8, 4) != bytes.size() ||
      !Zero(bytes.data() + 172, bytes.data() + 176) ||
      !Zero(bytes.data() + 240, bytes.data() + 288)) {
    return false;
  }
  SblrNameResolveJournalSnapshotV1 value;
  const auto state = GetLe(bytes.data() + 12, 4);
  if (state < 1 || state > 2) return false;
  value.state = static_cast<SblrNameResolveJournalStateV1>(state);
  Get(bytes.data() + 16, &value.key.database_uuid);
  Get(bytes.data() + 32, &value.key.statement_receipt_uuid);
  Get(bytes.data() + 48, &value.key.resolution_uuid);
  Get(bytes.data() + 64, &value.key.descriptor_sha256);
  Get(bytes.data() + 96, &value.key.canonical_name_sha256);
  value.key.namespace_generation = GetLe(bytes.data() + 128, 8);
  value.key.catalog_generation = GetLe(bytes.data() + 136, 8);
  value.key.security_epoch = GetLe(bytes.data() + 144, 8);
  value.key.resource_epoch = GetLe(bytes.data() + 152, 8);
  value.journal_generation = GetLe(bytes.data() + 160, 8);
  const auto result_size = GetLe(bytes.data() + 168, 4);
  Get(bytes.data() + 176, &value.canonical_result_sha256);
  Get(bytes.data() + 208, &value.record_evidence_sha256);
...
SblrNameResolveJournalResultV1 Refused(std::string code, std::string key,
                                       std::string detail = {}) {
  SblrNameResolveJournalResultV1 result;
  result.diagnostic =
      Diagnostic(std::move(code), std::move(key), std::move(detail));
  return result;
}

SblrNameResolveJournalResultV1 Loaded(
    SblrNameResolveJournalSnapshotV1 snapshot) {
  SblrNameResolveJournalResultV1 result;
  result.ok = true;
  result.found = true;
...
SblrNameResolveJournalResultV1 LoadExact(
  ...
  if (status != ReadStatus::ok || !Decode(bytes, &snapshot)) {
    return Refused("CATALOG.SNAPSHOT_STALE",
                   "sblr.name_resolve.journal_corrupt",
                   "durable NAME_RESOLVE journal is torn or noncanonical");
  }
  if (!SameKey(snapshot.key, key)) {
    return Refused("MGA.TRANSACTION.STALE",
                   "sblr.name_resolve.journal_identity_conflict",
                   "resolution identity is already bound to different authority");
  }
...
SblrNameResolveJournalResultV1 LookupSblrNameResolveJournalV1(
  ...
  if (!InputsValid(context, key)) {
    return Refused("SECURITY.ACCESS_DENIED",
                   "sblr.name_resolve.journal_hidden");
  }
...
SblrNameResolveJournalResultV1 EnsureSblrNameResolveJournalV1(
  ...
  if (bytes.empty()) {
    return Refused("SBLR.OPERAND.INVALID",
                   "sblr.name_resolve.journal_key_invalid");
  }
  ...
  if (created != CreateStatus::created || !Decode(bytes, &snapshot)) {
    return Refused("CATALOG.SNAPSHOT_STALE",
                   "sblr.name_resolve.journal_publish_failed",
                   "durable resolution identity publication failed");
  }
  return Loaded(std::move(snapshot));
}
...
SblrNameResolveJournalResultV1 PublishSblrNameResolveJournalResultV1(
  ...
  if (!existing.found) {
    return Refused("MGA.TRANSACTION.STALE",
                   "sblr.name_resolve.journal_identity_missing");
  }
  if (existing.snapshot.state == SblrNameResolveJournalStateV1::published) {
    if (existing.snapshot.canonical_result_bytes == canonical_result_bytes) {
      return existing;
    }
    return Refused("MGA.TRANSACTION.STALE",
                   "sblr.name_resolve.journal_result_conflict");
  }
  if (canonical_result_bytes.size() != kResultBytes ||
      existing.snapshot.journal_generation ==
          std::numeric_limits<std::uint64_t>::max()) {
    return Refused("SBLR.OPERAND.INVALID",
                   "sblr.name_resolve.journal_result_invalid");
  }
  auto published = existing.snapshot;
  published.state = SblrNameResolveJournalStateV1::published;
  ...
  if (bytes.empty() || !ReplaceFile(Path(context, key), bytes) ||
    ...
    return Refused("CATALOG.SNAPSHOT_STALE",
                   "sblr.name_resolve.journal_result_publish_failed",
                   "durable NAME_RESOLVE result publication failed");
  }
