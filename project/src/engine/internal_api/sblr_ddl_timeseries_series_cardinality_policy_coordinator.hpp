...
SblrDdlTimeseriesSeriesCardinalityPolicyCoordinationResult CompileSblrDdlTimeseriesSeriesCardinalityPolicyDescriptor(const EngineRequestContext&,const EngineUuid&,std::uint64_t,std::uint64_t,std::uint64_t);
