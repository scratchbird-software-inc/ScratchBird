...
namespace scratchbird::engine::internal_api { namespace { std::mutex m; std::unordered_map<std::string,bool> live,used; std::string key(const EngineRequestContext&c,const scratchbird::engine::sblr::SblrDdlAlterContinuousViewDescriptorV1&d){return c.session_uuid+"|"+std::to_string(d.availability)+"|"+std::to_string(d.body[16]);} }
