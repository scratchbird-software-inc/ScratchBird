...
SblrDdlAlterTypeCoordinationResult CompileSblrDdlAlterTypeDescriptor(
  ...
  SblrDdlAlterTypeCoordinationResult result;
  if (!HasPrivateTag(context, "private_ddl_alter_type_binder") ||
    ...
    result.diagnostic =
        Diagnostic("SBLR.OPERAND_INVALID",
                   "sblr.ddl_alter_type.coordination_invalid");
    return result;
  }
...
SblrDdlAlterTypeCoordinationResult ConsumeSblrDdlAlterTypeDescriptor(
    const EngineRequestContext& context,
    const scratchbird::engine::sblr::SblrDdlAlterTypeDescriptorV1&
        descriptor) {
  SblrDdlAlterTypeCoordinationResult result;
  if (scratchbird::engine::sblr::EncodeSblrDdlAlterTypeDescriptorV1(
          descriptor, true)
          .empty()) {
    result.diagnostic =
        Diagnostic("SBLR.OPERAND_INVALID",
  ...
  if (!HasPrivateTag(context, "private_ddl_alter_type")) {
    result.diagnostic = Diagnostic("SECURITY.ACCESS_DENIED",
                                   "sblr.ddl_alter_type.hidden");
    return result;
  }
  result.diagnostic = MissingExecutorEvidence();
  return result;
