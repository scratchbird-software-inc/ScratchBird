...
inline SblrDdlDropSynonymCoordinationResult CompileSblrDdlDropSynonymDescriptor(const EngineRequestContext& c,const EngineUuid& r,std::uint64_t o,std::uint32_t s,std::uint64_t a){auto x=c;for(auto& t:x.trace_tags)if(t=="private_ddl_drop_synonym_binder")t="private_ddl_drop_package_binder";return CompileSblrDdlDropPackageDescriptor(x,r,o,s,a);}
