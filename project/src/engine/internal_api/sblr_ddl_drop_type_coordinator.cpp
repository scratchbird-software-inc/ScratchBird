...
SblrDdlDropTypeCoordinationResult CompileSblrDdlDropTypeDescriptor(
  ...
  SblrDdlDropTypeCoordinationResult result;
...
SblrDdlDropTypeCoordinationResult ConsumeSblrDdlDropTypeDescriptor(
    ...
    const scratchbird::engine::sblr::SblrDdlDropTypeDescriptorV1& descriptor) {
  SblrDdlDropTypeCoordinationResult result;
  if (scratchbird::engine::sblr::EncodeSblrDdlDropTypeDescriptorV1(
          descriptor, true)
          .empty()) {
    result.diagnostic = Diagnostic("SBLR.OPERAND_INVALID",
                                   "sblr.ddl_drop_type.coordination_invalid");
  ...
  if (!HasPrivateTag(context, "private_ddl_drop_type")) {
    result.diagnostic = Diagnostic("SECURITY.ACCESS_DENIED",
                                   "sblr.ddl_drop_type.hidden");
    return result;
  }
  result.diagnostic = MissingExecutorEvidence();
  return result;
