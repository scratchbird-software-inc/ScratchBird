...
inline SblrDdlCreateFdwCoordinationResult CompileSblrDdlCreateFdwDescriptor(const EngineRequestContext& c,const EngineUuid& r,std::uint64_t o,std::uint32_t s,std::uint64_t a){auto x=c;for(auto& t:x.trace_tags)if(t=="private_ddl_create_fdw_binder")t="private_ddl_create_package_binder";return CompileSblrDdlCreatePackageDescriptor(x,r,o,s,a);}
