...
SblrDdlCreateMaterializedViewCoordinationResult CompileSblrDdlCreateMaterializedViewDescriptor(const EngineRequestContext& c,const EngineUuid& r,std::uint64_t o,std::uint32_t p,std::uint64_t a) { auto x=c; x.trace_tags.push_back("private_ddl_create_view_binder"); return CompileSblrDdlCreateViewDescriptor(x,r,o,p,a); }
