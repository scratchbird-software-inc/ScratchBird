...
constexpr std::size_t kMaximumResultBytes = 256 + 16U * 1024U * 1024U;
constexpr std::string_view kRecordDomain =
    "ScratchBird.ParseTextJournalRecord.V1";
...
SblrParseTextJournalHashV1 Hash(const std::uint8_t* bytes,
                                std::size_t size) {
...
SblrParseTextJournalHashV1 RecordHash(
...
std::string UuidText(const SblrParseTextJournalUuidV1& value) {
...
bool KeyValid(const SblrParseTextJournalKeyV1& key) {
  return NonZero(key.database_uuid) &&
         NonZero(key.statement_receipt_uuid) && NonZero(key.parse_uuid) &&
         NonZero(key.descriptor_sha256) &&
         NonZero(key.canonical_input_sha256) &&
         NonZero(key.language_profile_uuid) &&
         key.language_profile_generation != 0 &&
         key.catalog_generation != 0 && key.security_epoch != 0 &&
         key.resource_epoch != 0 &&
         key.executor_availability_generation != 0;
}

bool SameKey(const SblrParseTextJournalKeyV1& left,
             const SblrParseTextJournalKeyV1& right) {
  return left.database_uuid == right.database_uuid &&
         left.statement_receipt_uuid == right.statement_receipt_uuid &&
         left.parse_uuid == right.parse_uuid &&
         left.descriptor_sha256 == right.descriptor_sha256 &&
         left.canonical_input_sha256 == right.canonical_input_sha256 &&
         left.language_profile_uuid == right.language_profile_uuid &&
         left.language_profile_generation == right.language_profile_generation &&
...
std::string Path(const EngineRequestContext& context,
                 const SblrParseTextJournalKeyV1& key) {
  return context.database_path + ".sb.sblr_parse_text_journal.v1." +
         UuidText(key.parse_uuid);
}

std::vector<std::uint8_t> Encode(
    const SblrParseTextJournalSnapshotV1& snapshot) {
  if (!KeyValid(snapshot.key) || snapshot.journal_generation == 0 ||
      (snapshot.state != SblrParseTextJournalStateV1::begun &&
       snapshot.state != SblrParseTextJournalStateV1::published)) {
    return {};
  }
  if ((snapshot.state == SblrParseTextJournalStateV1::begun &&
       (!snapshot.canonical_result_bytes.empty() ||
        NonZero(snapshot.canonical_result_sha256))) ||
      (snapshot.state == SblrParseTextJournalStateV1::published &&
       (snapshot.canonical_result_bytes.empty() ||
        snapshot.canonical_result_bytes.size() > kMaximumResultBytes))) {
    return {};
  }
  const auto result_hash = snapshot.canonical_result_bytes.empty()
                               ? SblrParseTextJournalHashV1{}
  ...
  std::vector<std::uint8_t> bytes{'S', 'B', 'P', 'J'};
  PutLe(&bytes, 1, 2);
  ...
  Put(&bytes, snapshot.key.parse_uuid);
  Put(&bytes, snapshot.key.descriptor_sha256);
  Put(&bytes, snapshot.key.canonical_input_sha256);
  Put(&bytes, snapshot.key.language_profile_uuid);
  PutLe(&bytes, snapshot.key.language_profile_generation, 8);
...
bool Decode(const std::vector<std::uint8_t>& bytes,
  ...
  if (snapshot == nullptr || bytes.size() < kHeaderBytes ||
      bytes.size() > kHeaderBytes + kMaximumResultBytes ||
      !std::equal(bytes.begin(), bytes.begin() + 4, "SBPJ") ||
  ...
  SblrParseTextJournalSnapshotV1 value;
  const auto state = GetLe(bytes.data() + 12, 4);
  if (state < 1 || state > 2) return false;
  value.state = static_cast<SblrParseTextJournalStateV1>(state);
  Get(bytes.data() + 16, &value.key.database_uuid);
  Get(bytes.data() + 32, &value.key.statement_receipt_uuid);
  Get(bytes.data() + 48, &value.key.parse_uuid);
  Get(bytes.data() + 64, &value.key.descriptor_sha256);
  Get(bytes.data() + 96, &value.key.canonical_input_sha256);
  Get(bytes.data() + 128, &value.key.language_profile_uuid);
  value.key.language_profile_generation = GetLe(bytes.data() + 144, 8);
  value.key.catalog_generation = GetLe(bytes.data() + 152, 8);
...
ReadStatus ReadFile(const std::string& path,
  ...
  if (end < 0 || static_cast<std::uint64_t>(end) >
                     kHeaderBytes + kMaximumResultBytes) {
...
SblrParseTextJournalResultV1 Refused(std::string code, std::string key,
                                     std::string detail = {}) {
  SblrParseTextJournalResultV1 result;
  result.diagnostic =
      Diagnostic(std::move(code), std::move(key), std::move(detail));
  return result;
}

SblrParseTextJournalResultV1 Loaded(
    SblrParseTextJournalSnapshotV1 snapshot) {
  SblrParseTextJournalResultV1 result;
  result.ok = true;
  result.found = true;
...
SblrParseTextJournalResultV1 LoadExact(
  ...
  if (status != ReadStatus::ok || !Decode(bytes, &snapshot)) {
    return Refused("CATALOG.SNAPSHOT_STALE",
                   "sblr.parse_text.journal_corrupt",
                   "durable PARSE_TEXT journal is torn or noncanonical");
  }
  if (!SameKey(snapshot.key, key)) {
    return Refused("MGA.AUTHORITY_MISMATCH",
                   "sblr.parse_text.journal_identity_conflict",
                   "parse identity is already bound to different authority");
  }
...
SblrParseTextJournalResultV1 LookupSblrParseTextJournalV1(
  ...
  if (!InputsValid(context, key)) {
    return Refused("SECURITY.ACCESS_DENIED",
                   "sblr.parse_text.journal_hidden");
  }
...
SblrParseTextJournalResultV1 EnsureSblrParseTextJournalV1(
  ...
  if (bytes.empty()) {
    return Refused("SBLR.OPERAND_INVALID",
                   "sblr.parse_text.journal_key_invalid");
  }
  ...
  if (created != CreateStatus::created || !Decode(bytes, &snapshot)) {
    return Refused("CATALOG.SNAPSHOT_STALE",
                   "sblr.parse_text.journal_publish_failed",
                   "durable parse identity publication failed");
  }
  return Loaded(std::move(snapshot));
}
...
SblrParseTextJournalResultV1 PublishSblrParseTextJournalResultV1(
  ...
  if (!existing.found) {
    return Refused("MGA.AUTHORITY_MISMATCH",
                   "sblr.parse_text.journal_identity_missing");
  }
  if (existing.snapshot.state == SblrParseTextJournalStateV1::published) {
    if (existing.snapshot.canonical_result_bytes == canonical_result_bytes) {
      return existing;
    }
    return Refused("MGA.AUTHORITY_MISMATCH",
                   "sblr.parse_text.journal_result_conflict");
  }
  if (canonical_result_bytes.empty() ||
      canonical_result_bytes.size() > kMaximumResultBytes ||
      existing.snapshot.journal_generation ==
          std::numeric_limits<std::uint64_t>::max()) {
    return Refused("SBLR.OPERAND_INVALID",
                   "sblr.parse_text.journal_result_invalid");
  }
  auto published = existing.snapshot;
  published.state = SblrParseTextJournalStateV1::published;
  ++published.journal_generation;
  published.canonical_result_bytes = canonical_result_bytes;
  published.canonical_result_sha256 = Hash(
  ...
  if (bytes.empty() || !ReplaceFile(Path(context, key), bytes) ||
    ...
    return Refused("CATALOG.SNAPSHOT_STALE",
                   "sblr.parse_text.journal_result_publish_failed",
                   "durable PARSE_TEXT result publication failed");
  }
