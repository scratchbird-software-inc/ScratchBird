...
bool MatchesDmlDeleteDurableAuthorityOwnerV1(const EngineRequestContext& c,
  ...
  return c.security_context_present && c.authorization_context.present &&
      ...
      p::UuidValue(b.database_uuid) == c.database_uuid &&
      p::UuidValue(b.session_uuid) == c.session_uuid &&
      p::UuidValue(b.principal_uuid) == c.principal_uuid &&
      p::UuidValue(d.owning_transaction_uuid) == c.transaction_uuid &&
      d.owning_local_transaction_id == c.local_transaction_id &&
      p::UuidValue(d.authenticated_statement_receipt_uuid) == c.statement_receipt_uuid &&
      p::UuidValue(d.statement_snapshot_uuid) == c.statement_snapshot_uuid &&
      p::UuidValue(d.catalog_snapshot_uuid) == c.statement_metadata_snapshot_uuid &&
      d.catalog_generation == c.catalog_generation_id &&
      d.datatype_registry_generation == c.datatype_registry_generation &&
      p::UuidValue(b.datatypes.identity.vector_uuid) == c.datatype_catalog_snapshot_uuid &&
