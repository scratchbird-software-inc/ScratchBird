...
RelationalDagValidationResult ValidateTypedRelationalDag(
  ...
  // QOW-SOURCE-RCP-076-TYPED-DAG-FUNCTIONLESS-TIME-SERIES-V1
  const auto time_range_count = std::ranges::count_if(
      dag.expressions, [](const auto& expression) {
        return expression.operator_name == "TIME_RANGE";
      });
  const auto time_bucket_count = std::ranges::count_if(
      dag.expressions, [](const auto& expression) {
        return expression.operator_name == "TIME_BUCKET";
      });
  const auto time_downsample_count = std::ranges::count_if(
      dag.expressions, [](const auto& expression) {
        return expression.operator_name == "TIME_DOWNSAMPLE";
      });
  const auto time_series_node = std::ranges::find_if(
      dag.nodes, [](const auto& node) {
        return node.semantic_variant_id == "SBLR_MODEL_SOURCE_V1" ||
               node.semantic_variant_id == "SBLR_MODEL_AGGREGATE_V1";
      });
  const auto time_series_node_count = std::ranges::count_if(
      dag.nodes, [](const auto& node) {
        return node.semantic_variant_id == "SBLR_MODEL_SOURCE_V1" ||
               node.semantic_variant_id == "SBLR_MODEL_AGGREGATE_V1";
      });
  ...
  // QOW-SOURCE-RCP-078-TYPED-DAG-FUNCTIONLESS-SEARCH-V1
  const auto search_match_count = std::ranges::count_if(
      dag.expressions, [](const auto& expression) {
        return expression.operator_name == "SEARCH_MATCH";
      });
  const auto search_query_count = std::ranges::count_if(
      dag.expressions, [](const auto& expression) {
        return expression.operator_name == "SEARCH_TERMS" ||
               expression.operator_name == "SEARCH_PHRASE" ||
               expression.operator_name == "SEARCH_FUZZY";
      });
  const auto search_filter_count = std::ranges::count_if(
      dag.expressions, [](const auto& expression) {
        return expression.operator_name == "SEARCH_FILTER";
      });
  const auto search_analyzer_binding_count = std::ranges::count_if(
      dag.expressions, [](const auto& expression) {
        return expression.operator_name == "SEARCH_ANALYZER_BINDING";
      });
  const auto search_node = std::ranges::find_if(
      dag.nodes, [](const auto& node) {
        return node.semantic_variant_id == "SBLR_MODEL_SOURCE_V1" &&
               node.output_descriptor_ids.size() == 5 &&
  ...
  if (!multileg_model_wire &&
      ...
      !search_model_wire) {
    return refuse("SB_MODEL_OPERATION_SEMANTIC_REFUSED_V1", 0,
                  "search_model_operation_identity");
  }
  ...
  if (timestamp_model_wire != !dag.statement_timestamp.empty() ||
    ...
    return refuse(time_series_model_wire
                      ...
                      : (vector_model_wire || search_model_wire)
  ...
  for (const auto& expression : dag.expressions) {
    ...
    const bool functionless_time_series_operation =
        time_series_model_wire && function_call &&
        !expression.function_uuid.has_value() &&
    ...
    const bool functionless_search_operation =
        search_model_wire && function_call &&
        !expression.function_uuid.has_value() &&
  ...
  if (time_series_model_wire) {
    ...
    const bool is_downsample = time_downsample_count == 1;
    const bool is_bucket = time_bucket_count == 1;
    const auto expected_width = is_downsample ? 7u : (is_bucket ? 1u : 6u);
    if (range == dag.expressions.end() ||
        time_series_node->node_kind !=
            (is_downsample ? RelationalDagNodeKind::kAggregate
                           : RelationalDagNodeKind::kScan) ||
        !time_series_node->input_node_ids.empty() ||
        time_series_node->required_object_uuids.size() != 1 ||
        !canonical_uuid(time_series_node->required_object_uuids.front()) ||
        time_series_node->output_descriptor_ids.size() != expected_width ||
        range->expression_kind != RelationalExpressionKind::kFunctionCall ||
        range->function_uuid.has_value() || range->bound_name_uuid.has_value() ||
        range->literal_kind.has_value() ||
        range->literal_or_parameter_ref.has_value() ||
      ...
      return refuse("SB_MODEL_OPERATION_SEMANTIC_REFUSED_V1",
                    time_series_node->node_id, "time_series_exact_shape");
    }
    std::vector<const RelationalOutputRecord*> time_outputs;
    for (const auto& output : dag.outputs) {
      if (output.relation_node_id == time_series_node->node_id) {
        time_outputs.push_back(&output);
      }
    }
    std::ranges::sort(time_outputs, {}, [](const auto* output) {
      return output->ordinal;
    });
    if (time_outputs.size() != expected_width) {
      return refuse("SB_MODEL_TYPED_EXCHANGE_INVALID_V1",
                    time_series_node->node_id,
                    "time_series_output_descriptor_order");
    }
    const auto alias = expressions_by_id.at(range->child_expression_ids[0]);
    const auto start = expressions_by_id.at(range->child_expression_ids[1]);
    const auto end = expressions_by_id.at(range->child_expression_ids[2]);
    const auto start_descriptor =
        descriptors_by_id.at(start->result_descriptor_id);
    const auto end_descriptor = descriptors_by_id.at(end->result_descriptor_id);
    if (alias->expression_kind != RelationalExpressionKind::kIdentifier ||
        alias->bound_name_uuid !=
            time_series_node->required_object_uuids.front() ||
        start_descriptor->type_uuid != end_descriptor->type_uuid ||
        start_descriptor->nullability != RelationalNullability::kNonNull ||
        end_descriptor->nullability != RelationalNullability::kNonNull ||
        start->expression_kind != RelationalExpressionKind::kLiteral ||
        end->expression_kind != RelationalExpressionKind::kLiteral ||
        start->literal_kind != RelationalLiteralKind::kTemporal ||
        end->literal_kind != RelationalLiteralKind::kTemporal) {
      return refuse("SB_MODEL_TIME_SERIES_RANGE_INVALID_V1",
                    time_series_node->node_id,
                    "time_series_range_child_order_or_type");
    }
    std::vector<std::uint32_t> exact_bound_expression_ids;
    exact_bound_expression_ids.reserve(expected_width + 6);
    for (const auto* output : time_outputs) {
      exact_bound_expression_ids.push_back(output->expression_id);
    }
    if (is_bucket && !is_downsample) {
      if (time_series_node->bound_expression_ids.size() != 6 ||
          time_series_node->bound_expression_ids.front() !=
              exact_bound_expression_ids.front() ||
          time_series_node->bound_expression_ids.back() !=
              range->expression_id) {
        return refuse("SB_MODEL_OPERATION_SEMANTIC_REFUSED_V1",
                      time_series_node->node_id,
                      "time_series_operation_attachment");
      }
      const auto timestamp_id = bucket->child_expression_ids[1];
      std::unordered_set<std::uint32_t> hidden_source_ids;
      for (std::size_t ordinal = 1; ordinal < 5; ++ordinal) {
        const auto expression_id =
            time_series_node->bound_expression_ids[ordinal];
        const auto expression = expressions_by_id.at(expression_id);
        if (expression->expression_kind !=
                RelationalExpressionKind::kIdentifier ||
            expression_id == alias->expression_id ||
            expression_id == timestamp_id ||
            !hidden_source_ids.insert(expression_id).second) {
          return refuse("SB_MODEL_OPERATION_SEMANTIC_REFUSED_V1",
                        time_series_node->node_id,
                        "time_series_hidden_source_attachment");
        }
      }
    } else {
      if (is_bucket) {
        exact_bound_expression_ids.push_back(bucket->expression_id);
      }
      if (is_downsample) {
        exact_bound_expression_ids.push_back(downsample->expression_id);
      }
      exact_bound_expression_ids.push_back(range->expression_id);
      if (time_series_node->bound_expression_ids !=
          exact_bound_expression_ids) {
        return refuse("SB_MODEL_OPERATION_SEMANTIC_REFUSED_V1",
                      time_series_node->node_id,
                      "time_series_operation_attachment");
      }
    ...
    if (is_bucket) {
      const auto interval =
          expressions_by_id.at(bucket->child_expression_ids[0]);
      const auto timestamp =
          expressions_by_id.at(bucket->child_expression_ids[1]);
      const auto interval_descriptor =
          descriptors_by_id.at(interval->result_descriptor_id);
      const auto timestamp_descriptor =
          descriptors_by_id.at(timestamp->result_descriptor_id);
      if (bucket->expression_kind !=
              RelationalExpressionKind::kFunctionCall ||
          bucket->function_uuid.has_value() ||
          bucket->result_descriptor_id != timestamp->result_descriptor_id ||
          timestamp_descriptor->type_uuid != start_descriptor->type_uuid ||
          timestamp_descriptor->nullability !=
              RelationalNullability::kNonNull ||
          interval_descriptor->nullability !=
              RelationalNullability::kNonNull ||
          interval_descriptor->type_uuid == timestamp_descriptor->type_uuid ||
          interval->expression_kind != RelationalExpressionKind::kLiteral ||
          interval->literal_kind != RelationalLiteralKind::kTemporal ||
          !interval->literal_or_parameter_ref.has_value() ||
          !parse_fixed_interval(*interval->literal_or_parameter_ref,
                                &bucket_interval_ns) ||
          (!is_downsample &&
           time_series_node->output_descriptor_ids.front() !=
               bucket->result_descriptor_id)) {
        return refuse("SB_MODEL_TIME_SERIES_INTERVAL_INVALID_V1",
                      time_series_node->node_id,
                      "time_series_bucket_child_order_or_type");
      }
    }
    if (is_downsample) {
      const auto aggregate =
          expressions_by_id.at(downsample->child_expression_ids[0]);
      const auto interval =
          expressions_by_id.at(downsample->child_expression_ids[1]);
      const auto value =
          expressions_by_id.at(downsample->child_expression_ids[2]);
      const auto interval_descriptor =
          descriptors_by_id.at(interval->result_descriptor_id);
      const auto value_descriptor =
          descriptors_by_id.at(value->result_descriptor_id);
      const auto aggregate_text = aggregate->literal_or_parameter_ref;
      std::int64_t downsample_interval_ns = 0;
      const bool closed_aggregate =
          aggregate_text == "COUNT" || aggregate_text == "SUM" ||
          aggregate_text == "MIN" || aggregate_text == "MAX" ||
          aggregate_text == "AVG";
      if (interval->expression_kind != RelationalExpressionKind::kLiteral ||
          interval->literal_kind != RelationalLiteralKind::kTemporal ||
          !interval->literal_or_parameter_ref.has_value() ||
          !parse_fixed_interval(*interval->literal_or_parameter_ref,
                                &downsample_interval_ns) ||
          (is_bucket && downsample_interval_ns != bucket_interval_ns) ||
          interval_descriptor->nullability !=
              RelationalNullability::kNonNull ||
          interval_descriptor->type_uuid == value_descriptor->type_uuid) {
        return refuse("SB_MODEL_TIME_SERIES_INTERVAL_INVALID_V1",
                      time_series_node->node_id,
                      "time_series_intervals_invalid_or_not_equal");
      }
      if (downsample->expression_kind !=
              RelationalExpressionKind::kFunctionCall ||
          downsample->function_uuid.has_value() ||
          aggregate->expression_kind != RelationalExpressionKind::kLiteral ||
          aggregate->literal_kind != RelationalLiteralKind::kString ||
          !closed_aggregate ||
          value->expression_kind != RelationalExpressionKind::kIdentifier ||
          value_descriptor->nullability != RelationalNullability::kNonNull ||
          downsample->result_descriptor_id !=
              time_series_node->output_descriptor_ids[6]) {
        return refuse("SB_MODEL_TIME_SERIES_AGGREGATE_REFUSED_V1",
                      time_series_node->node_id,
                      "time_series_downsample_child_order_or_type");
      }
      for (std::size_t ordinal = 1;
           ordinal < time_series_node->output_descriptor_ids.size(); ++ordinal) {
        if (std::ranges::find(time_series_node->output_descriptor_ids.begin(),
                              time_series_node->output_descriptor_ids.begin() +
                                  static_cast<std::ptrdiff_t>(ordinal),
                              time_series_node->output_descriptor_ids[ordinal]) !=
            time_series_node->output_descriptor_ids.begin() +
                static_cast<std::ptrdiff_t>(ordinal)) {
          return refuse("SB_MODEL_TYPED_EXCHANGE_INVALID_V1",
                        time_series_node->node_id,
                        "time_series_output_descriptor_identity");
        }
      }
    }
    std::vector<std::uint32_t> time_series_expression_roots;
    const auto add_expression_root = [&](const std::uint32_t expression_id) {
      if (expression_id != 0) {
        time_series_expression_roots.push_back(expression_id);
      }
    };
    add_expression_root(range->expression_id);
    if (is_bucket) add_expression_root(bucket->expression_id);
    if (is_downsample) add_expression_root(downsample->expression_id);
    for (const auto& output : dag.outputs) {
      add_expression_root(output.expression_id);
    }
    for (const auto& node : dag.nodes) {
      for (const auto expression_id : node.bound_expression_ids) {
        add_expression_root(expression_id);
      }
    }
    ...
    std::unordered_set<std::uint32_t> reachable_time_series_expressions;
    while (!time_series_expression_roots.empty()) {
      const auto expression_id = time_series_expression_roots.back();
      time_series_expression_roots.pop_back();
      if (!reachable_time_series_expressions.insert(expression_id).second) {
        continue;
      }
      ...
      if (expression == expressions_by_id.end()) {
        return refuse("SBLR.PLAN_TREE.INVALID_HANDLE",
                      time_series_node->node_id,
                      "time_series_expression_root");
      }
      for (const auto child_id : expression->second->child_expression_ids) {
        time_series_expression_roots.push_back(child_id);
      }
    }
    if (reachable_time_series_expressions.size() != dag.expressions.size()) {
      return refuse("SBLR.PLAN_TREE.INVALID_HANDLE",
                    time_series_node->node_id,
                    "time_series_orphan_expression");
    }
    std::unordered_set<std::uint32_t> referenced_time_series_descriptors;
    for (const auto& expression : dag.expressions) {
      referenced_time_series_descriptors.insert(
          expression.result_descriptor_id);
    }
    for (const auto& node : dag.nodes) {
      referenced_time_series_descriptors.insert(
          node.output_descriptor_ids.begin(), node.output_descriptor_ids.end());
    }
    const auto expected_hidden_descriptor_count =
        (is_bucket || is_downsample) ? 1u : 0u;
    if (referenced_time_series_descriptors.size() +
            expected_hidden_descriptor_count !=
        dag.descriptors.size()) {
      return refuse("SBLR.PLAN_TREE.INVALID_HANDLE",
                    time_series_node->node_id,
                    "time_series_extra_descriptor");
    }
    bool exact_outputs = time_outputs.size() == expected_width;
    for (std::size_t ordinal = 0;
         exact_outputs && ordinal < time_outputs.size(); ++ordinal) {
      exact_outputs = time_outputs[ordinal]->ordinal == ordinal &&
                      time_outputs[ordinal]->visible &&
                      time_outputs[ordinal]->descriptor_id ==
                          time_series_node->output_descriptor_ids[ordinal];
    }
    if (!exact_outputs) {
      return refuse("SB_MODEL_TYPED_EXCHANGE_INVALID_V1",
    ...
    if (is_downsample) {
      ...
      for (std::size_t ordinal = 0; ordinal < kNames.size(); ++ordinal) {
        if (time_outputs[ordinal]->output_name_utf8 != kNames[ordinal]) {
          return refuse("SB_MODEL_TYPED_EXCHANGE_INVALID_V1",
                        time_series_node->node_id,
                        "time_series_downsample_output_name");
        }
      }
    } else if (is_bucket) {
      if (time_outputs.front()->output_name_utf8 != "bucket_start") {
        return refuse("SB_MODEL_TYPED_EXCHANGE_INVALID_V1",
                      time_series_node->node_id,
                      "time_series_bucket_output_name");
      }
    } else {
      static constexpr std::array<std::string_view, 6> kNames{
          "row_uuid", "series_uuid", "metric_uuid", "point_timestamp",
  ...
  for (const auto& property : dag.properties) {
    ...
    if (!planning_wire || !canonical_uuid(property.property_uuid) ||
        ...
        property.property_kind >
            RelationalPropertyKind::kSecurityVisibility ||
    ...
    const bool ordering =
        property.property_kind == RelationalPropertyKind::kOrdering ||
        property.property_kind == RelationalPropertyKind::kVectorOrdering ||
        property.property_kind ==
            RelationalPropertyKind::kTextScoreOrdering ||
        property.property_kind == RelationalPropertyKind::kTimeOrdering;
    const bool expression_set =
        property.property_kind == RelationalPropertyKind::kGrouping ||
        property.property_kind == RelationalPropertyKind::kPartitioning ||
        property.property_kind ==
            RelationalPropertyKind::kExpressionEquivalence ||
        property.property_kind == RelationalPropertyKind::kUniqueness;
    const bool window =
        property.property_kind == RelationalPropertyKind::kWindow;
    const bool distribution =
        property.property_kind == RelationalPropertyKind::kDistribution;
    const bool materialization =
        property.property_kind == RelationalPropertyKind::kMaterialization;
    const bool rewindability =
        property.property_kind == RelationalPropertyKind::kRewindability;
    const bool locality =
        property.property_kind == RelationalPropertyKind::kLocality;
    ...
    const bool specialized_enums_known =
        property.distribution_kind >=
            RelationalPropertyDistributionKind::kNone &&
        property.distribution_kind <=
            RelationalPropertyDistributionKind::kOwnerRouted &&
        property.materialization_kind >=
            RelationalPropertyMaterializationKind::kNone &&
        property.materialization_kind <=
            RelationalPropertyMaterializationKind::kSpillBacked &&
        property.rewindability_kind >=
            RelationalPropertyRewindabilityKind::kNone &&
        property.rewindability_kind <=
            RelationalPropertyRewindabilityKind::kMarkRestore &&
        property.locality_kind >= RelationalPropertyLocalityKind::kNone &&
        property.locality_kind <=
            RelationalPropertyLocalityKind::kRemoteAllowed;
    const bool shape_valid =
        specialized_enums_known &&
        ...
        (distribution && property.ordering_terms.empty() &&
         ...
         property.distribution_kind !=
             RelationalPropertyDistributionKind::kNone &&
         ((property.distribution_kind ==
               RelationalPropertyDistributionKind::kHashPartitioned ||
           property.distribution_kind ==
               RelationalPropertyDistributionKind::kRangePartitioned)
    ...
    if (!shape_valid) {
      return refuse("QOW-DIAG-LOGICAL-PROPERTY-SHAPE-V1",
                    property.origin_node_id, "property_shape");
    }
  }
  for (const auto& property : dag.properties) {
    for (const auto& dependency_uuid : property.dependency_property_uuids) {
...
PopulateCanonicalLogicalGraphFromAdmittedTypedRelationalDag(
  ...
  const auto time_series_node = std::ranges::find_if(
      dag.nodes, [&](const auto& node) {
        return time_downsample_count == 1
                   ? node.semantic_variant_id == "SBLR_MODEL_AGGREGATE_V1"
                   : node.semantic_variant_id == "SBLR_MODEL_SOURCE_V1";
      });
  ...
  for (const auto& node : dag.nodes) {
    ...
    if (exact_multileg_model &&
      ...
      logical_node.model_family_identity =
          plan::CanonicalLogicalModelFamilyIdentity::kKeyValue;
    } else if (exact_time_series_family &&
      ...
      logical_node.model_family_identity =
          plan::CanonicalLogicalModelFamilyIdentity::kVector;
    } else if (exact_search_family &&
               node.node_id == search_node->node_id) {
      logical_node.model_family_identity =
          plan::CanonicalLogicalModelFamilyIdentity::kSearch;
  ...
  for (const auto& property : dag.properties) {
    ...
    logical_property.distribution_kind =
        static_cast<plan::CanonicalLogicalDistributionKind>(
            property.distribution_kind);
    logical_property.materialization_kind =
        static_cast<plan::CanonicalLogicalMaterializationKind>(
            property.materialization_kind);
    logical_property.rewindability_kind =
        static_cast<plan::CanonicalLogicalRewindabilityKind>(
            property.rewindability_kind);
    logical_property.locality_kind =
        static_cast<plan::CanonicalLogicalLocalityKind>(
            property.locality_kind);
    logical_property.locality_uuid = property.locality_uuid;
    logical_property.security_visibility_context_uuid =
        property.security_visibility_context_uuid;
    logical_property.security_visibility_generation =
...
CanonicalRuntimeOptimizerStatisticsResult BuildRuntimeOptimizerStatistics(
  ...
  if (request.mga_authority.origin != executor::CanonicalMgaAuthorityOrigin::
    ...
    return refuse("QOW-DIAG-OPTIMIZER-ACTUALS-MGA-ORIGIN-V1", 0,
  ...
  if (!result_authority.ok) {
    return refuse(result_authority.diagnostic_code, 0,
...
CanonicalOptimizerSelectedExecutionResult ExecuteCanonicalOptimizerSelectedDag(
  ...
  if (!pre_publication_authority.ok) {
    return refuse(pre_publication_authority.diagnostic_code, 0,
                  pre_publication_authority.detail, false,
                  dispatch.data_access_observed);
  }
...
RefuseRetiredResultExpectationRoute(
  ...
  const auto lower_ascii = [](const std::string_view value) {
  ...
  const auto has_retired_suffix = [](const std::string_view value) {
    constexpr std::string_view suffix = "_assertion";
    return value.size() >= suffix.size() &&
           value.substr(value.size() - suffix.size()) == suffix;
  };

  CanonicalRetiredResultExpectationDisposition disposition;
  if (has_retired_suffix(lower_ascii(operation)) ||
      has_retired_suffix(lower_ascii(result_projection))) {
    disposition.applies = true;
  }

  constexpr std::string_view retired_fields[] = {
      "assertion_id:",
  ...
  for (const auto& option : option_envelopes) {
    const auto lowered = lower_ascii(option);
    for (const auto prefix : retired_fields) {
  ...
  if (disposition.applies) {
    ...
    disposition.detail =
        "assertion-only operation, result projection, and expected-result "
        "fields are retired; use the canonical ordinary result route";
  }
  return disposition;
}
...
bool TryParseSizeValue(const std::string& value, std::size_t* out) {
...
bool TryParseI64Value(const std::string& value, std::int64_t* out) {
  ...
  if (positive_sign && parsed < 0) return false;
...
std::int64_t ParseI64Value(const std::string& value) {
  std::int64_t parsed = 0;
  if (!TryParseI64Value(value, &parsed)) {
    throw std::invalid_argument("query_plan_int64_value_invalid");
  }
  return parsed;
}
...
plan::PhysicalAccessKind PlannedAccessKindForRequest(const EnginePlanOperationRequest& request,
  ...
  if (operation == "join" || operation == "inner_join" || operation == "equi_join" ||
      ...
      operation == "semi_join" || operation == "join_group_all_equality") {
  ...
  for (const auto& row : relation.rows) {
    ...
    for (const auto& field : row.fields) {
      ...
      if (field.second.is_null ||
        ...
        return exec::MakeBatch("query_plan_integer_relation_invalid", {});
      }
      tuple.values.push_back(value);
    }
    rows.push_back(std::move(tuple));
  }
  const auto column_count = relation.columns.empty()
                                ? (relation.rows.empty()
                                       ? 0
                                       : relation.rows.front().fields.size())
                                : relation.columns.size();
  return exec::MakeBatch(
      relation.descriptor_digest.empty() ? relation.relation_name
                                         : relation.descriptor_digest,
      std::move(rows), column_count);
}

bool WindowFunctionRequiresTypedResult(const std::string& function) {
  return function == "percent_rank" || function == "cume_dist" ||
         function == "nth_value" || function == "lag" ||
         function == "lead" || function == "first_value" ||
         function == "last_value";
}
...
EngineResultShape TypedWindowResultShape(const EngineQueryRelation& relation,
  ...
  std::size_t input_width = relation.columns.size();
  ...
  for (std::size_t column = 0; column < input_width; ++column) {
    EngineDescriptor descriptor;
    if (column < relation.columns.size()) {
      descriptor = relation.columns[column];
    } else {
    ...
    if (descriptor.canonical_type_name.empty()) {
      if (error_detail != nullptr) {
        *error_detail = "query_plan_window_result_descriptor_unresolved";
      }
      return {};
    }
  ...
  const bool value_function =
      function == "nth_value" || function == "lag" || function == "lead" ||
      function == "first_value" || function == "last_value";
  if (value_function) {
    if (function == "nth_value" && window_n == 0) {
  ...
  for (std::size_t row_index = 0; row_index < row_count; ++row_index) {
    ...
    if (function == "percent_rank") {
      ...
      if (function == "nth_value") {
        const auto nth_index = static_cast<std::size_t>(window_n - 1);
        if (nth_index <= row_index && nth_index < row_count) {
          requested_index = nth_index;
        }
      } else if (function == "lag") {
        if (row_index != 0) requested_index = row_index - 1;
      } else if (function == "lead") {
        if (row_index + 1 < row_count) requested_index = row_index + 1;
      } else if (function == "first_value") {
        if (row_count != 0) requested_index = 0;
      } else if (function == "last_value") {
        std::size_t peer_end = row_index + 1;
        while (peer_end < row_count &&
               rows[peer_end].order_value == rows[row_index].order_value) {
          ++peer_end;
        }
        requested_index = peer_end - 1;
      }
      if (!requested_index ||
        ...
        EngineTypedValue selected =
            NormalizeTypedValue(
                rows[*requested_index].row.fields[value_column].second);
...
EngineResultShape ArrayAggAggregateResultShape(const EngineQueryRelation& relation,
  ...
  std::map<std::int64_t, std::vector<OrderedValue>> groups;
  EngineDescriptor element_descriptor;
  bool element_descriptor_set = false;
  if (value_column < relation.columns.size()) {
    element_descriptor = relation.columns[value_column];
  ...
  if (!element_descriptor_set ||
      element_descriptor.canonical_type_name.empty()) {
    if (error_detail != nullptr) {
      *error_detail = "query_plan_array_agg_element_descriptor_unresolved";
    }
    return {};
  }
...
std::size_t ColumnIndexForRelation(const EngineQueryRelation& relation,
                                   ...
                                   std::size_t fallback) {
  if (field_name.empty()) return fallback;
  if (relation.rows.empty()) return std::numeric_limits<std::size_t>::max();
...
exec::Batch RelationToBatchByName(const EngineQueryRelation& relation,
  ...
  for (const auto& row : relation.rows) {
    ...
    for (const auto& field : field_order) {
      const auto value = RowFieldValueByName(row, field);
      if (!value) {
        if (error_detail != nullptr) {
          *error_detail =
              "set_operation_named_field_requires_non_null_int64_value";
        }
        return exec::MakeBatch("query_plan_set_relation_invalid", {});
      }
      tuple.values.push_back(*value);
    }
    rows.push_back(std::move(tuple));
  ...
  return exec::MakeBatch(
      relation.descriptor_digest.empty() ? relation.relation_name
                                         : relation.descriptor_digest,
      std::move(rows), field_order.size());
}

EngineResultShape BatchToResultShape(const exec::Batch& batch) {
  EngineResultShape shape;
  shape.result_kind = "query_rowset";
  std::size_t width = batch.column_count;
...
EnginePlanOperationResult ExecuteFastCrudCount(
  ...
  std::string result_column_name = OptionValue(request, "result_column_name:");
  if (result_column_name.empty()) result_column_name = "c0";
  result.result_shape = CountScalarResultShape(count, result_column_name);
...
EngineResultShape GenericGroupCountResultShape(const EngineQueryRelation& relation,
  ...
  EngineDescriptor key_descriptor;
  if (group_key_column < relation.columns.size()) {
    key_descriptor = relation.columns[group_key_column];
  } else if (!groups.empty()) {
    key_descriptor = InferredDescriptorForValue(groups.begin()->second.key_value);
  }
  if (key_descriptor.canonical_type_name.empty()) {
    if (error_detail != nullptr) {
      *error_detail = "query_plan_aggregate_group_key_descriptor_unresolved";
    }
    return {};
  }
  EngineResultShape shape;
...
EngineResultShape DerivedGroupConstantProjectionResultShape(
  ...
  for (const auto& row : relation.rows) {
    if (group_key_column >= row.fields.size()) {
      if (error_detail != nullptr) {
        *error_detail = "query_plan_derived_group_key_column_out_of_range";
      }
      return {};
    }
    auto typed = NormalizeTypedValue(row.fields[group_key_column].second);
...
EngineResultShape TypedInnerJoinResultShape(const EnginePlanOperationRequest& request,
  ...
  std::string normalized_algorithm(algorithm);
  if (!NormalizeJoinAlgorithm(&normalized_algorithm)) {
    if (error_detail != nullptr) {
      *error_detail = "query_plan_join_algorithm_unsupported";
    }
    return {};
  }
  if (normalized_algorithm == "nested_loop") {
...
EngineResultShape TypedLeftJoinResultShape(const EnginePlanOperationRequest& request,
                                           const EngineQueryRelation& left,
                                           const EngineQueryRelation& right,
                                           std::size_t left_key_column,
                                           std::size_t right_key_column,
  ...
  const auto emit_joined_row = [&](const EngineRowValue& left_row,
    ...
    for (std::size_t column = 0; column < right_width; ++column, ++output_column) {
      if (right_row == nullptr) {
        out.fields.push_back({"c" + std::to_string(output_column),
                              NullValue(shape.columns[output_column])});
      } else {
        out.fields.push_back({"c" + std::to_string(output_column),
                              JoinOutputValue(*right_row, column, shape.columns[output_column])});
      }
  ...
  for (const auto& left_row : left.rows) {
    ...
    if (error_detail != nullptr && !error_detail->empty()) return {};
    if (!key) {
      if (!emit_joined_row(left_row, nullptr)) return shape;
      continue;
    }
    const auto found = right_rows_by_key.find(*key);
    if (found == right_rows_by_key.end() || found->second.empty()) {
      if (!emit_joined_row(left_row, nullptr)) return shape;
      continue;
    }
    for (const auto* right_row : found->second) {
  ...
  if (!order_field.empty() || request.order_column != 0 ||
    ...
    const auto row_field_value = [](const EngineRowValue& row,
      ...
      for (const auto& [field_name, value] : row.fields) {
        if (field_name == key) return &value;
      }
      return nullptr;
    };
    std::stable_sort(shape.rows.begin(), shape.rows.end(), [&](const EngineRowValue& lhs,
                                                               const EngineRowValue& rhs) {
      const auto* lhs_value = row_field_value(lhs, order_key);
      const auto* rhs_value = row_field_value(rhs, order_key);
      if (lhs_value == nullptr || lhs_value->is_null) return false;
      if (rhs_value == nullptr || rhs_value->is_null) return true;
      std::int64_t lhs_int = 0;
      std::int64_t rhs_int = 0;
      if (TryParseI64Value(lhs_value->encoded_value, &lhs_int) &&
          TryParseI64Value(rhs_value->encoded_value, &rhs_int)) {
        return ascending ? lhs_int < rhs_int : lhs_int > rhs_int;
      }
      return ascending ? lhs_value->encoded_value < rhs_value->encoded_value
                       : lhs_value->encoded_value > rhs_value->encoded_value;
    });
  }
  if (offset != 0 || limit != 0) {
    ...
    for (std::size_t row = begin; row < end; ++row) {
      sliced.push_back(std::move(shape.rows[row]));
    }
    shape.rows = std::move(sliced);
  }
  return shape;
}
...
EngineResultShape TypedJoinGroupAllEqualityResultShape(
    const EngineQueryRelation& left,
    const EngineQueryRelation& right,
    std::size_t left_key_column,
    std::size_t right_key_column,
  ...
  shape.result_kind = "query_rowset";
  if (left_group_column == std::numeric_limits<std::size_t>::max()) {
    if (error_detail != nullptr) *error_detail = "query_plan_join_group_all_key_field_not_found";
    return shape;
  }
  shape.columns.push_back(DescriptorForRelationColumn(left, left_group_column));
  ...
  std::set<std::string> joined_subquery_keys;
  bool subquery_saw_null = false;
  std::map<std::string, EngineTypedValue> group_values_by_key;
  for (const auto& left_row : left.rows) {
    const auto join_key = JoinKeyForRow(left_row, left_key_column, error_detail);
    if (error_detail != nullptr && !error_detail->empty()) return {};
    if (left_group_column >= left_row.fields.size()) {
      if (error_detail != nullptr) {
        *error_detail = "query_plan_join_group_all_key_column_out_of_range";
      }
      return {};
    }
    EngineTypedValue group_value = NormalizeTypedValue(left_row.fields[left_group_column].second);
    const bool joined =
        join_key && right_rows_by_key.find(*join_key) != right_rows_by_key.end();
    if (group_value.is_null) {
      if (joined) subquery_saw_null = true;
      continue;
    }
    const auto group_key = EqualityKeyForTypedValue(group_value);
    if (group_key) group_values_by_key.emplace(*group_key, group_value);
    if (!joined) continue;
    if (group_key) joined_subquery_keys.insert(*group_key);
  }

  if (subquery_saw_null) {
    return shape;
  }
  for (const auto& [group_key, group_value] : group_values_by_key) {
    const bool all_predicate_true =
        joined_subquery_keys.empty() ||
        (joined_subquery_keys.size() == 1 &&
         *joined_subquery_keys.begin() == group_key);
    if (!all_predicate_true) continue;
    EngineRowValue out;
    out.fields.push_back({"c0", group_value});
    shape.rows.push_back(std::move(out));
  }
  return shape;
}
...
EngineResultShape PivotResultShape(const EnginePlanOperationRequest& request,
  ...
  if (aggregate_function.empty()) {
    if (error_detail != nullptr) {
      *error_detail = "query_plan_pivot_aggregate_function_required";
    }
    return {};
  }
...
bool RelationsHaveResolvedTypedValues(
    const std::vector<EngineQueryRelation>& relations,
    std::string* error_detail) {
  const auto refuse = [&](std::string detail) {
    if (error_detail != nullptr) *error_detail = std::move(detail);
    return false;
  };
  const auto descriptor_resolved = [](const EngineDescriptor& descriptor) {
    return !descriptor.descriptor_kind.empty() &&
           !descriptor.canonical_type_name.empty() &&
           !descriptor.encoded_descriptor.empty() &&
           QowCanonicalTypeFromDescriptorV1(descriptor) !=
  ...
  for (const auto& relation : relations) {
    for (const auto& descriptor : relation.columns) {
      if (!descriptor_resolved(descriptor)) {
        return refuse("query_relation_column_descriptor_unresolved");
      }
    }
    std::vector<std::string> field_names;
    if (!relation.rows.empty()) {
      field_names.reserve(relation.rows.front().fields.size());
      for (const auto& [name, value] : relation.rows.front().fields) {
        (void)value;
        if (name.empty() ||
            std::find(field_names.begin(), field_names.end(), name) !=
                field_names.end()) {
          return refuse("query_relation_field_identity_invalid");
        }
        field_names.push_back(name);
      }
    }
    const std::size_t expected_width = relation.columns.empty()
                                           ? field_names.size()
                                           : relation.columns.size();
    for (const auto& row : relation.rows) {
      if (row.fields.size() != expected_width) {
        return refuse("query_relation_row_width_mismatch");
      }
      for (std::size_t column = 0; column < row.fields.size(); ++column) {
        const auto& [name, value] = row.fields[column];
        if (column >= field_names.size() || name != field_names[column]) {
          return refuse("query_relation_field_order_mismatch");
        }
        if (!descriptor_resolved(value.descriptor)) {
          return refuse("query_relation_value_descriptor_unresolved");
        }
        if (!relation.columns.empty() &&
            !same_descriptor(value.descriptor, relation.columns[column])) {
          return refuse("query_relation_value_descriptor_mismatch");
        }
        if (value.isSqlNull()) {
          if (value.state != EngineValueState::sql_null || !value.is_null ||
              !value.encoded_value.empty() || !value.binary_value.empty()) {
            return refuse("query_relation_sql_null_state_invalid");
          }
        } else if (value.state != EngineValueState::value || value.is_null) {
          return refuse("query_relation_value_state_invalid");
        }
      }
    }
  }
...
plan::PhysicalAccessKind AccessKindForQueryOperation(const std::string& operation) {
  if (operation == "join" || operation == "inner_join" || operation == "equi_join" ||
      ...
      operation == "cube_count" || operation == "semi_join" ||
...
exec::Batch ExecuteQueryBatch(const EnginePlanOperationRequest& request,
  ...
  const auto require = [&](std::size_t index) -> exec::Batch {
    if (index >= relations.size()) {
      if (error_detail != nullptr) *error_detail = "query_relation_missing";
      return exec::MakeBatch("missing_relation", {});
    }
    return RelationToBatch(relations[index], error_detail);
  };

  exec::Batch batch = require(0);
  if (error_detail != nullptr && !error_detail->empty()) return batch;
  if (operation == "join" || operation == "inner_join" || operation == "equi_join" ||
      operation == "left_join" || operation == "left_outer_join" ||
      operation == "right_join" || operation == "right_outer_join" ||
      operation == "full_outer_join" || operation == "cross_join" ||
    ...
    if (error_detail != nullptr && !error_detail->empty()) return right;
    ...
    if (!NormalizeJoinAlgorithm(&algorithm)) {
      if (error_detail != nullptr) {
        *error_detail = "query_plan_join_algorithm_unsupported";
      }
      return exec::MakeBatch("query_plan_join_algorithm_invalid", {});
    }
    ...
    if (aggregate_function.empty()) {
      if (error_detail != nullptr) {
        *error_detail = "query_plan_aggregate_function_required";
      }
    ...
    if (!TryParseSizeValue(OptionValue(request, "recursive_iterations:"),
                           &max_iterations) ||
        max_iterations == 0) {
      if (error_detail != nullptr) {
        *error_detail = "query_plan_recursive_iteration_limit_required";
      }
      return exec::MakeBatch("query_plan_recursive_limit_invalid", {});
    }
    ...
    if (step_mode == "counter_add_until_lt") {
      ...
      for (const auto& anchor : batch.rows) {
        ...
        while (iteration < max_iterations && current < limit) {
          if (current > std::numeric_limits<std::int64_t>::max() - step) {
            if (error_detail != nullptr) {
              *error_detail = "query_plan_recursive_counter_overflow";
            }
            return exec::MakeBatch("query_plan_recursive_counter_invalid", {});
          }
          current += step;
        ...
        if (current < limit) {
          if (error_detail != nullptr) {
            *error_detail = "query_plan_recursive_iteration_limit_exceeded";
          }
          return exec::MakeBatch("query_plan_recursive_limit_exceeded", {});
        }
      }
    ...
    } else if (step_mode == "values_union_distinct" && relations.size() >= 2) {
      const exec::Batch recursive_step =
          RelationToBatch(relations[1], error_detail);
      ...
      if (batch.rows.size() != previous_count) {
        if (error_detail != nullptr) {
          *error_detail = "query_plan_recursive_iteration_limit_exceeded";
        }
        return exec::MakeBatch("query_plan_recursive_limit_exceeded", {});
      ...
      if (error_detail != nullptr) {
        *error_detail = "query_plan_recursive_step_descriptor_invalid";
      }
      return exec::MakeBatch("query_plan_recursive_step_invalid", {});
    }
    batch = exec::MaterializeCte(batch);
    ...
    batch = exec::MakeBatch("scalar_subquery", {{{value}}}, 1);
    ...
    const std::string method_text = OptionValue(request, "sample_method:");
    const std::string percent_text = OptionValue(request, "sample_percent:");
    if (method_text.empty() || percent_text.empty()) {
      if (error_detail != nullptr) {
        *error_detail = "query_plan_sample_method_and_percent_required";
      }
      return exec::MakeBatch("query_plan_sample_invalid", {});
    }
    const std::string method = LowerAscii(method_text);
    const double percent = ParseReal64Value(percent_text, -1.0);
    ...
    const std::string set_operation = operation == "set_operation"
        ...
        : operation;
    if (set_operation != "union" && set_operation != "union_distinct" &&
        set_operation != "union_all" && set_operation != "intersect" &&
        set_operation != "intersect_distinct" &&
        set_operation != "intersect_all" && set_operation != "except" &&
        set_operation != "except_distinct" &&
        set_operation != "except_all") {
      if (error_detail != nullptr) {
        *error_detail = "query_plan_set_operation_required_or_unsupported";
      }
      return exec::MakeBatch("query_plan_set_operation_invalid", {});
    }
      ...
      if (!project_field.empty() && relation_index < relations.size()) {
        ...
        if (!not_null_filter.empty()) {
          ...
          evidence->push_back({"query_set_relation_" + std::to_string(relation_index) + "_not_null_filter",
                               not_null_filter});
          return RelationToBatchByName(filtered, {project_field}, error_detail);
        }
        return RelationToBatchByName(relation_for_projection, {project_field},
                                     error_detail);
      }
      if (set_by_name && relation_index < relations.size()) {
        return RelationToBatchByName(relations[relation_index],
                                     FieldOrderForRelation(relations[0]),
                                     error_detail);
      }
    ...
    for (std::size_t relation_index = 1; relation_index < relations.size(); ++relation_index) {
      const auto right = projected_relation(relation_index);
      if (error_detail != nullptr && !error_detail->empty()) return right;
      apply_set_operation(right);
    }
    if (set_operation == "intersect_all") {
  ...
  return batch;
    ...
    return exec::MakeBatch("query_plan_execution_refused", {});
  }
}
...
bool RetiredResultExpectationRequested(const EnginePlanOperationRequest& request) {
  return RefuseRetiredResultExpectationRoute(
...
EnginePlanOperationResult EnginePlanOperationUncachedImpl(const EnginePlanOperationRequest& request) {
  ...
  if (execute) {
    ...
    if (operation == "join" || operation == "inner_join" || operation == "equi_join" ||
      ...
      if ((result_projection == "count" || result_projection == "count_result") &&
          !early_distinct_count_field.empty()) {
        if (relations->size() < 2) {
          return QueryFailure<EnginePlanOperationResult>(request.context,
        ...
        result.result_shape = CountScalarResultShape(joined_distinct_count);
        result.evidence.push_back({"query_join_kind", operation});
        result.evidence.push_back({"query_join_result_projection", "count_distinct"});
      ...
      if (!NormalizeJoinAlgorithm(&join_algorithm)) {
        return QueryFailure<EnginePlanOperationResult>(
            request.context, "query_plan_join_algorithm_unsupported");
      }
      result.evidence.push_back({"query_join_algorithm", join_algorithm});
      const bool join_group_all_equality = operation == "join_group_all_equality";
      if (join_group_all_equality) {
        if (result_projection != "rowset" && !result_projection.empty()) {
          return QueryFailure<EnginePlanOperationResult>(
              request.context,
              "query_plan_join_group_all_current_route_requires_rowset_projection");
        }
        ...
        result.result_shape =
            TypedJoinGroupAllEqualityResultShape((*relations)[0],
                                                 (*relations)[1],
                                                 left_key_column,
                                                 right_key_column,
                                                 group_column,
        ...
        result.evidence.push_back({"query_join_kind", "inner_group_all_equality"});
        result.evidence.push_back({"query_join_result_projection", "rowset"});
        result.evidence.push_back({"query_join_group_key_field", group_field});
        result.evidence.push_back({"query_join_all_predicate", "group_key_equals_all_join_projection"});
        result.plan_kind = operation;
        result.output_row_count = result.result_shape.rows.size();
        AddApiBehaviorEvidence(&result, "query_execution", operation);
        result.evidence.push_back({"query_relation_count", std::to_string(relations->size())});
      ...
      if (semi_join_operation) {
        if (result_projection != "count" &&
          ...
          return QueryFailure<EnginePlanOperationResult>(
              request.context,
              "query_plan_semi_join_requires_count_result");
        }
        ...
        result.result_shape = CountScalarResultShape(matched_left_row_count);
      ...
      if (left_join_operation) {
        if (result_projection != "rowset" &&
          ...
          return QueryFailure<EnginePlanOperationResult>(
              request.context,
              "query_plan_left_join_requires_rowset_or_count_result");
        }
        if (result_projection == "rowset") {
      ...
      if (right_join_operation) {
        if (result_projection != "count" && result_projection != "count_result") {
          return QueryFailure<EnginePlanOperationResult>(
              request.context,
              "query_plan_right_join_requires_count_result");
        }
      ...
      if (operation == "full_outer_join") {
        if (result_projection != "count" && result_projection != "count_result") {
          return QueryFailure<EnginePlanOperationResult>(
              request.context,
              "query_plan_full_outer_join_requires_count_result");
        }
        const bool unmatched_only =
            !OptionValue(request, "left_null_filter_field:").empty() ||
            !OptionValue(request, "right_null_filter_field:").empty();
      ...
      if (operation == "lateral_join") {
        if (result_projection != "count" && result_projection != "count_result") {
          return QueryFailure<EnginePlanOperationResult>(
              request.context,
              "query_plan_lateral_join_requires_count_result");
        }
        ...
        result.result_shape = CountScalarResultShape(lateral_row_count);
      ...
      if (operation == "grouping_sets_count" || operation == "rollup_count" ||
        ...
        if (result_projection != "count" && result_projection != "count_result") {
          return QueryFailure<EnginePlanOperationResult>(
              request.context,
              "query_plan_grouping_route_requires_count_result");
        }
        ...
        result.result_shape = CountScalarResultShape(grouping_row_count);
        result.evidence.push_back({"query_join_kind", operation});
        result.evidence.push_back({"query_join_result_projection", "count"});
        result.plan_kind = operation;
        result.output_row_count = result.result_shape.rows.size();
    ...
    std::string aggregate_function = request.aggregate_function;
    if (aggregate_function.empty()) aggregate_function = OptionValue(request, "aggregate_function:");
    const bool aggregate_operation =
        operation == "aggregate" || operation == "group" ||
        operation == "group_by";
    if (aggregate_operation && aggregate_function.empty()) {
      return QueryFailure<EnginePlanOperationResult>(
          request.context, "query_plan_aggregate_function_required");
    }
    ...
    if (typed_aggregate_route) {
      ...
      if (result_projection == "derived_constant") {
        result.result_shape = DerivedGroupConstantProjectionResultShape(
            request, relations->front(), group_key_column, &error_detail);
        if (!error_detail.empty()) {
          return QueryFailure<EnginePlanOperationResult>(request.context, error_detail);
        }
        result.plan_kind = operation;
        result.output_row_count = result.result_shape.rows.size();
        AddApiBehaviorEvidence(&result, "query_execution", operation);
        result.evidence.push_back({"query_aggregate", "derived_group_constant_projection"});
        result.evidence.push_back({"query_result_projection", "derived_constant"});
        result.evidence.push_back({"query_relation_count", std::to_string(relations->size())});
        result.evidence.push_back({"query_output_row_count",
                                   std::to_string(result.result_shape.rows.size())});
        return result;
    ...
    if (operation == "lag_window") window_function = "lag";
    if (operation == "lead_window") window_function = "lead";
    if (operation == "first_value_window") window_function = "first_value";
    if (operation == "last_value_window") window_function = "last_value";
    ...
    const bool window_operation =
        operation == "window" || operation == "lag_window" ||
        operation == "lead_window" || operation == "first_value_window" ||
        operation == "last_value_window" ||
        operation == "percent_rank_window" ||
        operation == "cume_dist_window" || operation == "nth_value_window";
    const bool typed_window_route =
        window_operation && WindowFunctionRequiresTypedResult(window_function);
    ...
    if (operation == "materialized_cte" &&
        (result_projection == "count" || result_projection == "count_result")) {
      result.result_shape =
          CountScalarResultShape(static_cast<std::uint64_t>(batch.rows.size()));
      result.plan_kind = operation;
      result.output_row_count = result.result_shape.rows.size();
      AddApiBehaviorEvidence(&result, "query_execution", operation);
      result.evidence.push_back({"query_cte_result_projection", "count"});
    ...
    if (IsSetOperationName(operation) &&
      ...
      result.output_row_count = result.result_shape.rows.size();
      AddApiBehaviorEvidence(&result, "query_execution", operation);
      result.evidence.push_back({"query_set_result_projection", "count"});
      result.evidence.push_back({"query_relation_count", std::to_string(relations->size())});
      result.evidence.push_back({"query_output_row_count", std::to_string(result.result_shape.rows.size())});
      return result;
    }
