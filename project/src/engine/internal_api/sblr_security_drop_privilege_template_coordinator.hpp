...
SblrSecurityDropPrivilegeTemplateCoordinationResult CompileSblrSecurityDropPrivilegeTemplateDescriptor(const EngineRequestContext&,const EngineUuid&,std::uint64_t,std::uint32_t,std::uint64_t);
