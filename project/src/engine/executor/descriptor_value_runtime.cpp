...
// engine-owned MGA statement context and the immutable descriptor handles;
// this executor surface never derives either from parser text or values.
CanonicalDescriptorProjectionResult ExecuteCanonicalDescriptorProjection(
  ...
  if (!dag_validation.accepted) {
    ...
    return refuse(ErrorDiagnostic(issue.diagnostic_id, issue.field_id));
  }
  if (request.selected_physical_node_id == 0 ||
      request.selected_physical_node_id !=
          request.physical_dag.root_physical_node_id) {
    return refuse(ErrorDiagnostic("SBLR.PLAN_TREE.INVALID_HANDLE",
