...
SblrQueryPreflightResult PreflightSblrQueryOperation(
  ...
  const bool exact_window = (request.envelope.operation_id=="engine.op.window"&&request.envelope.opcode=="SBLR_WINDOW"&&request.envelope.opcode_code==1285) || exact_diagnostic_refusal || exact_return_result_set || exact_kv_structured_read || exact_kv_structured_mutate || exact_kv_structured_scan || exact_kv_structured_stream_read || exact_kv_structured_stream_append || exact_kv_structured_timeseries || exact_system_config_set || exact_ddl_create_domain || exact_ddl_create_schema || exact_ddl_create_table || exact_ddl_drop_table || exact_ddl_create_index || exact_ddl_drop_index || exact_ddl_alter_domain || exact_ddl_create_view || exact_ddl_refresh_materialized_view || exact_ddl_drop_materialized_view || exact_ddl_create_type || exact_ddl_alter_type || exact_ddl_drop_type || exact_ddl_alter_view || exact_ddl_drop_view || exact_ddl_create_trigger || exact_ddl_alter_trigger || exact_ddl_create_procedure || exact_ddl_alter_procedure || exact_ddl_drop_procedure || exact_ddl_create_function || exact_ddl_alter_function || exact_ddl_drop_function || exact_ddl_create_package || exact_ddl_alter_package || exact_ddl_alter_sequence || exact_ddl_drop_package || exact_ddl_create_or_replace_srs || exact_ddl_drop_srs || exact_ddl_create_rewrite_rule;
