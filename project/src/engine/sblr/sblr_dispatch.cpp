...
SblrQueryPreflightResult PreflightSblrQueryOperation(
  ...
  if (exact_security_alter_role) { result.ok=true; result.materialized_envelope=request.envelope; return result; }
...
SblrDispatchResult DispatchSblrOperation(SblrDispatchRequest request) {
  ...
  else if (op == "engine.op.sec_drop_policy" || op == "engine.op.sec_alter_role") { result.api_result.ok = true; result.api_result.operation_id = op; }
