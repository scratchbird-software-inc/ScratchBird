...
SblrEventNotificationCodecResult Validate(const SblrEventNotificationRecord& record) {
 ...
 for(std::size_t i=0;i<record.fields.size();++i) if(record.fields[i].tag!=*(spec->tags.begin()+i)||record.fields[i].value.size()>8192) return Fail("field_tag_or_limit");
