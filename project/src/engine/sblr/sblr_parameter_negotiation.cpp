...
std::array<std::uint8_t,32> ComputeSblrParameterMappingSha256V1(const std::vector<SblrParameterMappingV1>&v){if(v.empty())return{};Bytes b;P32(&b,v.size());std::uint64_t prior=0;std::uint32_t slot=0;for(const auto&x:v){if(!x.occurrence_id||x.occurrence_id<=prior||x.slot_ordinal!=slot++||!Nz(x.slot_uuid)||!Nz(x.datatype_descriptor_uuid)||!x.datatype_descriptor_generation||x.direction<1||x.direction>3||x.nullable>1)return{};P64(&b,x.occurrence_id);P32(&b,x.slot_ordinal);b.insert(b.end(),x.slot_uuid.begin(),x.slot_uuid.end());b.insert(b.end(),x.datatype_descriptor_uuid.begin(),x.datatype_descriptor_uuid.end());P64(&b,x.datatype_descriptor_generation);b.push_back(x.direction);b.push_back(x.nullable);P16(&b,0);prior=x.occurrence_id;}return Hash("ScratchBird.SblrParameterDescriptorMappings.V1",b);}
...
Bytes EncodeSblrParameterNegotiateResultV1(const SblrParameterNegotiateResultV1&v){if(!Nz(v.preliminary_receipt_uuid)||!Nz(v.parameter_set_descriptor_uuid)||!v.descriptor_generation||!Nz(v.execution_uuid)||v.mappings.size()>4096||ComputeSblrParameterMappingSha256V1(v.mappings)!=v.mapping_sha256)return{};Bytes o;o.insert(o.end(),{'S','B','P','G'});P16(&o,1);P16(&o,112);P32(&o,112+56*v.mappings.size());P32(&o,0);o.insert(o.end(),v.preliminary_receipt_uuid.begin(),v.preliminary_receipt_uuid.end());P32(&o,v.mappings.size());P32(&o,56);o.insert(o.end(),v.parameter_set_descriptor_uuid.begin(),v.parameter_set_descriptor_uuid.end());P64(&o,v.descriptor_generation);o.insert(o.end(),v.execution_uuid.begin(),v.execution_uuid.end());o.insert(o.end(),v.mapping_sha256.begin(),v.mapping_sha256.end());for(const auto&x:v.mappings){P64(&o,x.occurrence_id);P32(&o,x.slot_ordinal);o.insert(o.end(),x.slot_uuid.begin(),x.slot_uuid.end());o.insert(o.end(),x.datatype_descriptor_uuid.begin(),x.datatype_descriptor_uuid.end());P64(&o,x.datatype_descriptor_generation);o.push_back(x.direction);o.push_back(x.nullable);P16(&o,0);}return o;}
bool DecodeSblrParameterNegotiateResultV1(const std::uint8_t*b,std::size_t n,SblrParameterNegotiateResultV1*out,std::string*d){if(!b||!out||n<112||n>229488||!std::equal(b,b+4,reinterpret_cast<const std::uint8_t*>("SBPG"))||U16(b+4)!=1||U16(b+6)!=112||U32(b+8)!=n||U32(b+12)||U32(b+36)!=56){if(d)*d="SBPG header invalid";return false;}auto c=U32(b+32);if(c>4096||112+std::size_t(c)*56!=n){if(d)*d="SBPG extent invalid";return false;}SblrParameterNegotiateResultV1 v;std::copy_n(b+16,16,v.preliminary_receipt_uuid.begin());std::copy_n(b+40,16,v.parameter_set_descriptor_uuid.begin());v.descriptor_generation=U64(b+56);std::copy_n(b+64,16,v.execution_uuid.begin());std::copy_n(b+80,32,v.mapping_sha256.begin());for(std::uint32_t i=0;i<c;++i){auto o=112+i*56;SblrParameterMappingV1 x;x.occurrence_id=U64(b+o);x.slot_ordinal=U32(b+o+8);std::copy_n(b+o+12,16,x.slot_uuid.begin());std::copy_n(b+o+28,16,x.datatype_descriptor_uuid.begin());x.datatype_descriptor_generation=U64(b+o+44);x.direction=b[o+52];x.nullable=b[o+53];if(U16(b+o+54)){if(d)*d="SBPG reserve invalid";return false;}v.mappings.push_back(x);}if(EncodeSblrParameterNegotiateResultV1(v)!=Bytes(b,b+n)){if(d)*d="SBPG reencode differs";return false;}*out=std::move(v);return true;}
...
SblrParameterWirePrevalidationV1 PrevalidateSblrParameterNegotiateResultV1(
  ...
  if(!b||n<112||!std::equal(b,b+4,reinterpret_cast<const std::uint8_t*>("SBPG"))||
     U16(b+4)!=1||U16(b+6)!=112||U32(b+8)!=n||U32(b+12)!=0||U32(b+36)!=56||
  ...
  if(count==0||count>(std::numeric_limits<std::size_t>::max()-112)/56||
     112+std::size_t(count)*56!=n)
    return SblrParameterWirePrevalidationV1::operand_invalid;
  std::uint64_t prior=0;
  for(std::uint32_t i=0;i<count;++i){const auto o=112+std::size_t(i)*56;
    const auto occurrence=U64(b+o);const auto direction=b[o+52];
    if(!occurrence||occurrence<=prior||U32(b+o+8)!=i||
       !std::any_of(b+o+12,b+o+28,[](auto x){return x!=0;})||
       !std::any_of(b+o+28,b+o+44,[](auto x){return x!=0;})||U64(b+o+44)==0||
       direction!=1||b[o+53]>1||U16(b+o+54)!=0)
      return SblrParameterWirePrevalidationV1::operand_invalid;
    prior=occurrence;}
  return count>4096||n>229488?SblrParameterWirePrevalidationV1::resource_exceeded:
      SblrParameterWirePrevalidationV1::ok;
}
