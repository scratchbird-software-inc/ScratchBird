...
SessionOperationResult HandleQueryNarrowBindingIssue(
  ...
  const auto request_key = UuidBytesToText(request.header.request_uuid);
...
SessionOperationResult HandleCoordinateDdlRenameObject(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=667;result.response_schema_id=sbps::kSchemaCoordinateDdlRenameObjectResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_rename_object_refused","DDL rename object coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlRenameObjectRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlRenameObjectRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND_INVALID",d);auto s=registry->sessions_by_uuid.find(UuidBytesToText(request.header.session_uuid));if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_rename_object_binder");auto q=scratchbird::engine::internal_api::CompileSblrDdlRenameObjectDescriptor(c,ru,v.occurrence,v.object_vector_occurrence,1);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlRenameObjectDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("DDL.RENAME_OBJECT_FAILED","RNOD_encode_failed");result.accepted=true;return result;}
