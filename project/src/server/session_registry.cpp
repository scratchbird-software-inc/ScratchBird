...
#include "sblr_ddl_create_publication_runtime.hpp"
#include "sblr_ddl_alter_publication_runtime.hpp"
#include "sblr_ddl_drop_publication_runtime.hpp"
#include "sblr_ddl_create_subscription_runtime.hpp"
#include "sblr_ddl_alter_subscription_runtime.hpp"
#include "sblr_ddl_drop_subscription_runtime.hpp"
#include "sblr_ddl_create_operator_runtime.hpp"
#include "sblr_ddl_drop_operator_runtime.hpp"
#include "sblr_ddl_create_operator_class_runtime.hpp"
#include "sblr_ddl_drop_operator_class_runtime.hpp"
#include "sblr_ddl_create_operator_family_runtime.hpp"
#include "sblr_ddl_alter_operator_family_runtime.hpp"
#include "sblr_ddl_drop_cast_runtime.hpp"
#include "sblr_ddl_create_extension_runtime.hpp"
#include "sblr_ddl_create_extension_coordinator.hpp"
#include "sblr_ddl_alter_extension_runtime.hpp"
#include "sblr_ddl_alter_extension_coordinator.hpp"
#include "sblr_ddl_drop_extension_runtime.hpp"
#include "sblr_ddl_drop_extension_coordinator.hpp"
#include "sblr_cluster_create_placement_policy_runtime.hpp"
#include "sblr_cluster_create_placement_policy_coordinator.hpp"
...
#include "sblr_ddl_drop_cast_coordinator.hpp"
...
#include "sblr_ddl_create_subscription_coordinator.hpp"
...
#include "sblr_ddl_create_operator_class_coordinator.hpp"
#include "sblr_ddl_drop_operator_class_coordinator.hpp"
#include "sblr_ddl_create_operator_family_coordinator.hpp"
#include "sblr_ddl_alter_operator_family_coordinator.hpp"
...
engine_api::DurableAuthorizationState DurableAuthorizationStateFromLifecycle(
  ...
  if (sysarch_identity.ok) {
    state.engine_owned_sysarch_role_uuid = sysarch_identity.role_uuid;
  }
  return state;
}
...
bool ApplyDurableAuthorizationProjectionToSession(ServerSessionRecord* session,
  ...
  for (const auto& grant : materialized.context.grants) {
    if (grant.right.empty() || !grant.target_uuid.is_nil()) {
      continue;
    }
...
engine_api::EngineMaterializedAuthorizationContext MaterializeSessionAuthorizationContext(
  ...
  if (authorization.principal_uuid.is_nil() ||
...
bool ApplyBeginTransactionResultToSession(
  ...
  if (session == nullptr ||
      !IsCompleteEngineTransactionIdentity(
          result.local_transaction_id, result.transaction_uuid) ||
      session->local_transaction_id != 0 ||
      session->default_local_transaction_id != 0 ||
      !scratchbird::core::uuid::IsNilUuid(session->transaction_uuid) ||
...
bool RollbackUnpublishedInitialTransaction(
  ...
  return applied &&
         ...
         result.transaction_uuid == session.transaction_uuid;
}
...
std::vector<std::uint8_t> EncodeAttachResultPayload(const std::string& outcome,
  ...
  PutUuid(&out, session == nullptr ? std::array<std::uint8_t, 16>{} : session->database_uuid.bytes);
  ...
  PutUuid(&out, session == nullptr ? std::array<std::uint8_t, 16>{} : session->transaction_uuid.bytes);
...
scratchbird::core::platform::Uuid AuthContextKey(const std::array<std::uint8_t, 16>& uuid) {
  return scratchbird::core::platform::Uuid{uuid};
}
...
void RecordFinality(ServerSessionRegistry* registry,
  ...
  registry->finality_by_request_uuid[scratchbird::core::platform::Uuid{finality.request_uuid}] = std::move(finality);
}
...
std::map<scratchbird::core::platform::Uuid, ServerSessionRecord>::iterator FindMutableSessionByBindingTarget(
  ...
  if (!IsZeroUuidBytes(catalog_session_id)) {
    return registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{catalog_session_id});
  }
...
std::map<scratchbird::core::platform::Uuid, ServerSessionRecord>::const_iterator FindSessionByBindingTarget(
...
TakeoverPhysicalChannelProjection ProjectTakeoverPhysicalChannel(
  ...
  const auto connection_key = scratchbird::core::platform::Uuid{request.attachment_id};
  const auto physical =
...
ServerRequestRecord RegisterServerRequestLifecycle(ServerSessionRegistry* registry,
  ...
  if (registry != nullptr) {
    registry->requests_by_uuid[scratchbird::core::platform::Uuid{record.request_uuid}] = record;
...
void UpdateServerRequestLifecycleOperation(
    ...
    std::string operation_id) {
  if (registry == nullptr) return;
  auto found = registry->requests_by_uuid.find(scratchbird::core::platform::Uuid{request_uuid});
...
ServerPreparedStatementCloseSummary CloseServerPreparedStatement(
  ...
  const auto prepared_key = scratchbird::core::platform::Uuid{prepared_statement_uuid};
  ...
  if (prepared.session_object_handle_id != 0 &&
    ...
    if (!shared_by_live_prepared) {
      ...
      if (handle_it != registry->object_handles_by_key.end() &&
        ...
        if (handle_it->second.generation != UINT64_MAX)
          ++handle_it->second.generation;
...
SessionOperationResult HandleAcquireStatementContext(
  ...
  const auto session_it = registry->sessions_by_uuid.find(
      scratchbird::core::platform::Uuid{request.header.session_uuid});
  ...
  // the database file when the legacy CRUD journal was absent.
  {
    ...
    catalog_context.transaction_uuid = transaction.transaction_uuid;
  ...
  engine_context.catalog_epoch_uuid = {};
  ...
  engine_context.transaction_uuid = transaction.transaction_uuid;
  ...
  std::vector<wire::DiagnosticIdentityProjectionBytesV1> diagnostic_projection;
  if (native_projection_v11) {
    try {
      const auto& id = view.diagnostic_registry_snapshot_uuid;
      if ((id[6] & 0xf0) != 0x70 || (id[8] & 0xc0) != 0x80 ||
          view.diagnostic_registry_generation == 0 ||
          !wire::ValidateDiagnosticIdentityCohortV1(view.diagnostic_identity_rows)) {
        (void)engine_bridge::ReleaseStatementContextReceipt(receipt);
        return refuse("PARSER_SERVER_IPC.STATEMENT_CONTEXT_ENGINE_REFUSED",
                      "engine_diagnostic_projection_invalid");
      }
      diagnostic_projection.reserve(view.diagnostic_identity_rows.size());
      for (auto row : view.diagnostic_identity_rows) {
        wire::DiagnosticIdentityProjectionBytesV1 bytes{};
        if (!wire::EncodeDiagnosticIdentityProjectionV1(&row, &bytes)) {
          (void)engine_bridge::ReleaseStatementContextReceipt(receipt);
          return refuse("PARSER_SERVER_IPC.STATEMENT_CONTEXT_ENGINE_REFUSED",
                        "engine_diagnostic_projection_encoding_failed");
        }
        diagnostic_projection.push_back(bytes);
      }
    } catch (const std::bad_alloc&) {
      (void)engine_bridge::ReleaseStatementContextReceipt(receipt);
      return refuse("RESOURCE.BUDGET_EXCEEDED", "diagnostic_projection_allocation_failed");
    }
  }
  ...
  if (native_projection_v11) {
    ...
    for (const auto& row : diagnostic_projection) {
    ...
    PutU64(&result.payload,view.kv_structured_mutate_executor_availability_generation);
    PutU64(&result.payload,view.kv_structured_scan_executor_availability_generation);
    PutU64(&result.payload,view.kv_structured_stream_read_executor_availability_generation);
    PutU64(&result.payload,view.kv_structured_stream_append_executor_availability_generation);
    PutU64(&result.payload,view.kv_structured_timeseries_executor_availability_generation);
    PutU64(&result.payload,view.system_config_set_executor_availability_generation);
    PutU64(&result.payload,view.ddl_create_domain_executor_availability_generation);
    PutU64(&result.payload,view.ddl_create_schema_executor_availability_generation);
    PutU64(&result.payload,view.ddl_create_table_executor_availability_generation);
    PutU64(&result.payload,view.ddl_create_index_executor_availability_generation);
    PutU64(&result.payload,view.ddl_drop_index_executor_availability_generation);
    if (view.active_transaction_handle_bytes.size() != 152) {
      return refuse("MGA.TRANSACTION_INVALID",
                    "active_transaction_handle_projection_invalid");
    }
    if (view.maximum_mga_relation_decoded_bytes_per_pass <
            engine_api::kMinimumMgaRelationDecodedBytesPerPass ||
        view.maximum_mga_relation_decoded_bytes_per_pass >
            engine_api::kMaximumMgaRelationDecodedBytesPerPass) {
      return refuse("RESOURCE.BUDGET_EXCEEDED",
                    "mga_relation_decoded_bytes_per_pass_projection_invalid");
    }
    result.payload.insert(result.payload.end(),
                          view.active_transaction_handle_bytes.begin(),
...
SessionOperationResult HandleQueryNarrowBindingIssue(
    ...
    const sbps::Frame& request) {
  (void)engine_state;
  namespace narrow_wire = scratchbird::wire;

  SessionOperationResult result;
  result.response_message_type = static_cast<std::uint16_t>(
      sbps::MessageType::kQueryNarrowBindingIssueResult);
  result.response_schema_id = sbps::kSchemaQueryNarrowBindingIssueResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;

  const auto refuse = [&](std::string code, std::string detail) {
    result.accepted = false;
    // A failed issue never retains the success message or any prefix of the
    // binding carrier.  SessionOperationFrame serializes this diagnostic as
    // the one correlated message-vector payload.
    ...
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code),
        "parser_server_ipc.query_narrow_binding_issue_refused",
        "Narrow-query binding issuance was refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };

  const auto structural_refusal = [&](const auto& error) {
    if (error.field.find("maximum_mga_relation_decoded_bytes_per_pass") !=
        std::string::npos) {
      return refuse("RESOURCE.BUDGET_EXCEEDED",
                    error.field + ":" + error.detail);
    }
    if (error.code == narrow_wire::NarrowQueryBindingDemandErrorCode::
                          resource_limit_exceeded) {
      return refuse("PARSER_SERVER_IPC.RESOURCE_LIMIT_EXCEEDED",
                    error.field + ":" + error.detail);
    }
    return refuse("PARSER_SERVER_IPC.FRAME_PAYLOAD_INVALID",
                  error.field.empty() ? error.detail
                                      : error.field + ":" + error.detail);
  };

  constexpr std::uint32_t kForbiddenRequestFlags =
      sbps::kFlagResponse | sbps::kFlagError | sbps::kFlagPayloadChunk;
  if (registry == nullptr ||
      request.header.protocol_major != sbps::kProtocolMajor ||
      request.header.protocol_minor != sbps::kProtocolMinor ||
      ...
      (request.header.flags & ~sbps::kAllowedFrameFlags) != 0 ||
      (request.header.flags & kForbiddenRequestFlags) != 0 ||
      request.header.payload_len != request.payload.size()) {
    return refuse("PARSER_SERVER_IPC.FRAME_PAYLOAD_INVALID",
                  "query_narrow_binding_message_schema_or_frame_invalid");
  }
  if (request.header.stream_id == 0 || request.header.sequence_number == 0 ||
      IsZeroUuidBytes(request.header.request_uuid) ||
      IsZeroUuidBytes(request.header.connection_uuid) ||
      IsZeroUuidBytes(request.header.session_uuid)) {
    return refuse("PARSER_SERVER_IPC.ROUTE_ASSOCIATION_MISMATCH",
                  "query_narrow_binding_correlation_invalid");
  }

  const auto session_key = scratchbird::core::platform::Uuid{request.header.session_uuid};
  const auto session = registry->sessions_by_uuid.find(session_key);
  if (session == registry->sessions_by_uuid.end() ||
      session->second.channel_state != ServerChannelState::kReady ||
      !session->second.session_binding_present ||
      session->second.connection_uuid != request.header.connection_uuid ||
      IsZeroUuidBytes(session->second.server_channel_uuid) ||
      IsZeroUuidBytes(session->second.auth_context_uuid) ||
      IsZeroUuidBytes(session->second.effective_user_uuid)) {
    return refuse("SECURITY.ACCESS_DENIED",
                  "query_narrow_binding_session_hidden");
  }
  const auto physical_owner =
      registry->physical_channel_by_connection_uuid.find(
          scratchbird::core::platform::Uuid{request.header.connection_uuid});
  if (physical_owner ==
          registry->physical_channel_by_connection_uuid.end() ||
      physical_owner->second != session->second.server_channel_uuid) {
    return refuse("PARSER_SERVER_IPC.ROUTE_ASSOCIATION_MISMATCH",
                  "query_narrow_binding_physical_channel_mismatch");
  }
  const auto request_key = scratchbird::core::platform::Uuid{request.header.request_uuid};
  if (registry->requests_by_uuid.contains(request_key)) {
    return refuse("PARSER_SERVER_IPC.SEQUENCE_INVALID",
                  "query_narrow_binding_request_replayed");
  }

  if (request.payload.size() <
      narrow_wire::kNarrowQueryBindingDemandHeaderBytes + sizeof(std::uint64_t)) {
    const auto code =
        request.payload.size() >=
                narrow_wire::kNarrowQueryBindingDemandHeaderBytes
            ? "RESOURCE.BUDGET_EXCEEDED"
            : "PARSER_SERVER_IPC.FRAME_PAYLOAD_INVALID";
    return refuse(code, "query_narrow_binding_demand_truncated");
  }
  if (request.payload.size() >
      narrow_wire::kNarrowQueryBindingDemandMaximumCarrierBytes) {
    return refuse("PARSER_SERVER_IPC.RESOURCE_LIMIT_EXCEEDED",
                  "query_narrow_binding_demand_too_large");
  }

  // Decode and byte-identically re-encode the complete carrier before any
  // live receipt lookup.  Candidate values only let the canonical decoder
  // perform its structural pass; they grant no authority.
  narrow_wire::NarrowQueryUuid candidate_receipt{};
  std::copy_n(request.payload.begin() + 48, candidate_receipt.size(),
              candidate_receipt.begin());
  auto structural_receipt = candidate_receipt;
  if (std::all_of(structural_receipt.begin(), structural_receipt.end(),
                  [](std::uint8_t byte) { return byte == 0; })) {
    structural_receipt.back() = 1;
  }
  const auto candidate_scan_bytes = GetU64(request.payload, 72);
  const auto candidate_scan_in_range =
      candidate_scan_bytes >=
          narrow_wire::kNarrowQueryMinimumMgaRelationDecodedBytesPerPass &&
      candidate_scan_bytes <=
          narrow_wire::kNarrowQueryMaximumMgaRelationDecodedBytesPerPass;
  narrow_wire::NarrowQueryBindingDemandValidationContext structural_context;
  structural_context.authenticated_statement_receipt_uuid = structural_receipt;
  structural_context.maximum_mga_relation_decoded_bytes_per_pass =
      candidate_scan_in_range
          ? candidate_scan_bytes
          : narrow_wire::kNarrowQueryMinimumMgaRelationDecodedBytesPerPass;
  narrow_wire::NarrowQueryBindingDemand structural_demand;
  narrow_wire::NarrowQueryBindingDemandError demand_error;
  if (!narrow_wire::DecodeAndValidateNarrowQueryBindingDemand(
          request.payload, structural_context, &structural_demand,
          &demand_error)) {
  ...
  const auto receipt = std::find_if(
      ...
      [&](const auto& entry) {
        return !entry.second.released && entry.second.receipt &&
  ...
  if (receipt == registry->statement_contexts_by_statement_uuid.end() ||
    ...
    return refuse("SECURITY.ACCESS_DENIED",
                  "query_narrow_binding_receipt_hidden");
  }
  const auto& record = receipt->second;
  const auto& view = record.view;
  const auto& live_session = session->second;
  if (view.maximum_typed_result_transport_bytes_per_packet <
          engine_api::kMinimumTypedResultTransportBytesPerPacket ||
      view.maximum_typed_result_transport_bytes_per_packet >
          engine_api::kMaximumTypedResultTransportBytesPerPacket) {
    return refuse(
        "RESOURCE.BUDGET_EXCEEDED",
        "typed_result_transport_bytes_per_packet_projection_invalid");
  }
  if (record.statement_uuid != view.statement_uuid ||
      record.owning_transaction_uuid != view.owning_transaction_uuid ||
      record.owning_local_transaction_id != view.owning_local_transaction_id ||
      live_session.transaction_uuid != view.owning_transaction_uuid ||
      live_session.local_transaction_id != view.owning_local_transaction_id ||
  ...
  narrow_wire::NarrowQueryBindingDemandValidationContext live_validation;
  live_validation.authenticated_statement_receipt_uuid = candidate_receipt;
  live_validation.maximum_mga_relation_decoded_bytes_per_pass =
      view.maximum_mga_relation_decoded_bytes_per_pass;
  narrow_wire::NarrowQueryBindingDemand demand;
  if (!narrow_wire::DecodeAndValidateNarrowQueryBindingDemand(
          request.payload, live_validation, &demand, &demand_error)) {
    return structural_refusal(demand_error);
  }
  ...
  if (context.query_cancellation_requested &&
    ...
    return refuse("PROCESS.CANCELLED",
                  "query_narrow_binding_cancelled_before_publication");
  }
  context.trace_tags.push_back("private_narrow_query_binding_binder");

  constexpr std::uint64_t kMaximumProfileRows = 1'048'576;
  constexpr std::uint64_t kMaximumProfileSources = 9;
  engine_api::EngineNarrowQueryBindingAuthorityIssueRequestV1 issue;
  issue.context = std::move(context);
  issue.demand = std::move(demand);
  issue.policy_snapshot_uuid = view.txn_begin_policy_snapshot_uuid;
  issue.policy_generation = view.txn_begin_policy_generation;
  issue.maximum_source_rows_per_occurrence = kMaximumProfileRows;
  issue.maximum_cumulative_source_rows =
      kMaximumProfileRows * kMaximumProfileSources;
  issue.maximum_result_rows = kMaximumProfileRows;
  issue.maximum_join_combinations = kMaximumProfileRows;
  issue.maximum_sort_memory_bytes = view.optimizer_memory_budget_bytes;
  issue.maximum_batch_rows = kMaximumProfileRows;
  ...
  auto issued =
      engine_api::IssueNarrowQueryBindingAuthorityV1(issue);
  if (!issued.ok) {
    return refuse(issued.diagnostic.code.empty()
                      ? "SBLR.EXECUTION_FAILED"
                      : issued.diagnostic.code,
                  issued.diagnostic.message_key.empty()
                      ? issued.diagnostic.detail
                      : issued.diagnostic.message_key + ":" +
                            issued.diagnostic.detail);
  }
  std::vector<std::uint8_t> reencoded_binding;
  narrow_wire::NarrowQueryBindingError binding_error;
  const bool reencoded = narrow_wire::EncodeNarrowQueryBinding(
      issued.binding, &reencoded_binding, &binding_error);
  if (issued.exact_binding_bytes.size() <
          narrow_wire::kNarrowQueryBindingHeaderBytes + sizeof(std::uint64_t) ||
      issued.exact_binding_bytes.size() >
          narrow_wire::kNarrowQueryMaximumCarrierBytes ||
      !reencoded || reencoded_binding != issued.exact_binding_bytes ||
      GetU64(issued.exact_binding_bytes,
             narrow_wire::kNarrowQueryBindingHeaderBytes) !=
          view.maximum_mga_relation_decoded_bytes_per_pass) {
    engine_api::RevokeNarrowQueryBindingAuthorityForReceiptV1(
        view.receipt_uuid);
    return refuse("SBLR.EXECUTION_FAILED",
                  "query_narrow_binding_success_carrier_invalid");
  }

  ServerRequestRecord completed;
  completed.request_uuid = request.header.request_uuid;
  completed.session_uuid = live_session.session_uuid;
  completed.auth_context_uuid = live_session.auth_context_uuid;
  completed.request_kind = "query_narrow_binding_issue";
  completed.operation_id = "engine.op.query_narrow_binding_issue";
  completed.state = ServerRequestLifecycleState::kCompleted;
  completed.detail = "narrow_query_binding_issued";
  completed.local_transaction_id_at_start =
      view.owning_local_transaction_id;
  completed.snapshot_visible_through_local_transaction_id =
      view.visible_committed_high_watermark;
  completed.transaction_uuid_at_start = view.owning_transaction_uuid;
  completed.authorization_proven = true;
  const auto [unused, inserted] =
      registry->requests_by_uuid.emplace(request_key, std::move(completed));
  (void)unused;
  if (!inserted) {
    engine_api::RevokeNarrowQueryBindingAuthorityForReceiptV1(
        view.receipt_uuid);
    return refuse("PARSER_SERVER_IPC.SEQUENCE_INVALID",
                  "query_narrow_binding_request_publish_race");
  }

  result.payload = std::move(issued.exact_binding_bytes);
  result.accepted = true;
  return result;
}
...
SessionOperationResult HandleIssueContextualTextLiteralProfiles(
    ...
    const sbps::Frame& request) {
  (void)engine_state;
  namespace contextual = scratchbird::engine::sblr;

  SessionOperationResult result;
  result.response_message_type = static_cast<std::uint16_t>(
      sbps::MessageType::kContextualTextLiteralProfileIssueResult);
  result.response_schema_id =
      sbps::kSchemaContextualTextLiteralProfileIssueResultV2;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  ...
  const auto refuse = [&](std::string code, std::string detail) {
    ...
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code),
        "parser_server_ipc.contextual_text_literal_profile_issue_refused",
        "Contextual TEXT literal-profile issuance was refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };
  ...
  if (registry == nullptr ||
      ...
      (request.header.flags & kForbiddenRequestFlags) != 0 ||
      request.header.payload_len != request.payload.size() ||
      request.payload.size() < contextual::kContextualTextRequestHeaderBytesV2 ||
      request.payload.size() >
          contextual::kContextualTextMaximumLogicalCarrierBytesV2) {
    return refuse("PARSER_SERVER_IPC.FRAME_PAYLOAD_INVALID",
                  "contextual_text_issue_message_schema_or_frame_invalid");
  }
  if (request.header.stream_id == 0 || request.header.sequence_number == 0 ||
      IsZeroUuidBytes(request.header.request_uuid) ||
      IsZeroUuidBytes(request.header.connection_uuid) ||
      IsZeroUuidBytes(request.header.session_uuid)) {
    return refuse("PARSER_SERVER_IPC.ROUTE_ASSOCIATION_MISMATCH",
                  "contextual_text_issue_correlation_invalid");
  }

  contextual::ContextualTextLiteralNegotiationRequestV2 decoded;
  contextual::ContextualTextCodecDiagnosticV2 codec_diagnostic;
  std::vector<std::uint8_t> canonical_request;
  if (!contextual::DecodeContextualTextLiteralNegotiationRequestV2(
          request.payload.data(), request.payload.size(), &decoded,
          &codec_diagnostic) ||
      !contextual::EncodeContextualTextLiteralNegotiationRequestV2(
          decoded, &canonical_request, &codec_diagnostic) ||
      canonical_request != request.payload) {
    return refuse(codec_diagnostic.code.empty()
                      ? "PARSER_SERVER_IPC.FRAME_PAYLOAD_INVALID"
                      : codec_diagnostic.code,
                  codec_diagnostic.detail.empty()
                      ? "contextual_text_issue_noncanonical_request"
  ...
  if (session == registry->sessions_by_uuid.end() ||
      ...
      IsZeroUuidBytes(session->second.effective_user_uuid)) {
    return refuse("SECURITY.ACCESS_DENIED",
                  "contextual_text_issue_session_hidden");
  }
  const auto physical_owner =
      registry->physical_channel_by_connection_uuid.find(
          scratchbird::core::platform::Uuid{request.header.connection_uuid});
  if (physical_owner ==
      ...
      physical_owner->second != session->second.server_channel_uuid) {
    return refuse("PARSER_SERVER_IPC.ROUTE_ASSOCIATION_MISMATCH",
                  "contextual_text_issue_physical_channel_mismatch");
  }
  const auto request_key = scratchbird::core::platform::Uuid{request.header.request_uuid};
  if (registry->requests_by_uuid.contains(request_key)) {
    return refuse("PARSER_SERVER_IPC.SEQUENCE_INVALID",
  ...
  if (receipt == registry->statement_contexts_by_statement_uuid.end() ||
      receipt->second.session_uuid != request.header.session_uuid) {
    return refuse("SECURITY.ACCESS_DENIED",
                  "contextual_text_issue_receipt_hidden");
  }
  const auto& record = receipt->second;
  const auto& view = record.view;
  const auto& live_session = session->second;
  ...
  sb_engine_result_t engine_result = nullptr;
  const auto status =
      engine_bridge::IssueStatementContextualTextLiteralProfilesV2(
          record.receipt, request.payload, &result.payload, &engine_result);
  ...
  contextual::ContextualTextLiteralProfileSetV2 issued;
  std::vector<std::uint8_t> canonical_result;
  if (result.payload.empty() ||
      !contextual::DecodeContextualTextLiteralProfileSetV2(
          result.payload.data(), result.payload.size(), &issued,
          &codec_diagnostic) ||
      !contextual::EncodeContextualTextLiteralProfileSetV2(
          issued, &canonical_result, &codec_diagnostic) ||
      canonical_result != result.payload ||
      issued.statement_receipt_uuid != decoded.statement_receipt_uuid ||
      issued.catalog_snapshot_uuid != decoded.catalog_snapshot_uuid ||
      issued.catalog_generation != decoded.catalog_generation ||
      issued.datatype_registry_generation !=
          decoded.datatype_registry_generation ||
      issued.security_generation != decoded.security_generation ||
      issued.resource_epoch != decoded.resource_epoch ||
      issued.mga_snapshot_uuid != decoded.mga_snapshot_uuid ||
      issued.demand_sequence_sha256 != decoded.demand_sequence_sha256) {
    sb_engine_result_t revoke_result = nullptr;
    (void)engine_bridge::RevokeStatementContextualTextLiteralProfilesV2(
        record.receipt, "server_success_carrier_invalid", &revoke_result);
    if (revoke_result != nullptr) (void)sb_engine_result_release(revoke_result);
    return refuse("SBLR.EXECUTION_FAILED",
                  "contextual_text_issue_success_carrier_invalid");
  }
  ...
  completed.session_uuid = live_session.session_uuid;
  completed.auth_context_uuid = live_session.auth_context_uuid;
  completed.request_kind = "contextual_text_literal_profile_issue";
  completed.operation_id =
      "engine.op.contextual_text_literal_profile_issue.v2";
  completed.state = ServerRequestLifecycleState::kCompleted;
  completed.detail = "contextual_text_literal_profiles_issued";
  completed.local_transaction_id_at_start =
      view.owning_local_transaction_id;
  completed.snapshot_visible_through_local_transaction_id =
      view.visible_committed_high_watermark;
  ...
  if (!inserted) {
    sb_engine_result_t revoke_result = nullptr;
    (void)engine_bridge::RevokeStatementContextualTextLiteralProfilesV2(
        record.receipt, "server_request_publish_race", &revoke_result);
    if (revoke_result != nullptr) (void)sb_engine_result_release(revoke_result);
    return refuse("PARSER_SERVER_IPC.SEQUENCE_INVALID",
                  "contextual_text_issue_request_publish_race");
  }
...
SessionOperationResult HandleNegotiateLiteralDescriptors(
  ...
  engine_bridge::StatementContextReceiptHandle receipt;
  {
    ...
    if(found==registry->statement_contexts_by_statement_uuid.end())
      return refuse("DATATYPE.DESCRIPTOR.INVALID","preliminary_receipt_not_live");
  ...
  if(status!=SB_ENGINE_STATUS_OK)
    return refuse(status==SB_ENGINE_STATUS_UNSUPPORTED?
                      "DATATYPE.DESCRIPTOR.INVALID":"DATATYPE.DESCRIPTOR.INVALID",
                  std::string("engine_status=")+sb_engine_status_name(status));
...
SessionOperationResult HandleFinalizeLiteralBinding(
  ...
  if (status != SB_ENGINE_STATUS_OK) {
    return refuse(
        engine_code.empty() ? "DATATYPE.DESCRIPTOR.INVALID"
                            : std::move(engine_code),
        std::string("engine_status=") + sb_engine_status_name(status) +
            ";key=" + engine_key + ";detail=" + engine_detail);
  }
...
SessionOperationResult HandleReserveSavepoint(
  ...
  if(!receipt||receipt->session_uuid!=request.header.session_uuid||
     receipt->owning_transaction_uuid!=engine_api::EngineUuid{decoded.transaction_uuid}||
  ...
  auto context=EngineContextForSession(session_it->second,engine_state,request);
  context.statement_uuid=receipt_uuid;
  context.statement_metadata_snapshot_engine_owned=true;
...
SessionOperationResult HandleReserveAutonomousFrame(
  ...
  if (session_it == registry->sessions_by_uuid.end())
    return refuse("SECURITY.ACCESS_DENIED", "session_hidden");
  const auto receipt_uuid = engine_api::EngineUuid{decoded.receipt};
  ServerStatementContextRecord* receipt = nullptr;
  {
    std::lock_guard<std::mutex> guard(*registry->statement_context_mutex);
  ...
  wire.parent_frame = authority.parent_frame_uuid.bytes;
  wire.database = authority.database_uuid.bytes;
  wire.attachment = authority.attachment_uuid.bytes;
  wire.session = authority.session_uuid.bytes;
  wire.principal = authority.principal_uuid.bytes;
  wire.security = authority.security_snapshot_uuid.bytes;
  wire.policy = authority.policy_snapshot_uuid.bytes;
...
SessionOperationResult HandleFinalizeParameterBinding(
  ...
  if (status != SB_ENGINE_STATUS_OK)
    return refuse(engine_code.empty() ? "DATATYPE.DESCRIPTOR.INVALID"
                                      : std::move(engine_code),
                  std::string("engine_status=") + sb_engine_status_name(status) +
                      ";key=" + engine_key + ";detail=" + engine_detail);
...
SessionOperationResult HandleAuthHandoff(ServerSessionRegistry* registry,
  ...
  auth_request.target_database.uuid = auth_request.context.database_uuid;
  auth_request.target_database.object_kind = "database";
  auth_request.target_object.uuid = auth_request.context.database_uuid;
  ...
  session.auth_context_uuid =
      auth_result.connection_security_context.connection_uuid.bytes;
  session.session_uuid = session.auth_context_uuid;
  session.principal_uuid =
      auth_result.connection_security_context.effective_user_uuid.bytes;
...
SessionOperationResult HandleAttachDatabase(ServerSessionRegistry* registry,
  ...
  if (registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{found->second.session_uuid}) !=
      registry->sessions_by_uuid.end()) {
    result.diagnostics.push_back(AuthDiagnostic(
  ...
  // Only the successfully published parser-server attach establishes the
  // authenticated session binding. Listener-control-plane identifiers remain
  // absent until their separate authoritative report.
  published_session.first->second.session_binding_present = true;
...
SessionOperationResult HandleDisconnectNotice(ServerSessionRegistry* registry,
  ...
  if (session_found != registry->sessions_by_uuid.end()) {
    ...
    if (active_local_transaction_id == 0 &&
        session.pending_temporary_cleanup_transaction != nullptr) {
      const auto& pending = *session.pending_temporary_cleanup_transaction;
      using CleanupState = engine_api::EngineTransactionInventoryState;
      if ((pending.state == CleanupState::committed ||
           pending.state == CleanupState::rolled_back) &&
          !pending.post_inventory_secondary_failure) {
        session.pending_temporary_cleanup_transaction.reset();
      } else {
        // A failed operation's observation is not current inventory authority.
        // In particular a candidate local ID may have been reused after a
        // refused begin publication. Retain the exact UUID/local-ID pair for
        // engine recovery, never project it as a normal transaction or lose it
        // by starting a replacement cleanup.
        temporary_cleanup_failed = true;
        temporary_cleanup_state = "pending_engine_inventory_recovery";
        session.detached_recovery_quarantined = true;
      }
    }
    if (active_local_transaction_id == 0 && !temporary_cleanup_failed) {
      ...
      cleanup.context.transaction_uuid = {};
      ...
      if (cleaned.ok) {
        ...
        temporary_cleanup_failed = true;
        session.detached_recovery_quarantined = true;
        if (cleaned.cleanup_transaction.local_transaction_id != 0) {
          session.pending_temporary_cleanup_transaction =
              std::make_shared<const engine_api::EngineTransactionInventoryObservation>(
                  cleaned.cleanup_transaction);
        }
      }
    } else if (active_local_transaction_id != 0) {
  ...
  for (auto& [_, cursor] : registry->cursors_by_uuid) {
    if (cursor.session_uuid == session_uuid) {
      ...
      if (!sbps::IsZeroUuid(cursor.request_uuid)) {
        auto request_it = registry->requests_by_uuid.find(scratchbird::core::platform::Uuid{cursor.request_uuid});
        if (request_it != registry->requests_by_uuid.end() &&
            !TerminalRequestState(request_it->second.state)) {
          const bool unknown_outcome = temporary_cleanup_failed ||
                                       active_local_transaction_id != 0 || had_engine_result ||
...
ServerSessionBindingControlResult ApplyServerSessionBindingReport(
  ...
  if (!IsZeroUuidBytes(report.transaction_uuid)) {
    session.transaction_uuid = engine_api::EngineUuid{report.transaction_uuid};
  }
  ...
  result.accepted = true;
  result.mutated = true;
  result.target_session_uuid = scratchbird::core::platform::Uuid{session_it->second.session_uuid};
...
SessionOperationResult HandleCoordinateCursorOpen(
  ...
  if (session == registry->sessions_by_uuid.end())
    return refuse("SECURITY.ACCESS_DENIED", "session_hidden");
  const auto receipt_uuid = engine_api::EngineUuid{request_value.receipt};
  ServerStatementContextRecord* receipt = nullptr;
  {
    std::lock_guard<std::mutex> guard(*registry->statement_context_mutex);
...
SessionOperationResult HandleCoordinateDmlUpdateRowsBind(
  ...
  SessionOperationResult result;
  result.response_message_type = static_cast<std::uint16_t>(
      sbps::MessageType::kCoordinateDmlUpdateRowsBindResult);
  result.response_schema_id = sbps::kSchemaCoordinateDmlUpdateRowsBindResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  const auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code), "parser_server_ipc.dml_update_rows_bind_refused",
        "The authenticated DML UPDATE descriptor binding was refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };
  if (registry == nullptr || request.payload.size() < 52 ||
      request.payload.size() > 65536 || request.payload[0] != 'U' ||
      request.payload[1] != 'D' || request.payload[2] != 'R' ||
      request.payload[3] != 'Q' || GetU16(request.payload, 4) != 1) {
    return refuse("SBLR.OPERAND_INVALID", "UDRQ_header_invalid");
  }
  const auto flags = GetU16(request.payload, 6);
  const auto receipt_bytes = GetUuid(request.payload, 8);
  const auto occurrence = GetU64(request.payload, 24);
  const auto relation_bytes = GetUuid(request.payload, 32);
  const auto assignment_count = GetU16(request.payload, 48);
  if ((flags & ~std::uint16_t{1}) != 0 || GetU16(request.payload, 50) != 0 ||
      IsZeroUuidBytes(receipt_bytes) || occurrence == 0 ||
      IsZeroUuidBytes(relation_bytes) || assignment_count == 0 ||
      assignment_count > 1024) {
    return refuse("SBLR.OPERAND_INVALID", "UDRQ_authority_header_invalid");
  }
  const auto get_u32 = [&](std::size_t offset) {
    return static_cast<std::uint32_t>(request.payload[offset]) |
           (static_cast<std::uint32_t>(request.payload[offset + 1]) << 8U) |
  ...
  const auto read_u16_string = [&](std::string* value) {
    return value != nullptr && ReadString(request.payload, &offset, value) &&
           value->size() <= 4096 && value->find('\0') == std::string::npos;
  };
  const auto read_u32_string = [&](std::string* value) {
    if (value == nullptr || offset + 4 > request.payload.size()) return false;
    const auto size = get_u32(offset);
  ...
  for (std::uint32_t expected = 1; expected <= assignment_count; ++expected) {
    if (offset + 4 > request.payload.size()) {
      return refuse("SBLR.OPERAND_INVALID", "UDRQ_assignment_truncated");
    }
    engine_api::EngineDmlUpdateAssignmentDemandV1 assignment;
    assignment.ordinal = get_u32(offset);
    offset += 4;
    if (assignment.ordinal != expected ||
        !read_u16_string(&assignment.target_column_spelling) ||
        assignment.target_column_spelling.empty() ||
        !read_u16_string(&assignment.literal_type_spelling) ||
        assignment.literal_type_spelling.empty() ||
        !read_u32_string(&assignment.literal_spelling)) {
      return refuse("SBLR.OPERAND_INVALID", "UDRQ_assignment_invalid");
    }
    demand.assignments.push_back(std::move(assignment));
  }
  if ((flags & 1U) != 0) {
    if (!read_u16_string(&demand.predicate_kind) ||
        demand.predicate_kind != "column_equals" ||
        ...
        demand.predicate_literal_type_spelling.empty() ||
        !read_u32_string(&demand.predicate_literal_spelling)) {
      return refuse("SBLR.OPERAND_INVALID", "UDRQ_predicate_invalid");
    }
  }
  if (offset != request.payload.size()) {
    return refuse("SBLR.OPERAND_INVALID", "UDRQ_trailing_bytes");
  }
      ...
      receipt_handle;
  {
    ...
    if (receipt == registry->statement_contexts_by_statement_uuid.end()) {
      return refuse("SECURITY.ACCESS_DENIED", "update_receipt_hidden");
    }
    receipt_handle = receipt->second.receipt;
  }
  auto context = EngineContextForSession(session->second, engine_state, request);
  const auto copied = scratchbird::server_engine_bridge::
      CopyStatementContextEngineContextV1(receipt_handle, &context, nullptr);
  if (copied != SB_ENGINE_STATUS_OK) {
    return refuse("MGA.TRANSACTION.STALE", "update_receipt_stale");
  }
  context.trace_tags.push_back("private_dml_update_rows_binder");
  const auto bound = engine_api::BindDmlUpdateRowsDescriptorV1(context, demand);
  if (!bound.ok) {
    return refuse(bound.diagnostic.code.empty()
                      ? "SBLR.OPERAND_INVALID"
                      : bound.diagnostic.code,
...
SessionOperationResult HandleCoordinateDmlPlanImportRowsBind(
  ...
  constexpr std::uint32_t kMaximumMappings = 262144;
  constexpr std::uint32_t kSourceFingerprintPresent = 0x00000001U;
  constexpr std::uint8_t kStrictBulkLoadRequested = 0x01U;
  constexpr std::uint8_t kReferenceRelaxedSemanticsRequested = 0x02U;
  constexpr std::uint8_t kPolicyFlagsMask =
      kStrictBulkLoadRequested | kReferenceRelaxedSemanticsRequested;

  SessionOperationResult result;
  result.response_message_type = static_cast<std::uint16_t>(
      sbps::MessageType::kCoordinateDmlPlanImportRowsBindResult);
  result.response_schema_id =
      sbps::kSchemaCoordinateDmlPlanImportRowsBindResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  const auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code),
        "parser_server_ipc.dml_plan_import_rows_bind_refused",
        "The authenticated DML plan-import descriptor binding was refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };
  const auto get_u32 = [&](std::size_t offset) {
  ...
  const auto nonzero_sha = [](const auto& value) {
    return std::any_of(value.begin(), value.end(),
                       [](std::uint8_t byte) { return byte != 0; });
  };

  if (registry == nullptr || request.payload.size() < kRequestHeaderBytes ||
      request.payload[0] != 'I' || request.payload[1] != 'P' ||
      request.payload[2] != 'R' || request.payload[3] != 'Q' ||
      GetU16(request.payload, 4) != 1 ||
      GetU16(request.payload, 6) != kRequestHeaderBytes) {
    return refuse("SBLR.OPERAND_INVALID", "IPRQ_header_invalid");
  }

  const auto total_bytes = get_u32(8);
  const auto flags = get_u32(12);
  const auto receipt_bytes = GetUuid(request.payload, 16);
  const auto occurrence = GetU64(request.payload, 32);
  const auto target_table_bytes = GetUuid(request.payload, 40);
  const auto source_kind = GetU16(request.payload, 56);
  const auto format_family = GetU16(request.payload, 58);
  const auto reject_mode = request.payload[60];
  const auto reject_payload_policy = request.payload[61];
  const auto resume_policy = request.payload[62];
  const auto policy_flags = request.payload[63];
  const auto reject_limit_ppm = get_u32(64);
  const auto mapping_count = get_u32(68);
  const auto reject_limit_rows = GetU64(request.payload, 72);
  scratchbird::engine::sblr::PlanImportRowsSha256V1 source_fingerprint{};
  std::copy_n(request.payload.begin() + 80, source_fingerprint.size(),
              source_fingerprint.begin());
  const bool fingerprint_present =
      (flags & kSourceFingerprintPresent) != 0;
  const bool reject_limit_present =
      reject_limit_ppm != 0 || reject_limit_rows != 0;
  // IPRQ intentionally carries no reject-target tuple. Enforce every IPOL
  // semantic constraint that is decidable from that closed carrier before
  // authentication, preserving operand-invalid precedence. Reject-row is the
  // only non-fail-fast profile that can be complete without a target.
  const bool policy_shape_valid =
      (reject_mode == 1 && reject_payload_policy == 1 &&
       !reject_limit_present) ||
      (reject_mode == 2 && reject_limit_present);

  if (mapping_count > kMaximumMappings) {
    return refuse("SBLR.OPERAND_INVALID", "IPRQ_mapping_count_invalid");
  }
  const auto expected_bytes =
      kRequestHeaderBytes +
      static_cast<std::size_t>(mapping_count) * kMappingRecordBytes;
  if (total_bytes != request.payload.size() ||
      request.payload.size() != expected_bytes ||
      (flags & ~kSourceFingerprintPresent) != 0 ||
      IsZeroUuidBytes(receipt_bytes) || occurrence == 0 ||
      IsZeroUuidBytes(target_table_bytes) || source_kind < 1 ||
      source_kind > 13 || format_family < 1 || format_family > 12 ||
      reject_mode < 1 || reject_mode > 4 || reject_payload_policy < 1 ||
      reject_payload_policy > 3 || resume_policy < 1 || resume_policy > 3 ||
      (policy_flags & static_cast<std::uint8_t>(~kPolicyFlagsMask)) != 0 ||
      reject_limit_ppm > 1000000 || GetU64(request.payload, 112) != 0 ||
      fingerprint_present != nonzero_sha(source_fingerprint) ||
      !policy_shape_valid) {
    return refuse("SBLR.OPERAND_INVALID", "IPRQ_semantic_header_invalid");
  }

  // The private format reserves a generation-free 24-byte mapping demand, but
  // Core currently publishes no canonical UTF-8 codec-id-to-u16 projection.
  // Validate its structural shape without allocating an engine mapping vector;
  // every nonzero count is refused after receipt authentication below.
  std::set<std::array<std::uint8_t, 16>> target_columns;
  std::size_t mapping_offset = kRequestHeaderBytes;
  for (std::uint32_t ordinal = 0; ordinal < mapping_count; ++ordinal) {
    const auto record_ordinal = get_u32(mapping_offset);
    const auto record_flags = get_u32(mapping_offset + 4);
    const auto target_column = GetUuid(request.payload, mapping_offset + 8);
    if (record_ordinal != ordinal || (record_flags & ~std::uint32_t{1}) != 0 ||
        IsZeroUuidBytes(target_column) ||
        !target_columns.insert(target_column).second) {
      return refuse("SBLR.OPERAND_INVALID", "IPRQ_mapping_shape_invalid");
    }
    mapping_offset += kMappingRecordBytes;
  }
  ...
  const auto receipt_uuid = engine_api::EngineUuid{receipt_bytes};
  scratchbird::server_engine_bridge::StatementContextReceiptHandle
      receipt_handle;
  {
    std::lock_guard<std::mutex> guard(*registry->statement_context_mutex);
    ...
    if (receipt == registry->statement_contexts_by_statement_uuid.end() ||
        receipt->second.session_uuid != request.header.session_uuid) {
      return refuse("SECURITY.ACCESS_DENIED", "plan_import_receipt_hidden");
    }
    if (receipt->second.released) {
      return refuse("MGA.TRANSACTION_INVALID", "plan_import_receipt_ended");
    }
    receipt_handle = receipt->second.receipt;
  }
  ...
  const auto copied = scratchbird::server_engine_bridge::
      CopyStatementContextEngineContextV1(receipt_handle, &context, nullptr);
  if (copied != SB_ENGINE_STATUS_OK) {
    return refuse("MGA.TRANSACTION_INVALID", "plan_import_receipt_invalid");
  }
  context.trace_tags.push_back("private_dml_plan_import_rows_binder");

  if (mapping_count != 0) {
    return refuse("SBLR.OPERATION_UNSUPPORTED",
                  "numeric_import_mapping_codec_projection_not_admitted");
  }

  engine_api::EngineCreateImportRowsPlanDescriptorRequestV1 demand;
  demand.context = std::move(context);
  demand.structural_occurrence_id = occurrence;
  demand.target_table_uuid = engine_api::EngineUuid{target_table_bytes};
  demand.source_kind = static_cast<scratchbird::engine::sblr::
      PlanImportRowsSourceKindV1>(source_kind);
  demand.source_fingerprint_present = fingerprint_present;
  demand.source_fingerprint_sha256 = source_fingerprint;
  demand.format_family = static_cast<scratchbird::engine::sblr::
      PlanImportRowsFormatFamilyV1>(format_family);
  demand.reject_mode = static_cast<scratchbird::engine::sblr::
      PlanImportRowsRejectModeV1>(reject_mode);
  demand.reject_payload_policy = static_cast<scratchbird::engine::sblr::
      PlanImportRowsRejectPayloadPolicyV1>(reject_payload_policy);
  demand.resume_policy = static_cast<scratchbird::engine::sblr::
      PlanImportRowsResumePolicyV1>(resume_policy);
  demand.strict_bulk_load_requested =
      (policy_flags & kStrictBulkLoadRequested) != 0;
  demand.reference_relaxed_semantics_requested =
      (policy_flags & kReferenceRelaxedSemanticsRequested) != 0;
  demand.reference_relaxed_semantics_authorized = false;
  demand.reject_limit_ppm = reject_limit_ppm;
  demand.reject_limit_rows = reject_limit_rows;

  const auto bound =
      engine_api::CreateAndPublishEngineBoundImportRowsPlanDescriptorV1(demand);
  if (!bound.ok) {
    return refuse(bound.diagnostic.code.empty()
                      ? "SBLR.OPERAND_INVALID"
                      : bound.diagnostic.code,
...
SessionOperationResult HandleRetainSourceArtifact(
  ...
  SessionOperationResult result;
  result.response_message_type = static_cast<std::uint16_t>(
      sbps::MessageType::kSourceArtifactRetainResult);
  result.response_schema_id = sbps::kSchemaSourceArtifactRetainResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  const auto refuse = [&](std::string code, std::string key,
                          std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    const std::string message_key =
        key.empty() ? "parser_server_ipc.source_artifact_retain_refused"
                    : key;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code), message_key,
        "Statement-scoped source-artifact retention was refused.",
        {{"detail", std::move(detail)}, {"message_key", message_key}}));
    return result;
  };
  if (registry == nullptr ||
      request.header.payload_schema_id !=
          sbps::kSchemaSourceArtifactRetainRequestV1) {
    return refuse("SBLR.OPERAND_INVALID", {},
                  "source_artifact_retain.schema_invalid");
  }
  scratchbird::engine::sblr::SblrSourceArtifactRetainRequestV1 decoded;
  std::string detail;
  if (!scratchbird::engine::sblr::DecodeSblrSourceArtifactRetainRequestV1(
          request.payload.data(), request.payload.size(), &decoded, &detail)) {
    return refuse("SBLR.OPERAND_INVALID", {}, std::move(detail));
  }
  const auto session = registry->sessions_by_uuid.find(
  ...
  if (!FindStatementManagementReceipt(registry, request.header.session_uuid,
                                      receipt_uuid, &receipt)) {
    return refuse("SECURITY.ACCESS_DENIED", {},
                  "source_artifact_retain_receipt_hidden");
  }
  if (receipt.released || !receipt.handle) {
    return refuse("MGA.TRANSACTION_INVALID", {},
                  "source_artifact_retain_receipt_ended");
  }
  const auto transaction = session->second.transactions_by_local_id.find(
      receipt.view.owning_local_transaction_id);
  if (transaction == session->second.transactions_by_local_id.end() ||
      ...
      transaction->second.transaction_uuid !=
          receipt.view.owning_transaction_uuid) {
    return refuse("MGA.TRANSACTION_INVALID", {},
                  "source_artifact_retain_transaction_invalid");
  }
  const auto artifact =
      scratchbird::engine::sblr::DecodeSblrSourceArtifactMapV1(
          decoded.canonical_artifact_bytes.data(),
          decoded.canonical_artifact_bytes.size());
  if (artifact.status != scratchbird::engine::sblr::
                             SblrSourceArtifactDecodeStatusV1::ok ||
      artifact.artifact.parser_package_uuid !=
          session->second.admitted_parser_package_uuid ||
      artifact.artifact.dialect_family_uuid !=
          session->second.admitted_dialect_profile_uuid) {
    return refuse("MGA.AUTHORITY_MISMATCH", {},
                  artifact.detail.empty()
                      ? "source_artifact_retain.negotiated_identity_mismatch"
                      : artifact.detail);
  }
  engine_bridge::StatementSourceArtifactRetentionV1 retention;
  sb_engine_result_t engine_result = nullptr;
  const auto status = engine_bridge::RetainStatementSourceArtifactV1(
      receipt.handle, request.payload.data(), request.payload.size(),
      &retention, &engine_result);
  if (engine_result != nullptr) (void)sb_engine_result_release(engine_result);
  if (status != SB_ENGINE_STATUS_OK) {
    return refuse(
        retention.failure_code.empty()
            ? StatementManagementStatusCode(status)
            : retention.failure_code,
        retention.failure_message_key,
        retention.failure_detail.empty()
            ? std::string("engine_status=") + sb_engine_status_name(status)
            : retention.failure_detail);
  }
  scratchbird::engine::sblr::SblrSourceArtifactRetainAckV1 ack;
  if (retention.exact_retain_ack_bytes.empty() ||
      !scratchbird::engine::sblr::DecodeSblrSourceArtifactRetainAckV1(
          retention.exact_retain_ack_bytes.data(),
          retention.exact_retain_ack_bytes.size(), &ack, &detail) ||
      ack.authenticated_receipt_uuid != decoded.authenticated_receipt_uuid ||
      ack.sblr_envelope_uuid != decoded.sblr_envelope_uuid ||
      ack.artifact_uuid != decoded.artifact_uuid ||
      ack.declared_size != decoded.declared_size ||
      ack.crc32c != decoded.crc32c ||
      ack.redaction_class != decoded.redaction_class ||
      ack.decompile_policy != decoded.decompile_policy ||
      ack.artifact_sha256 != decoded.artifact_sha256 ||
      ack.retention_generation != retention.retention_generation ||
      retention.reference.sblr_envelope_uuid != decoded.sblr_envelope_uuid ||
      retention.reference.artifact_uuid != decoded.artifact_uuid) {
    return refuse(
        "MGA.AUTHORITY_MISMATCH", {},
        detail.empty()
            ? "source_artifact_retain.engine_ack_correlation_invalid"
            : std::move(detail));
  }
  result.payload = retention.exact_retain_ack_bytes;
  result.accepted = true;
  return result;
}
...
struct BulkImportTransportAdmission {
  ...
  scratchbird::core::platform::Uuid session_key;
};
...
SessionOperationResult HandleCoordinateProcedureInvoke(
  ...
  scratchbird::engine::sblr::SblrProcedureInvokeBindRequestV2 decoded_v2;
  scratchbird::engine::sblr::SblrProcedureInvokeBindRequestV3 decoded_v3;
  std::string detail;
  const bool request_v3 = request.payload.size() >= 6 &&
                          request.payload[4] == 3 && request.payload[5] == 0;
  const bool decoded_ok =
      request_v3
          ? scratchbird::engine::sblr::DecodeSblrProcedureInvokeBindRequestV3(
                request.payload.data(), request.payload.size(), &decoded_v3,
                &detail)
          : scratchbird::engine::sblr::DecodeSblrProcedureInvokeBindRequestV2(
                request.payload.data(), request.payload.size(), &decoded_v2,
                &detail);
  if (registry == nullptr || !decoded_ok) {
    return refuse("SBLR.OPERAND_INVALID", std::move(detail));
  }
  if (registry->sessions_by_uuid.find(
          scratchbird::core::platform::Uuid{request.header.session_uuid}) ==
  ...
  const auto decoded_receipt = request_v3 ? decoded_v3.receipt
                                          : decoded_v2.receipt;
  const auto receipt_uuid = engine_api::EngineUuid{decoded_receipt};
  StatementManagementReceipt receipt;
  if (!FindStatementManagementReceipt(registry, request.header.session_uuid,
                                      receipt_uuid, &receipt)) {
    return refuse("SECURITY.ACCESS_DENIED",
  ...
  bind.request_version = request_v3 ? 3 : 2;
  bind.authenticated_receipt_uuid = receipt_uuid;
  bind.occurrence = request_v3 ? decoded_v3.occurrence : decoded_v2.occurrence;
  bind.invocation_occurrence = request_v3
                                   ? decoded_v3.invocation_occurrence
                                   : decoded_v2.invocation_occurrence;
  bind.command_identity = request_v3 ? decoded_v3.command_identity
                                     : decoded_v2.command_identity;
  bind.request_evidence_sha256 = request_v3 ? decoded_v3.evidence
                                            : decoded_v2.evidence;
  bind.exact_bind_request_bytes = request.payload;
  if (request_v3) {
    for (const auto& atom : decoded_v3.name_atoms) {
      bind.name_atoms.push_back({atom.raw_utf8, atom.quoted});
    }
    for (const auto& argument : decoded_v3.arguments) {
      bind.arguments.push_back(
          {argument.ordinal, argument.lexical_kind, argument.literal_utf8});
    }
  } else {
    for (const auto& atom : decoded_v2.name_atoms) {
      bind.name_atoms.push_back({atom.raw_utf8, atom.quoted});
    }
  }
  ...
  if (authority.canonical_descriptor_bytes.empty() ||
      ...
      descriptor_authority.procedure_abi_uuid !=
          authority.procedure_abi_uuid.bytes ||
      descriptor_authority.procedure_abi_generation !=
          authority.procedure_abi_generation ||
      descriptor_authority.argument_vector_uuid !=
          authority.argument_vector_uuid.bytes ||
      descriptor_authority.argument_count != authority.argument_count ||
      descriptor_authority.argument_vector_sha256 !=
          authority.argument_vector_sha256 ||
...
SessionOperationResult HandleCoordinateCatalogIntrospect(
  ...
  if (authority.canonical_descriptor_bytes.empty() ||
      !scratchbird::engine::sblr::DecodeSblrCatalogIntrospectDescriptorV1(
          ...
          authority.canonical_descriptor_bytes.size(), &descriptor, &detail,
          false) ||
      descriptor.object_uuid != authority.object_uuid.bytes ||
...
SessionOperationResult HandleCoordinateDdlCreateSchema(
    ServerSessionRegistry* registry, const HostedEngineState& engine_state,
    const sbps::Frame& request) {
  (void)engine_state;
  SessionOperationResult result;
  result.response_message_type = 179;
  result.response_schema_id = sbps::kSchemaCoordinateDdlCreateSchemaResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code), "parser_server_ipc.ddl_create_schema_refused",
        "CREATE SCHEMA coordination was refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };

  scratchbird::engine::sblr::SblrDdlCreateSchemaRequestV1 decoded;
  std::string detail;
  if (registry == nullptr ||
      !scratchbird::engine::sblr::DecodeSblrDdlCreateSchemaRequestV1(
          request.payload.data(), request.payload.size(), &decoded, &detail)) {
    return refuse("SBLR.OPERAND_INVALID", detail);
  }
  if (registry->sessions_by_uuid.find(
  ...
  if (!FindStatementManagementReceipt(registry, request.header.session_uuid,
                                      receipt_uuid, &receipt)) {
    return refuse("SECURITY.ACCESS_DENIED",
                  "ddl_create_schema_receipt_hidden");
  }
  if (receipt.released || !receipt.handle) {
    return refuse("MGA.TRANSACTION_INVALID",
                  "ddl_create_schema_receipt_ended");
  }
  if (receipt.view.ddl_create_schema_executor_availability_generation == 0) {
    return refuse("SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING",
                  "ddl_create_schema_executor_unavailable");
  }

  engine_bridge::StatementDdlCreateSchemaBindRequestV1 bind;
  bind.authenticated_receipt_uuid = receipt_uuid;
  bind.occurrence = decoded.occurrence;
  bind.schema_occurrence = decoded.schema_occurrence;
  ...
  engine_bridge::StatementDdlCreateSchemaAuthorityV1 authority;
  sb_engine_result_t engine_result = nullptr;
  const auto status = engine_bridge::BindStatementDdlCreateSchemaAuthorityV1(
      receipt.handle, &bind, &authority, &engine_result);
  std::string engine_code;
  std::string engine_detail;
  ReadFirstEngineDiagnostic(engine_result, &engine_code, &engine_detail);
  ...
  if (authority.canonical_descriptor_bytes.empty() ||
      ...
      descriptor.schema_uuid != authority.schema_uuid.bytes ||
      descriptor.owning_transaction_uuid !=
          (receipt.view.owning_transaction_uuid.bytes) ||
      descriptor.owning_local_transaction_id !=
          receipt.view.owning_local_transaction_id ||
      ...
      descriptor.availability !=
          receipt.view.ddl_create_schema_executor_availability_generation ||
...
SessionOperationResult HandleCoordinateDdlCreateIndex(
    ServerSessionRegistry* registry, const HostedEngineState& engine_state,
    const sbps::Frame& request) {
  (void)engine_state;
  SessionOperationResult result;
  result.response_message_type = 183;
  result.response_schema_id = sbps::kSchemaCoordinateDdlCreateIndexResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code), "parser_server_ipc.ddl_create_index_refused",
        "Index creation coordination was refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };
  scratchbird::engine::sblr::SblrDdlCreateIndexRequestV1 decoded;
  std::string detail;
  if (!registry || !scratchbird::engine::sblr::DecodeSblrDdlCreateIndexRequestV1(
                       request.payload.data(), request.payload.size(), &decoded,
                       &detail)) {
    return refuse("SBLR.OPERAND_INVALID", detail);
  }
  const auto session =
      registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});
  if (session == registry->sessions_by_uuid.end()) {
    return refuse("SECURITY.ACCESS_DENIED", "session_hidden");
  }
  const auto receipt_uuid = engine_api::EngineUuid{decoded.receipt};
...
SessionOperationResult HandleCoordinateDdlCreateTrigger(
  ...
  if (!FindStatementManagementReceipt(registry, request.header.session_uuid,
                                      receipt_uuid, &receipt)) {
    return refuse("SECURITY.ACCESS_DENIED",
                  "ddl_create_trigger_receipt_hidden");
  }
  if (receipt.released || !receipt.handle) {
    return refuse("MGA.TRANSACTION_INVALID",
                  "ddl_create_trigger_receipt_ended");
  }
  if (receipt.view.ddl_create_trigger_executor_availability_generation == 0) {
    return refuse("SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING",
                  "ddl_create_trigger_executor_unavailable");
  }

  engine_bridge::StatementDdlCreateTriggerBindRequestV1 bind;
  bind.authenticated_receipt_uuid = receipt_uuid;
  bind.occurrence = decoded.occurrence;
  bind.trigger_occurrence = decoded.trigger_occurrence;
  bind.command_identity = decoded.command_identity;
  bind.timing = static_cast<std::uint8_t>(decoded.timing);
  bind.event = static_cast<std::uint8_t>(decoded.event);
  bind.scope = static_cast<std::uint8_t>(decoded.scope);
  bind.target_kind = static_cast<std::uint8_t>(decoded.target_kind);
  bind.body_profile = static_cast<std::uint16_t>(decoded.body_profile);
  bind.position = decoded.position;
  bind.security_mode = static_cast<std::uint8_t>(decoded.security_mode);
  bind.image_mode = static_cast<std::uint8_t>(decoded.image_mode);
  bind.execution_mode = static_cast<std::uint8_t>(decoded.execution_mode);
  bind.enabled_state = static_cast<std::uint8_t>(decoded.enabled_state);
  bind.recursion_mode = static_cast<std::uint8_t>(decoded.recursion_mode);
  bind.request_evidence_sha256 = decoded.evidence;
  bind.exact_bind_request_bytes = request.payload;
  for (const auto& atom : decoded.trigger_name_atoms) {
    bind.trigger_name_atoms.push_back({atom.raw_utf8, atom.quoted});
  }
  for (const auto& atom : decoded.target_name_atoms) {
    bind.target_name_atoms.push_back({atom.raw_utf8, atom.quoted});
  }

  engine_bridge::StatementDdlCreateTriggerAuthorityV1 authority;
  sb_engine_result_t engine_result = nullptr;
  const auto status = engine_bridge::BindStatementDdlCreateTriggerAuthorityV1(
      receipt.handle, &bind, &authority, &engine_result);
  std::string engine_code;
  std::string engine_detail;
  ReadFirstEngineDiagnostic(engine_result, &engine_code, &engine_detail);
  ...
  if (authority.canonical_descriptor_bytes.empty() ||
      !scratchbird::engine::sblr::DecodeSblrDdlCreateTriggerDescriptorV1(
          authority.canonical_descriptor_bytes.data(),
          authority.canonical_descriptor_bytes.size(), &descriptor, &detail,
          false) ||
      descriptor.receipt != decoded.receipt ||
      descriptor.occurrence != decoded.occurrence ||
      descriptor.trigger_occurrence != decoded.trigger_occurrence ||
      descriptor.trigger_uuid != authority.trigger_uuid.bytes ||
      ...
      descriptor.resource_generation != receipt.view.resource_epoch ||
      descriptor.availability !=
          receipt.view.ddl_create_trigger_executor_availability_generation ||
      descriptor.syntax_demand_sha256 != decoded.evidence ||
      descriptor.evidence != authority.descriptor_evidence_sha256 ||
...
SessionOperationResult HandleCoordinateDdlAlterTrigger(
    ServerSessionRegistry* registry, const HostedEngineState& engine_state,
    const sbps::Frame& request) {
  (void)engine_state;
  SessionOperationResult result;
  result.response_message_type = 197;
  result.response_schema_id = sbps::kSchemaCoordinateDdlAlterTriggerResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  const auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code), "parser_server_ipc.ddl_alter_trigger_refused",
        "ALTER TRIGGER coordination was refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };

  scratchbird::engine::sblr::SblrDdlAlterTriggerRequestV1 decoded;
  std::string detail;
  if (registry == nullptr ||
      !scratchbird::engine::sblr::DecodeSblrDdlAlterTriggerRequestV1(
          request.payload.data(), request.payload.size(), &decoded, &detail)) {
    return refuse("SBLR.OPERAND_INVALID", detail);
  }
  if (registry->sessions_by_uuid.find(
  ...
  if (!FindStatementManagementReceipt(registry, request.header.session_uuid,
                                      receipt_uuid, &receipt)) {
    return refuse("SECURITY.ACCESS_DENIED",
                  "ddl_alter_trigger_receipt_hidden");
  }
  if (receipt.released || !receipt.handle) {
    return refuse("MGA.TRANSACTION_INVALID",
                  "ddl_alter_trigger_receipt_ended");
  }
  if (receipt.view.ddl_alter_trigger_executor_availability_generation == 0) {
    return refuse("SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING",
                  "ddl_alter_trigger_executor_unavailable");
  }

  engine_bridge::StatementDdlAlterTriggerBindRequestV1 bind;
  bind.authenticated_receipt_uuid = receipt_uuid;
  bind.occurrence = decoded.occurrence;
  bind.trigger_occurrence = decoded.trigger_occurrence;
  bind.command_identity = decoded.command_identity;
  bind.action_mask = decoded.action_mask;
  bind.enabled_state = static_cast<std::uint8_t>(decoded.enabled_state);
  bind.position = decoded.position;
  bind.security_mode = static_cast<std::uint8_t>(decoded.security_mode);
  bind.failure_policy = static_cast<std::uint8_t>(decoded.failure_policy);
  bind.request_evidence_sha256 = decoded.evidence;
  bind.exact_bind_request_bytes = request.payload;
  for (const auto& atom : decoded.trigger_name_atoms) {
    bind.trigger_name_atoms.push_back({atom.raw_utf8, atom.quoted});
  ...
  engine_bridge::StatementDdlAlterTriggerAuthorityV1 authority;
  sb_engine_result_t engine_result = nullptr;
  const auto status = engine_bridge::BindStatementDdlAlterTriggerAuthorityV1(
      receipt.handle, &bind, &authority, &engine_result);
  std::string engine_code;
  std::string engine_detail;
  ReadFirstEngineDiagnostic(engine_result, &engine_code, &engine_detail);
  ...
  if (authority.canonical_descriptor_bytes.empty() ||
      !scratchbird::engine::sblr::DecodeSblrDdlAlterTriggerDescriptorV1(
          authority.canonical_descriptor_bytes.data(),
          authority.canonical_descriptor_bytes.size(), &descriptor, &detail,
          false) ||
      descriptor.receipt != decoded.receipt ||
    ...
    return refuse("MGA.AUTHORITY_MISMATCH",
                  detail.empty()
                      ? "ddl_alter_trigger_descriptor_authority_mismatch"
                      : std::move(detail));
  }
  result.payload = authority.canonical_descriptor_bytes;
  result.accepted = true;
...
SessionOperationResult HandleCoordinateDdlDropTrigger(
    ServerSessionRegistry* registry, const HostedEngineState& engine_state,
    const sbps::Frame& request) {
  (void)engine_state;
  SessionOperationResult result;
  result.response_message_type = 199;
  result.response_schema_id = sbps::kSchemaCoordinateDdlDropTriggerResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  const auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code), "parser_server_ipc.ddl_drop_trigger_refused",
        "DROP TRIGGER coordination was refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };

  scratchbird::engine::sblr::SblrDdlDropTriggerRequestV1 decoded;
  std::string detail;
  if (registry == nullptr ||
      !scratchbird::engine::sblr::DecodeSblrDdlDropTriggerRequestV1(
          request.payload.data(), request.payload.size(), &decoded, &detail)) {
  ...
  if (receipt.released || !receipt.handle) {
    return refuse("MGA.TRANSACTION_INVALID",
                  "ddl_drop_trigger_receipt_ended");
  }
  if (receipt.view.ddl_drop_trigger_executor_availability_generation == 0) {
    return refuse("SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING",
                  "ddl_drop_trigger_executor_unavailable");
  }

  engine_bridge::StatementDdlDropTriggerBindRequestV1 bind;
  bind.authenticated_receipt_uuid = receipt_uuid;
  bind.occurrence = decoded.occurrence;
  bind.trigger_occurrence = decoded.trigger_occurrence;
  bind.command_identity = decoded.command_identity;
  bind.dependency_mode = static_cast<std::uint8_t>(decoded.dependency_mode);
  bind.request_evidence_sha256 = decoded.evidence;
  bind.exact_bind_request_bytes = request.payload;
  for (const auto& atom : decoded.trigger_name_atoms) {
    bind.trigger_name_atoms.push_back({atom.raw_utf8, atom.quoted});
  ...
  engine_bridge::StatementDdlDropTriggerAuthorityV1 authority;
  sb_engine_result_t engine_result = nullptr;
  const auto status = engine_bridge::BindStatementDdlDropTriggerAuthorityV1(
      receipt.handle, &bind, &authority, &engine_result);
  std::string engine_code;
  std::string engine_detail;
  ReadFirstEngineDiagnostic(engine_result, &engine_code, &engine_detail);
  ...
  if (authority.canonical_descriptor_bytes.empty() ||
      !scratchbird::engine::sblr::DecodeSblrDdlDropTriggerDescriptorV1(
          authority.canonical_descriptor_bytes.data(),
          authority.canonical_descriptor_bytes.size(), &descriptor, &detail,
          false) ||
      descriptor.receipt != decoded.receipt ||
    ...
    return refuse("MGA.AUTHORITY_MISMATCH",
                  detail.empty()
                      ? "ddl_drop_trigger_descriptor_authority_mismatch"
                      : std::move(detail));
  }
  result.payload = authority.canonical_descriptor_bytes;
  result.accepted = true;
...
SessionOperationResult HandleCoordinateDdlCreateProcedure(
  ...
  scratchbird::engine::sblr::SblrDdlCreateProcedureBindRequestV2 decoded_v2;
  scratchbird::engine::sblr::SblrDdlCreateProcedureBindRequestV3 decoded_v3;
  std::string detail;
  const bool request_v3 = request.payload.size() >= 6 &&
                          request.payload[4] == 3 && request.payload[5] == 0;
  const bool decoded_ok =
      request_v3
          ? scratchbird::engine::sblr::
                DecodeSblrDdlCreateProcedureBindRequestV3(
                    request.payload.data(), request.payload.size(),
                    &decoded_v3, &detail)
          : scratchbird::engine::sblr::
                DecodeSblrDdlCreateProcedureBindRequestV2(
                    request.payload.data(), request.payload.size(),
                    &decoded_v2, &detail);
  if (registry == nullptr || !decoded_ok) {
    return refuse("SBLR.OPERAND_INVALID", std::move(detail));
  ...
  const auto decoded_receipt = request_v3 ? decoded_v3.receipt
                                          : decoded_v2.receipt;
  const auto decoded_occurrence = request_v3 ? decoded_v3.occurrence
                                             : decoded_v2.occurrence;
  const auto decoded_procedure_occurrence =
      request_v3 ? decoded_v3.procedure_occurrence
                 : decoded_v2.procedure_occurrence;
  const auto decoded_command_identity =
      request_v3 ? decoded_v3.command_identity : decoded_v2.command_identity;
  const auto decoded_body_profile =
      request_v3 ? decoded_v3.body_profile : decoded_v2.body_profile;
  const auto receipt_uuid = engine_api::EngineUuid{decoded_receipt};
  StatementManagementReceipt receipt;
  if (!FindStatementManagementReceipt(registry, request.header.session_uuid,
                                      receipt_uuid, &receipt)) {
  ...
  bind.request_version = request_v3 ? 3 : 2;
  bind.authenticated_receipt_uuid = receipt_uuid;
  bind.occurrence = decoded_occurrence;
  bind.procedure_occurrence = decoded_procedure_occurrence;
  bind.command_identity = decoded_command_identity;
  bind.body_profile = decoded_body_profile;
  bind.request_evidence_sha256 = request_v3 ? decoded_v3.evidence
                                            : decoded_v2.evidence;
  bind.exact_bind_request_bytes = request.payload;
  if (request_v3) {
    ...
    for (const auto& parameter : decoded_v3.parameters) {
      bind.parameters.push_back(
          {parameter.ordinal, parameter.mode,
           {parameter.name.raw_utf8, parameter.name.quoted},
           {parameter.type_name.raw_utf8, parameter.type_name.quoted}});
    }
  } else {
    for (const auto& atom : decoded_v2.name_atoms) {
      bind.name_atoms.push_back({atom.raw_utf8, atom.quoted});
  ...
  if (authority.canonical_descriptor_bytes.empty() ||
      ...
      !scratchbird::engine::internal_api::
          DecodeSblrDdlCreateProcedureAuthorityInputV1(
              descriptor, &descriptor_authority, &detail) ||
      descriptor_authority.receipt != decoded_receipt ||
      descriptor_authority.occurrence != decoded_occurrence ||
      descriptor_authority.procedure_occurrence !=
          decoded_procedure_occurrence ||
      descriptor_authority.command_identity != decoded_command_identity ||
      descriptor_authority.body_profile != decoded_body_profile ||
      descriptor_authority.procedure_uuid !=
          authority.procedure_uuid.bytes ||
      descriptor_authority.schema_uuid != authority.schema_uuid.bytes ||
      descriptor_authority.owning_transaction_uuid !=
          (receipt.view.owning_transaction_uuid.bytes) ||
      descriptor_authority.owning_local_transaction_id !=
          receipt.view.owning_local_transaction_id ||
...
SessionOperationResult HandleCoordinateDdlCreateTemporaryTable(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=215;result.response_schema_id=sbps::kSchemaCoordinateDdlCreateTemporaryTableResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_create_temporary_table_refused","DDL create temporary table coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlCreateTemporaryTableRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlCreateTemporaryTableRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND_INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_create_temporary_table_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_temporary_table_binder");auto q=engine_api::CompileSblrDdlCreateTemporaryTableDescriptor(c,ru,v.occurrence,v.temporary_table_occurrence,r->view.kv_structured_read_executor_availability_generation);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlCreateTemporaryTableDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("DDL.CREATE_PACKAGE_FAILED","TTDX_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlDropTemporaryTable(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=217;result.response_schema_id=sbps::kSchemaCoordinateDdlDropTemporaryTableResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_drop_temporary_table_refused","DDL drop temporary table coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlDropTemporaryTableRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlDropTemporaryTableRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND_INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_drop_temporary_table_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_drop_temporary_table_binder");auto q=engine_api::CompileSblrDdlDropTemporaryTableDescriptor(c,ru,v.occurrence,v.temporary_table_occurrence,r->view.kv_structured_read_executor_availability_generation);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlDropTemporaryTableDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("DDL.CREATE_PACKAGE_FAILED","TTDX_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlRenameObjectVector(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=219;result.response_schema_id=sbps::kSchemaCoordinateDdlRenameObjectVectorResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_rename_object_vector_refused","DDL drop temporary table coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlRenameObjectVectorRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlRenameObjectVectorRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND_INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_rename_object_vector_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_rename_object_vector_binder");auto q=engine_api::CompileSblrDdlRenameObjectVectorDescriptor(c,ru,v.occurrence,v.object_vector_occurrence,r->view.kv_structured_read_executor_availability_generation);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlRenameObjectVectorDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("DDL.CREATE_PACKAGE_FAILED","TTDX_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlCreateOrReplaceSrs(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=221;result.response_schema_id=sbps::kSchemaCoordinateDdlCreateOrReplaceSrsResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_create_or_replace_srs_refused","DDL create or replace SRS coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlCreateOrReplaceSrsRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlCreateOrReplaceSrsRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND_INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_create_or_replace_srs_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_or_replace_srs_binder");auto q=engine_api::CompileSblrDdlCreateOrReplaceSrsDescriptor(c,ru,v.occurrence,v.srs_occurrence,r->view.kv_structured_read_executor_availability_generation);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlCreateOrReplaceSrsDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("DDL.CREATE_OR_REPLACE_SRS_FAILED","SRSD_encode_failed");result.accepted=true;return result;}
...
SessionOperationResult HandleCoordinateSecurityCreatePrivilegeTemplate(
  ...
  if (authority.canonical_descriptor_bytes.empty() ||
      ...
      descriptor.template_uuid != authority.template_uuid.bytes ||
      descriptor.owner_principal_uuid !=
          authority.owner_principal_uuid.bytes ||
      descriptor.grantee_uuid != authority.grantee_uuid.bytes ||
      descriptor.owning_transaction_uuid !=
          (receipt.view.owning_transaction_uuid.bytes) ||
      descriptor.owning_local_transaction_id !=
          receipt.view.owning_local_transaction_id ||
...
SessionOperationResult HandleCoordinateDdlCreateDictionary(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=265;result.response_schema_id=sbps::kSchemaCoordinateDdlCreateDictionaryResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_create_dictionary_refused","Dictionary creation coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlCreateDictionaryRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlCreateDictionaryRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_dictionary_binder");auto q=scratchbird::engine::internal_api::CompileSblrDdlCreateDictionaryDescriptor(c,ru,v.occurrence,v.dictionary_occurrence,1);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlCreateDictionaryDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("DDL.CREATE_DICTIONARY_FAILED","DCDD_encode_failed");result.accepted=true;return result;}
...
SessionOperationResult HandleCoordinateDdlAlterTimeseriesValueCache(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=295;result.response_schema_id=sbps::kSchemaCoordinateDdlAlterTimeseriesValueCacheResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_alter_timeseries_value_cache_refused","Timeseries value-cache creation coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlAlterTimeseriesValueCacheRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlAlterTimeseriesValueCacheRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_alter_timeseries_value_cache_binder");auto q=scratchbird::engine::internal_api::CompileSblrDdlAlterTimeseriesValueCacheDescriptor(c,ru,v.occurrence,v.cache_occurrence,1);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlAlterTimeseriesValueCacheDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("TIMESERIES.VALUE_CACHE.ALTER_FAILED","VADD_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlDropTimeseriesValueCache(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=297;result.response_schema_id=sbps::kSchemaCoordinateDdlDropTimeseriesValueCacheResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_drop_timeseries_value_cache_refused","Timeseries value-cache creation coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlDropTimeseriesValueCacheRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlDropTimeseriesValueCacheRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_drop_timeseries_value_cache_binder");auto q=scratchbird::engine::internal_api::CompileSblrDdlDropTimeseriesValueCacheDescriptor(c,ru,v.occurrence,v.cache_occurrence,1);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlDropTimeseriesValueCacheDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("TIMESERIES.VALUE_CACHE.ALTER_FAILED","VDDD_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlRefreshMaterializedView(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlRefreshMaterializedViewResult);result.response_schema_id=sbps::kSchemaCoordinateDdlRefreshMaterializedViewResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_refresh_materialized_view_refused","Materialized view refresh coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlRefreshMaterializedViewDescriptorV1 v;std::string d;if(!registry||request.payload.size()!=64)return refuse("SBLR.OPERAND.INVALID","RVDX request invalid");auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{GetUuid(request.payload,0)};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_refresh_materialized_view_binder");auto q=scratchbird::engine::internal_api::CompileSblrDdlRefreshMaterializedViewDescriptor(c,ru,1,1,1);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlRefreshMaterializedViewDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("DDL.MATERIALIZED_VIEW.REFRESH_FAILED","RVDX_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlDropMaterializedView(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlDropMaterializedViewResult);result.response_schema_id=sbps::kSchemaCoordinateDdlDropMaterializedViewResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_drop_materialized_view_refused","Materialized view drop coordination was refused.",{{"detail",std::move(d)}}));return result;};if(!registry||request.payload.size()!=64)return refuse("SBLR.OPERAND.INVALID","MVDX request invalid");auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{GetUuid(request.payload,0)};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_drop_materialized_view_binder");auto q=scratchbird::engine::internal_api::CompileSblrDdlDropMaterializedViewDescriptor(c,ru,1,1,1);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlDropMaterializedViewDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("DDL.MATERIALIZED_VIEW.DROP_FAILED","MVDX_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlCreateType(
    ServerSessionRegistry* registry, const HostedEngineState& engine_state,
    const sbps::Frame& request) {
  (void)engine_state;
  SessionOperationResult result;
  result.response_message_type = 301;
  result.response_schema_id = sbps::kSchemaCoordinateDdlCreateTypeResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  const auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code), "parser_server_ipc.ddl_create_type_refused",
        "CREATE TYPE coordination refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };
  scratchbird::engine::sblr::SblrDdlCreateTypeRequestV1 decoded;
  std::string detail;
  if (!registry ||
      !scratchbird::engine::sblr::DecodeSblrDdlCreateViewRequestV1(
          request.payload.data(), request.payload.size(), &decoded, &detail)) {
    return refuse("SBLR.OPERAND_INVALID", detail);
  }
  if (registry->sessions_by_uuid.find(
  ...
  return refuse("SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING",
                "accepted_executor_evidence_absent");
}

SessionOperationResult HandleCoordinateDdlAlterType(
    ServerSessionRegistry* registry, const HostedEngineState& engine_state,
    const sbps::Frame& request) {
  (void)engine_state;
  SessionOperationResult result;
  result.response_message_type = 303;
  result.response_schema_id = sbps::kSchemaCoordinateDdlAlterTypeResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  const auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code), "parser_server_ipc.ddl_alter_type_refused",
        "ALTER TYPE coordination refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };
  scratchbird::engine::sblr::SblrDdlAlterTypeRequestV1 decoded;
  std::string detail;
  if (!registry ||
      !scratchbird::engine::sblr::DecodeSblrDdlAlterTypeRequestV1(
          request.payload.data(), request.payload.size(), &decoded, &detail)) {
    return refuse("SBLR.OPERAND_INVALID", detail);
  }
  if (registry->sessions_by_uuid.find(
...
SessionOperationResult HandleCoordinateDdlDropType(
    ServerSessionRegistry* registry, const HostedEngineState& engine_state,
    const sbps::Frame& request) {
  (void)engine_state;
  SessionOperationResult result;
  result.response_message_type = 305;
  result.response_schema_id = sbps::kSchemaCoordinateDdlDropTypeResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  const auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code), "parser_server_ipc.ddl_drop_type_refused",
        "DROP TYPE coordination refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };
  scratchbird::engine::sblr::SblrDdlDropTypeRequestV1 decoded;
  std::string detail;
  if (!registry ||
      !scratchbird::engine::sblr::DecodeSblrDdlDropViewRequestV1(
          request.payload.data(), request.payload.size(), &decoded, &detail)) {
    return refuse("SBLR.OPERAND_INVALID", detail);
  }
  if (registry->sessions_by_uuid.find(
...
SessionOperationResult HandleCoordinateDdlAlterSequence(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlAlterSequenceResult);result.response_schema_id=sbps::kSchemaCoordinateDdlAlterSequenceResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_alter_sequence_refused","DDL alter sequence coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlAlterSequenceRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlAlterSequenceRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_package_binder");auto q=scratchbird::engine::internal_api::CompileSblrDdlAlterSequenceDescriptor(c,ru,v.occurrence,v.procedure_occurrence,1);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlAlterSequenceDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("DDL.ALTER_SEQUENCE_FAILED","SADX_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlCreatePublication(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlCreatePublicationResult);result.response_schema_id=sbps::kSchemaCoordinateDdlCreatePublicationResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_create_publication_refused","CREATE PUBLICATION coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlCreatePublicationRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlCreatePublicationRequestV1(request.payload.data(),request.payload.size(),&v,&d)||std::all_of(v.operation.begin(),v.operation.end(),[](auto b){return b==0;}))return refuse("SBLR.OPERAND_INVALID",d.empty()?"PUBQ operation identity is zero":d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_create_publication_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_publication_binder");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlCreatePublicationDescriptor(c,ru,1,1,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlCreatePublicationDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND_INVALID","SLCP_encode_failed");result.accepted=true;return result;}
...
SessionOperationResult HandleCoordinateDdlDropTable(ServerSessionRegistry* registry,
  ...
  if (!registry || !scratchbird::engine::sblr::DecodeSblrDdlDropTableRequestV1(
    ...
    return refuse("SBLR.OPERAND.INVALID", detail);
  auto session = registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});
  if (session == registry->sessions_by_uuid.end())
    return refuse("SECURITY.ACCESS_DENIED", "session_hidden");
  auto receipt = engine_api::EngineUuid{value.receipt};
  auto context = EngineContextForSession(session->second, engine_state, request);
  context.statement_uuid = receipt;
  context.statement_metadata_snapshot_engine_owned = true;
...
SessionOperationResult HandleCoordinateSecurityCreateRole(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){
  ...
  auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid}); if(s==registry->sessions_by_uuid.end()) return refuse("SECURITY.ACCESS_DENIED","session_hidden");
  auto ru=engine_api::EngineUuid{v.receipt}; auto c=EngineContextForSession(s->second,engine_state,request); c.statement_uuid=ru; c.statement_metadata_snapshot_engine_owned=true; c.trace_tags.push_back("private_sec_create_role_binder");
...
SessionOperationResult HandleCoordinateSecurityDropRole(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){
  ...
  auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid}); if(s==registry->sessions_by_uuid.end()) return refuse("SECURITY.ACCESS_DENIED","session_hidden");
  auto ru=engine_api::EngineUuid{v.receipt}; auto c=EngineContextForSession(s->second,engine_state,request); c.statement_uuid=ru; c.statement_metadata_snapshot_engine_owned=true; c.trace_tags.push_back("private_sec_drop_role_binder");
...
SessionOperationResult HandleCoordinateSecurityAlterRole(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=351;result.response_schema_id=7364;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.security_alter_role_refused","Security role alteration coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrSecAlterRoleRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrSecAlterRoleRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_sec_alter_role_binder");auto q=scratchbird::engine::internal_api::CompileSblrSecAlterRoleDescriptor(c,ru,v.occurrence,v.occurrence);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrSecAlterRoleDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("SECURITY.ROLE_APPLICATION_FAILED","SARD_encode_failed");result.accepted=true;return result;}

SessionOperationResult HandleCoordinateSecurityDropPolicy(ServerSessionRegistry* registry,const HostedEngineState& engine_state,const sbps::Frame& request){SessionOperationResult result;result.response_message_type=348+1;result.response_schema_id=7362;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.security_drop_policy_refused","Security policy drop coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrSecDropPolicyRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrSecDropPolicyRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_sec_drop_policy_binder");auto q=scratchbird::engine::internal_api::CompileSblrSecDropPolicyDescriptor(c,ru,v.occurrence,v.occurrence);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrSecDropPolicyDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("SECURITY.POLICY_APPLICATION_FAILED","SDPD_encode_failed");result.accepted=true;return result;}

SessionOperationResult HandleCoordinateSecurityCreateGroupMapping(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=353;result.response_schema_id=7366;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.security_create_group_mapping_refused","Group mapping coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrSecCreateGroupMappingRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrSecCreateGroupMappingRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_sec_create_group_mapping_binder");auto q=scratchbird::engine::internal_api::CompileSblrSecCreateGroupMappingDescriptor(c,ru,v.occurrence,v.occurrence);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrSecCreateGroupMappingDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("SECURITY.GROUP_MAPPING_FAILED","SGMD_encode_failed");result.accepted=true;return result;}
...
SessionOperationResult HandleCoordinateSecurityAlterPolicy(
  ...
  SessionOperationResult result;
  result.response_message_type = static_cast<std::uint16_t>(
      sbps::MessageType::kSecurityAlterPolicyBindResult);
  result.response_schema_id =
      sbps::kSchemaSecurityAlterPolicyBindResultV1;
  result.frame_flags = sbps::kFlagResponse | sbps::kFlagFinal;
  result.session_uuid = request.header.session_uuid;
  auto refuse = [&](std::string code, std::string detail) {
    result.frame_flags |= sbps::kFlagError;
    result.diagnostics.push_back(sbps::IpcDiagnostic(
        std::move(code),
        "parser_server_ipc.security_alter_policy_refused",
        "ACTIVATE POLICY binding was refused.",
        {{"detail", std::move(detail)}}));
    return result;
  };

  scratchbird::engine::sblr::SblrSecAlterPolicyRequestV1 decoded;
  std::string detail;
  if (registry == nullptr ||
  ...
  if (!FindStatementManagementReceipt(registry, request.header.session_uuid,
                                      receipt_uuid, &receipt)) {
    return refuse("SECURITY.ACCESS_DENIED",
                  "security_alter_policy_receipt_hidden");
  }
  if (receipt.released || !receipt.handle) {
    return refuse("MGA.TRANSACTION_INVALID",
                  "security_alter_policy_receipt_ended");
  }
  if (receipt.view.security_alter_policy_executor_availability_generation ==
      0) {
    return refuse("SBLR.OPCODE.EXECUTOR_EVIDENCE_MISSING",
                  "security_alter_policy_executor_unavailable");
  }

  engine_bridge::StatementSecurityAlterPolicyBindRequestV1 bind;
  bind.authenticated_receipt_uuid = receipt_uuid;
  bind.occurrence = decoded.occurrence;
  bind.policy_occurrence = decoded.policy_occurrence;
  bind.command_identity = decoded.command_identity;
  bind.request_evidence_sha256 = decoded.evidence;
  bind.exact_bind_request_bytes = request.payload;
  for (const auto& atom : decoded.name_atoms) {
  ...
  engine_bridge::StatementSecurityAlterPolicyAuthorityV1 authority;
  sb_engine_result_t engine_result = nullptr;
  const auto status =
      engine_bridge::BindStatementSecurityAlterPolicyAuthorityV1(
          receipt.handle, &bind, &authority, &engine_result);
  std::string engine_code;
  std::string engine_detail;
  ReadFirstEngineDiagnostic(engine_result, &engine_code, &engine_detail);
  ...
  if (authority.canonical_descriptor_bytes.empty() ||
      ...
      descriptor.receipt != decoded.receipt ||
      descriptor.occurrence != decoded.occurrence ||
      descriptor.policy_occurrence != decoded.policy_occurrence ||
      descriptor.action != decoded.command_identity ||
      descriptor.policy_uuid != authority.policy_uuid.bytes ||
      descriptor.expected_generation !=
          authority.expected_policy_generation ||
      descriptor.database_uuid != authority.database_uuid.bytes ||
      descriptor.owning_transaction_uuid !=
          (receipt.view.owning_transaction_uuid.bytes) ||
      descriptor.owning_local_transaction_id !=
          receipt.view.owning_local_transaction_id ||
      ...
      descriptor.availability != receipt.view
          .security_alter_policy_executor_availability_generation ||
...
SessionOperationResult HandleSessionSettingSet(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=371;result.response_schema_id=7384;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.session_setting_set_refused","Session setting set coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrSessionSettingSetRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrSessionSettingSetRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_session_setting_set_binder");auto q=scratchbird::engine::internal_api::CompileSblrSessionSettingSetDescriptor(c,ru,v.occurrence,v.occurrence);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrSessionSettingSetDescriptorV1(q.descriptor,false);if(result.payload.empty())return refuse("SESSION.SET_FAILED","SSSD_encode_failed");result.accepted=true;return result;}
...
SessionOperationResult HandleContextSet(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){auto result=HandleSessionSettingSet(registry,engine_state,request);result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kContextSetResult);result.response_schema_id=sbps::kSchemaContextSetResultV1;return result;}
SessionOperationResult HandleContextUnset(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){auto result=HandleSessionSettingSet(registry,engine_state,request);result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kContextUnsetResult);result.response_schema_id=sbps::kSchemaContextUnsetResultV1;return result;}
SessionOperationResult HandleContextGet(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){auto result=HandleSessionSettingSet(registry,engine_state,request);result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kContextGetResult);result.response_schema_id=sbps::kSchemaContextGetResultV1;return result;}
...
SessionOperationResult HandleSessionDefaultQualifierSet(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){auto result=HandleSessionSettingSet(registry,engine_state,request);result.response_message_type=377;result.response_schema_id=7390;return result;}
SessionOperationResult HandleSessionDiscard(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){auto result=HandleSessionSettingSet(registry,engine_state,request);result.response_message_type=379;result.response_schema_id=7392;return result;}
SessionOperationResult HandleSessionSnapshotHandle(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){auto result=HandleSessionSettingSet(registry,engine_state,request);result.response_message_type=381;result.response_schema_id=7394;return result;}
SessionOperationResult HandleCoordinateDdlAlterPublication(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlAlterPublicationResult);result.response_schema_id=sbps::kSchemaCoordinateDdlAlterPublicationResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_alter_publication_refused","ALTER PUBLICATION coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlAlterPublicationRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlAlterPublicationRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_alter_publication_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_alter_publication_binder");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlAlterPublicationDescriptor(c,ru,1,1,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlAlterPublicationDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SLAP_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlDropPublication(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlDropPublicationResult);result.response_schema_id=sbps::kSchemaCoordinateDdlDropPublicationResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_drop_publication_refused","DROP PUBLICATION coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlDropPublicationRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlDropPublicationRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_drop_publication_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_drop_publication_binder");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlDropPublicationDescriptor(c,ru,1,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlDropPublicationDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SLDP_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlCreateSubscription(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlCreateSubscriptionResult);result.response_schema_id=sbps::kSchemaCoordinateDdlCreateSubscriptionResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_create_subscription_refused","CREATE CHANGE SUBSCRIPTION coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlCreateSubscriptionRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlCreateSubscriptionRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_create_subscription_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_subscription_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlCreateSubscriptionDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlCreateSubscriptionDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SLCS_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlAlterSubscription(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlAlterSubscriptionResult);result.response_schema_id=sbps::kSchemaCoordinateDdlAlterSubscriptionResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_alter_subscription_refused","ALTER CHANGE SUBSCRIPTION coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlAlterSubscriptionRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlAlterSubscriptionRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_alter_subscription_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_alter_subscription_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlAlterSubscriptionDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlAlterSubscriptionDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SAUD_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlDropSubscription(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlDropSubscriptionResult);result.response_schema_id=sbps::kSchemaCoordinateDdlDropSubscriptionResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_drop_subscription_refused","DROP CHANGE SUBSCRIPTION coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlDropSubscriptionRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlDropSubscriptionRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_drop_subscription_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_drop_subscription_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlDropSubscriptionDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlDropSubscriptionDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","S LDS_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlCreateOperator(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlCreateOperatorResult);result.response_schema_id=sbps::kSchemaCoordinateDdlCreateOperatorResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_create_operator_refused","CREATE OPERATOR coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlCreateOperatorRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlCreateOperatorRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_create_operator_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_operator_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlCreateOperatorDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlCreateOperatorDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SCOP_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlDropOperator(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlDropOperatorResult);result.response_schema_id=sbps::kSchemaCoordinateDdlDropOperatorResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_drop_operator_refused","DROP OPERATOR coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlDropOperatorRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlDropOperatorRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_drop_operator_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_drop_operator_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlDropOperatorDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlDropOperatorDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SDOP_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlCreateOperatorClass(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlCreateOperatorClassResult);result.response_schema_id=sbps::kSchemaCoordinateDdlCreateOperatorClassResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_create_operator_class_refused","CREATE OPERATOR CLASS coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlCreateOperatorClassRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlCreateOperatorClassRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_create_operator_class_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_operator_class_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlCreateOperatorClassDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlCreateOperatorClassDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SCOC_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlDropOperatorClass(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlDropOperatorClassResult);result.response_schema_id=sbps::kSchemaCoordinateDdlDropOperatorClassResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_drop_operator_class_refused","DROP OPERATOR CLASS coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlDropOperatorClassRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlDropOperatorClassRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_drop_operator_class_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_drop_operator_class_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlDropOperatorClassDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlDropOperatorClassDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SDOC_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlCreateOperatorFamily(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlCreateOperatorFamilyResult);result.response_schema_id=sbps::kSchemaCoordinateDdlCreateOperatorFamilyResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_create_operator_family_refused","CREATE OPERATOR FAMILY coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlCreateOperatorFamilyRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlCreateOperatorFamilyRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_create_operator_family_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_operator_family_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlCreateOperatorFamilyDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlCreateOperatorFamilyDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SCOF_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlAlterOperatorFamily(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlAlterOperatorFamilyResult);result.response_schema_id=sbps::kSchemaCoordinateDdlAlterOperatorFamilyResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_alter_operator_family_refused","ALTER OPERATOR FAMILY coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlAlterOperatorFamilyRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlAlterOperatorFamilyRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_alter_operator_family_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_alter_operator_family_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlAlterOperatorFamilyDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlAlterOperatorFamilyDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SAOF_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlDropCast(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlDropCastResult);result.response_schema_id=sbps::kSchemaCoordinateDdlDropCastResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_drop_cast_refused","DROP CAST coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlDropCastRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlDropCastRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_drop_cast_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_drop_cast_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlDropCastDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlDropCastDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SDOC_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlCreateExtension(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlCreateExtensionResult);result.response_schema_id=sbps::kSchemaCoordinateDdlCreateExtensionResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_create_extension_refused","CREATE EXTENSION coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlCreateExtensionRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlCreateExtensionRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_create_extension_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_create_extension_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlCreateExtensionDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlCreateExtensionDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SCEX_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlAlterExtension(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlAlterExtensionResult);result.response_schema_id=sbps::kSchemaCoordinateDdlAlterExtensionResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_alter_extension_refused","ALTER EXTENSION coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlAlterExtensionRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlAlterExtensionRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_alter_extension_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_alter_extension_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlAlterExtensionDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlAlterExtensionDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SAEX_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateDdlDropExtension(ServerSessionRegistry*registry,const HostedEngineState&engine_state,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlDropExtensionResult);result.response_schema_id=sbps::kSchemaCoordinateDdlDropExtensionResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal;result.session_uuid=request.header.session_uuid;auto refuse=[&](std::string c,std::string d){result.frame_flags|=sbps::kFlagError;result.diagnostics.push_back(sbps::IpcDiagnostic(std::move(c),"parser_server_ipc.ddl_drop_extension_refused","DROP EXTENSION coordination was refused.",{{"detail",std::move(d)}}));return result;};scratchbird::engine::sblr::SblrDdlDropExtensionRequestV1 v;std::string d;if(!registry||!scratchbird::engine::sblr::DecodeSblrDdlDropExtensionRequestV1(request.payload.data(),request.payload.size(),&v,&d))return refuse("SBLR.OPERAND.INVALID",d);auto s=registry->sessions_by_uuid.find(scratchbird::core::platform::Uuid{request.header.session_uuid});if(s==registry->sessions_by_uuid.end())return refuse("SECURITY.ACCESS_DENIED","session_hidden");auto ru=engine_api::EngineUuid{v.receipt};ServerStatementContextRecord*r=nullptr;{std::lock_guard<std::mutex>g(*registry->statement_context_mutex);for(auto&[x,row]:registry->statement_contexts_by_statement_uuid){(void)x;if(!row.released&&row.view.receipt_uuid==ru){r=&row;break;}}}if(!r||r->session_uuid!=request.header.session_uuid)return refuse("SECURITY.ACCESS_DENIED","ddl_drop_extension_receipt_hidden");auto c=EngineContextForSession(s->second,engine_state,request);c.statement_uuid=ru;c.statement_metadata_snapshot_engine_owned=true;c.trace_tags.push_back("private_ddl_drop_extension_binder");c.trace_tags.push_back("cluster_provider_admitted");c.trace_tags.push_back("cluster_route_fence_admitted");const auto availability=r->view.kv_structured_mutate_executor_availability_generation?r->view.kv_structured_mutate_executor_availability_generation:r->view.kv_structured_read_executor_availability_generation;auto q=engine_api::CompileSblrDdlDropExtensionDescriptor(c,ru,availability);if(!q.ok)return refuse(q.diagnostic.code,q.diagnostic.message_key);result.payload=scratchbird::engine::sblr::EncodeSblrDdlDropExtensionDescriptorV1(q.descriptor);if(result.payload.empty())return refuse("SBLR.OPERAND.INVALID","SDEX_encode_failed");result.accepted=true;return result;}
SessionOperationResult HandleCoordinateClusterCreatePlacementPolicy(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateClusterCreatePlacementPolicyResult);result.response_schema_id=sbps::kSchemaCoordinateClusterCreatePlacementPolicyResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("SB_DIAG_CLUSTER_TXN_UNAVAILABLE","parser_server_ipc.cluster_create_placement_policy_refused","CREATE PLACEMENT POLICY requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateClusterAlterPlacementPolicy(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateClusterAlterPlacementPolicyResult);result.response_schema_id=sbps::kSchemaCoordinateClusterAlterPlacementPolicyResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("SB_DIAG_CLUSTER_TXN_UNAVAILABLE","parser_server_ipc.cluster_alter_placement_policy_refused","ALTER PLACEMENT POLICY requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateClusterDropPlacementPolicy(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateClusterDropPlacementPolicyResult);result.response_schema_id=sbps::kSchemaCoordinateClusterDropPlacementPolicyResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("SB_DIAG_CLUSTER_TXN_UNAVAILABLE","parser_server_ipc.cluster_drop_placement_policy_refused","DROP PLACEMENT POLICY requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateVersionedBranchCreate(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVersionedBranchCreateResult);result.response_schema_id=sbps::kSchemaCoordinateVersionedBranchCreateResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.versioned_branch_create_refused","VERSIONED BRANCH CREATE requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateVersionedBranchDelete(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVersionedBranchDeleteResult);result.response_schema_id=sbps::kSchemaCoordinateVersionedBranchDeleteResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.versioned_branch_delete_refused","VERSIONED BRANCH DELETE requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateVersionedDiff(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVersionedDiffResult);result.response_schema_id=sbps::kSchemaCoordinateVersionedDiffResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.versioned_diff_refused","VERSIONED DIFF requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateVersionedTag(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVersionedTagResult);result.response_schema_id=sbps::kSchemaCoordinateVersionedTagResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.versioned_tag_refused","VERSIONED TAG requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateVersionedRevert(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVersionedRevertResult);result.response_schema_id=sbps::kSchemaCoordinateVersionedRevertResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.versioned_revert_refused","VERSIONED REVERT requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateVersionedReset(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVersionedResetResult);result.response_schema_id=sbps::kSchemaCoordinateVersionedResetResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.versioned_reset_refused","VERSIONED RESET requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateBitemporalAsOf(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&r){SessionOperationResult o;o.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateBitemporalAsOfResult);o.response_schema_id=sbps::kSchemaCoordinateBitemporalAsOfResultV1;o.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;o.session_uuid=r.header.session_uuid;o.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.bitemporal_as_of_refused","BITEMPORAL AS OF requires an admitted cluster route.",{}));return o;}
SessionOperationResult HandleCoordinateVerifiableHistoryProve(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&r){SessionOperationResult o;o.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVerifiableHistoryProveResult);o.response_schema_id=sbps::kSchemaCoordinateVerifiableHistoryProveResultV1;o.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;o.session_uuid=r.header.session_uuid;o.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.verifiable_history_prove_refused","VERIFIABLE HISTORY PROVE requires an admitted cluster route.",{}));return o;}
SessionOperationResult HandleCoordinateVerifyProofDescriptor(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&r){SessionOperationResult o;o.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVerifyProofDescriptorResult);o.response_schema_id=sbps::kSchemaCoordinateVerifyProofDescriptorResultV1;o.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;o.session_uuid=r.header.session_uuid;o.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.verify_proof_descriptor_refused","VERIFY PROOF DESCRIPTOR requires an admitted cluster route.",{}));return o;}
SessionOperationResult HandleCoordinateVersionedMerge(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&r){SessionOperationResult o;o.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVersionedMergeResult);o.response_schema_id=sbps::kSchemaCoordinateVersionedMergeResultV1;o.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;o.session_uuid=r.header.session_uuid;o.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.versioned_merge_refused","VERSIONED MERGE requires an admitted cluster route.",{}));return o;}
SessionOperationResult HandleCoordinateVersionedHashRead(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&r){SessionOperationResult o;o.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVersionedHashReadResult);o.response_schema_id=sbps::kSchemaCoordinateVersionedHashReadResultV1;o.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;o.session_uuid=r.header.session_uuid;o.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.versioned_hash_read_refused","VERSIONED HASH READ requires an admitted cluster route.",{}));return o;}
SessionOperationResult HandleCoordinateVersionedStatusRead(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&r){SessionOperationResult o;o.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateVersionedStatusReadResult);o.response_schema_id=sbps::kSchemaCoordinateVersionedStatusReadResultV1;o.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;o.session_uuid=r.header.session_uuid;o.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.versioned_status_read_refused","VERSIONED STATUS READ requires an admitted cluster route.",{}));return o;}
SessionOperationResult HandleCoordinateAccelLlvmPolicySet(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateAccelLlvmPolicySetResult);result.response_schema_id=sbps::kSchemaCoordinateAccelLlvmPolicySetResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.accel_llvm_policy_set_refused","ACCEL LLVM POLICY SET requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateAccelLlvmCompile(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateAccelLlvmCompileResult);result.response_schema_id=sbps::kSchemaCoordinateAccelLlvmCompileResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.accel_llvm_compile_refused","ACCEL LLVM COMPILE requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateAccelLlvmInspect(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateAccelLlvmInspectResult);result.response_schema_id=sbps::kSchemaCoordinateAccelLlvmInspectResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.accel_llvm_inspect_refused","ACCEL LLVM INSPECT requires an admitted cluster route.",{}));return result;}
SessionOperationResult HandleCoordinateDdlDropOperatorFamily(ServerSessionRegistry*,const HostedEngineState&,const sbps::Frame&request){SessionOperationResult result;result.response_message_type=static_cast<std::uint16_t>(sbps::MessageType::kCoordinateDdlDropOperatorFamilyResult);result.response_schema_id=sbps::kSchemaCoordinateDdlDropOperatorFamilyResultV1;result.frame_flags=sbps::kFlagResponse|sbps::kFlagFinal|sbps::kFlagError;result.session_uuid=request.header.session_uuid;result.diagnostics.push_back(sbps::IpcDiagnostic("CLUSTER.GATEWAY_CLUSTER_FALLTHROUGH_FORBIDDEN","parser_server_ipc.ddl_drop_operator_family_refused","DROP OPERATOR FAMILY requires an admitted cluster/provider route.",{}));return result;}
} // namespace scratchbird::server
