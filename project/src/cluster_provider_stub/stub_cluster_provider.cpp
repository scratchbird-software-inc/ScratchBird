...
internal_api::EngineApiResult InspectClusterProvider(const ClusterProviderRequest& request) {
  // Inspection is cluster-required, not a successful local metadata query.
  // Host descriptor inspection remains available through DescribeClusterProvider.
  // An unbacked compile/link stub has no cluster result or row identity to emit.
