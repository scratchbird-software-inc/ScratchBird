...
UdrResult sbu_sqlite_management_package_request(std::string_view operation_name,
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.SQLITE.MGMT_OPERATION_UNKNOWN",
                      "SQLite management package request names must be registered in the standard donor management ABI.");
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/donor_regression/sqlite/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::sqlite::MessageVectorToJson({})};
}
