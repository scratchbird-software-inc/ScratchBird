...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_sqlite\","
      << "\"package_logical_name\":\"sqlite-v3_53\","
      << "\"package_call_name\":\"sbup_sqlite\","
      << "\"reference_family\":\"sqlite\","
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::sqlite::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::sqlite::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::sqlite::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::sqlite::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_sqlite_parser_support", {}}};
  return {false, {}, scratchbird::parser::sqlite::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::sqlite::Diagnostic> diagnostics{
      {"UDR.SQLITE.SECURITY_DENIED", "ERROR",
       "SQLite parser-support UDR denied the requested operation.",
       "sbu_sqlite_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::sqlite::Diagnostic> diagnostics{
      {"UDR.SQLITE.CONTEXT_MISSING", "ERROR",
       "SQLite parser-support UDR requires engine-supplied trusted context.",
       "sbu_sqlite_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "sqlite_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrStatus SqliteLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuSqlitePackageIdentity) {
    return {false, "UDR.SQLITE.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_sqlite_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_sqlite_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_sqlite_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_sqlite_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_sqlite_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_sqlite_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_sqlite_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_sqlite_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_sqlite_debug_capabilities(input.context_packet));
}
...
UdrResult sbu_sqlite_validate_syntax(std::string_view sql_text,
  ...
  if (profile != "sqlite") {
    return Diagnostic("UDR.SQLITE.PROFILE_MISMATCH",
                      "SQLite parser-support UDR only accepts the sqlite profile.");
  }
  const auto result = scratchbird::parser::sqlite::ParseStatement(sql_text);
...
UdrResult sbu_sqlite_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_sqlite_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"sqlite\",\"statement_family\":\"" +
...
UdrResult sbu_sqlite_install_environment(std::string_view context_packet,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_sqlite_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.SQLITE.INSTALL_MODE_INVALID",
                      "SQLite environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_sqlite_install_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  ...
  return {true,
          "{\"installer\":\"sbu_sqlite_parser_support\","
          ...
          "\"catalog_overlays\":[\"SQLITE_SCHEMA\",\"SQLITE_MASTER\",\"PRAGMA_TABLE_INFO\",\"PRAGMA_INDEX_LIST\",\"SQLITE_SEQUENCE\",\"SQLITE_STAT1\",\"SQLITE_STAT4\"],"
          "\"real_sqlite_file_effects\":false}",
          scratchbird::parser::sqlite::MessageVectorToJson({})};
}

UdrResult sbu_sqlite_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_sqlite_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_sqlite_verify_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"SQLITE_SCHEMA\",\"SQLITE_MASTER\",\"PRAGMA_TABLE_INFO\",\"PRAGMA_INDEX_LIST\",\"SQLITE_SEQUENCE\",\"SQLITE_STAT1\",\"SQLITE_STAT4\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":7,"
...
UdrResult sbu_sqlite_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
    ...
    return Diagnostic("UDR.SQLITE.MGMT_INVENTORY_POLICY_DENIED",
                      "SQLite management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
...
UdrResult sbu_sqlite_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_sqlite_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.SQLITE.MGMT_OPERATION_UNKNOWN",
                      "SQLite management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuSqlitePackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/sqlite/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::sqlite::MessageVectorToJson({})};
}

UdrResult sbu_sqlite_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_SQLITE.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::sqlite::SqlitePackageIdentityJson(),
          scratchbird::parser::sqlite::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_sqlite_package_descriptor() {
  ...
  descriptor.package_uuid = kSbuSqlitePackageIdentity;
  descriptor.package_name = std::string(kSbuSqlitePackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "sqlite-parser-support-enterprise-closure";
  descriptor.binary_hash = "sha256:sbu_sqlite_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.sqlite";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_sqlite_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_sqlite_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_sqlite_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_sqlite_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_sqlite_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_sqlite_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_sqlite_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_sqlite_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_sqlite_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &SqliteLifecycle;
  descriptor.shutdown = &SqliteLifecycle;
  return descriptor;
}
