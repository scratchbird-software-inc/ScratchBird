...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_opensearch\","
      << "\"package_logical_name\":\"opensearch-v3_6\","
      << "\"package_call_name\":\"sbup_opensearch\","
      << "\"reference_family\":\"opensearch\","
      ...
      << "\"real_opensearch_file_effects\":false,"
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token = scratchbird::parser::opensearch::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back({scratchbird::parser::opensearch::TrimAscii(token.substr(0, separator)),
                       scratchbird::parser::opensearch::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::opensearch::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message), "sbu_opensearch_parser_support", {}}};
  return {false, {}, scratchbird::parser::opensearch::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::opensearch::Diagnostic> diagnostics{
      {"UDR.OPENSEARCH.SECURITY_DENIED", "ERROR",
       "OpenSearch parser-support UDR denied the requested operation.",
       "sbu_opensearch_parser_support",
...
UdrResult MissingContext(std::string_view function_name, std::string_view required_fields) {
  const std::vector<scratchbird::parser::opensearch::Diagnostic> diagnostics{
      {"UDR.OPENSEARCH.CONTEXT_MISSING", "ERROR",
       "OpenSearch parser-support UDR requires engine-supplied trusted context.",
       "sbu_opensearch_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid, const CatalogOverlayInstallState& state) {
  ...
  std::ostringstream out;
  out << "opensearch_catalog_overlay_" << hash;
  return out.str();
}
...
std::string OverlayArrayJson() {
  return "[\"OPENSEARCH_INDICES\",\"OPENSEARCH_MAPPINGS\",\"OPENSEARCH_ALIASES\",\"OPENSEARCH_TEMPLATES\",\"OPENSEARCH_PIPELINES\",\"OPENSEARCH_SECURITY\",\"OPENSEARCH_CAT\",\"OPENSEARCH_TASKS\",\"OPENSEARCH_SNAPSHOTS\",\"OPENSEARCH_PLUGINS\"]";
}
...
scratchbird::udr::runtime::UdrStatus OpensearchLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuOpensearchPackageIdentity) {
    return {false, "UDR.OPENSEARCH.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}

scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_verify_environment(input.context_packet));
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_opensearch_validate_syntax(std::string_view sql_text, std::string_view profile) {
  if (profile != "opensearch") {
    return Diagnostic("UDR.OPENSEARCH.PROFILE_MISMATCH", "OpenSearch parser-support UDR only accepts the opensearch profile.");
  }
  const auto result = scratchbird::parser::opensearch::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_opensearch_parse_to_sblr(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_opensearch_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  if (!RequireContextField(context_packet, "resolver", "uuid", "sbu_opensearch_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  const auto result = scratchbird::parser::opensearch::ParseStatement(sql_text);
  return {result.ok, result.sblr_envelope, result.message_vector_json};
}

UdrResult sbu_opensearch_normalize(std::string_view sql_text, std::string_view profile) {
  if (profile != "opensearch") {
    return Diagnostic("UDR.OPENSEARCH.PROFILE_MISMATCH", "OpenSearch parser-support UDR only accepts the opensearch profile.");
  }
  const auto result = scratchbird::parser::opensearch::ParseStatement(sql_text);
...
UdrResult sbu_opensearch_describe_statement(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_opensearch_describe_statement", "engine_context=trusted", failure)) return failure;
  const auto result = scratchbird::parser::opensearch::ParseStatement(sql_text);
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"opensearch\",\"statement_family\":\"" + result.statement_family +
              "\",\"operation_family\":\"" + result.operation_family +
              "\",\"authority_disposition\":\"" + result.authority_disposition + "\"}",
          result.message_vector_json};
}

UdrResult sbu_opensearch_install_environment(std::string_view context_packet, std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_opensearch_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (install_mode != "install" && install_mode != "verify" && install_mode != "force_reinstall" &&
    ...
    return Diagnostic("UDR.OPENSEARCH.INSTALL_MODE_INVALID", "OpenSearch environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_opensearch_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  ...
  return {true,
          "{\"installer\":\"sbu_opensearch_parser_support\","
          ...
          "\"real_opensearch_file_effects\":false}",
          scratchbird::parser::opensearch::MessageVectorToJson({})};
}

UdrResult sbu_opensearch_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_opensearch_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_opensearch_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
...
UdrResult sbu_opensearch_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" && render_policy != "allow_debug_artifacts" && render_policy != "summary") {
    return Diagnostic("UDR.OPENSEARCH.MGMT_INVENTORY_POLICY_DENIED", "OpenSearch management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy), scratchbird::parser::opensearch::MessageVectorToJson({})};
}

UdrResult sbu_opensearch_management_package_request(std::string_view operation_name, std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_opensearch_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.OPENSEARCH.MGMT_OPERATION_UNKNOWN", "OpenSearch management package request names must be registered in the standard compatibility management ABI.");
  }
  if (!RequireTrustedContext(context_packet, function_name, required_context, failure)) return failure;
  if (!RequireContextField(context_packet, "package_uuid", kSbuOpensearchPackageUuid, function_name, required_context, failure)) return failure;
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/opensearch/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::opensearch::MessageVectorToJson({})};
}

UdrResult sbu_opensearch_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_OPENSEARCH.DEBUG_POLICY_DENIED", "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::opensearch::OpensearchPackageIdentityJson(), scratchbird::parser::opensearch::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_opensearch_package_descriptor() {
  ...
  descriptor.package_uuid = kSbuOpensearchPackageIdentity;
  descriptor.package_name = std::string(kSbuOpensearchPackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "opensearch-parser-support-beta-closure";
  descriptor.binary_hash = "sha256:sbu_opensearch_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.opensearch";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_opensearch_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_opensearch_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_opensearch_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_opensearch_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_opensearch_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_opensearch_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_opensearch_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_opensearch_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_opensearch_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &OpensearchLifecycle;
  descriptor.shutdown = &OpensearchLifecycle;
  return descriptor;
}
