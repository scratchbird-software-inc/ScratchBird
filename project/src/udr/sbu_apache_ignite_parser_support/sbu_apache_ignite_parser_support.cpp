...
struct CatalogOverlayInstallState {
  ...
  int overlay_profile_version{136};
};
...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_apache_ignite\","
      << "\"package_logical_name\":\"apache_ignite-v2_17\","
      << "\"package_call_name\":\"sbup_apache_ignite\","
      << "\"reference_family\":\"apache_ignite\","
      ...
      << "\"real_apache_ignite_file_effects\":false,"
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token = scratchbird::parser::apache_ignite::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back({scratchbird::parser::apache_ignite::TrimAscii(token.substr(0, separator)),
                       scratchbird::parser::apache_ignite::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::apache_ignite::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message), "sbu_apache_ignite_parser_support", {}}};
  return {false, {}, scratchbird::parser::apache_ignite::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::apache_ignite::Diagnostic> diagnostics{
      {"UDR.APACHE_IGNITE.SECURITY_DENIED", "ERROR",
       "Apache Ignite parser-support UDR denied the requested operation.",
       "sbu_apache_ignite_parser_support",
...
UdrResult MissingContext(std::string_view function_name, std::string_view required_fields) {
  const std::vector<scratchbird::parser::apache_ignite::Diagnostic> diagnostics{
      {"UDR.APACHE_IGNITE.CONTEXT_MISSING", "ERROR",
       "Apache Ignite parser-support UDR requires engine-supplied trusted context.",
       "sbu_apache_ignite_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid, const CatalogOverlayInstallState& state) {
  ...
  std::ostringstream out;
  out << "apache_ignite_catalog_overlay_" << hash;
  return out.str();
}
...
std::string OverlayArrayJson() {
  return "[\"IGNITE_CACHES\",\"IGNITE_TABLES\",\"IGNITE_INDEXES\",\"IGNITE_SQL_FIELDS\",\"IGNITE_METRICS\",\"IGNITE_SYSTEM_VIEWS\",\"IGNITE_USERS\",\"IGNITE_ROLES\",\"IGNITE_BASELINE\",\"IGNITE_SNAPSHOTS\"]";
}
...
scratchbird::udr::runtime::UdrStatus ApacheIgniteLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuApacheIgnitePackageIdentity) {
    return {false, "UDR.APACHE_IGNITE.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}

scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_apache_ignite_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_apache_ignite_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_apache_ignite_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_apache_ignite_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_apache_ignite_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_apache_ignite_verify_environment(input.context_packet));
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_apache_ignite_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_apache_ignite_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_apache_ignite_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_apache_ignite_validate_syntax(std::string_view sql_text, std::string_view profile) {
  if (profile != "apache_ignite") {
    return Diagnostic("UDR.APACHE_IGNITE.PROFILE_MISMATCH", "Apache Ignite parser-support UDR only accepts the apache_ignite profile.");
  }
  const auto result = scratchbird::parser::apache_ignite::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_apache_ignite_parse_to_sblr(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_apache_ignite_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  if (!RequireContextField(context_packet, "resolver", "uuid", "sbu_apache_ignite_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  const auto result = scratchbird::parser::apache_ignite::ParseStatement(sql_text);
  return {result.ok, result.sblr_envelope, result.message_vector_json};
}

UdrResult sbu_apache_ignite_normalize(std::string_view sql_text, std::string_view profile) {
  if (profile != "apache_ignite") {
    return Diagnostic("UDR.APACHE_IGNITE.PROFILE_MISMATCH", "Apache Ignite parser-support UDR only accepts the apache_ignite profile.");
  }
  const auto result = scratchbird::parser::apache_ignite::ParseStatement(sql_text);
...
UdrResult sbu_apache_ignite_describe_statement(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_apache_ignite_describe_statement", "engine_context=trusted", failure)) return failure;
  const auto result = scratchbird::parser::apache_ignite::ParseStatement(sql_text);
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"apache_ignite\",\"statement_family\":\"" + result.statement_family +
              "\",\"operation_family\":\"" + result.operation_family +
              "\",\"authority_disposition\":\"" + result.authority_disposition + "\"}",
          result.message_vector_json};
}

UdrResult sbu_apache_ignite_install_environment(std::string_view context_packet, std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_apache_ignite_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (install_mode != "install" && install_mode != "verify" && install_mode != "force_reinstall" &&
    ...
    return Diagnostic("UDR.APACHE_IGNITE.INSTALL_MODE_INVALID", "Apache Ignite environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_apache_ignite_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  ...
  return {true,
          "{\"installer\":\"sbu_apache_ignite_parser_support\","
          "\"role\":\"environment_installer\","
          "\"dialect\":\"apache_ignite\","
          ...
          "\"real_apache_ignite_file_effects\":false}",
          scratchbird::parser::apache_ignite::MessageVectorToJson({})};
}

UdrResult sbu_apache_ignite_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_apache_ignite_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_apache_ignite_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
...
UdrResult sbu_apache_ignite_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" && render_policy != "allow_debug_artifacts" && render_policy != "summary") {
    return Diagnostic("UDR.APACHE_IGNITE.MGMT_INVENTORY_POLICY_DENIED", "Apache Ignite management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy), scratchbird::parser::apache_ignite::MessageVectorToJson({})};
}

UdrResult sbu_apache_ignite_management_package_request(std::string_view operation_name, std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_apache_ignite_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.APACHE_IGNITE.MGMT_OPERATION_UNKNOWN", "Apache Ignite management package request names must be registered in the standard compatibility management ABI.");
  }
  if (!RequireTrustedContext(context_packet, function_name, required_context, failure)) return failure;
  if (!RequireContextField(context_packet, "package_uuid", kSbuApacheIgnitePackageUuid, function_name, required_context, failure)) return failure;
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/apache_ignite/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::apache_ignite::MessageVectorToJson({})};
}

UdrResult sbu_apache_ignite_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_APACHE_IGNITE.DEBUG_POLICY_DENIED", "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::apache_ignite::ApacheIgnitePackageIdentityJson(), scratchbird::parser::apache_ignite::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_apache_ignite_package_descriptor() {
  ...
  descriptor.package_uuid = kSbuApacheIgnitePackageIdentity;
  descriptor.package_name = std::string(kSbuApacheIgnitePackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "apache_ignite-parser-support-beta-closure";
  descriptor.binary_hash = "sha256:sbu_apache_ignite_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.apache_ignite";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_apache_ignite_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_apache_ignite_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_apache_ignite_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_apache_ignite_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_apache_ignite_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_apache_ignite_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_apache_ignite_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_apache_ignite_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_apache_ignite_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &ApacheIgniteLifecycle;
  descriptor.shutdown = &ApacheIgniteLifecycle;
  return descriptor;
}
