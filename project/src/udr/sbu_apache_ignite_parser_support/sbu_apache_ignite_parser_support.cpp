...
UdrResult sbu_apache_ignite_management_package_request(std::string_view operation_name, std::string_view context_packet) {
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.APACHE_IGNITE.MGMT_OPERATION_UNKNOWN", "Apache Ignite management package request names must be registered in the standard donor management ABI.");
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/donor_regression/apache_ignite/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::apache_ignite::MessageVectorToJson({})};
}
