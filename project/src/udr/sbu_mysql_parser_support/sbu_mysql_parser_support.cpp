...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_mysql\","
      << "\"package_logical_name\":\"mysql-v8_4\","
      << "\"package_call_name\":\"sbup_mysql\","
      << "\"reference_family\":\"mysql\","
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::mysql::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::mysql::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::mysql::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::mysql::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_mysql_parser_support", {}}};
  return {false, {}, scratchbird::parser::mysql::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::mysql::Diagnostic> diagnostics{
      {"UDR.MYSQL.SECURITY_DENIED", "ERROR",
       "MySQL parser-support UDR denied the requested operation.",
       "sbu_mysql_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::mysql::Diagnostic> diagnostics{
      {"UDR.MYSQL.CONTEXT_MISSING", "ERROR",
       "MySQL parser-support UDR requires engine-supplied trusted context.",
       "sbu_mysql_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "mysql_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrStatus MysqlLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuMysqlPackageIdentity) {
    return {false, "UDR.MYSQL.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_mysql_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mysql_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mysql_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mysql_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mysql_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mysql_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mysql_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_mysql_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mysql_debug_capabilities(input.context_packet));
}
...
UdrResult sbu_mysql_validate_syntax(std::string_view sql_text,
  ...
  if (profile != "mysql") {
    return Diagnostic("UDR.MYSQL.PROFILE_MISMATCH",
                      "MySQL parser-support UDR only accepts the mysql profile.");
  }
  const auto result = scratchbird::parser::mysql::ParseStatement(sql_text);
...
UdrResult sbu_mysql_parse_to_sblr(std::string_view sql_text,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mysql_parse_to_sblr",
...
UdrResult sbu_mysql_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mysql_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"mysql\",\"statement_family\":\"" +
...
UdrResult sbu_mysql_install_environment(std::string_view context_packet,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mysql_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.MYSQL.INSTALL_MODE_INVALID",
                      "MySQL environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_mysql_install_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  ...
  return {true,
          "{\"installer\":\"sbu_mysql_parser_support\","
          ...
          "\"catalog_overlays\":[\"INFORMATION_SCHEMA\",\"MYSQL\",\"PERFORMANCE_SCHEMA\",\"SYS\"],"
          "\"real_mysql_file_effects\":false}",
          scratchbird::parser::mysql::MessageVectorToJson({})};
}

UdrResult sbu_mysql_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mysql_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_mysql_verify_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"INFORMATION_SCHEMA\",\"MYSQL\",\"PERFORMANCE_SCHEMA\",\"SYS\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":8,"
...
UdrResult sbu_mysql_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
    ...
    return Diagnostic("UDR.MYSQL.MGMT_INVENTORY_POLICY_DENIED",
                      "MySQL management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
...
UdrResult sbu_mysql_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_mysql_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.MYSQL.MGMT_OPERATION_UNKNOWN",
                      "MySQL management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuMysqlPackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/mysql/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::mysql::MessageVectorToJson({})};
}

UdrResult sbu_mysql_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_MYSQL.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::mysql::MysqlPackageIdentityJson(),
          scratchbird::parser::mysql::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_mysql_package_descriptor() {
  ...
  descriptor.package_uuid = kSbuMysqlPackageIdentity;
  descriptor.package_name = std::string(kSbuMysqlPackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "mysql-parser-support-enterprise-closure";
  descriptor.binary_hash = "sha256:sbu_mysql_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.mysql";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_mysql_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_mysql_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_mysql_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_mysql_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_mysql_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_mysql_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_mysql_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_mysql_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_mysql_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &MysqlLifecycle;
  descriptor.shutdown = &MysqlLifecycle;
  return descriptor;
}
