...
UdrResult sbu_neo4j_management_package_request(std::string_view operation_name, std::string_view context_packet) {
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.NEO4J.MGMT_OPERATION_UNKNOWN", "Neo4j management package request names must be registered in the standard donor management ABI.");
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/donor_regression/neo4j/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::neo4j::MessageVectorToJson({})};
}
