...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_opensearch_sql_ppl\","
      << "\"package_logical_name\":\"opensearch_sql_ppl-v3_6\","
      << "\"package_call_name\":\"sbup_opensearch_sql_ppl\","
      << "\"reference_family\":\"opensearch_sql_ppl\","
      ...
      << "\"real_opensearch_sql_ppl_file_effects\":false,"
      << "\"inventory_detail\":\""
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::opensearch_sql_ppl::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::opensearch_sql_ppl::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::opensearch_sql_ppl::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::opensearch_sql_ppl::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_opensearch_sql_ppl_parser_support", {}}};
  return {false, {}, scratchbird::parser::opensearch_sql_ppl::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::opensearch_sql_ppl::Diagnostic> diagnostics{
      {"UDR.OPENSEARCH_SQL_PPL.SECURITY_DENIED", "ERROR",
       "OpenSearch SQL/PPL parser-support UDR denied the requested operation.",
       "sbu_opensearch_sql_ppl_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::opensearch_sql_ppl::Diagnostic> diagnostics{
      {"UDR.OPENSEARCH_SQL_PPL.CONTEXT_MISSING", "ERROR",
       "OpenSearch SQL/PPL parser-support UDR requires engine-supplied trusted context.",
       "sbu_opensearch_sql_ppl_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "opensearch_sql_ppl_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_opensearch_sql_ppl_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_sql_ppl_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_sql_ppl_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_sql_ppl_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_sql_ppl_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_sql_ppl_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_sql_ppl_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_opensearch_sql_ppl_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_opensearch_sql_ppl_debug_capabilities(input.context_packet));
}
...
UdrResult sbu_opensearch_sql_ppl_validate_syntax(std::string_view sql_text,
  ...
  if (profile != "opensearch_sql_ppl") {
    return Diagnostic("UDR.OPENSEARCH_SQL_PPL.PROFILE_MISMATCH",
                      "OpenSearch SQL/PPL parser-support UDR only accepts the opensearch_sql_ppl profile.");
  }
  const auto result = scratchbird::parser::opensearch_sql_ppl::ParseStatement(sql_text);
...
UdrResult sbu_opensearch_sql_ppl_parse_to_sblr(std::string_view sql_text,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_opensearch_sql_ppl_parse_to_sblr",
...
UdrResult sbu_opensearch_sql_ppl_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_opensearch_sql_ppl_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"opensearch_sql_ppl\",\"statement_family\":\"" +
...
UdrResult sbu_opensearch_sql_ppl_install_environment(std::string_view context_packet,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_opensearch_sql_ppl_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.OPENSEARCH_SQL_PPL.INSTALL_MODE_INVALID",
                      "OpenSearch SQL/PPL environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  ...
  return {true,
          "{\"installer\":\"sbu_opensearch_sql_ppl_parser_support\","
          "\"role\":\"environment_installer\","
          "\"dialect\":\"opensearch_sql_ppl\","
          ...
          "\"catalog_overlays\":[\"DATASOURCES\",\"INDEXES\",\"MAPPINGS\",\"FIELDS\",\"PPL_PIPELINES\",\"SQL_PLUGIN\",\"PPL_PLUGIN\",\"ML_MODELS\",\"SEARCH_PIPELINES\"],"
          "\"real_opensearch_sql_ppl_file_effects\":false}",
          scratchbird::parser::opensearch_sql_ppl::MessageVectorToJson({})};
}

UdrResult sbu_opensearch_sql_ppl_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_opensearch_sql_ppl_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"DATASOURCES\",\"INDEXES\",\"MAPPINGS\",\"FIELDS\",\"PPL_PIPELINES\",\"SQL_PLUGIN\",\"PPL_PLUGIN\",\"ML_MODELS\",\"SEARCH_PIPELINES\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":9,"
...
UdrResult sbu_opensearch_sql_ppl_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
    ...
    return Diagnostic("UDR.OPENSEARCH_SQL_PPL.MGMT_INVENTORY_POLICY_DENIED",
                      "OpenSearch SQL/PPL management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
...
UdrResult sbu_opensearch_sql_ppl_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_opensearch_sql_ppl_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.OPENSEARCH_SQL_PPL.MGMT_OPERATION_UNKNOWN",
                      "OpenSearch SQL/PPL management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuOpensearchSqlPplPackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/opensearch_sql_ppl/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::opensearch_sql_ppl::MessageVectorToJson({})};
}

UdrResult sbu_opensearch_sql_ppl_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_OPENSEARCH_SQL_PPL.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::opensearch_sql_ppl::OpensearchSqlPplPackageIdentityJson(),
          scratchbird::parser::opensearch_sql_ppl::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_opensearch_sql_ppl_package_descriptor() {
  ...
  descriptor.binary_hash = "sha256:sbu_opensearch_sql_ppl_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.opensearch_sql_ppl";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_opensearch_sql_ppl_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_opensearch_sql_ppl_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_opensearch_sql_ppl_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_opensearch_sql_ppl_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_opensearch_sql_ppl_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_opensearch_sql_ppl_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_opensearch_sql_ppl_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_opensearch_sql_ppl_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_opensearch_sql_ppl_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &OpensearchSqlPplLifecycle;
  descriptor.shutdown = &OpensearchSqlPplLifecycle;
  return descriptor;
}
