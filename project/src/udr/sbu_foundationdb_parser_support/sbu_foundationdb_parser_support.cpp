...
UdrResult sbu_foundationdb_management_package_request(std::string_view operation_name, std::string_view context_packet) {
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.FOUNDATIONDB.MGMT_OPERATION_UNKNOWN", "FoundationDB management package request names must be registered in the standard donor management ABI.");
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/donor_regression/foundationdb/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::foundationdb::MessageVectorToJson({})};
}
