...
namespace scratchbird::udr::foundationdb_parser_support {
namespace {
...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_foundationdb\","
      << "\"package_logical_name\":\"foundationdb-v7_3\","
      << "\"package_call_name\":\"sbup_foundationdb\","
      << "\"reference_family\":\"foundationdb\","
      ...
      << "\"real_foundationdb_file_effects\":false,"
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token = scratchbird::parser::foundationdb::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back({scratchbird::parser::foundationdb::TrimAscii(token.substr(0, separator)),
                       scratchbird::parser::foundationdb::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::foundationdb::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message), "sbu_foundationdb_parser_support", {}}};
  return {false, {}, scratchbird::parser::foundationdb::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::foundationdb::Diagnostic> diagnostics{
      {"UDR.FOUNDATIONDB.SECURITY_DENIED", "ERROR",
       "FoundationDB parser-support UDR denied the requested operation.",
       "sbu_foundationdb_parser_support",
...
UdrResult MissingContext(std::string_view function_name, std::string_view required_fields) {
  const std::vector<scratchbird::parser::foundationdb::Diagnostic> diagnostics{
      {"UDR.FOUNDATIONDB.CONTEXT_MISSING", "ERROR",
       "FoundationDB parser-support UDR requires engine-supplied trusted context.",
       "sbu_foundationdb_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid, const CatalogOverlayInstallState& state) {
  ...
  std::ostringstream out;
  out << "foundationdb_catalog_overlay_" << hash;
  return out.str();
}
...
std::string OverlayArrayJson() {
  return "[\"FOUNDATIONDB_STATUS\",\"FOUNDATIONDB_DIRECTORIES\",\"FOUNDATIONDB_SUBSPACES\",\"FOUNDATIONDB_TENANTS\",\"FOUNDATIONDB_WATCHES\",\"FOUNDATIONDB_VERSIONS\",\"FOUNDATIONDB_CONFLICT_RANGES\",\"FOUNDATIONDB_METRICS\",\"FOUNDATIONDB_CLUSTER_STATUS\"]";
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_foundationdb_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_foundationdb_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_foundationdb_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_foundationdb_validate_syntax(std::string_view sql_text, std::string_view profile) {
  if (profile != "foundationdb") {
    return Diagnostic("UDR.FOUNDATIONDB.PROFILE_MISMATCH", "FoundationDB parser-support UDR only accepts the foundationdb profile.");
  }
  const auto result = scratchbird::parser::foundationdb::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_foundationdb_parse_to_sblr(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_foundationdb_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  if (!RequireContextField(context_packet, "resolver", "uuid", "sbu_foundationdb_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  const auto result = scratchbird::parser::foundationdb::ParseStatement(sql_text);
  return {result.ok, result.sblr_envelope, result.message_vector_json};
}

UdrResult sbu_foundationdb_normalize(std::string_view sql_text, std::string_view profile) {
  if (profile != "foundationdb") {
    return Diagnostic("UDR.FOUNDATIONDB.PROFILE_MISMATCH", "FoundationDB parser-support UDR only accepts the foundationdb profile.");
  }
  const auto result = scratchbird::parser::foundationdb::ParseStatement(sql_text);
...
UdrResult sbu_foundationdb_describe_statement(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_foundationdb_describe_statement", "engine_context=trusted", failure)) return failure;
  const auto result = scratchbird::parser::foundationdb::ParseStatement(sql_text);
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"foundationdb\",\"statement_family\":\"" + result.statement_family +
              "\",\"operation_family\":\"" + result.operation_family +
              "\",\"authority_disposition\":\"" + result.authority_disposition + "\"}",
          result.message_vector_json};
}

UdrResult sbu_foundationdb_install_environment(std::string_view context_packet, std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_foundationdb_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (install_mode != "install" && install_mode != "verify" && install_mode != "force_reinstall" &&
    ...
    return Diagnostic("UDR.FOUNDATIONDB.INSTALL_MODE_INVALID", "FoundationDB environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_foundationdb_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  ...
  return {true,
          "{\"installer\":\"sbu_foundationdb_parser_support\","
          "\"role\":\"environment_installer\","
          "\"dialect\":\"foundationdb\","
          ...
          "\"real_foundationdb_file_effects\":false}",
          scratchbird::parser::foundationdb::MessageVectorToJson({})};
}

UdrResult sbu_foundationdb_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_foundationdb_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_foundationdb_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
...
UdrResult sbu_foundationdb_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" && render_policy != "allow_debug_artifacts" && render_policy != "summary") {
    return Diagnostic("UDR.FOUNDATIONDB.MGMT_INVENTORY_POLICY_DENIED", "FoundationDB management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy), scratchbird::parser::foundationdb::MessageVectorToJson({})};
}

UdrResult sbu_foundationdb_management_package_request(std::string_view operation_name, std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_foundationdb_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.FOUNDATIONDB.MGMT_OPERATION_UNKNOWN", "FoundationDB management package request names must be registered in the standard compatibility management ABI.");
  }
  if (!RequireTrustedContext(context_packet, function_name, required_context, failure)) return failure;
  if (!RequireContextField(context_packet, "package_uuid", kSbuFoundationdbPackageUuid, function_name, required_context, failure)) return failure;
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/foundationdb/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::foundationdb::MessageVectorToJson({})};
}

UdrResult sbu_foundationdb_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_FOUNDATIONDB.DEBUG_POLICY_DENIED", "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::foundationdb::FoundationdbPackageIdentityJson(), scratchbird::parser::foundationdb::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_foundationdb_package_descriptor() {
  ...
  descriptor.binary_hash = "sha256:sbu_foundationdb_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.foundationdb";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_foundationdb_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_foundationdb_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_foundationdb_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_foundationdb_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_foundationdb_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_foundationdb_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_foundationdb_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_foundationdb_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_foundationdb_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &FoundationdbLifecycle;
  descriptor.shutdown = &FoundationdbLifecycle;
  return descriptor;
}
