...
UdrResult sbu_postgresql_management_package_request(std::string_view operation_name,
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.POSTGRESQL.MGMT_OPERATION_UNKNOWN",
                      "PostgreSQL management package request names must be registered in the standard donor management ABI.");
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/donor_regression/postgresql/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::postgresql::MessageVectorToJson({})};
}
