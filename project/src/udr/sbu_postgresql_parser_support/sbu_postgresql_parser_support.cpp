...
namespace scratchbird::udr::postgresql_parser_support {
namespace {
...
struct CatalogOverlayInstallState {
  ...
  int overlay_profile_version{183};
};
...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_postgresql\","
      << "\"package_logical_name\":\"postgresql-v16\","
      << "\"package_call_name\":\"sbup_postgresql\","
      << "\"reference_family\":\"postgresql\","
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::postgresql::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::postgresql::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::postgresql::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::postgresql::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_postgresql_parser_support", {}}};
  return {false, {}, scratchbird::parser::postgresql::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::postgresql::Diagnostic> diagnostics{
      {"UDR.POSTGRESQL.SECURITY_DENIED", "ERROR",
       "PostgreSQL parser-support UDR denied the requested operation.",
       "sbu_postgresql_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::postgresql::Diagnostic> diagnostics{
      {"UDR.POSTGRESQL.CONTEXT_MISSING", "ERROR",
       "PostgreSQL parser-support UDR requires engine-supplied trusted context.",
       "sbu_postgresql_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  hash ^= static_cast<std::uint32_t>(state.overlay_revision * 173u);
  hash ^= static_cast<std::uint32_t>(state.overlay_profile_version * 23u);
  hash ^= state.installed ? 0x7f4a7c15u : 0x1b873593u;
  std::ostringstream out;
  out << "postgresql_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_postgresql_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_postgresql_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_postgresql_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_postgresql_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_postgresql_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_postgresql_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_postgresql_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_postgresql_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_postgresql_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_postgresql_validate_syntax(std::string_view sql_text,
                                         std::string_view profile) {
  if (profile != "postgresql") {
    return Diagnostic("UDR.POSTGRESQL.PROFILE_MISMATCH",
                      "PostgreSQL parser-support UDR only accepts the postgresql profile.");
  }
  const auto result = scratchbird::parser::postgresql::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_postgresql_parse_to_sblr(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_postgresql_parse_to_sblr",
...
UdrResult sbu_postgresql_normalize(std::string_view sql_text,
                                   std::string_view profile) {
  if (profile != "postgresql") {
    return Diagnostic("UDR.POSTGRESQL.PROFILE_MISMATCH",
                      "PostgreSQL parser-support UDR only accepts the postgresql profile.");
...
UdrResult sbu_postgresql_describe_statement(std::string_view sql_text,
                                            std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_postgresql_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"postgresql\",\"statement_family\":\"" +
...
UdrResult sbu_postgresql_install_environment(std::string_view context_packet,
                                             std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_postgresql_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.POSTGRESQL.INSTALL_MODE_INVALID",
                      "PostgreSQL environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_postgresql_install_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  ...
  return {true,
          "{\"installer\":\"sbu_postgresql_parser_support\","
          ...
          "\"catalog_overlays\":[\"PG_CATALOG\",\"INFORMATION_SCHEMA\",\"PG_STAT\",\"PG_AUTH\",\"PG_REPLICATION\"],"
          "\"real_postgresql_file_effects\":false}",
          scratchbird::parser::postgresql::MessageVectorToJson({})};
}

UdrResult sbu_postgresql_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_postgresql_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_postgresql_verify_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"PG_CATALOG\",\"INFORMATION_SCHEMA\",\"PG_STAT\",\"PG_AUTH\",\"PG_REPLICATION\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":9,"
...
UdrResult sbu_postgresql_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
      render_policy != "allow_debug_artifacts" &&
      render_policy != "summary") {
    return Diagnostic("UDR.POSTGRESQL.MGMT_INVENTORY_POLICY_DENIED",
                      "PostgreSQL management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy),
          scratchbird::parser::postgresql::MessageVectorToJson({})};
}

UdrResult sbu_postgresql_management_package_request(std::string_view operation_name,
                                                    std::string_view context_packet) {
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.POSTGRESQL.MGMT_OPERATION_UNKNOWN",
                      "PostgreSQL management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuPostgresqlPackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/postgresql/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::postgresql::MessageVectorToJson({})};
}

UdrResult sbu_postgresql_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_POSTGRESQL.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::postgresql::PostgresqlPackageIdentityJson(),
          scratchbird::parser::postgresql::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_postgresql_package_descriptor() {
  ...
  descriptor.binary_hash = "sha256:sbu_postgresql_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.postgresql";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_postgresql_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_postgresql_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_postgresql_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_postgresql_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_postgresql_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_postgresql_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_postgresql_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_postgresql_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_postgresql_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &PostgresqlLifecycle;
  descriptor.shutdown = &PostgresqlLifecycle;
  return descriptor;
}
