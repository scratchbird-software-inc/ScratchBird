...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_cockroachdb\","
      << "\"package_logical_name\":\"cockroachdb-v26_1\","
      << "\"package_call_name\":\"sbup_cockroachdb\","
      << "\"reference_family\":\"cockroachdb\","
      ...
      << "\"real_cockroachdb_file_effects\":false,"
      << "\"inventory_detail\":\""
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::cockroachdb::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::cockroachdb::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::cockroachdb::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::cockroachdb::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_cockroachdb_parser_support", {}}};
  return {false, {}, scratchbird::parser::cockroachdb::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::cockroachdb::Diagnostic> diagnostics{
      {"UDR.COCKROACHDB.SECURITY_DENIED", "ERROR",
       "CockroachDB parser-support UDR denied the requested operation.",
       "sbu_cockroachdb_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::cockroachdb::Diagnostic> diagnostics{
      {"UDR.COCKROACHDB.CONTEXT_MISSING", "ERROR",
       "CockroachDB parser-support UDR requires engine-supplied trusted context.",
       "sbu_cockroachdb_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "cockroachdb_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrStatus CockroachdbLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuCockroachdbPackageIdentity) {
    return {false, "UDR.COCKROACHDB.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_cockroachdb_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_cockroachdb_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_cockroachdb_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_cockroachdb_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_cockroachdb_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_cockroachdb_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_cockroachdb_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_cockroachdb_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_cockroachdb_debug_capabilities(input.context_packet));
}
...
UdrResult sbu_cockroachdb_validate_syntax(std::string_view sql_text,
  ...
  if (profile != "cockroachdb") {
    return Diagnostic("UDR.COCKROACHDB.PROFILE_MISMATCH",
                      "CockroachDB parser-support UDR only accepts the cockroachdb profile.");
  }
  const auto result = scratchbird::parser::cockroachdb::ParseStatement(sql_text);
...
UdrResult sbu_cockroachdb_parse_to_sblr(std::string_view sql_text,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_cockroachdb_parse_to_sblr",
                             "engine_context=trusted;resolver=uuid", failure)) {
    return failure;
  }
  if (!RequireContextField(context_packet, "resolver", "uuid",
                           "sbu_cockroachdb_parse_to_sblr",
                           "engine_context=trusted;resolver=uuid", failure)) {
    return failure;
  }
  const auto result = scratchbird::parser::cockroachdb::ParseStatement(sql_text);
...
UdrResult sbu_cockroachdb_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_cockroachdb_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"cockroachdb\",\"statement_family\":\"" +
...
UdrResult sbu_cockroachdb_install_environment(std::string_view context_packet,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_cockroachdb_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.COCKROACHDB.INSTALL_MODE_INVALID",
                      "CockroachDB environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  ...
  return {true,
          "{\"installer\":\"sbu_cockroachdb_parser_support\","
          ...
          "\"catalog_overlays\":[\"CRDB_INTERNAL\",\"INFORMATION_SCHEMA\",\"SYSTEM\",\"SHOW RANGES\",\"SHOW JOBS\",\"CLUSTER_SETTINGS\",\"ZONE_CONFIGS\",\"TABLE_STATISTICS\",\"USERS\",\"ROLE_MEMBERS\"],"
          "\"real_cockroachdb_file_effects\":false}",
          scratchbird::parser::cockroachdb::MessageVectorToJson({})};
}

UdrResult sbu_cockroachdb_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_cockroachdb_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"CRDB_INTERNAL\",\"INFORMATION_SCHEMA\",\"SYSTEM\",\"SHOW RANGES\",\"SHOW JOBS\",\"CLUSTER_SETTINGS\",\"ZONE_CONFIGS\",\"TABLE_STATISTICS\",\"USERS\",\"ROLE_MEMBERS\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":9,"
...
UdrResult sbu_cockroachdb_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
    ...
    return Diagnostic("UDR.COCKROACHDB.MGMT_INVENTORY_POLICY_DENIED",
                      "CockroachDB management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
...
UdrResult sbu_cockroachdb_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_cockroachdb_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.COCKROACHDB.MGMT_OPERATION_UNKNOWN",
                      "CockroachDB management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuCockroachdbPackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/cockroachdb/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::cockroachdb::MessageVectorToJson({})};
}

UdrResult sbu_cockroachdb_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_COCKROACHDB.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::cockroachdb::CockroachdbPackageIdentityJson(),
          scratchbird::parser::cockroachdb::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_cockroachdb_package_descriptor() {
  ...
  descriptor.package_uuid = kSbuCockroachdbPackageIdentity;
  descriptor.package_name = std::string(kSbuCockroachdbPackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "cockroachdb-parser-support-enterprise-closure";
  descriptor.binary_hash = "sha256:sbu_cockroachdb_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.cockroachdb";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_cockroachdb_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_cockroachdb_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_cockroachdb_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_cockroachdb_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_cockroachdb_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_cockroachdb_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_cockroachdb_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_cockroachdb_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_cockroachdb_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &CockroachdbLifecycle;
  descriptor.shutdown = &CockroachdbLifecycle;
  return descriptor;
}
