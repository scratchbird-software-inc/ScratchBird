...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_tikv\","
      << "\"package_logical_name\":\"tikv-v8_5\","
      << "\"package_call_name\":\"sbup_tikv\","
      << "\"reference_family\":\"tikv\","
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token = scratchbird::parser::tikv::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back({scratchbird::parser::tikv::TrimAscii(token.substr(0, separator)),
                       scratchbird::parser::tikv::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::tikv::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message), "sbu_tikv_parser_support", {}}};
  return {false, {}, scratchbird::parser::tikv::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::tikv::Diagnostic> diagnostics{
      {"UDR.TIKV.SECURITY_DENIED", "ERROR",
       "TiKV parser-support UDR denied the requested operation.",
       "sbu_tikv_parser_support",
...
UdrResult MissingContext(std::string_view function_name, std::string_view required_fields) {
  const std::vector<scratchbird::parser::tikv::Diagnostic> diagnostics{
      {"UDR.TIKV.CONTEXT_MISSING", "ERROR",
       "TiKV parser-support UDR requires engine-supplied trusted context.",
       "sbu_tikv_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid, const CatalogOverlayInstallState& state) {
  ...
  std::ostringstream out;
  out << "tikv_catalog_overlay_" << hash;
  return out.str();
}
...
std::string OverlayArrayJson() {
  return "[\"TIKV_REGIONS\",\"TIKV_STORES\",\"TIKV_LOCKS\",\"TIKV_MVCC\",\"TIKV_RAW_CF\",\"TIKV_SCHEDULER\",\"TIKV_COPROCESSOR\",\"TIKV_IMPORT_JOBS\"]";
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tikv_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tikv_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tikv_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_tikv_validate_syntax(std::string_view sql_text, std::string_view profile) {
  if (profile != "tikv") {
    return Diagnostic("UDR.TIKV.PROFILE_MISMATCH", "TiKV parser-support UDR only accepts the tikv profile.");
  }
  const auto result = scratchbird::parser::tikv::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_tikv_parse_to_sblr(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_tikv_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  if (!RequireContextField(context_packet, "resolver", "uuid", "sbu_tikv_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  const auto result = scratchbird::parser::tikv::ParseStatement(sql_text);
  return {result.ok, result.sblr_envelope, result.message_vector_json};
}

UdrResult sbu_tikv_normalize(std::string_view sql_text, std::string_view profile) {
  if (profile != "tikv") {
    return Diagnostic("UDR.TIKV.PROFILE_MISMATCH", "TiKV parser-support UDR only accepts the tikv profile.");
  }
  const auto result = scratchbird::parser::tikv::ParseStatement(sql_text);
...
UdrResult sbu_tikv_describe_statement(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_tikv_describe_statement", "engine_context=trusted", failure)) return failure;
  const auto result = scratchbird::parser::tikv::ParseStatement(sql_text);
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"tikv\",\"statement_family\":\"" + result.statement_family +
              "\",\"operation_family\":\"" + result.operation_family +
              "\",\"authority_disposition\":\"" + result.authority_disposition + "\"}",
          result.message_vector_json};
}

UdrResult sbu_tikv_install_environment(std::string_view context_packet, std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_tikv_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (install_mode != "install" && install_mode != "verify" && install_mode != "force_reinstall" &&
    ...
    return Diagnostic("UDR.TIKV.INSTALL_MODE_INVALID", "TiKV environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_tikv_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  ...
  return {true,
          "{\"installer\":\"sbu_tikv_parser_support\","
          ...
          "\"real_tikv_file_effects\":false}",
          scratchbird::parser::tikv::MessageVectorToJson({})};
}

UdrResult sbu_tikv_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_tikv_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_tikv_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
...
UdrResult sbu_tikv_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" && render_policy != "allow_debug_artifacts" && render_policy != "summary") {
    return Diagnostic("UDR.TIKV.MGMT_INVENTORY_POLICY_DENIED", "TiKV management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy), scratchbird::parser::tikv::MessageVectorToJson({})};
}

UdrResult sbu_tikv_management_package_request(std::string_view operation_name, std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_tikv_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.TIKV.MGMT_OPERATION_UNKNOWN", "TiKV management package request names must be registered in the standard compatibility management ABI.");
  }
  if (!RequireTrustedContext(context_packet, function_name, required_context, failure)) return failure;
  if (!RequireContextField(context_packet, "package_uuid", kSbuTikvPackageUuid, function_name, required_context, failure)) return failure;
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/tikv/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::tikv::MessageVectorToJson({})};
}

UdrResult sbu_tikv_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_TIKV.DEBUG_POLICY_DENIED", "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::tikv::TikvPackageIdentityJson(), scratchbird::parser::tikv::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_tikv_package_descriptor() {
  ...
  descriptor.binary_hash = "sha256:sbu_tikv_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.tikv";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_tikv_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_tikv_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_tikv_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_tikv_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_tikv_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_tikv_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_tikv_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_tikv_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_tikv_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &TikvLifecycle;
  descriptor.shutdown = &TikvLifecycle;
  return descriptor;
}
