...
UdrResult sbu_vitess_management_package_request(std::string_view operation_name,
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.VITESS.MGMT_OPERATION_UNKNOWN",
                      "Vitess management package request names must be registered in the standard donor management ABI.");
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/donor_regression/vitess/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::vitess::MessageVectorToJson({})};
}
