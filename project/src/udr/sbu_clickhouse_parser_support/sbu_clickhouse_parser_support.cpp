...
UdrResult sbu_clickhouse_management_package_request(std::string_view operation_name,
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.CLICKHOUSE.MGMT_OPERATION_UNKNOWN",
                      "ClickHouse management package request names must be registered in the standard donor management ABI.");
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/donor_regression/clickhouse/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::clickhouse::MessageVectorToJson({})};
}
