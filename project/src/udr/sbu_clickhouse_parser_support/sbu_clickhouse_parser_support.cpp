...
namespace scratchbird::udr::clickhouse_parser_support {
namespace {
...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_clickhouse\","
      << "\"package_logical_name\":\"clickhouse-v25_12\","
      << "\"package_call_name\":\"sbup_clickhouse\","
      << "\"reference_family\":\"clickhouse\","
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::clickhouse::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::clickhouse::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::clickhouse::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::clickhouse::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_clickhouse_parser_support", {}}};
  return {false, {}, scratchbird::parser::clickhouse::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::clickhouse::Diagnostic> diagnostics{
      {"UDR.CLICKHOUSE.SECURITY_DENIED", "ERROR",
       "ClickHouse parser-support UDR denied the requested operation.",
       "sbu_clickhouse_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::clickhouse::Diagnostic> diagnostics{
      {"UDR.CLICKHOUSE.CONTEXT_MISSING", "ERROR",
       "ClickHouse parser-support UDR requires engine-supplied trusted context.",
       "sbu_clickhouse_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "clickhouse_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrStatus ClickhouseLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuClickhousePackageIdentity) {
    return {false, "UDR.CLICKHOUSE.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_clickhouse_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_clickhouse_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_clickhouse_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_clickhouse_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_clickhouse_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_clickhouse_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_clickhouse_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_clickhouse_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_clickhouse_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_clickhouse_validate_syntax(std::string_view sql_text,
                                    std::string_view profile) {
  if (profile != "clickhouse") {
    return Diagnostic("UDR.CLICKHOUSE.PROFILE_MISMATCH",
                      "ClickHouse parser-support UDR only accepts the clickhouse profile.");
  }
  const auto result = scratchbird::parser::clickhouse::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_clickhouse_parse_to_sblr(std::string_view sql_text,
                                  std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_clickhouse_parse_to_sblr",
                             "engine_context=trusted;resolver=uuid", failure)) {
    return failure;
  }
  if (!RequireContextField(context_packet, "resolver", "uuid",
                           "sbu_clickhouse_parse_to_sblr",
                           "engine_context=trusted;resolver=uuid", failure)) {
    return failure;
  }
  const auto result = scratchbird::parser::clickhouse::ParseStatement(sql_text);
...
UdrResult sbu_clickhouse_normalize(std::string_view sql_text,
                              std::string_view profile) {
  if (profile != "clickhouse") {
    return Diagnostic("UDR.CLICKHOUSE.PROFILE_MISMATCH",
                      "ClickHouse parser-support UDR only accepts the clickhouse profile.");
...
UdrResult sbu_clickhouse_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_clickhouse_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"clickhouse\",\"statement_family\":\"" +
...
UdrResult sbu_clickhouse_install_environment(std::string_view context_packet,
                                        std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_clickhouse_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.CLICKHOUSE.INSTALL_MODE_INVALID",
                      "ClickHouse environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_clickhouse_install_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  ...
  return {true,
          "{\"installer\":\"sbu_clickhouse_parser_support\","
          ...
          "\"catalog_overlays\":[\"SYSTEM.TABLES\",\"SYSTEM.COLUMNS\",\"SYSTEM.DATABASES\",\"SYSTEM.SETTINGS\",\"SYSTEM.FUNCTIONS\",\"SYSTEM.DICTIONARIES\",\"SYSTEM.USERS\",\"SYSTEM.CLUSTERS\",\"SYSTEM.FORMATS\",\"INFORMATION_SCHEMA\"],"
          "\"real_clickhouse_file_effects\":false}",
          scratchbird::parser::clickhouse::MessageVectorToJson({})};
}

UdrResult sbu_clickhouse_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_clickhouse_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_clickhouse_verify_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"SYSTEM.TABLES\",\"SYSTEM.COLUMNS\",\"SYSTEM.DATABASES\",\"SYSTEM.SETTINGS\",\"SYSTEM.FUNCTIONS\",\"SYSTEM.DICTIONARIES\",\"SYSTEM.USERS\",\"SYSTEM.CLUSTERS\",\"SYSTEM.FORMATS\",\"INFORMATION_SCHEMA\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":10,"
...
UdrResult sbu_clickhouse_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
      render_policy != "allow_debug_artifacts" &&
      render_policy != "summary") {
    return Diagnostic("UDR.CLICKHOUSE.MGMT_INVENTORY_POLICY_DENIED",
                      "ClickHouse management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy),
          scratchbird::parser::clickhouse::MessageVectorToJson({})};
}

UdrResult sbu_clickhouse_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_clickhouse_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.CLICKHOUSE.MGMT_OPERATION_UNKNOWN",
                      "ClickHouse management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuClickhousePackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/clickhouse/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::clickhouse::MessageVectorToJson({})};
}

UdrResult sbu_clickhouse_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_CLICKHOUSE.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::clickhouse::ClickhousePackageIdentityJson(),
          scratchbird::parser::clickhouse::MessageVectorToJson({})};
}

scratchbird::udr::runtime::UdrPackageDescriptor sbu_clickhouse_package_descriptor() {
  scratchbird::udr::runtime::UdrPackageDescriptor descriptor;
  descriptor.package_uuid = kSbuClickhousePackageIdentity;
  descriptor.package_name = std::string(kSbuClickhousePackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "clickhouse-parser-support-enterprise-closure";
  descriptor.binary_hash = "sha256:sbu_clickhouse_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.clickhouse";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_clickhouse_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_clickhouse_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_clickhouse_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_clickhouse_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_clickhouse_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_clickhouse_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_clickhouse_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_clickhouse_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_clickhouse_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &ClickhouseLifecycle;
  descriptor.shutdown = &ClickhouseLifecycle;
  return descriptor;
}
