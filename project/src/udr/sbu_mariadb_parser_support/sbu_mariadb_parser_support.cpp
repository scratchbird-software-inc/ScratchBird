...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::mariadb::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::mariadb::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::mariadb::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::mariadb::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_mariadb_parser_support", {}}};
  return {false, {}, scratchbird::parser::mariadb::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::mariadb::Diagnostic> diagnostics{
      {"UDR.MARIADB.SECURITY_DENIED", "ERROR",
       "MariaDB parser-support UDR denied the requested operation.",
       "sbu_mariadb_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::mariadb::Diagnostic> diagnostics{
      {"UDR.MARIADB.CONTEXT_MISSING", "ERROR",
       "MariaDB parser-support UDR requires engine-supplied trusted context.",
       "sbu_mariadb_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "mariadb_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_mariadb_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mariadb_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mariadb_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mariadb_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mariadb_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mariadb_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mariadb_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_mariadb_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mariadb_debug_capabilities(input.context_packet));
}
...
UdrResult sbu_mariadb_validate_syntax(std::string_view sql_text,
  ...
  if (profile != "mariadb") {
    return Diagnostic("UDR.MARIADB.PROFILE_MISMATCH",
                      "MariaDB parser-support UDR only accepts the mariadb profile.");
  }
  const auto result = scratchbird::parser::mariadb::ParseStatement(sql_text);
...
UdrResult sbu_mariadb_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mariadb_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"mariadb\",\"statement_family\":\"" +
...
UdrResult sbu_mariadb_install_environment(std::string_view context_packet,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mariadb_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.MARIADB.INSTALL_MODE_INVALID",
                      "MariaDB environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_mariadb_install_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  ...
  return {true,
          "{\"installer\":\"sbu_mariadb_parser_support\","
          ...
          "\"catalog_overlays\":[\"INFORMATION_SCHEMA\",\"MYSQL\",\"PERFORMANCE_SCHEMA\",\"SYS\",\"INFORMATION_SCHEMA.SEQUENCES\",\"INFORMATION_SCHEMA.ROUTINES\",\"SHOW GRANTS\",\"SHOW COLUMNS\",\"SHOW INDEX\"],"
          "\"real_mariadb_file_effects\":false}",
...
UdrResult sbu_mariadb_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mariadb_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_mariadb_verify_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"INFORMATION_SCHEMA\",\"MYSQL\",\"PERFORMANCE_SCHEMA\",\"SYS\",\"INFORMATION_SCHEMA.SEQUENCES\",\"INFORMATION_SCHEMA.ROUTINES\",\"SHOW GRANTS\",\"SHOW COLUMNS\",\"SHOW INDEX\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":9,"
...
UdrResult sbu_mariadb_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
    ...
    return Diagnostic("UDR.MARIADB.MGMT_INVENTORY_POLICY_DENIED",
                      "MariaDB management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
...
UdrResult sbu_mariadb_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_mariadb_management_package_request";
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuMariadbPackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
...
UdrResult sbu_mariadb_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_MARIADB.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::mariadb::MariadbPackageIdentityJson(),
          scratchbird::parser::mariadb::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_mariadb_package_descriptor() {
  ...
  descriptor.binary_hash = "sha256:sbu_mariadb_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.mariadb";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_mariadb_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_mariadb_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_mariadb_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_mariadb_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_mariadb_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_mariadb_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_mariadb_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_mariadb_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_mariadb_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &MariadbLifecycle;
  descriptor.shutdown = &MariadbLifecycle;
  return descriptor;
}
