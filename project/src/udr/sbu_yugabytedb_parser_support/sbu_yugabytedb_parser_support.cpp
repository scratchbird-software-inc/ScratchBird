...
UdrResult sbu_yugabytedb_management_package_request(std::string_view operation_name,
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.YUGABYTEDB.MGMT_OPERATION_UNKNOWN",
                      "YugabyteDB management package request names must be registered in the standard donor management ABI.");
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/donor_regression/yugabytedb/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::yugabytedb::MessageVectorToJson({})};
}
