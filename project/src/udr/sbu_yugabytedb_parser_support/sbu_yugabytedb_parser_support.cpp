...
namespace scratchbird::udr::yugabytedb_parser_support {
namespace {
...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_yugabytedb\","
      << "\"package_logical_name\":\"yugabytedb-v2025_2\","
      << "\"package_call_name\":\"sbup_yugabytedb\","
      << "\"reference_family\":\"yugabytedb\","
      ...
      << "\"real_yugabytedb_file_effects\":false,"
      << "\"inventory_detail\":\""
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::yugabytedb::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::yugabytedb::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::yugabytedb::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::yugabytedb::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_yugabytedb_parser_support", {}}};
  return {false, {}, scratchbird::parser::yugabytedb::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::yugabytedb::Diagnostic> diagnostics{
      {"UDR.YUGABYTEDB.SECURITY_DENIED", "ERROR",
       "YugabyteDB parser-support UDR denied the requested operation.",
       "sbu_yugabytedb_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::yugabytedb::Diagnostic> diagnostics{
      {"UDR.YUGABYTEDB.CONTEXT_MISSING", "ERROR",
       "YugabyteDB parser-support UDR requires engine-supplied trusted context.",
       "sbu_yugabytedb_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "yugabytedb_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrStatus YugabytedbLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuYugabytedbPackageIdentity) {
    return {false, "UDR.YUGABYTEDB.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_yugabytedb_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_yugabytedb_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_yugabytedb_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_yugabytedb_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_yugabytedb_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_yugabytedb_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_yugabytedb_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_yugabytedb_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_yugabytedb_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_yugabytedb_validate_syntax(std::string_view sql_text,
                                    std::string_view profile) {
  if (profile != "yugabytedb") {
    return Diagnostic("UDR.YUGABYTEDB.PROFILE_MISMATCH",
                      "YugabyteDB parser-support UDR only accepts the yugabytedb profile.");
  }
  const auto result = scratchbird::parser::yugabytedb::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_yugabytedb_parse_to_sblr(std::string_view sql_text,
                                  std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_yugabytedb_parse_to_sblr",
...
UdrResult sbu_yugabytedb_normalize(std::string_view sql_text,
                              std::string_view profile) {
  if (profile != "yugabytedb") {
    return Diagnostic("UDR.YUGABYTEDB.PROFILE_MISMATCH",
                      "YugabyteDB parser-support UDR only accepts the yugabytedb profile.");
...
UdrResult sbu_yugabytedb_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_yugabytedb_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"yugabytedb\",\"statement_family\":\"" +
...
UdrResult sbu_yugabytedb_install_environment(std::string_view context_packet,
                                        std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_yugabytedb_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.YUGABYTEDB.INSTALL_MODE_INVALID",
                      "YugabyteDB environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  ...
  return {true,
          "{\"installer\":\"sbu_yugabytedb_parser_support\","
          ...
          "\"catalog_overlays\":[\"YB_CATALOG\",\"PG_CATALOG\",\"INFORMATION_SCHEMA\",\"YCQL_KEYSPACES\",\"YB_TABLETS\",\"YB_SERVERS\",\"YB_TABLEGROUPS\",\"YB_XCLUSTER\",\"YB_CDC_STREAMS\",\"PG_ROLES\"],"
          "\"real_yugabytedb_file_effects\":false}",
          scratchbird::parser::yugabytedb::MessageVectorToJson({})};
}

UdrResult sbu_yugabytedb_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_yugabytedb_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"YB_CATALOG\",\"PG_CATALOG\",\"INFORMATION_SCHEMA\",\"YCQL_KEYSPACES\",\"YB_TABLETS\",\"YB_SERVERS\",\"YB_TABLEGROUPS\",\"YB_XCLUSTER\",\"YB_CDC_STREAMS\",\"PG_ROLES\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":9,"
...
UdrResult sbu_yugabytedb_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
      render_policy != "allow_debug_artifacts" &&
      render_policy != "summary") {
    return Diagnostic("UDR.YUGABYTEDB.MGMT_INVENTORY_POLICY_DENIED",
                      "YugabyteDB management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy),
          scratchbird::parser::yugabytedb::MessageVectorToJson({})};
}

UdrResult sbu_yugabytedb_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_yugabytedb_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.YUGABYTEDB.MGMT_OPERATION_UNKNOWN",
                      "YugabyteDB management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuYugabytedbPackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/yugabytedb/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::yugabytedb::MessageVectorToJson({})};
}

UdrResult sbu_yugabytedb_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_YUGABYTEDB.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::yugabytedb::YugabytedbPackageIdentityJson(),
          scratchbird::parser::yugabytedb::MessageVectorToJson({})};
}

scratchbird::udr::runtime::UdrPackageDescriptor sbu_yugabytedb_package_descriptor() {
  scratchbird::udr::runtime::UdrPackageDescriptor descriptor;
  descriptor.package_uuid = kSbuYugabytedbPackageIdentity;
  descriptor.package_name = std::string(kSbuYugabytedbPackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "yugabytedb-parser-support-enterprise-closure";
  descriptor.binary_hash = "sha256:sbu_yugabytedb_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.yugabytedb";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_yugabytedb_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_yugabytedb_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_yugabytedb_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_yugabytedb_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_yugabytedb_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_yugabytedb_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_yugabytedb_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_yugabytedb_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_yugabytedb_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &YugabytedbLifecycle;
  descriptor.shutdown = &YugabytedbLifecycle;
  return descriptor;
}
