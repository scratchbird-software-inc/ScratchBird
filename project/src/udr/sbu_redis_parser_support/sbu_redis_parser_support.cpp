...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_redis\","
      << "\"package_logical_name\":\"redis-v8_6\","
      << "\"package_call_name\":\"sbup_redis\","
      << "\"reference_family\":\"redis\","
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::redis::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::redis::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::redis::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::redis::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_redis_parser_support", {}}};
  return {false, {}, scratchbird::parser::redis::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::redis::Diagnostic> diagnostics{
      {"UDR.REDIS.SECURITY_DENIED", "ERROR",
       "Redis parser-support UDR denied the requested operation.",
       "sbu_redis_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::redis::Diagnostic> diagnostics{
      {"UDR.REDIS.CONTEXT_MISSING", "ERROR",
       "Redis parser-support UDR requires engine-supplied trusted context.",
       "sbu_redis_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "redis_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrStatus RedisLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuRedisPackageIdentity) {
    return {false, "UDR.REDIS.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_redis_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_redis_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_redis_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_redis_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_redis_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_redis_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_redis_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_redis_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_redis_debug_capabilities(input.context_packet));
}
...
UdrResult sbu_redis_validate_syntax(std::string_view sql_text,
  ...
  if (profile != "redis") {
    return Diagnostic("UDR.REDIS.PROFILE_MISMATCH",
                      "Redis parser-support UDR only accepts the redis profile.");
  }
  const auto result = scratchbird::parser::redis::ParseStatement(sql_text);
...
UdrResult sbu_redis_parse_to_sblr(std::string_view sql_text,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_redis_parse_to_sblr",
...
UdrResult sbu_redis_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_redis_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"redis\",\"statement_family\":\"" +
...
UdrResult sbu_redis_install_environment(std::string_view context_packet,
  ...
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_redis_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.REDIS.INSTALL_MODE_INVALID",
                      "Redis environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  ...
  return {true,
          "{\"installer\":\"sbu_redis_parser_support\","
          ...
          "\"catalog_overlays\":[\"COMMAND\",\"INFO\",\"ACL\",\"KEYSPACE\",\"STREAMS\",\"MEMORY\",\"LATENCY\",\"MODULE\",\"CLUSTER_RESERVED\"],"
          "\"real_redis_file_effects\":false}",
          scratchbird::parser::redis::MessageVectorToJson({})};
}

UdrResult sbu_redis_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_redis_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"COMMAND\",\"INFO\",\"ACL\",\"KEYSPACE\",\"STREAMS\",\"MEMORY\",\"LATENCY\",\"MODULE\",\"CLUSTER_RESERVED\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":9,"
...
UdrResult sbu_redis_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
    ...
    return Diagnostic("UDR.REDIS.MGMT_INVENTORY_POLICY_DENIED",
                      "Redis management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
...
UdrResult sbu_redis_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_redis_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.REDIS.MGMT_OPERATION_UNKNOWN",
                      "Redis management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuRedisPackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/redis/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::redis::MessageVectorToJson({})};
}

UdrResult sbu_redis_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_REDIS.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::redis::RedisPackageIdentityJson(),
          scratchbird::parser::redis::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_redis_package_descriptor() {
  ...
  descriptor.package_uuid = kSbuRedisPackageIdentity;
  descriptor.package_name = std::string(kSbuRedisPackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "redis-parser-support-enterprise-closure";
  descriptor.binary_hash = "sha256:sbu_redis_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.redis";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_redis_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_redis_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_redis_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_redis_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_redis_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_redis_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_redis_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_redis_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_redis_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &RedisLifecycle;
  descriptor.shutdown = &RedisLifecycle;
  return descriptor;
}
