...
namespace scratchbird::udr::tidb_parser_support {
namespace {
...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_tidb\","
      << "\"package_logical_name\":\"tidb-v8_5\","
      << "\"package_call_name\":\"sbup_tidb\","
      << "\"reference_family\":\"tidb\","
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::tidb::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::tidb::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::tidb::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::tidb::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_tidb_parser_support", {}}};
  return {false, {}, scratchbird::parser::tidb::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::tidb::Diagnostic> diagnostics{
      {"UDR.TIDB.SECURITY_DENIED", "ERROR",
       "TiDB parser-support UDR denied the requested operation.",
       "sbu_tidb_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::tidb::Diagnostic> diagnostics{
      {"UDR.TIDB.CONTEXT_MISSING", "ERROR",
       "TiDB parser-support UDR requires engine-supplied trusted context.",
       "sbu_tidb_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "tidb_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrStatus TidbLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuTidbPackageIdentity) {
    return {false, "UDR.TIDB.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_tidb_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tidb_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tidb_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tidb_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tidb_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tidb_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tidb_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_tidb_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_tidb_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_tidb_validate_syntax(std::string_view sql_text,
                                    std::string_view profile) {
  if (profile != "tidb") {
    return Diagnostic("UDR.TIDB.PROFILE_MISMATCH",
                      "TiDB parser-support UDR only accepts the tidb profile.");
  }
  const auto result = scratchbird::parser::tidb::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_tidb_parse_to_sblr(std::string_view sql_text,
                                  std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_tidb_parse_to_sblr",
                             "engine_context=trusted;resolver=uuid", failure)) {
    return failure;
  }
  if (!RequireContextField(context_packet, "resolver", "uuid",
                           "sbu_tidb_parse_to_sblr",
                           "engine_context=trusted;resolver=uuid", failure)) {
    return failure;
  }
  const auto result = scratchbird::parser::tidb::ParseStatement(sql_text);
...
UdrResult sbu_tidb_normalize(std::string_view sql_text,
                              std::string_view profile) {
  if (profile != "tidb") {
    return Diagnostic("UDR.TIDB.PROFILE_MISMATCH",
                      "TiDB parser-support UDR only accepts the tidb profile.");
...
UdrResult sbu_tidb_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_tidb_describe_statement",
...
UdrResult sbu_tidb_install_environment(std::string_view context_packet,
                                        std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_tidb_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.TIDB.INSTALL_MODE_INVALID",
                      "TiDB environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_tidb_install_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  ...
  return {true,
          "{\"installer\":\"sbu_tidb_parser_support\","
          ...
          "\"catalog_overlays\":[\"INFORMATION_SCHEMA\",\"METRICS_SCHEMA\",\"PERFORMANCE_SCHEMA\",\"TIDB\",\"CLUSTER_INFO\",\"TIDB_SERVERS\",\"PLACEMENT_POLICIES\",\"RESOURCE_GROUPS\",\"SHOW GRANTS\",\"SHOW COLUMNS\",\"SHOW INDEX\"],"
          "\"real_tidb_file_effects\":false}",
          scratchbird::parser::tidb::MessageVectorToJson({})};
}

UdrResult sbu_tidb_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_tidb_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
                                "sbu_tidb_verify_environment",
                                "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"INFORMATION_SCHEMA\",\"METRICS_SCHEMA\",\"PERFORMANCE_SCHEMA\",\"TIDB\",\"CLUSTER_INFO\",\"TIDB_SERVERS\",\"PLACEMENT_POLICIES\",\"RESOURCE_GROUPS\",\"SHOW GRANTS\",\"SHOW COLUMNS\",\"SHOW INDEX\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":9,"
...
UdrResult sbu_tidb_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
      render_policy != "allow_debug_artifacts" &&
      render_policy != "summary") {
    return Diagnostic("UDR.TIDB.MGMT_INVENTORY_POLICY_DENIED",
                      "TiDB management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy),
          scratchbird::parser::tidb::MessageVectorToJson({})};
}

UdrResult sbu_tidb_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_tidb_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.TIDB.MGMT_OPERATION_UNKNOWN",
                      "TiDB management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuTidbPackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/tidb/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::tidb::MessageVectorToJson({})};
}

UdrResult sbu_tidb_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_TIDB.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::tidb::TidbPackageIdentityJson(),
          scratchbird::parser::tidb::MessageVectorToJson({})};
}

scratchbird::udr::runtime::UdrPackageDescriptor sbu_tidb_package_descriptor() {
  scratchbird::udr::runtime::UdrPackageDescriptor descriptor;
  descriptor.package_uuid = kSbuTidbPackageIdentity;
  descriptor.package_name = std::string(kSbuTidbPackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "tidb-parser-support-enterprise-closure";
  descriptor.binary_hash = "sha256:sbu_tidb_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.tidb";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_tidb_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_tidb_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_tidb_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_tidb_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_tidb_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_tidb_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_tidb_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_tidb_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_tidb_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &TidbLifecycle;
  descriptor.shutdown = &TidbLifecycle;
  return descriptor;
}
