...
struct CatalogOverlayInstallState {
  ...
  int overlay_profile_version{111};
};
...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_immudb\","
      << "\"package_logical_name\":\"immudb-v1_11\","
      << "\"package_call_name\":\"sbup_immudb\","
      << "\"reference_family\":\"immudb\","
      ...
      << "\"real_immudb_file_effects\":false,"
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token = scratchbird::parser::immudb::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back({scratchbird::parser::immudb::TrimAscii(token.substr(0, separator)),
                       scratchbird::parser::immudb::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::immudb::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message), "sbu_immudb_parser_support", {}}};
  return {false, {}, scratchbird::parser::immudb::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::immudb::Diagnostic> diagnostics{
      {"UDR.IMMUDB.SECURITY_DENIED", "ERROR",
       "immudb parser-support UDR denied the requested operation.",
       "sbu_immudb_parser_support",
...
UdrResult MissingContext(std::string_view function_name, std::string_view required_fields) {
  const std::vector<scratchbird::parser::immudb::Diagnostic> diagnostics{
      {"UDR.IMMUDB.CONTEXT_MISSING", "ERROR",
       "immudb parser-support UDR requires engine-supplied trusted context.",
       "sbu_immudb_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid, const CatalogOverlayInstallState& state) {
  ...
  std::ostringstream out;
  out << "immudb_catalog_overlay_" << hash;
  return out.str();
}
...
std::string OverlayArrayJson() {
  return "[\"PG_TYPE\",\"DATABASES\",\"TABLES\",\"COLUMNS\",\"INDEXES\","
         "\"CURRENT_STATE\",\"TX_BY_ID\",\"HISTORY\",\"IMMUDB_USERS\",\"IMMUDB_PERMISSIONS\"]";
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_immudb_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_immudb_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_immudb_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_immudb_validate_syntax(std::string_view sql_text, std::string_view profile) {
  if (profile != "immudb") {
    return Diagnostic("UDR.IMMUDB.PROFILE_MISMATCH", "immudb parser-support UDR only accepts the immudb profile.");
  }
  const auto result = scratchbird::parser::immudb::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_immudb_parse_to_sblr(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_immudb_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  if (!RequireContextField(context_packet, "resolver", "uuid", "sbu_immudb_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  const auto result = scratchbird::parser::immudb::ParseStatement(sql_text);
  return {result.ok, result.sblr_envelope, result.message_vector_json};
}

UdrResult sbu_immudb_normalize(std::string_view sql_text, std::string_view profile) {
  if (profile != "immudb") {
    return Diagnostic("UDR.IMMUDB.PROFILE_MISMATCH", "immudb parser-support UDR only accepts the immudb profile.");
  }
  const auto result = scratchbird::parser::immudb::ParseStatement(sql_text);
...
UdrResult sbu_immudb_describe_statement(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_immudb_describe_statement", "engine_context=trusted", failure)) return failure;
  const auto result = scratchbird::parser::immudb::ParseStatement(sql_text);
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"immudb\",\"statement_family\":\"" + result.statement_family +
              "\",\"operation_family\":\"" + result.operation_family +
              "\",\"authority_disposition\":\"" + result.authority_disposition + "\"}",
          result.message_vector_json};
}

UdrResult sbu_immudb_install_environment(std::string_view context_packet, std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_immudb_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (install_mode != "install" && install_mode != "verify" && install_mode != "force_reinstall" &&
    ...
    return Diagnostic("UDR.IMMUDB.INSTALL_MODE_INVALID", "immudb environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_immudb_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  ...
  return {true,
          "{\"installer\":\"sbu_immudb_parser_support\","
          ...
          "\"real_immudb_file_effects\":false}",
          scratchbird::parser::immudb::MessageVectorToJson({})};
}

UdrResult sbu_immudb_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_immudb_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_immudb_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
...
UdrResult sbu_immudb_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" && render_policy != "allow_debug_artifacts" && render_policy != "summary") {
    return Diagnostic("UDR.IMMUDB.MGMT_INVENTORY_POLICY_DENIED", "immudb management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy), scratchbird::parser::immudb::MessageVectorToJson({})};
}

UdrResult sbu_immudb_management_package_request(std::string_view operation_name, std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_immudb_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.IMMUDB.MGMT_OPERATION_UNKNOWN", "immudb management package request names must be registered in the standard compatibility management ABI.");
  }
  if (!RequireTrustedContext(context_packet, function_name, required_context, failure)) return failure;
  if (!RequireContextField(context_packet, "package_uuid", kSbuImmudbPackageUuid, function_name, required_context, failure)) return failure;
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/immudb/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::immudb::MessageVectorToJson({})};
}

UdrResult sbu_immudb_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_IMMUDB.DEBUG_POLICY_DENIED", "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::immudb::ImmudbPackageIdentityJson(), scratchbird::parser::immudb::MessageVectorToJson({})};
}
...
scratchbird::udr::runtime::UdrPackageDescriptor sbu_immudb_package_descriptor() {
  ...
  descriptor.binary_hash = "sha256:sbu_immudb_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.immudb";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_immudb_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_immudb_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_immudb_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_immudb_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_immudb_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_immudb_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_immudb_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_immudb_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_immudb_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &ImmudbLifecycle;
  descriptor.shutdown = &ImmudbLifecycle;
  return descriptor;
}
