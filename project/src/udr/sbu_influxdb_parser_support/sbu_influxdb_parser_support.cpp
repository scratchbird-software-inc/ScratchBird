...
namespace scratchbird::udr::influxdb_parser_support {
namespace {
...
struct CatalogOverlayInstallState {
  ...
  int overlay_profile_version{139};
};
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token = scratchbird::parser::influxdb::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back({scratchbird::parser::influxdb::TrimAscii(token.substr(0, separator)),
                       scratchbird::parser::influxdb::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::influxdb::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message), "sbu_influxdb_parser_support", {}}};
  return {false, {}, scratchbird::parser::influxdb::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::influxdb::Diagnostic> diagnostics{
      {"UDR.INFLUXDB.SECURITY_DENIED", "ERROR",
       "InfluxDB parser-support UDR denied the requested operation.",
       "sbu_influxdb_parser_support",
...
UdrResult MissingContext(std::string_view function_name, std::string_view required_fields) {
  const std::vector<scratchbird::parser::influxdb::Diagnostic> diagnostics{
      {"UDR.INFLUXDB.CONTEXT_MISSING", "ERROR",
       "InfluxDB parser-support UDR requires engine-supplied trusted context.",
       "sbu_influxdb_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid, const CatalogOverlayInstallState& state) {
  ...
  std::ostringstream out;
  out << "influxdb_catalog_overlay_" << hash;
  return out.str();
}
...
std::string OverlayArrayJson() {
  return "[\"INFLUXDB_DATABASES\",\"INFLUXDB_BUCKETS\",\"INFLUXDB_MEASUREMENTS\",\"INFLUXDB_TAG_KEYS\",\"INFLUXDB_FIELD_KEYS\",\"INFLUXDB_RETENTION_POLICIES\",\"INFLUXDB_CONTINUOUS_QUERIES\",\"INFLUXDB_SUBSCRIPTIONS\",\"INFLUXDB_TASKS\"]";
}
...
scratchbird::udr::runtime::UdrStatus InfluxdbLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuInfluxdbPackageIdentity) {
    return {false, "UDR.INFLUXDB.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}

scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_influxdb_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_influxdb_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_influxdb_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_influxdb_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_influxdb_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_influxdb_verify_environment(input.context_packet));
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_influxdb_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_influxdb_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_influxdb_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_influxdb_validate_syntax(std::string_view sql_text, std::string_view profile) {
  if (profile != "influxdb") {
    return Diagnostic("UDR.INFLUXDB.PROFILE_MISMATCH", "InfluxDB parser-support UDR only accepts the influxdb profile.");
  }
  const auto result = scratchbird::parser::influxdb::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_influxdb_parse_to_sblr(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_influxdb_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  if (!RequireContextField(context_packet, "resolver", "uuid", "sbu_influxdb_parse_to_sblr", "engine_context=trusted;resolver=uuid", failure)) return failure;
  const auto result = scratchbird::parser::influxdb::ParseStatement(sql_text);
  return {result.ok, result.sblr_envelope, result.message_vector_json};
}

UdrResult sbu_influxdb_normalize(std::string_view sql_text, std::string_view profile) {
  if (profile != "influxdb") {
    return Diagnostic("UDR.INFLUXDB.PROFILE_MISMATCH", "InfluxDB parser-support UDR only accepts the influxdb profile.");
  }
  const auto result = scratchbird::parser::influxdb::ParseStatement(sql_text);
...
UdrResult sbu_influxdb_describe_statement(std::string_view sql_text, std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_influxdb_describe_statement", "engine_context=trusted", failure)) return failure;
  const auto result = scratchbird::parser::influxdb::ParseStatement(sql_text);
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"influxdb\",\"statement_family\":\"" + result.statement_family +
              "\",\"operation_family\":\"" + result.operation_family +
              "\",\"authority_disposition\":\"" + result.authority_disposition + "\"}",
          result.message_vector_json};
}

UdrResult sbu_influxdb_install_environment(std::string_view context_packet, std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_influxdb_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (install_mode != "install" && install_mode != "verify" && install_mode != "force_reinstall" &&
    ...
    return Diagnostic("UDR.INFLUXDB.INSTALL_MODE_INVALID", "InfluxDB environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_influxdb_install_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  ...
  return {true,
          "{\"installer\":\"sbu_influxdb_parser_support\","
...
UdrResult sbu_influxdb_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_influxdb_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  if (!RequireContextFieldValue(context_packet, "catalog_uuid", "sbu_influxdb_verify_environment", "engine_context=trusted;catalog_uuid=<uuid>", failure)) return failure;
  const auto catalog_uuid = ContextFieldValue(context_packet, "catalog_uuid");
...
UdrResult sbu_influxdb_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" && render_policy != "allow_debug_artifacts" && render_policy != "summary") {
    return Diagnostic("UDR.INFLUXDB.MGMT_INVENTORY_POLICY_DENIED", "InfluxDB management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy), scratchbird::parser::influxdb::MessageVectorToJson({})};
}

UdrResult sbu_influxdb_management_package_request(std::string_view operation_name, std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_influxdb_management_package_request";
...
UdrResult sbu_influxdb_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_INFLUXDB.DEBUG_POLICY_DENIED", "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::influxdb::InfluxdbPackageIdentityJson(), scratchbird::parser::influxdb::MessageVectorToJson({})};
}

scratchbird::udr::runtime::UdrPackageDescriptor sbu_influxdb_package_descriptor() {
  scratchbird::udr::runtime::UdrPackageDescriptor descriptor;
  descriptor.package_uuid = kSbuInfluxdbPackageIdentity;
  descriptor.package_name = std::string(kSbuInfluxdbPackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "influxdb-parser-support-beta-closure";
  descriptor.binary_hash = "sha256:sbu_influxdb_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.influxdb";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_influxdb_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_influxdb_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_influxdb_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_influxdb_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_influxdb_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_influxdb_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_influxdb_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_influxdb_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_influxdb_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &InfluxdbLifecycle;
  descriptor.shutdown = &InfluxdbLifecycle;
  return descriptor;
}
