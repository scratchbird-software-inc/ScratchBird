...
namespace scratchbird::udr::mongodb_parser_support {
namespace {
...
std::string ManagementInventoryJson(std::string_view render_policy) {
  ...
  out << "{\"package\":\"sbup_mongodb\","
      << "\"package_logical_name\":\"mongodb-v8_2\","
      << "\"package_call_name\":\"sbup_mongodb\","
      << "\"reference_family\":\"mongodb\","
      ...
      << "\"real_mongodb_file_effects\":false,"
      << "\"inventory_detail\":\""
...
std::vector<ContextField> ParseContextPacket(std::string_view context_packet) {
  ...
  while (begin <= context_packet.size()) {
    ...
    const auto token =
        scratchbird::parser::mongodb::TrimAscii(context_packet.substr(begin, end - begin));
    const auto separator = token.find('=');
    if (separator != std::string::npos) {
      fields.push_back(
          {scratchbird::parser::mongodb::TrimAscii(token.substr(0, separator)),
           scratchbird::parser::mongodb::TrimAscii(token.substr(separator + 1))});
    }
...
UdrResult Diagnostic(std::string_view code, std::string_view message) {
  const std::vector<scratchbird::parser::mongodb::Diagnostic> diagnostics{
      {std::string(code), "ERROR", std::string(message),
       "sbu_mongodb_parser_support", {}}};
  return {false, {}, scratchbird::parser::mongodb::MessageVectorToJson(diagnostics)};
}

UdrResult SecurityDenied(std::string_view function_name, std::string_view reason) {
  const std::vector<scratchbird::parser::mongodb::Diagnostic> diagnostics{
      {"UDR.MONGODB.SECURITY_DENIED", "ERROR",
       "MongoDB parser-support UDR denied the requested operation.",
       "sbu_mongodb_parser_support",
...
UdrResult MissingContext(std::string_view function_name,
  ...
  const std::vector<scratchbird::parser::mongodb::Diagnostic> diagnostics{
      {"UDR.MONGODB.CONTEXT_MISSING", "ERROR",
       "MongoDB parser-support UDR requires engine-supplied trusted context.",
       "sbu_mongodb_parser_support",
       {{"udr_function", std::string(function_name)},
...
std::string OverlayHash(std::string_view catalog_uuid,
  ...
  std::ostringstream out;
  out << "mongodb_catalog_overlay_" << hash;
  return out.str();
}
...
scratchbird::udr::runtime::UdrStatus MongodbLifecycle(const scratchbird::udr::runtime::UdrUuid& package_uuid) {
  if (package_uuid != kSbuMongodbPackageIdentity) {
    return {false, "UDR.MONGODB.PACKAGE_UUID_MISMATCH", "unexpected_package_uuid"};
  }
  return {true, "UDR.OK", {}};
}
...
scratchbird::udr::runtime::UdrCallResult RuntimeValidateSyntax(
  ...
  return ToRuntimeResult(sbu_mongodb_validate_syntax(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeParseToSblr(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mongodb_parse_to_sblr(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeNormalize(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mongodb_normalize(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDescribeStatement(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mongodb_describe_statement(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeInstallEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mongodb_install_environment(input.context_packet, input.payload));
}

scratchbird::udr::runtime::UdrCallResult RuntimeVerifyEnvironment(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mongodb_verify_environment(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementOperationInventory(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mongodb_management_operation_inventory(input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeManagementPackageRequest(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(
      sbu_mongodb_management_package_request(input.payload, input.context_packet));
}

scratchbird::udr::runtime::UdrCallResult RuntimeDebugCapabilities(
    const scratchbird::udr::runtime::UdrCallInput& input) {
  return ToRuntimeResult(sbu_mongodb_debug_capabilities(input.context_packet));
}

} // namespace

UdrResult sbu_mongodb_validate_syntax(std::string_view sql_text,
                                    std::string_view profile) {
  if (profile != "mongodb") {
    return Diagnostic("UDR.MONGODB.PROFILE_MISMATCH",
                      "MongoDB parser-support UDR only accepts the mongodb profile.");
  }
  const auto result = scratchbird::parser::mongodb::ParseStatement(sql_text);
  return {result.ok, {}, result.message_vector_json};
}

UdrResult sbu_mongodb_parse_to_sblr(std::string_view sql_text,
                                  std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mongodb_parse_to_sblr",
...
UdrResult sbu_mongodb_normalize(std::string_view sql_text,
                              std::string_view profile) {
  if (profile != "mongodb") {
    return Diagnostic("UDR.MONGODB.PROFILE_MISMATCH",
                      "MongoDB parser-support UDR only accepts the mongodb profile.");
...
UdrResult sbu_mongodb_describe_statement(std::string_view sql_text,
                                       std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mongodb_describe_statement",
  ...
  if (!result.ok) return {false, {}, result.message_vector_json};
  return {true,
          "{\"dialect\":\"mongodb\",\"statement_family\":\"" +
...
UdrResult sbu_mongodb_install_environment(std::string_view context_packet,
                                        std::string_view install_mode) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mongodb_install_environment",
  ...
  if (install_mode != "install" && install_mode != "verify" &&
    ...
    return Diagnostic("UDR.MONGODB.INSTALL_MODE_INVALID",
                      "MongoDB environment installer mode must be install, verify, force_reinstall, upgrade, or repair_version_drift.");
  }
  ...
  return {true,
          "{\"installer\":\"sbu_mongodb_parser_support\","
          ...
          "\"catalog_overlays\":[\"admin.system.users\",\"admin.system.roles\",\"config.collections\",\"config.chunks\",\"system.profile\",\"listCollections\",\"listIndexes\",\"serverStatus\",\"currentOp\"],"
          "\"real_mongodb_file_effects\":false}",
          scratchbird::parser::mongodb::MessageVectorToJson({})};
}

UdrResult sbu_mongodb_verify_environment(std::string_view context_packet) {
  UdrResult failure;
  if (!RequireTrustedContext(context_packet, "sbu_mongodb_verify_environment",
                             "engine_context=trusted;catalog_uuid=<uuid>", failure)) {
    return failure;
  }
  if (!RequireContextFieldValue(context_packet, "catalog_uuid",
  ...
  return {true,
          ...
          "\"catalog_overlays_present\":[\"admin.system.users\",\"admin.system.roles\",\"config.collections\",\"config.chunks\",\"system.profile\",\"listCollections\",\"listIndexes\",\"serverStatus\",\"currentOp\"],"
          "\"overlay_revision\":" + std::to_string(state.overlay_revision) + ","
          "\"overlay_profile_version\":" +
              std::to_string(state.overlay_profile_version) + ","
          "\"overlay_row_count\":9,"
...
UdrResult sbu_mongodb_management_operation_inventory(std::string_view render_policy) {
  if (render_policy != "release_evidence" &&
      render_policy != "allow_debug_artifacts" &&
      render_policy != "summary") {
    return Diagnostic("UDR.MONGODB.MGMT_INVENTORY_POLICY_DENIED",
                      "MongoDB management ABI inventory requires release evidence, summary, or debug artifact policy.");
  }
  return {true, ManagementInventoryJson(render_policy),
          scratchbird::parser::mongodb::MessageVectorToJson({})};
}

UdrResult sbu_mongodb_management_package_request(std::string_view operation_name,
                                               std::string_view context_packet) {
  constexpr std::string_view function_name = "sbu_mongodb_management_package_request";
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.MONGODB.MGMT_OPERATION_UNKNOWN",
                      "MongoDB management package request names must be registered in the standard compatibility management ABI.");
  }
  ...
  if (!RequireContextField(context_packet, "package_uuid",
                           kSbuMongodbPackageUuid, function_name,
                           required_context, failure)) {
    return failure;
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/reference_regression/mongodb/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::mongodb::MessageVectorToJson({})};
}

UdrResult sbu_mongodb_debug_capabilities(std::string_view render_policy) {
  if (render_policy != "allow_debug_artifacts") {
    return Diagnostic("SBU_MONGODB.DEBUG_POLICY_DENIED",
                      "Debug capability reporting requires explicit debug artifact policy.");
  }
  return {true, scratchbird::parser::mongodb::MongodbPackageIdentityJson(),
          scratchbird::parser::mongodb::MessageVectorToJson({})};
}

scratchbird::udr::runtime::UdrPackageDescriptor sbu_mongodb_package_descriptor() {
  scratchbird::udr::runtime::UdrPackageDescriptor descriptor;
  descriptor.package_uuid = kSbuMongodbPackageIdentity;
  descriptor.package_name = std::string(kSbuMongodbPackageName);
  descriptor.abi_version = "sb_udr_v1";
  descriptor.source_revision = "mongodb-parser-support-enterprise-closure";
  descriptor.binary_hash = "sha256:sbu_mongodb_parser_support_builtin";
  descriptor.signature_policy = "builtin-trusted-private";
  descriptor.capability_role = "parser_support.mongodb";
  descriptor.trusted_cpp = true;
  descriptor.entrypoints = {
      {"sbu_mongodb_validate_syntax", "parser.validate_syntax", &RuntimeValidateSyntax},
      {"sbu_mongodb_parse_to_sblr", "parser.parse_to_sblr", &RuntimeParseToSblr},
      {"sbu_mongodb_normalize", "parser.normalize", &RuntimeNormalize},
      {"sbu_mongodb_describe_statement", "parser.describe_statement", &RuntimeDescribeStatement},
      {"sbu_mongodb_install_environment", "parser.install_environment", &RuntimeInstallEnvironment},
      {"sbu_mongodb_verify_environment", "parser.verify_environment", &RuntimeVerifyEnvironment},
      {"sbu_mongodb_management_operation_inventory", "parser.management_abi_inventory", &RuntimeManagementOperationInventory},
      {"sbu_mongodb_management_package_request", "parser.management_abi_dispatch", &RuntimeManagementPackageRequest},
      {"sbu_mongodb_debug_capabilities", "parser.debug_capabilities", &RuntimeDebugCapabilities},
  };
  descriptor.init = &MongodbLifecycle;
  descriptor.shutdown = &MongodbLifecycle;
  return descriptor;
}
