...
UdrResult sbu_mongodb_management_package_request(std::string_view operation_name,
  ...
  if (!IsManagementOperation(operation_name)) {
    return Diagnostic("UDR.MONGODB.MGMT_OPERATION_UNKNOWN",
                      "MongoDB management package request names must be registered in the standard donor management ABI.");
  }
  ...
  return {true,
          ...
          "\"support_evidence_ref\":\"project/tests/donor_regression/mongodb/management_package_abi/management_package_abi_manifest.csv\"}",
          scratchbird::parser::mongodb::MessageVectorToJson({})};
}
