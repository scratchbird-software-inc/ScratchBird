...
int RunListenerWorker() {
  ...
  hello.protocol = Env("SB_PROTOCOL_FAMILY", "mysql");
  ...
  hello.profile_id = Env("SB_PARSER_PROFILE_ID", "mysql.default");
  hello.bundle_contract_id = Env("SB_PARSER_BUNDLE_CONTRACT_ID", "mysql.bundle.default@1");
  ...
  for (;;) {
    ...
    if (inbound.frame.opcode == scratchbird::listener::ListenerControlOpcode::kHandoffSocket) {
      ...
      handoff_ack.reason = handoff_ack.accepted
                               ? ""
                               : (connection_id == 0 ? "mysql_handoff_payload_invalid"
                                                     : "mysql_client_fd_missing");
      scratchbird::listener::ListenerControlFrame response;
...
int main(int argc, char** argv) {
  for (int i = 1; i < argc; ++i) {
...
#ifdef _WIN32
      std::cerr << "sbp_mysql listener-worker mode is not attached on Windows.\n";
      return EXIT_FAILURE;
#else
    ...
    if (std::string(argv[i]) == "--package-identity") {
      std::cout << scratchbird::parser::mysql::MysqlPackageIdentityJson() << '\n';
      return EXIT_SUCCESS;
    }
    if (std::string(argv[i]) == "--surface-report") {
      std::cout << scratchbird::parser::mysql::MysqlSurfaceReportJson() << '\n';
      return EXIT_SUCCESS;
    }
  }

  const std::string sql = argc > 1 ? argv[1] : "select 1";
  const auto result = scratchbird::parser::mysql::ParseStatement(sql);
  if (!result.ok) {
