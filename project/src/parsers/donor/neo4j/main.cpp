...
int RunListenerWorker() {
  ...
  hello.protocol = Env("SB_PROTOCOL_FAMILY", "neo4j");
  ...
  hello.profile_id = Env("SB_PARSER_PROFILE_ID", "neo4j.default");
  hello.bundle_contract_id = Env("SB_PARSER_BUNDLE_CONTRACT_ID", "neo4j.bundle.default@1");
  ...
  for (;;) {
    ...
    if (inbound.frame.opcode == scratchbird::listener::ListenerControlOpcode::kHandoffSocket) {
      ...
      handoff_ack.reason = handoff_ack.accepted
                               ? ""
                               : (connection_id == 0 ? "neo4j_handoff_payload_invalid"
                                                     : "neo4j_client_fd_missing");
      scratchbird::listener::ListenerControlFrame response;
      ...
      if (handoff_ack.accepted) {
        const int rc = scratchbird::parser::donor::ServeTextWorkerSession(
            handoff_fd, scratchbird::parser::neo4j::Profile());
        CloseFd(handoff_fd);
...
int main(int argc, char** argv) {
  for (int i = 1; i < argc; ++i) {
...
#ifdef _WIN32
      std::cerr << "sbp_neo4j listener-worker mode is not attached on Windows.\n";
      return EXIT_FAILURE;
#else
    ...
    if (std::string(argv[i]) == "--package-identity") {
      std::cout << scratchbird::parser::neo4j::Neo4jPackageIdentityJson() << '\n';
      return EXIT_SUCCESS;
    }
    if (std::string(argv[i]) == "--surface-report") {
      std::cout << scratchbird::parser::neo4j::Neo4jSurfaceReportJson() << '\n';
      return EXIT_SUCCESS;
    }
  }

  const std::string sql = argc > 1 ? argv[1] : "match (n:Account) return n";
  const auto result = scratchbird::parser::neo4j::ParseStatement(sql);
  if (!result.ok) {
