...
int RunListenerWorker() {
  ...
  hello.protocol = Env("SB_PROTOCOL_FAMILY", "apache_ignite");
  ...
  hello.profile_id = Env("SB_PARSER_PROFILE_ID", "apache_ignite.default");
  hello.bundle_contract_id = Env("SB_PARSER_BUNDLE_CONTRACT_ID", "apache_ignite.bundle.default@1");
  ...
  for (;;) {
    ...
    if (inbound.frame.opcode == scratchbird::listener::ListenerControlOpcode::kHandoffSocket) {
      ...
      handoff_ack.reason = handoff_ack.accepted
                               ? ""
                               : (connection_id == 0 ? "apache_ignite_handoff_payload_invalid"
                                                     : "apache_ignite_client_fd_missing");
      scratchbird::listener::ListenerControlFrame response;
      ...
      if (handoff_ack.accepted) {
        const int rc = scratchbird::parser::donor::ServeTextWorkerSession(
            handoff_fd, scratchbird::parser::apache_ignite::Profile());
        CloseFd(handoff_fd);
...
int main(int argc, char** argv) {
  for (int i = 1; i < argc; ++i) {
...
#ifdef _WIN32
      std::cerr << "sbp_apache_ignite listener-worker mode is not attached on Windows.\n";
      return EXIT_FAILURE;
#else
    ...
    if (std::string(argv[i]) == "--package-identity") {
      std::cout << scratchbird::parser::apache_ignite::ApacheIgnitePackageIdentityJson() << '\n';
      return EXIT_SUCCESS;
    }
    if (std::string(argv[i]) == "--surface-report") {
      std::cout << scratchbird::parser::apache_ignite::ApacheIgniteSurfaceReportJson() << '\n';
      return EXIT_SUCCESS;
    }
  }

  const std::string sql = argc > 1 ? argv[1] : "select * from City";
  const auto result = scratchbird::parser::apache_ignite::ParseStatement(sql);
  if (!result.ok) {
