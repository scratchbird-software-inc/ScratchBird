...
int RunListenerWorker() {
  ...
  hello.protocol = Env("SB_PROTOCOL_FAMILY", "opensearch_sql_ppl");
  ...
  hello.profile_id = Env("SB_PARSER_PROFILE_ID", "opensearch_sql_ppl.default");
  hello.bundle_contract_id = Env("SB_PARSER_BUNDLE_CONTRACT_ID", "opensearch_sql_ppl.bundle.default@1");
  ...
  for (;;) {
    ...
    if (inbound.frame.opcode == scratchbird::listener::ListenerControlOpcode::kHandoffSocket) {
      ...
      handoff_ack.reason = handoff_ack.accepted
                               ? ""
                               : (connection_id == 0 ? "opensearch_sql_ppl_handoff_payload_invalid"
                                                     : "opensearch_sql_ppl_client_fd_missing");
      scratchbird::listener::ListenerControlFrame response;
      ...
      if (handoff_ack.accepted) {
        const int rc = scratchbird::parser::donor::ServeTextWorkerSession(
            handoff_fd, scratchbird::parser::opensearch_sql_ppl::Profile());
        CloseFd(handoff_fd);
...
int main(int argc, char** argv) {
  for (int i = 1; i < argc; ++i) {
...
#ifdef _WIN32
      std::cerr << "sbp_opensearch_sql_ppl listener-worker mode is not attached on Windows.\n";
      return EXIT_FAILURE;
#else
    ...
    if (std::string(argv[i]) == "--package-identity") {
      std::cout << scratchbird::parser::opensearch_sql_ppl::OpensearchSqlPplPackageIdentityJson() << '\n';
      return EXIT_SUCCESS;
    }
    if (std::string(argv[i]) == "--surface-report") {
      std::cout << scratchbird::parser::opensearch_sql_ppl::OpensearchSqlPplSurfaceReportJson() << '\n';
      return EXIT_SUCCESS;
    }
  }

  const std::string sql = argc > 1 ? argv[1] : "select 1";
  const auto result = scratchbird::parser::opensearch_sql_ppl::ParseStatement(sql);
  if (!result.ok) {
