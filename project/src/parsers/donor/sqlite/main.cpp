...
int RunListenerWorker() {
  ...
  hello.protocol = Env("SB_PROTOCOL_FAMILY", "sqlite");
  ...
  hello.profile_id = Env("SB_PARSER_PROFILE_ID", "sqlite.default");
  hello.bundle_contract_id = Env("SB_PARSER_BUNDLE_CONTRACT_ID", "sqlite.bundle.default@1");
  ...
  for (;;) {
    ...
    if (inbound.frame.opcode == scratchbird::listener::ListenerControlOpcode::kHandoffSocket) {
      ...
      handoff_ack.reason = handoff_ack.accepted
                               ? ""
                               : (connection_id == 0 ? "sqlite_handoff_payload_invalid"
                                                     : "sqlite_client_fd_missing");
      scratchbird::listener::ListenerControlFrame response;
      ...
      if (handoff_ack.accepted) {
        const int rc = scratchbird::parser::donor::ServeTextWorkerSession(
            handoff_fd, scratchbird::parser::sqlite::Profile());
        CloseFd(handoff_fd);
...
int main(int argc, char** argv) {
  for (int i = 1; i < argc; ++i) {
...
#ifdef _WIN32
      std::cerr << "sbp_sqlite listener-worker mode is not attached on Windows.\n";
      return EXIT_FAILURE;
#else
    ...
    if (std::string(argv[i]) == "--package-identity") {
      std::cout << scratchbird::parser::sqlite::SqlitePackageIdentityJson() << '\n';
      return EXIT_SUCCESS;
    }
    if (std::string(argv[i]) == "--surface-report") {
      std::cout << scratchbird::parser::sqlite::SqliteSurfaceReportJson() << '\n';
      return EXIT_SUCCESS;
    }
  }

  const std::string sql = argc > 1 ? argv[1] : "select 1";
  const auto result = scratchbird::parser::sqlite::ParseStatement(sql);
  if (!result.ok) {
