...
using Descriptor = ReferenceVariableCompatibilityDescriptor;
...
constexpr std::array<Descriptor, 29> kReferenceVariableCompatibility = {{
    {"SBSQL-DCA62654CB0F", "@@", "tsql_mysql_system_variable_root", "", "", "",
     "", "expression.system_variable_read", "SBLR_SYSTEM_VARIABLE_READ",
     "sblr.expression.runtime.v3", "SB_DIAG_REFERENCE_VARIABLE_NAME_REQUIRED",
     false, true, true},
    ...
    {"SBSQL-73AAF62A5CC3", "SYS_CONTEXT('USERENV',name)",
     ...
     "SBLR_SYSTEM_VARIABLE_READ", "sblr.expression.runtime.v3",
     "SB_DIAG_FUNCTION_RUNTIME_REFUSAL", false, true, true},
    {"REFERENCE-VARIABLE-COLON-IDENTIFIER", ":identifier",
     "reference_host_or_bind_variable", "ctx_reference_bind_variable",
     "", "SBSQL-FD4E4EFCCC17", "REFERENCE_BIND_VARIABLE",
     "expression.system_variable_read", "SBLR_SYSTEM_VARIABLE_READ",
     "sblr.expression.runtime.v3", "SB_DIAG_REFERENCE_VARIABLE_BIND_REQUIRED",
     false, true, true},
    {"REFERENCE-VARIABLE-AT-IDENTIFIER", "@identifier",
     "reference_session_variable", "ctx_reference_session_variable",
     "", "SBSQL-FD4E4EFCCC17", "REFERENCE_SESSION_VARIABLE",
     "expression.system_variable_read", "SBLR_SYSTEM_VARIABLE_READ",
     "sblr.expression.runtime.v3", "SB_DIAG_REFERENCE_VARIABLE_BIND_REQUIRED",
     false, true, true},
}};
...
BuiltinReferenceVariableCompatibilityDescriptors() {
  return {kReferenceVariableCompatibility.data(),
          kReferenceVariableCompatibility.size()};
}

const ReferenceVariableCompatibilityDescriptor*
FindReferenceVariableCompatibilityBySurfaceId(std::string_view surface_id) {
  for (const auto& descriptor : BuiltinReferenceVariableCompatibilityDescriptors()) {
...
FindReferenceVariableCompatibilityBySpelling(std::string_view reference_spelling) {
  for (const auto& descriptor : BuiltinReferenceVariableCompatibilityDescriptors()) {
    if (EqualAsciiFold(descriptor.reference_spelling, reference_spelling)) {
      return &descriptor;
    }
  }
  return nullptr;
...
bool IsReferenceVariableCompatibilitySurface(std::string_view surface_id) {
  return FindReferenceVariableCompatibilityBySurfaceId(surface_id) != nullptr;
}

bool IsReferenceVariableCompatibilitySpelling(std::string_view reference_spelling) {
  return FindReferenceVariableCompatibilityBySpelling(reference_spelling) != nullptr;
}

ReferenceVariableSblrBinding LowerReferenceVariableCompatibilityBySpelling(
    std::string_view reference_spelling) {
  ReferenceVariableSblrBinding binding;
  const auto* descriptor =
      FindReferenceVariableCompatibilityBySpelling(reference_spelling);
  if (descriptor == nullptr) {
    return binding;
  }

  binding.surface_id = std::string(descriptor->surface_id);
  binding.reference_spelling = std::string(descriptor->reference_spelling);
  ...
  binding.operands.push_back(
      {"text", "reference_source_spelling",
