...
std::size_t ContentLength(std::string_view headers) {
  ...
  while (std::getline(input, line)) {
    ...
    const auto value_view =
        scratchbird::parser::compatibility::TrimAscii(std::string_view(line).substr(colon + 1));
...
std::string ExtractDiagnosticMessage(std::string_view json) {
  ...
  if (pos == std::string_view::npos) return "compatibility InfluxDB parser refused the request";
...
std::string ResponseBodyForPath(std::string_view request_path) {
  const auto lower = ToLower(request_path);
  if (lower.rfind("/api/v3/query_sql", 0) == 0 ||
      lower.rfind("/api/v3/query_influxql", 0) == 0) {
    return "[{\"sb_reference_probe\":1,\"scratchbird_reference_probe\":1,"
           "\"authority\":\"parser_listener_only_engine_mga_final\"}]\n";
  }
  if (lower.rfind("/api/v3/write_lp", 0) == 0) {
    return "{\"status\":\"ok\",\"scratchbird_reference_probe\":1}\n";
  }
  return "{\"status\":\"ok\",\"scratchbird_reference_probe\":1}\n";
}
...
int ServeInfluxdbHttpWorkerSession(int fd, InfluxHttpParseFn parse_statement) {
#ifdef _WIN32
  (void)fd;
  ...
  const auto parsed = parse_statement != nullptr ? parse_statement(parser_text)
                                                 : compatibility::ParseResult{};
  if (!parsed.ok) {
  ...
  const auto response = ResponseBodyForPath(RequestPath(request));
