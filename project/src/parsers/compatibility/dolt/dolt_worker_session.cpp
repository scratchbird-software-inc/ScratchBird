...
#include <unistd.h>
#endif
namespace scratchbird::parser::dolt { namespace {
...
std::vector<std::uint8_t> Handshake(){std::vector<std::uint8_t> p{10};wire::AppendNullString(&p,"1.75-Dolt-ScratchBird");
#ifdef _WIN32
wire::AppendU32(&p,0);
#else
wire::AppendU32(&p,static_cast<std::uint32_t>(::getpid()));
#endif
const std::string a="sbdolt01";p.insert(p.end(),a.begin(),a.end());p.push_back(0);wire::AppendU16(&p,static_cast<std::uint16_t>(kCaps));p.push_back(45);wire::AppendU16(&p,2);wire::AppendU16(&p,static_cast<std::uint16_t>(kCaps>>16));p.push_back(21);p.insert(p.end(),10,0);const std::string b="sbdolt02auth";p.insert(p.end(),b.begin(),b.end());p.push_back(0);wire::AppendNullString(&p,"caching_sha2_password");return p;}
...
std::string Message(std::string_view j){constexpr std::string_view k="\"message\":\"";auto a=j.find(k);if(a==j.npos)return "Dolt parser refused the statement";a+=k.size();auto b=j.find('"',a);return std::string(j.substr(a,b==j.npos?j.npos:b-a));}
