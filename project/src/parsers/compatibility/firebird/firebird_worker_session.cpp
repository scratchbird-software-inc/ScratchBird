...
const scratchbird::parser::compatibility::DialectProfile& FirebirdPolicyProfile() {
  static const scratchbird::parser::compatibility::DialectProfile profile{
      "firebird",
      "Firebird",
      "sbp_firebird",
...
std::string HandleLine(std::string_view line, bool* close) {
  ...
  if (upper == "CONNECTION_SANDBOX_REPORT") {
    return "SANDBOX " + scratchbird::parser::compatibility::ConnectionSandboxReportJson(
                            FirebirdPolicyProfile()) +
           "\n";
  }
  if (upper == "DIALECT_VARIANT_REPORT") {
    return "VARIANTS " + scratchbird::parser::compatibility::DialectVariantReportJson(
                             FirebirdPolicyProfile()) +
           "\n";
  }
