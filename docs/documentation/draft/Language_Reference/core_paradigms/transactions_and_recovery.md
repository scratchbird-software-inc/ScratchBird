...
Transactions are local to the database that owns the data. Bridge operations may create a local transaction and one or more remote transactions, but each participating database keeps its own MGA authority.

Commit, rollback, cleanup, retention, stream retention, backup-forward coverage, and recovery-required state are not parser decisions. SBsql can request and inspect these states. The engine determines visibility, finality, horizons, and repair/refusal behavior.

Use explicit transaction control for scripts that mix DDL, DML, backup, restore, bridge, or migration work. Autocommit is a session behavior, not a substitute for knowing where transaction boundaries are.
