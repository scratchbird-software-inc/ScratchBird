...
`MERGE` binds a source table expression, target relation, match condition, and action list. `UPSERT` binds insert input plus conflict handling. Both require deterministic conflict and result-shape rules.

Use these forms when a single transaction must coordinate conditional insert/update/delete behavior while preserving constraints and index maintenance.

Example:

```sql
upsert into app.customer (customer_id, customer_name)
values (:id, :name)
...
```ebnf
merge_statement         ::= "MERGE" "INTO" table_ref "USING" table_expression merge_condition merge_action_list returning_clause? ;
```

```ebnf
upsert_statement        ::= "UPSERT" "INTO" table_ref column_list? values_source conflict_clause? returning_clause? ;
```
