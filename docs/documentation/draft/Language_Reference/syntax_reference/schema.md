...
The schema-tree model, session schema variables, recursive branch layout, name-resolution order, search-path behavior, and SBsql-parser sandbox rules are documented in [schema_tree_and_name_resolution.md](schema_tree_and_name_resolution.md).
...
| Show | `SHOW SCHEMA ...`, `SHOW SCHEMAS` | Returns only authorized schema branches and properties; sandboxed SBsql sessions see their attached root as their schema root. |
...
Schema lifecycle operations operate on resolver scope. They must not expose private parent branches to sessions that are sandboxed below their attached database or workarea root.
