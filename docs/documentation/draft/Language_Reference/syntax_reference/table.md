...
| --- | --- | --- |
| Integer and numeric | `int`, `bigint`, `decimal(18,2)` | Choose exact numeric descriptors for keys, counters, money, and audit values. Use explicit precision and scale for decimals. |
| Floating point | `float`, `real` | Use for approximate scientific or ranking values. Do not use where exact decimal equality is required. |
...
| Temporal | `date`, `time`, `timestamp`, `timestamptz`, intervals where admitted | Time-zone, precision, and calendar behavior are descriptor-owned. |
