# sys.catalog.donor_type_mapping Catalog Reference

This page is part of the SBsql Language Reference Manual. It is generated from the SBsql grammar, surface registry, SBLR routing matrix, built-in operation registries, catalog-definition material, and parser/engine proof fixtures. It explains the user-facing language contract without treating SQL text as engine authority.

Generation task: `catalog_sys_catalog_donor_type_mapping`


## Role

`sys.catalog.donor_type_mapping` is a system catalog surface. It records durable metadata used by the binder, engine verifier, optimizer, security layer, support diagnostics, bridge rendering, or transaction model.
...
| donor_family | enum domain | Donor engine family. |
| donor_version_profile | text/domain | Version/profile. |
| donor_type_name | text domain | Donor spelling. |
| donor_type_code | nullable text/domain | Donor OID/code/protocol identifier. |
...
| compatibility_mode | enum domain | strict_scratchbird, donor_emulation, bridge_only, degraded, render_only, unsupported_by_policy. |

## Full Definition Extract

### Catalog Table `sys.catalog.donor_type_mapping`
...
```sql
select *
from sys.catalog.donor_type_mapping
limit 20;
```
