...
- `Cross-Platform Compilation` - the project has now been ported and runs on linux (primary), windows and mac.  Preliminary work for porting the drivers and embedded engine to iOS and Android is underway.  Installation medium testing for end-user deployment is underway.
- `Reference-system compatibility parsers` - source and prior evidence for 25 public compatibility parser lanes remain in the tree, but non-SBsql parsers are temporarily excluded from normal builds while the shared SBLR boundary stabilizes. They require the explicit `SB_BUILD_COMPATIBILITY_PARSERS=ON` build gate. This is not a production or drop-in compatibility claim. See `docs/compatibility-parsers/README.md`.
