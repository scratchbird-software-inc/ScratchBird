...
- `Reference-system compatibility parsers` - the 25 public compatibility parser lanes have moved beyond skeleton status. Their current beta parser surfaces are gate-backed as mapped to ScratchBird authority, emulated through controlled ScratchBird routes, parser-only presentation, or deterministic refusal. This is not a production or drop-in compatibility claim. See `docs/compatibility-parsers/README.md`.
