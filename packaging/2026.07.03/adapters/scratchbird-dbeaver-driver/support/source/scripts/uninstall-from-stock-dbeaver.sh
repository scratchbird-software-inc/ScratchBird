...
usage() {
  cat <<'EOF'
Usage:
  uninstall-from-stock-dbeaver.sh [path/to/dbeaver-install-or-launcher]

Description:
  Removes the ScratchBird DBeaver feature from an existing packaged DBeaver
  application using the p2 director. If no install path is provided, the
  script attempts to discover DBeaver from DBEAVER_HOME, PATH, and common
  install locations.
EOF
}
...
if ! grep -q "${FEATURE_IU//./\\.}" "${ROOTS_OUTPUT}"; then
  cat <<EOF
ScratchBird is not currently installed in this DBeaver instance.
...
if grep -q "${FEATURE_IU//./\\.}" "${ROOTS_OUTPUT}"; then
  echo "ScratchBird feature is still present in the installed roots output." >&2
  exit 1
fi
