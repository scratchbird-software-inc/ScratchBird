...
exports.METADATA_TYPE_INFO_QUERY = exports.METADATA_ROUTINES_QUERY = exports.METADATA_FUNCTIONS_QUERY = exports.METADATA_PROCEDURES_QUERY = exports.METADATA_COLUMN_PRIVILEGES_QUERY = exports.METADATA_TABLE_PRIVILEGES_QUERY = exports.METADATA_FOREIGN_KEYS_QUERY = exports.METADATA_PRIMARY_KEYS_QUERY = exports.METADATA_CONSTRAINTS_QUERY = exports.METADATA_INDEX_COLUMNS_QUERY = exports.METADATA_INDEXES_QUERY = exports.METADATA_COLUMNS_QUERY = exports.METADATA_TABLES_QUERY = exports.METADATA_CATALOGS_QUERY = exports.METADATA_SCHEMAS_QUERY = void 0;
exports.normalizeMetadataCollectionName = normalizeMetadataCollectionName;
exports.resolveMetadataCollectionQuery = resolveMetadataCollectionQuery;
exports.normalizeMetadataRestrictions = normalizeMetadataRestrictions;
exports.filterMetadataRowsByRestrictions = filterMetadataRowsByRestrictions;
exports.expandSchemaPaths = expandSchemaPaths;
exports.listMetadataSchemaPaths = listMetadataSchemaPaths;
exports.buildMetadataSchemaTree = buildMetadataSchemaTree;
exports.expandSchemaMetadataRows = expandSchemaMetadataRows;
exports.shapeMetadataRowsForCollection = shapeMetadataRowsForCollection;
exports.filterMetadataRowsForCollectionFamily = filterMetadataRowsForCollectionFamily;
exports.METADATA_SCHEMAS_QUERY = "SELECT schema_id, schema_name, owner_id, default_tablespace_id FROM sys.schemas WHERE is_valid = 1 ORDER BY schema_name";
exports.METADATA_CATALOGS_QUERY = "SELECT schema_id AS catalog_id, schema_name AS catalog_name FROM sys.schemas WHERE is_valid = 1 ORDER BY schema_name";
exports.METADATA_TABLES_QUERY = "SELECT t.table_id, t.schema_id, s.schema_name, t.table_name, t.table_type, t.owner_id FROM sys.tables t LEFT JOIN sys.schemas s ON s.schema_id = t.schema_id WHERE t.is_valid = 1 ORDER BY s.schema_name, t.table_name";
exports.METADATA_COLUMNS_QUERY = "SELECT c.column_id, c.table_id, t.table_name, t.schema_id, s.schema_name, c.column_name, c.data_type_id, c.data_type_name, c.ordinal_position, c.is_nullable, c.default_value, c.domain_id, c.collation_id, c.charset_id, c.is_identity, c.is_generated, c.generation_expression FROM sys.columns c LEFT JOIN sys.tables t ON t.table_id = c.table_id LEFT JOIN sys.schemas s ON s.schema_id = t.schema_id WHERE c.is_valid = 1 ORDER BY s.schema_name, t.table_name, c.ordinal_position";
exports.METADATA_INDEXES_QUERY = "SELECT i.index_id, i.table_id, t.table_name, t.schema_id, s.schema_name, i.index_name, i.index_type, i.is_unique FROM sys.indexes i LEFT JOIN sys.tables t ON t.table_id = i.table_id LEFT JOIN sys.schemas s ON s.schema_id = t.schema_id WHERE i.is_valid = 1 ORDER BY s.schema_name, t.table_name, i.index_name";
exports.METADATA_INDEX_COLUMNS_QUERY = "SELECT ic.index_id, i.index_name, ic.column_id, ic.column_name, ic.ordinal_position, ic.is_included, i.table_id, t.table_name, t.schema_id, s.schema_name FROM sys.index_columns ic LEFT JOIN sys.indexes i ON i.index_id = ic.index_id LEFT JOIN sys.tables t ON t.table_id = i.table_id LEFT JOIN sys.schemas s ON s.schema_id = t.schema_id ORDER BY s.schema_name, t.table_name, i.index_name, ic.ordinal_position";
exports.METADATA_CONSTRAINTS_QUERY = "SELECT * FROM information_schema.table_constraints";
exports.METADATA_PRIMARY_KEYS_QUERY = exports.METADATA_CONSTRAINTS_QUERY;
exports.METADATA_FOREIGN_KEYS_QUERY = exports.METADATA_CONSTRAINTS_QUERY;
exports.METADATA_TABLE_PRIVILEGES_QUERY = "SELECT t.table_id, t.table_name, t.schema_id, s.schema_name, t.owner_id AS grantor_id, t.owner_id AS grantee_id, 'ALL' AS privilege_type, 'YES' AS is_grantable FROM sys.tables t LEFT JOIN sys.schemas s ON s.schema_id = t.schema_id WHERE t.is_valid = 1 ORDER BY s.schema_name, t.table_name";
exports.METADATA_COLUMN_PRIVILEGES_QUERY = "SELECT c.table_id, t.table_name, t.schema_id, s.schema_name, c.column_id, c.column_name, 'ALL' AS privilege_type, 'YES' AS is_grantable FROM sys.columns c LEFT JOIN sys.tables t ON t.table_id = c.table_id LEFT JOIN sys.schemas s ON s.schema_id = t.schema_id WHERE c.is_valid = 1 ORDER BY s.schema_name, t.table_name, c.ordinal_position";
exports.METADATA_PROCEDURES_QUERY = "SELECT * FROM information_schema.routines";
exports.METADATA_FUNCTIONS_QUERY = exports.METADATA_PROCEDURES_QUERY;
exports.METADATA_ROUTINES_QUERY = exports.METADATA_PROCEDURES_QUERY;
exports.METADATA_TYPE_INFO_QUERY = "SELECT DISTINCT data_type_id, data_type_name, data_type_name AS type_name FROM sys.columns WHERE is_valid = 1 ORDER BY data_type_name";
const METADATA_COLLECTION_QUERIES = {
    catalogs: exports.METADATA_CATALOGS_QUERY,
    schemas: exports.METADATA_SCHEMAS_QUERY,
    tables: exports.METADATA_TABLES_QUERY,
    columns: exports.METADATA_COLUMNS_QUERY,
    indexes: exports.METADATA_INDEXES_QUERY,
    index_columns: exports.METADATA_INDEX_COLUMNS_QUERY,
    constraints: exports.METADATA_CONSTRAINTS_QUERY,
    primary_keys: exports.METADATA_PRIMARY_KEYS_QUERY,
    foreign_keys: exports.METADATA_FOREIGN_KEYS_QUERY,
    table_privileges: exports.METADATA_TABLE_PRIVILEGES_QUERY,
    column_privileges: exports.METADATA_COLUMN_PRIVILEGES_QUERY,
    procedures: exports.METADATA_PROCEDURES_QUERY,
    functions: exports.METADATA_FUNCTIONS_QUERY,
    routines: exports.METADATA_ROUTINES_QUERY,
    type_info: exports.METADATA_TYPE_INFO_QUERY,
};
const METADATA_COLLECTION_ALIASES = {
    catalogs: "catalogs",
    catalog: "catalogs",
    schemas: "schemas",
    schema: "schemas",
...
const METADATA_RESTRICTION_KEY_ALIASES = {
...
const METADATA_COLLECTION_RESTRICTION_KEYS = {
    catalogs: ["catalog"],
...
function normalizeMetadataCollectionName(collectionName) {
...
function resolveMetadataCollectionQuery(collectionName) {
    return METADATA_COLLECTION_QUERIES[normalizeMetadataCollectionName(collectionName)];
}
function normalizeMetadataRestrictions(restrictions) {
    if (!restrictions) {
        return {};
    }
    const out = {};
...
function filterMetadataRowsByRestrictions(rows, restrictions, collectionName) {
...
function expandSchemaPaths(schemaPaths) {
    const out = [];
    const seen = new Set();
...
function listMetadataSchemaPaths(rows, options) {
    const deduped = [];
    const seen = new Set();
...
function buildMetadataSchemaTree(rows, options) {
...
function expandSchemaMetadataRows(rows) {
    const out = [];
    const seen = new Set();
    for (const row of rows) {
        const schemaPath = readSchemaPath(row);
...
function shapeMetadataRowsForCollection(rows, collectionName, options) {
...
function filterMetadataRowsForCollectionFamily(rows, collectionName) {
...
function buildMetadataRestrictionBindings(rows, restrictions, collectionName) {
...
function rowsHaveAnyAlias(rows, aliases) {
    for (const row of rows) {
...
function metadataRowMatchesBindings(row, bindings) {
...
function metadataRowValueByAlias(row, alias) {
...
function metadataRestrictionAliases(key) {
...
function shapeMetadataRow(source, collectionName, catalogName) {
    const row = { ...source };
    ...
    const effectiveCatalog = firstStringValue(row, [
        "catalog_name",
        "table_catalog",
        "table_cat",
        "TABLE_CAT",
...
function assignIfMissing(row, key, value) {
...
function firstStringValue(row, keys) {
...
function metadataTypeMatches(row, keys, expectedValues) {
...
function splitSchemaPath(value) {
    return value
        .split(".")
        .map((segment) => segment.trim())
        .filter((segment) => segment.length > 0);
}
function readSchemaPath(row) {
...
function createSyntheticSchemaRow(sample, schemaPath) {
    const synthetic = {};
