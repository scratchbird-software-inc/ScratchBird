...
- Core runtime packages are staged under `server/engine`,
  `server/ipc-server`, and `server/sbparser`. `SBParser` and its UDR belong to
  the server tree, not the CLI tools package.
- CLI tools, drivers, adapters, and optional UDR packages must carry their
  installable payloads, source/support material, examples or usage notes, legal
  notices, checksums, and package manifests.
- Bundled proofs must be public-release proofs. Do not copy private workplans,
  private audits, private notes, or private repository paths into this tree.
