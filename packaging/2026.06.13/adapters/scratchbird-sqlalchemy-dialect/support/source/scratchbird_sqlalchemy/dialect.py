...
class ScratchBirdDialect(DefaultDialect):
    ...
    @classmethod
    def dbapi(cls):  # pragma: no cover - compatibility for older SQLAlchemy loaders
        return cls.import_dbapi()

    def create_connect_args(self, url):
        connect_args: Dict[str, Any] = {
            "host": url.host or "localhost",
            "port": int(url.port or 3092),
        }
        ...
        for raw_key, raw_value in (url.query or {}).items():
    ...
    def _fetchall(self, connection, sql: str, params: Optional[Dict[str, Any]] = None):
    ...
    def get_schema_names(self, connection, **kw):
        ...
        return [_row_get(row, "schema_name", 0) for row in rows]
    ...
    def get_table_names(self, connection, schema: Optional[str] = None, **kw):
        sql = (
            ...
            "WHERE t.is_valid = 1 "
            "AND (t.table_type IS NULL OR upper(t.table_type) <> 'VIEW')"
        )
        params: Dict[str, Any] = {}
        if schema:
        ...
        rows = self._fetchall(connection, sql, params)
        return [_row_get(row, "table_name", 0) for row in rows]

    def has_table(self, connection, table_name: str, schema: Optional[str] = None, **kw):
        sql = (
            "SELECT 1 "
            "FROM sys.tables t "
            "JOIN sys.schemas s ON s.schema_id = t.schema_id "
            "WHERE t.is_valid = 1 AND t.table_name = :table"
        )
    ...
    def get_view_names(self, connection, schema: Optional[str] = None, **kw):
        sql = (
            ...
            "FROM sys.tables t "
            "JOIN sys.schemas s ON s.schema_id = t.schema_id "
            "WHERE t.is_valid = 1 AND upper(t.table_type) = 'VIEW'"
        )
        params: Dict[str, Any] = {}
        if schema:
            sql += " AND s.schema_name = :schema"
    ...
    def get_columns(self, connection, table_name: str, schema: Optional[str] = None, **kw):
        params: Dict[str, Any] = {"table": table_name}
        sql = (
            "SELECT c.column_name, c.data_type_name, c.is_nullable, c.default_value, c.is_identity "
            "FROM sys.columns c "
        ...
        columns = []
        for row in rows:
            col_name = _row_get(row, "column_name", 0)
            data_type_name = _row_get(row, "data_type_name", 1)
            is_nullable = _row_get(row, "is_nullable", 2)
            default_value = _row_get(row, "default_value", 3)
            is_identity = _row_get(row, "is_identity", 4)
            columns.append(
                {
                    "name": col_name,
                    "type": _map_type(str(data_type_name) if data_type_name is not None else None),
                    "nullable": bool(is_nullable) if is_nullable is not None else True,
                    "default": default_value,
                    "autoincrement": bool(is_identity) if is_identity is not None else False,
                }
            )
        return columns
    ...
    def get_pk_constraint(self, connection, table_name: str, schema: Optional[str] = None, **kw):
        ...
        rows = self._fetchall(connection, sql, params)
        columns = [_row_get(row, "column_name", 0) for row in rows]
        name = _row_get(rows[0], "constraint_name", 1) if rows else None
    ...
    def get_foreign_keys(self, connection, table_name: str, schema: Optional[str] = None, **kw):
        ...
        for row in rows:
            name = _row_get(row, "constraint_name", 0)
            entry = keys.setdefault(
                name,
                {
                    "name": name,
                    "constrained_columns": [],
                    "referred_schema": _row_get(row, "table_schema", 2),
                    "referred_table": _row_get(row, "table_name", 3),
                    "referred_columns": [],
                    "options": {},
                },
            )
            entry["constrained_columns"].append(_row_get(row, "column_name", 1))
            entry["referred_columns"].append(_row_get(row, "column_name", 4))

            on_update = _row_get(row, "update_rule", 5)
            on_delete = _row_get(row, "delete_rule", 6)
            if on_update:
    ...
    def get_indexes(self, connection, table_name: str, schema: Optional[str] = None, **kw):
        sql = (
            "SELECT i.index_name, i.is_unique, ic.column_name, ic.ordinal_position "
            "FROM sys.indexes i "
            "JOIN sys.tables t ON t.table_id = i.table_id "
            "JOIN sys.schemas s ON s.schema_id = t.schema_id "
            "LEFT JOIN sys.index_columns ic ON ic.index_id = i.index_id "
            "WHERE t.is_valid = 1 AND i.is_valid = 1 AND t.table_name = :table"
        )
        params: Dict[str, Any] = {"table": table_name}
        if schema:
            sql += " AND s.schema_name = :schema"
        ...
        for row in rows:
            name = _row_get(row, "index_name", 0)
            entry = indexes.setdefault(
                name,
                {
                    "name": name,
                    "column_names": [],
                    "unique": bool(_row_get(row, "is_unique", 1)),
                },
            )
            column_name = _row_get(row, "column_name", 2)
            if column_name is not None:
                entry["column_names"].append(column_name)
