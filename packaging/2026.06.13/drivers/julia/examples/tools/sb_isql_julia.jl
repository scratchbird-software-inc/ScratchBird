...
function execute_copy_in!(conn, sql::String, payload::Vector{UInt8})::Int
    query_payload = UInt8[]
    ScratchBird.append_le!(query_payload, UInt32(0))
    ScratchBird.append_le!(query_payload, UInt32(0))
    ScratchBird.append_le!(query_payload, UInt32(0))
    append!(query_payload, Vector{UInt8}(codeunits(sql)))
    push!(query_payload, 0)
    ScratchBird.send_message!(conn, ScratchBird.MSG_QUERY, query_payload)
    rows_copied = 0
    copy_started = false
    while true
        ...
        elseif msg_type == ScratchBird.MSG_READY
            ...
            copy_started || throw(ScratchBird.ScratchBirdError("HY000", "COPY FROM STDIN did not enter COPY input mode"))
            return rows_copied
        elseif msg_type == ScratchBird.MSG_ERROR
            ScratchBird.raise_error(body)
        end
    end
end
